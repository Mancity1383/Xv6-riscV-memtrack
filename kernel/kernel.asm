
kernel/kernel:     file format elf64-littleriscv


Disassembly of section .text:

0000000080000000 <_entry>:
_entry:
        # set up a stack for C.
        # stack0 is declared in start.c,
        # with a 4096-byte stack per CPU.
        # sp = stack0 + ((hartid + 1) * 4096)
        la sp, stack0
    80000000:	00008117          	auipc	sp,0x8
    80000004:	87010113          	addi	sp,sp,-1936 # 80007870 <stack0>
        li a0, 1024*4
    80000008:	6505                	lui	a0,0x1
        csrr a1, mhartid
    8000000a:	f14025f3          	csrr	a1,mhartid
        addi a1, a1, 1
    8000000e:	0585                	addi	a1,a1,1
        mul a0, a0, a1
    80000010:	02b50533          	mul	a0,a0,a1
        add sp, sp, a0
    80000014:	912a                	add	sp,sp,a0
        # jump to start() in start.c
        call start
    80000016:	04a000ef          	jal	80000060 <start>

000000008000001a <spin>:
spin:
        j spin
    8000001a:	a001                	j	8000001a <spin>

000000008000001c <timerinit>:
}

// ask each hart to generate timer interrupts.
void
timerinit()
{
    8000001c:	1141                	addi	sp,sp,-16
    8000001e:	e422                	sd	s0,8(sp)
    80000020:	0800                	addi	s0,sp,16
#define MIE_STIE (1L << 5)  // supervisor timer
static inline uint64
r_mie()
{
  uint64 x;
  asm volatile("csrr %0, mie" : "=r" (x) );
    80000022:	304027f3          	csrr	a5,mie
  // enable supervisor-mode timer interrupts.
  w_mie(r_mie() | MIE_STIE);
    80000026:	0207e793          	ori	a5,a5,32
}

static inline void 
w_mie(uint64 x)
{
  asm volatile("csrw mie, %0" : : "r" (x));
    8000002a:	30479073          	csrw	mie,a5
static inline uint64
r_menvcfg()
{
  uint64 x;
  // asm volatile("csrr %0, menvcfg" : "=r" (x) );
  asm volatile("csrr %0, 0x30a" : "=r" (x) );
    8000002e:	30a027f3          	csrr	a5,0x30a
  
  // enable the sstc extension (i.e. stimecmp).
  w_menvcfg(r_menvcfg() | (1L << 63)); 
    80000032:	577d                	li	a4,-1
    80000034:	177e                	slli	a4,a4,0x3f
    80000036:	8fd9                	or	a5,a5,a4

static inline void 
w_menvcfg(uint64 x)
{
  // asm volatile("csrw menvcfg, %0" : : "r" (x));
  asm volatile("csrw 0x30a, %0" : : "r" (x));
    80000038:	30a79073          	csrw	0x30a,a5

static inline uint64
r_mcounteren()
{
  uint64 x;
  asm volatile("csrr %0, mcounteren" : "=r" (x) );
    8000003c:	306027f3          	csrr	a5,mcounteren
  
  // allow supervisor to use stimecmp and time.
  w_mcounteren(r_mcounteren() | 2);
    80000040:	0027e793          	ori	a5,a5,2
  asm volatile("csrw mcounteren, %0" : : "r" (x));
    80000044:	30679073          	csrw	mcounteren,a5
// machine-mode cycle counter
static inline uint64
r_time()
{
  uint64 x;
  asm volatile("csrr %0, time" : "=r" (x) );
    80000048:	c01027f3          	rdtime	a5
  
  // ask for the very first timer interrupt.
  w_stimecmp(r_time() + 1000000);
    8000004c:	000f4737          	lui	a4,0xf4
    80000050:	24070713          	addi	a4,a4,576 # f4240 <_entry-0x7ff0bdc0>
    80000054:	97ba                	add	a5,a5,a4
  asm volatile("csrw 0x14d, %0" : : "r" (x));
    80000056:	14d79073          	csrw	stimecmp,a5
}
    8000005a:	6422                	ld	s0,8(sp)
    8000005c:	0141                	addi	sp,sp,16
    8000005e:	8082                	ret

0000000080000060 <start>:
{
    80000060:	1141                	addi	sp,sp,-16
    80000062:	e406                	sd	ra,8(sp)
    80000064:	e022                	sd	s0,0(sp)
    80000066:	0800                	addi	s0,sp,16
  asm volatile("csrr %0, mstatus" : "=r" (x) );
    80000068:	300027f3          	csrr	a5,mstatus
  x &= ~MSTATUS_MPP_MASK;
    8000006c:	7779                	lui	a4,0xffffe
    8000006e:	7ff70713          	addi	a4,a4,2047 # ffffffffffffe7ff <end+0xffffffff7ffdda87>
    80000072:	8ff9                	and	a5,a5,a4
  x |= MSTATUS_MPP_S;
    80000074:	6705                	lui	a4,0x1
    80000076:	80070713          	addi	a4,a4,-2048 # 800 <_entry-0x7ffff800>
    8000007a:	8fd9                	or	a5,a5,a4
  asm volatile("csrw mstatus, %0" : : "r" (x));
    8000007c:	30079073          	csrw	mstatus,a5
  asm volatile("csrw mepc, %0" : : "r" (x));
    80000080:	00001797          	auipc	a5,0x1
    80000084:	dbc78793          	addi	a5,a5,-580 # 80000e3c <main>
    80000088:	34179073          	csrw	mepc,a5
  asm volatile("csrw satp, %0" : : "r" (x));
    8000008c:	4781                	li	a5,0
    8000008e:	18079073          	csrw	satp,a5
  asm volatile("csrw medeleg, %0" : : "r" (x));
    80000092:	67c1                	lui	a5,0x10
    80000094:	17fd                	addi	a5,a5,-1 # ffff <_entry-0x7fff0001>
    80000096:	30279073          	csrw	medeleg,a5
  asm volatile("csrw mideleg, %0" : : "r" (x));
    8000009a:	30379073          	csrw	mideleg,a5
  asm volatile("csrr %0, sie" : "=r" (x) );
    8000009e:	104027f3          	csrr	a5,sie
  w_sie(r_sie() | SIE_SEIE | SIE_STIE);
    800000a2:	2207e793          	ori	a5,a5,544
  asm volatile("csrw sie, %0" : : "r" (x));
    800000a6:	10479073          	csrw	sie,a5
  asm volatile("csrw pmpaddr0, %0" : : "r" (x));
    800000aa:	57fd                	li	a5,-1
    800000ac:	83a9                	srli	a5,a5,0xa
    800000ae:	3b079073          	csrw	pmpaddr0,a5
  asm volatile("csrw pmpcfg0, %0" : : "r" (x));
    800000b2:	47bd                	li	a5,15
    800000b4:	3a079073          	csrw	pmpcfg0,a5
  timerinit();
    800000b8:	f65ff0ef          	jal	8000001c <timerinit>
  asm volatile("csrr %0, mhartid" : "=r" (x) );
    800000bc:	f14027f3          	csrr	a5,mhartid
  w_tp(id);
    800000c0:	2781                	sext.w	a5,a5
}

static inline void 
w_tp(uint64 x)
{
  asm volatile("mv tp, %0" : : "r" (x));
    800000c2:	823e                	mv	tp,a5
  asm volatile("mret");
    800000c4:	30200073          	mret
}
    800000c8:	60a2                	ld	ra,8(sp)
    800000ca:	6402                	ld	s0,0(sp)
    800000cc:	0141                	addi	sp,sp,16
    800000ce:	8082                	ret

00000000800000d0 <consolewrite>:
//
// user write()s to the console go here.
//
int
consolewrite(int user_src, uint64 src, int n)
{
    800000d0:	7119                	addi	sp,sp,-128
    800000d2:	fc86                	sd	ra,120(sp)
    800000d4:	f8a2                	sd	s0,112(sp)
    800000d6:	f4a6                	sd	s1,104(sp)
    800000d8:	0100                	addi	s0,sp,128
  char buf[32];
  int i = 0;

  while(i < n){
    800000da:	06c05a63          	blez	a2,8000014e <consolewrite+0x7e>
    800000de:	f0ca                	sd	s2,96(sp)
    800000e0:	ecce                	sd	s3,88(sp)
    800000e2:	e8d2                	sd	s4,80(sp)
    800000e4:	e4d6                	sd	s5,72(sp)
    800000e6:	e0da                	sd	s6,64(sp)
    800000e8:	fc5e                	sd	s7,56(sp)
    800000ea:	f862                	sd	s8,48(sp)
    800000ec:	f466                	sd	s9,40(sp)
    800000ee:	8aaa                	mv	s5,a0
    800000f0:	8b2e                	mv	s6,a1
    800000f2:	8a32                	mv	s4,a2
  int i = 0;
    800000f4:	4481                	li	s1,0
    int nn = sizeof(buf);
    if(nn > n - i)
    800000f6:	02000c13          	li	s8,32
    800000fa:	02000c93          	li	s9,32
      nn = n - i;
    if(either_copyin(buf, user_src, src+i, nn) == -1)
    800000fe:	5bfd                	li	s7,-1
    80000100:	a035                	j	8000012c <consolewrite+0x5c>
    if(nn > n - i)
    80000102:	0009099b          	sext.w	s3,s2
    if(either_copyin(buf, user_src, src+i, nn) == -1)
    80000106:	86ce                	mv	a3,s3
    80000108:	01648633          	add	a2,s1,s6
    8000010c:	85d6                	mv	a1,s5
    8000010e:	f8040513          	addi	a0,s0,-128
    80000112:	1c0020ef          	jal	800022d2 <either_copyin>
    80000116:	03750e63          	beq	a0,s7,80000152 <consolewrite+0x82>
      break;
    uartwrite(buf, nn);
    8000011a:	85ce                	mv	a1,s3
    8000011c:	f8040513          	addi	a0,s0,-128
    80000120:	778000ef          	jal	80000898 <uartwrite>
    i += nn;
    80000124:	009904bb          	addw	s1,s2,s1
  while(i < n){
    80000128:	0144da63          	bge	s1,s4,8000013c <consolewrite+0x6c>
    if(nn > n - i)
    8000012c:	409a093b          	subw	s2,s4,s1
    80000130:	0009079b          	sext.w	a5,s2
    80000134:	fcfc57e3          	bge	s8,a5,80000102 <consolewrite+0x32>
    80000138:	8966                	mv	s2,s9
    8000013a:	b7e1                	j	80000102 <consolewrite+0x32>
    8000013c:	7906                	ld	s2,96(sp)
    8000013e:	69e6                	ld	s3,88(sp)
    80000140:	6a46                	ld	s4,80(sp)
    80000142:	6aa6                	ld	s5,72(sp)
    80000144:	6b06                	ld	s6,64(sp)
    80000146:	7be2                	ld	s7,56(sp)
    80000148:	7c42                	ld	s8,48(sp)
    8000014a:	7ca2                	ld	s9,40(sp)
    8000014c:	a819                	j	80000162 <consolewrite+0x92>
  int i = 0;
    8000014e:	4481                	li	s1,0
    80000150:	a809                	j	80000162 <consolewrite+0x92>
    80000152:	7906                	ld	s2,96(sp)
    80000154:	69e6                	ld	s3,88(sp)
    80000156:	6a46                	ld	s4,80(sp)
    80000158:	6aa6                	ld	s5,72(sp)
    8000015a:	6b06                	ld	s6,64(sp)
    8000015c:	7be2                	ld	s7,56(sp)
    8000015e:	7c42                	ld	s8,48(sp)
    80000160:	7ca2                	ld	s9,40(sp)
  }

  return i;
}
    80000162:	8526                	mv	a0,s1
    80000164:	70e6                	ld	ra,120(sp)
    80000166:	7446                	ld	s0,112(sp)
    80000168:	74a6                	ld	s1,104(sp)
    8000016a:	6109                	addi	sp,sp,128
    8000016c:	8082                	ret

000000008000016e <consoleread>:
// user_dist indicates whether dst is a user
// or kernel address.
//
int
consoleread(int user_dst, uint64 dst, int n)
{
    8000016e:	711d                	addi	sp,sp,-96
    80000170:	ec86                	sd	ra,88(sp)
    80000172:	e8a2                	sd	s0,80(sp)
    80000174:	e4a6                	sd	s1,72(sp)
    80000176:	e0ca                	sd	s2,64(sp)
    80000178:	fc4e                	sd	s3,56(sp)
    8000017a:	f852                	sd	s4,48(sp)
    8000017c:	f456                	sd	s5,40(sp)
    8000017e:	f05a                	sd	s6,32(sp)
    80000180:	1080                	addi	s0,sp,96
    80000182:	8aaa                	mv	s5,a0
    80000184:	8a2e                	mv	s4,a1
    80000186:	89b2                	mv	s3,a2
  uint target;
  int c;
  char cbuf;

  target = n;
    80000188:	00060b1b          	sext.w	s6,a2
  acquire(&cons.lock);
    8000018c:	0000f517          	auipc	a0,0xf
    80000190:	6e450513          	addi	a0,a0,1764 # 8000f870 <cons>
    80000194:	23b000ef          	jal	80000bce <acquire>
  while(n > 0){
    // wait until interrupt handler has put some
    // input into cons.buffer.
    while(cons.r == cons.w){
    80000198:	0000f497          	auipc	s1,0xf
    8000019c:	6d848493          	addi	s1,s1,1752 # 8000f870 <cons>
      if(killed(myproc())){
        release(&cons.lock);
        return -1;
      }
      sleep(&cons.r, &cons.lock);
    800001a0:	0000f917          	auipc	s2,0xf
    800001a4:	76890913          	addi	s2,s2,1896 # 8000f908 <cons+0x98>
  while(n > 0){
    800001a8:	0b305d63          	blez	s3,80000262 <consoleread+0xf4>
    while(cons.r == cons.w){
    800001ac:	0984a783          	lw	a5,152(s1)
    800001b0:	09c4a703          	lw	a4,156(s1)
    800001b4:	0af71263          	bne	a4,a5,80000258 <consoleread+0xea>
      if(killed(myproc())){
    800001b8:	778010ef          	jal	80001930 <myproc>
    800001bc:	7a9010ef          	jal	80002164 <killed>
    800001c0:	e12d                	bnez	a0,80000222 <consoleread+0xb4>
      sleep(&cons.r, &cons.lock);
    800001c2:	85a6                	mv	a1,s1
    800001c4:	854a                	mv	a0,s2
    800001c6:	567010ef          	jal	80001f2c <sleep>
    while(cons.r == cons.w){
    800001ca:	0984a783          	lw	a5,152(s1)
    800001ce:	09c4a703          	lw	a4,156(s1)
    800001d2:	fef703e3          	beq	a4,a5,800001b8 <consoleread+0x4a>
    800001d6:	ec5e                	sd	s7,24(sp)
    }

    c = cons.buf[cons.r++ % INPUT_BUF_SIZE];
    800001d8:	0000f717          	auipc	a4,0xf
    800001dc:	69870713          	addi	a4,a4,1688 # 8000f870 <cons>
    800001e0:	0017869b          	addiw	a3,a5,1
    800001e4:	08d72c23          	sw	a3,152(a4)
    800001e8:	07f7f693          	andi	a3,a5,127
    800001ec:	9736                	add	a4,a4,a3
    800001ee:	01874703          	lbu	a4,24(a4)
    800001f2:	00070b9b          	sext.w	s7,a4

    if(c == C('D')){  // end-of-file
    800001f6:	4691                	li	a3,4
    800001f8:	04db8663          	beq	s7,a3,80000244 <consoleread+0xd6>
      }
      break;
    }

    // copy the input byte to the user-space buffer.
    cbuf = c;
    800001fc:	fae407a3          	sb	a4,-81(s0)
    if(either_copyout(user_dst, dst, &cbuf, 1) == -1)
    80000200:	4685                	li	a3,1
    80000202:	faf40613          	addi	a2,s0,-81
    80000206:	85d2                	mv	a1,s4
    80000208:	8556                	mv	a0,s5
    8000020a:	07e020ef          	jal	80002288 <either_copyout>
    8000020e:	57fd                	li	a5,-1
    80000210:	04f50863          	beq	a0,a5,80000260 <consoleread+0xf2>
      break;

    dst++;
    80000214:	0a05                	addi	s4,s4,1
    --n;
    80000216:	39fd                	addiw	s3,s3,-1

    if(c == '\n'){
    80000218:	47a9                	li	a5,10
    8000021a:	04fb8d63          	beq	s7,a5,80000274 <consoleread+0x106>
    8000021e:	6be2                	ld	s7,24(sp)
    80000220:	b761                	j	800001a8 <consoleread+0x3a>
        release(&cons.lock);
    80000222:	0000f517          	auipc	a0,0xf
    80000226:	64e50513          	addi	a0,a0,1614 # 8000f870 <cons>
    8000022a:	23d000ef          	jal	80000c66 <release>
        return -1;
    8000022e:	557d                	li	a0,-1
    }
  }
  release(&cons.lock);

  return target - n;
}
    80000230:	60e6                	ld	ra,88(sp)
    80000232:	6446                	ld	s0,80(sp)
    80000234:	64a6                	ld	s1,72(sp)
    80000236:	6906                	ld	s2,64(sp)
    80000238:	79e2                	ld	s3,56(sp)
    8000023a:	7a42                	ld	s4,48(sp)
    8000023c:	7aa2                	ld	s5,40(sp)
    8000023e:	7b02                	ld	s6,32(sp)
    80000240:	6125                	addi	sp,sp,96
    80000242:	8082                	ret
      if(n < target){
    80000244:	0009871b          	sext.w	a4,s3
    80000248:	01677a63          	bgeu	a4,s6,8000025c <consoleread+0xee>
        cons.r--;
    8000024c:	0000f717          	auipc	a4,0xf
    80000250:	6af72e23          	sw	a5,1724(a4) # 8000f908 <cons+0x98>
    80000254:	6be2                	ld	s7,24(sp)
    80000256:	a031                	j	80000262 <consoleread+0xf4>
    80000258:	ec5e                	sd	s7,24(sp)
    8000025a:	bfbd                	j	800001d8 <consoleread+0x6a>
    8000025c:	6be2                	ld	s7,24(sp)
    8000025e:	a011                	j	80000262 <consoleread+0xf4>
    80000260:	6be2                	ld	s7,24(sp)
  release(&cons.lock);
    80000262:	0000f517          	auipc	a0,0xf
    80000266:	60e50513          	addi	a0,a0,1550 # 8000f870 <cons>
    8000026a:	1fd000ef          	jal	80000c66 <release>
  return target - n;
    8000026e:	413b053b          	subw	a0,s6,s3
    80000272:	bf7d                	j	80000230 <consoleread+0xc2>
    80000274:	6be2                	ld	s7,24(sp)
    80000276:	b7f5                	j	80000262 <consoleread+0xf4>

0000000080000278 <consputc>:
{
    80000278:	1141                	addi	sp,sp,-16
    8000027a:	e406                	sd	ra,8(sp)
    8000027c:	e022                	sd	s0,0(sp)
    8000027e:	0800                	addi	s0,sp,16
  if(c == BACKSPACE){
    80000280:	10000793          	li	a5,256
    80000284:	00f50863          	beq	a0,a5,80000294 <consputc+0x1c>
    uartputc_sync(c);
    80000288:	6a4000ef          	jal	8000092c <uartputc_sync>
}
    8000028c:	60a2                	ld	ra,8(sp)
    8000028e:	6402                	ld	s0,0(sp)
    80000290:	0141                	addi	sp,sp,16
    80000292:	8082                	ret
    uartputc_sync('\b'); uartputc_sync(' '); uartputc_sync('\b');
    80000294:	4521                	li	a0,8
    80000296:	696000ef          	jal	8000092c <uartputc_sync>
    8000029a:	02000513          	li	a0,32
    8000029e:	68e000ef          	jal	8000092c <uartputc_sync>
    800002a2:	4521                	li	a0,8
    800002a4:	688000ef          	jal	8000092c <uartputc_sync>
    800002a8:	b7d5                	j	8000028c <consputc+0x14>

00000000800002aa <consoleintr>:
// do erase/kill processing, append to cons.buf,
// wake up consoleread() if a whole line has arrived.
//
void
consoleintr(int c)
{
    800002aa:	1101                	addi	sp,sp,-32
    800002ac:	ec06                	sd	ra,24(sp)
    800002ae:	e822                	sd	s0,16(sp)
    800002b0:	e426                	sd	s1,8(sp)
    800002b2:	1000                	addi	s0,sp,32
    800002b4:	84aa                	mv	s1,a0
  acquire(&cons.lock);
    800002b6:	0000f517          	auipc	a0,0xf
    800002ba:	5ba50513          	addi	a0,a0,1466 # 8000f870 <cons>
    800002be:	111000ef          	jal	80000bce <acquire>

  switch(c){
    800002c2:	47d5                	li	a5,21
    800002c4:	08f48f63          	beq	s1,a5,80000362 <consoleintr+0xb8>
    800002c8:	0297c563          	blt	a5,s1,800002f2 <consoleintr+0x48>
    800002cc:	47a1                	li	a5,8
    800002ce:	0ef48463          	beq	s1,a5,800003b6 <consoleintr+0x10c>
    800002d2:	47c1                	li	a5,16
    800002d4:	10f49563          	bne	s1,a5,800003de <consoleintr+0x134>
  case C('P'):  // Print process list.
    procdump();
    800002d8:	044020ef          	jal	8000231c <procdump>
      }
    }
    break;
  }
  
  release(&cons.lock);
    800002dc:	0000f517          	auipc	a0,0xf
    800002e0:	59450513          	addi	a0,a0,1428 # 8000f870 <cons>
    800002e4:	183000ef          	jal	80000c66 <release>
}
    800002e8:	60e2                	ld	ra,24(sp)
    800002ea:	6442                	ld	s0,16(sp)
    800002ec:	64a2                	ld	s1,8(sp)
    800002ee:	6105                	addi	sp,sp,32
    800002f0:	8082                	ret
  switch(c){
    800002f2:	07f00793          	li	a5,127
    800002f6:	0cf48063          	beq	s1,a5,800003b6 <consoleintr+0x10c>
    if(c != 0 && cons.e-cons.r < INPUT_BUF_SIZE){
    800002fa:	0000f717          	auipc	a4,0xf
    800002fe:	57670713          	addi	a4,a4,1398 # 8000f870 <cons>
    80000302:	0a072783          	lw	a5,160(a4)
    80000306:	09872703          	lw	a4,152(a4)
    8000030a:	9f99                	subw	a5,a5,a4
    8000030c:	07f00713          	li	a4,127
    80000310:	fcf766e3          	bltu	a4,a5,800002dc <consoleintr+0x32>
      c = (c == '\r') ? '\n' : c;
    80000314:	47b5                	li	a5,13
    80000316:	0cf48763          	beq	s1,a5,800003e4 <consoleintr+0x13a>
      consputc(c);
    8000031a:	8526                	mv	a0,s1
    8000031c:	f5dff0ef          	jal	80000278 <consputc>
      cons.buf[cons.e++ % INPUT_BUF_SIZE] = c;
    80000320:	0000f797          	auipc	a5,0xf
    80000324:	55078793          	addi	a5,a5,1360 # 8000f870 <cons>
    80000328:	0a07a683          	lw	a3,160(a5)
    8000032c:	0016871b          	addiw	a4,a3,1
    80000330:	0007061b          	sext.w	a2,a4
    80000334:	0ae7a023          	sw	a4,160(a5)
    80000338:	07f6f693          	andi	a3,a3,127
    8000033c:	97b6                	add	a5,a5,a3
    8000033e:	00978c23          	sb	s1,24(a5)
      if(c == '\n' || c == C('D') || cons.e-cons.r == INPUT_BUF_SIZE){
    80000342:	47a9                	li	a5,10
    80000344:	0cf48563          	beq	s1,a5,8000040e <consoleintr+0x164>
    80000348:	4791                	li	a5,4
    8000034a:	0cf48263          	beq	s1,a5,8000040e <consoleintr+0x164>
    8000034e:	0000f797          	auipc	a5,0xf
    80000352:	5ba7a783          	lw	a5,1466(a5) # 8000f908 <cons+0x98>
    80000356:	9f1d                	subw	a4,a4,a5
    80000358:	08000793          	li	a5,128
    8000035c:	f8f710e3          	bne	a4,a5,800002dc <consoleintr+0x32>
    80000360:	a07d                	j	8000040e <consoleintr+0x164>
    80000362:	e04a                	sd	s2,0(sp)
    while(cons.e != cons.w &&
    80000364:	0000f717          	auipc	a4,0xf
    80000368:	50c70713          	addi	a4,a4,1292 # 8000f870 <cons>
    8000036c:	0a072783          	lw	a5,160(a4)
    80000370:	09c72703          	lw	a4,156(a4)
          cons.buf[(cons.e-1) % INPUT_BUF_SIZE] != '\n'){
    80000374:	0000f497          	auipc	s1,0xf
    80000378:	4fc48493          	addi	s1,s1,1276 # 8000f870 <cons>
    while(cons.e != cons.w &&
    8000037c:	4929                	li	s2,10
    8000037e:	02f70863          	beq	a4,a5,800003ae <consoleintr+0x104>
          cons.buf[(cons.e-1) % INPUT_BUF_SIZE] != '\n'){
    80000382:	37fd                	addiw	a5,a5,-1
    80000384:	07f7f713          	andi	a4,a5,127
    80000388:	9726                	add	a4,a4,s1
    while(cons.e != cons.w &&
    8000038a:	01874703          	lbu	a4,24(a4)
    8000038e:	03270263          	beq	a4,s2,800003b2 <consoleintr+0x108>
      cons.e--;
    80000392:	0af4a023          	sw	a5,160(s1)
      consputc(BACKSPACE);
    80000396:	10000513          	li	a0,256
    8000039a:	edfff0ef          	jal	80000278 <consputc>
    while(cons.e != cons.w &&
    8000039e:	0a04a783          	lw	a5,160(s1)
    800003a2:	09c4a703          	lw	a4,156(s1)
    800003a6:	fcf71ee3          	bne	a4,a5,80000382 <consoleintr+0xd8>
    800003aa:	6902                	ld	s2,0(sp)
    800003ac:	bf05                	j	800002dc <consoleintr+0x32>
    800003ae:	6902                	ld	s2,0(sp)
    800003b0:	b735                	j	800002dc <consoleintr+0x32>
    800003b2:	6902                	ld	s2,0(sp)
    800003b4:	b725                	j	800002dc <consoleintr+0x32>
    if(cons.e != cons.w){
    800003b6:	0000f717          	auipc	a4,0xf
    800003ba:	4ba70713          	addi	a4,a4,1210 # 8000f870 <cons>
    800003be:	0a072783          	lw	a5,160(a4)
    800003c2:	09c72703          	lw	a4,156(a4)
    800003c6:	f0f70be3          	beq	a4,a5,800002dc <consoleintr+0x32>
      cons.e--;
    800003ca:	37fd                	addiw	a5,a5,-1
    800003cc:	0000f717          	auipc	a4,0xf
    800003d0:	54f72223          	sw	a5,1348(a4) # 8000f910 <cons+0xa0>
      consputc(BACKSPACE);
    800003d4:	10000513          	li	a0,256
    800003d8:	ea1ff0ef          	jal	80000278 <consputc>
    800003dc:	b701                	j	800002dc <consoleintr+0x32>
    if(c != 0 && cons.e-cons.r < INPUT_BUF_SIZE){
    800003de:	ee048fe3          	beqz	s1,800002dc <consoleintr+0x32>
    800003e2:	bf21                	j	800002fa <consoleintr+0x50>
      consputc(c);
    800003e4:	4529                	li	a0,10
    800003e6:	e93ff0ef          	jal	80000278 <consputc>
      cons.buf[cons.e++ % INPUT_BUF_SIZE] = c;
    800003ea:	0000f797          	auipc	a5,0xf
    800003ee:	48678793          	addi	a5,a5,1158 # 8000f870 <cons>
    800003f2:	0a07a703          	lw	a4,160(a5)
    800003f6:	0017069b          	addiw	a3,a4,1
    800003fa:	0006861b          	sext.w	a2,a3
    800003fe:	0ad7a023          	sw	a3,160(a5)
    80000402:	07f77713          	andi	a4,a4,127
    80000406:	97ba                	add	a5,a5,a4
    80000408:	4729                	li	a4,10
    8000040a:	00e78c23          	sb	a4,24(a5)
        cons.w = cons.e;
    8000040e:	0000f797          	auipc	a5,0xf
    80000412:	4ec7af23          	sw	a2,1278(a5) # 8000f90c <cons+0x9c>
        wakeup(&cons.r);
    80000416:	0000f517          	auipc	a0,0xf
    8000041a:	4f250513          	addi	a0,a0,1266 # 8000f908 <cons+0x98>
    8000041e:	35b010ef          	jal	80001f78 <wakeup>
    80000422:	bd6d                	j	800002dc <consoleintr+0x32>

0000000080000424 <consoleinit>:

void
consoleinit(void)
{
    80000424:	1141                	addi	sp,sp,-16
    80000426:	e406                	sd	ra,8(sp)
    80000428:	e022                	sd	s0,0(sp)
    8000042a:	0800                	addi	s0,sp,16
  initlock(&cons.lock, "cons");
    8000042c:	00007597          	auipc	a1,0x7
    80000430:	bd458593          	addi	a1,a1,-1068 # 80007000 <etext>
    80000434:	0000f517          	auipc	a0,0xf
    80000438:	43c50513          	addi	a0,a0,1084 # 8000f870 <cons>
    8000043c:	712000ef          	jal	80000b4e <initlock>

  uartinit();
    80000440:	400000ef          	jal	80000840 <uartinit>

  // connect read and write system calls
  // to consoleread and consolewrite.
  devsw[CONSOLE].read = consoleread;
    80000444:	0001f797          	auipc	a5,0x1f
    80000448:	79c78793          	addi	a5,a5,1948 # 8001fbe0 <devsw>
    8000044c:	00000717          	auipc	a4,0x0
    80000450:	d2270713          	addi	a4,a4,-734 # 8000016e <consoleread>
    80000454:	eb98                	sd	a4,16(a5)
  devsw[CONSOLE].write = consolewrite;
    80000456:	00000717          	auipc	a4,0x0
    8000045a:	c7a70713          	addi	a4,a4,-902 # 800000d0 <consolewrite>
    8000045e:	ef98                	sd	a4,24(a5)
}
    80000460:	60a2                	ld	ra,8(sp)
    80000462:	6402                	ld	s0,0(sp)
    80000464:	0141                	addi	sp,sp,16
    80000466:	8082                	ret

0000000080000468 <printint>:

static char digits[] = "0123456789abcdef";

static void
printint(long long xx, int base, int sign)
{
    80000468:	7139                	addi	sp,sp,-64
    8000046a:	fc06                	sd	ra,56(sp)
    8000046c:	f822                	sd	s0,48(sp)
    8000046e:	0080                	addi	s0,sp,64
  char buf[20];
  int i;
  unsigned long long x;

  if(sign && (sign = (xx < 0)))
    80000470:	c219                	beqz	a2,80000476 <printint+0xe>
    80000472:	08054063          	bltz	a0,800004f2 <printint+0x8a>
    x = -xx;
  else
    x = xx;
    80000476:	4881                	li	a7,0
    80000478:	fc840693          	addi	a3,s0,-56

  i = 0;
    8000047c:	4781                	li	a5,0
  do {
    buf[i++] = digits[x % base];
    8000047e:	00007617          	auipc	a2,0x7
    80000482:	29260613          	addi	a2,a2,658 # 80007710 <digits>
    80000486:	883e                	mv	a6,a5
    80000488:	2785                	addiw	a5,a5,1
    8000048a:	02b57733          	remu	a4,a0,a1
    8000048e:	9732                	add	a4,a4,a2
    80000490:	00074703          	lbu	a4,0(a4)
    80000494:	00e68023          	sb	a4,0(a3)
  } while((x /= base) != 0);
    80000498:	872a                	mv	a4,a0
    8000049a:	02b55533          	divu	a0,a0,a1
    8000049e:	0685                	addi	a3,a3,1
    800004a0:	feb773e3          	bgeu	a4,a1,80000486 <printint+0x1e>

  if(sign)
    800004a4:	00088a63          	beqz	a7,800004b8 <printint+0x50>
    buf[i++] = '-';
    800004a8:	1781                	addi	a5,a5,-32
    800004aa:	97a2                	add	a5,a5,s0
    800004ac:	02d00713          	li	a4,45
    800004b0:	fee78423          	sb	a4,-24(a5)
    800004b4:	0028079b          	addiw	a5,a6,2

  while(--i >= 0)
    800004b8:	02f05963          	blez	a5,800004ea <printint+0x82>
    800004bc:	f426                	sd	s1,40(sp)
    800004be:	f04a                	sd	s2,32(sp)
    800004c0:	fc840713          	addi	a4,s0,-56
    800004c4:	00f704b3          	add	s1,a4,a5
    800004c8:	fff70913          	addi	s2,a4,-1
    800004cc:	993e                	add	s2,s2,a5
    800004ce:	37fd                	addiw	a5,a5,-1
    800004d0:	1782                	slli	a5,a5,0x20
    800004d2:	9381                	srli	a5,a5,0x20
    800004d4:	40f90933          	sub	s2,s2,a5
    consputc(buf[i]);
    800004d8:	fff4c503          	lbu	a0,-1(s1)
    800004dc:	d9dff0ef          	jal	80000278 <consputc>
  while(--i >= 0)
    800004e0:	14fd                	addi	s1,s1,-1
    800004e2:	ff249be3          	bne	s1,s2,800004d8 <printint+0x70>
    800004e6:	74a2                	ld	s1,40(sp)
    800004e8:	7902                	ld	s2,32(sp)
}
    800004ea:	70e2                	ld	ra,56(sp)
    800004ec:	7442                	ld	s0,48(sp)
    800004ee:	6121                	addi	sp,sp,64
    800004f0:	8082                	ret
    x = -xx;
    800004f2:	40a00533          	neg	a0,a0
  if(sign && (sign = (xx < 0)))
    800004f6:	4885                	li	a7,1
    x = -xx;
    800004f8:	b741                	j	80000478 <printint+0x10>

00000000800004fa <printf>:
}

// Print to the console.
int
printf(char *fmt, ...)
{
    800004fa:	7131                	addi	sp,sp,-192
    800004fc:	fc86                	sd	ra,120(sp)
    800004fe:	f8a2                	sd	s0,112(sp)
    80000500:	e8d2                	sd	s4,80(sp)
    80000502:	0100                	addi	s0,sp,128
    80000504:	8a2a                	mv	s4,a0
    80000506:	e40c                	sd	a1,8(s0)
    80000508:	e810                	sd	a2,16(s0)
    8000050a:	ec14                	sd	a3,24(s0)
    8000050c:	f018                	sd	a4,32(s0)
    8000050e:	f41c                	sd	a5,40(s0)
    80000510:	03043823          	sd	a6,48(s0)
    80000514:	03143c23          	sd	a7,56(s0)
  va_list ap;
  int i, cx, c0, c1, c2;
  char *s;

  if(panicking == 0)
    80000518:	00007797          	auipc	a5,0x7
    8000051c:	32c7a783          	lw	a5,812(a5) # 80007844 <panicking>
    80000520:	c3a1                	beqz	a5,80000560 <printf+0x66>
    acquire(&pr.lock);

  va_start(ap, fmt);
    80000522:	00840793          	addi	a5,s0,8
    80000526:	f8f43423          	sd	a5,-120(s0)
  for(i = 0; (cx = fmt[i] & 0xff) != 0; i++){
    8000052a:	000a4503          	lbu	a0,0(s4)
    8000052e:	28050763          	beqz	a0,800007bc <printf+0x2c2>
    80000532:	f4a6                	sd	s1,104(sp)
    80000534:	f0ca                	sd	s2,96(sp)
    80000536:	ecce                	sd	s3,88(sp)
    80000538:	e4d6                	sd	s5,72(sp)
    8000053a:	e0da                	sd	s6,64(sp)
    8000053c:	f862                	sd	s8,48(sp)
    8000053e:	f466                	sd	s9,40(sp)
    80000540:	f06a                	sd	s10,32(sp)
    80000542:	ec6e                	sd	s11,24(sp)
    80000544:	4981                	li	s3,0
    if(cx != '%'){
    80000546:	02500a93          	li	s5,37
    i++;
    c0 = fmt[i+0] & 0xff;
    c1 = c2 = 0;
    if(c0) c1 = fmt[i+1] & 0xff;
    if(c1) c2 = fmt[i+2] & 0xff;
    if(c0 == 'd'){
    8000054a:	06400b13          	li	s6,100
      printint(va_arg(ap, int), 10, 1);
    } else if(c0 == 'l' && c1 == 'd'){
    8000054e:	06c00c13          	li	s8,108
      printint(va_arg(ap, uint64), 10, 1);
      i += 1;
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'd'){
      printint(va_arg(ap, uint64), 10, 1);
      i += 2;
    } else if(c0 == 'u'){
    80000552:	07500c93          	li	s9,117
      printint(va_arg(ap, uint64), 10, 0);
      i += 1;
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'u'){
      printint(va_arg(ap, uint64), 10, 0);
      i += 2;
    } else if(c0 == 'x'){
    80000556:	07800d13          	li	s10,120
      printint(va_arg(ap, uint64), 16, 0);
      i += 1;
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'x'){
      printint(va_arg(ap, uint64), 16, 0);
      i += 2;
    } else if(c0 == 'p'){
    8000055a:	07000d93          	li	s11,112
    8000055e:	a01d                	j	80000584 <printf+0x8a>
    acquire(&pr.lock);
    80000560:	0000f517          	auipc	a0,0xf
    80000564:	3b850513          	addi	a0,a0,952 # 8000f918 <pr>
    80000568:	666000ef          	jal	80000bce <acquire>
    8000056c:	bf5d                	j	80000522 <printf+0x28>
      consputc(cx);
    8000056e:	d0bff0ef          	jal	80000278 <consputc>
      continue;
    80000572:	84ce                	mv	s1,s3
  for(i = 0; (cx = fmt[i] & 0xff) != 0; i++){
    80000574:	0014899b          	addiw	s3,s1,1
    80000578:	013a07b3          	add	a5,s4,s3
    8000057c:	0007c503          	lbu	a0,0(a5)
    80000580:	20050b63          	beqz	a0,80000796 <printf+0x29c>
    if(cx != '%'){
    80000584:	ff5515e3          	bne	a0,s5,8000056e <printf+0x74>
    i++;
    80000588:	0019849b          	addiw	s1,s3,1
    c0 = fmt[i+0] & 0xff;
    8000058c:	009a07b3          	add	a5,s4,s1
    80000590:	0007c903          	lbu	s2,0(a5)
    if(c0) c1 = fmt[i+1] & 0xff;
    80000594:	20090b63          	beqz	s2,800007aa <printf+0x2b0>
    80000598:	0017c783          	lbu	a5,1(a5)
    c1 = c2 = 0;
    8000059c:	86be                	mv	a3,a5
    if(c1) c2 = fmt[i+2] & 0xff;
    8000059e:	c789                	beqz	a5,800005a8 <printf+0xae>
    800005a0:	009a0733          	add	a4,s4,s1
    800005a4:	00274683          	lbu	a3,2(a4)
    if(c0 == 'd'){
    800005a8:	03690963          	beq	s2,s6,800005da <printf+0xe0>
    } else if(c0 == 'l' && c1 == 'd'){
    800005ac:	05890363          	beq	s2,s8,800005f2 <printf+0xf8>
    } else if(c0 == 'u'){
    800005b0:	0d990663          	beq	s2,s9,8000067c <printf+0x182>
    } else if(c0 == 'x'){
    800005b4:	11a90d63          	beq	s2,s10,800006ce <printf+0x1d4>
    } else if(c0 == 'p'){
    800005b8:	15b90663          	beq	s2,s11,80000704 <printf+0x20a>
      printptr(va_arg(ap, uint64));
    } else if(c0 == 'c'){
    800005bc:	06300793          	li	a5,99
    800005c0:	18f90563          	beq	s2,a5,8000074a <printf+0x250>
      consputc(va_arg(ap, uint));
    } else if(c0 == 's'){
    800005c4:	07300793          	li	a5,115
    800005c8:	18f90b63          	beq	s2,a5,8000075e <printf+0x264>
      if((s = va_arg(ap, char*)) == 0)
        s = "(null)";
      for(; *s; s++)
        consputc(*s);
    } else if(c0 == '%'){
    800005cc:	03591b63          	bne	s2,s5,80000602 <printf+0x108>
      consputc('%');
    800005d0:	02500513          	li	a0,37
    800005d4:	ca5ff0ef          	jal	80000278 <consputc>
    800005d8:	bf71                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, int), 10, 1);
    800005da:	f8843783          	ld	a5,-120(s0)
    800005de:	00878713          	addi	a4,a5,8
    800005e2:	f8e43423          	sd	a4,-120(s0)
    800005e6:	4605                	li	a2,1
    800005e8:	45a9                	li	a1,10
    800005ea:	4388                	lw	a0,0(a5)
    800005ec:	e7dff0ef          	jal	80000468 <printint>
    800005f0:	b751                	j	80000574 <printf+0x7a>
    } else if(c0 == 'l' && c1 == 'd'){
    800005f2:	01678f63          	beq	a5,s6,80000610 <printf+0x116>
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'd'){
    800005f6:	03878b63          	beq	a5,s8,8000062c <printf+0x132>
    } else if(c0 == 'l' && c1 == 'u'){
    800005fa:	09978e63          	beq	a5,s9,80000696 <printf+0x19c>
    } else if(c0 == 'l' && c1 == 'x'){
    800005fe:	0fa78563          	beq	a5,s10,800006e8 <printf+0x1ee>
    } else if(c0 == 0){
      break;
    } else {
      // Print unknown % sequence to draw attention.
      consputc('%');
    80000602:	8556                	mv	a0,s5
    80000604:	c75ff0ef          	jal	80000278 <consputc>
      consputc(c0);
    80000608:	854a                	mv	a0,s2
    8000060a:	c6fff0ef          	jal	80000278 <consputc>
    8000060e:	b79d                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 10, 1);
    80000610:	f8843783          	ld	a5,-120(s0)
    80000614:	00878713          	addi	a4,a5,8
    80000618:	f8e43423          	sd	a4,-120(s0)
    8000061c:	4605                	li	a2,1
    8000061e:	45a9                	li	a1,10
    80000620:	6388                	ld	a0,0(a5)
    80000622:	e47ff0ef          	jal	80000468 <printint>
      i += 1;
    80000626:	0029849b          	addiw	s1,s3,2
    8000062a:	b7a9                	j	80000574 <printf+0x7a>
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'd'){
    8000062c:	06400793          	li	a5,100
    80000630:	02f68863          	beq	a3,a5,80000660 <printf+0x166>
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'u'){
    80000634:	07500793          	li	a5,117
    80000638:	06f68d63          	beq	a3,a5,800006b2 <printf+0x1b8>
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'x'){
    8000063c:	07800793          	li	a5,120
    80000640:	fcf691e3          	bne	a3,a5,80000602 <printf+0x108>
      printint(va_arg(ap, uint64), 16, 0);
    80000644:	f8843783          	ld	a5,-120(s0)
    80000648:	00878713          	addi	a4,a5,8
    8000064c:	f8e43423          	sd	a4,-120(s0)
    80000650:	4601                	li	a2,0
    80000652:	45c1                	li	a1,16
    80000654:	6388                	ld	a0,0(a5)
    80000656:	e13ff0ef          	jal	80000468 <printint>
      i += 2;
    8000065a:	0039849b          	addiw	s1,s3,3
    8000065e:	bf19                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 10, 1);
    80000660:	f8843783          	ld	a5,-120(s0)
    80000664:	00878713          	addi	a4,a5,8
    80000668:	f8e43423          	sd	a4,-120(s0)
    8000066c:	4605                	li	a2,1
    8000066e:	45a9                	li	a1,10
    80000670:	6388                	ld	a0,0(a5)
    80000672:	df7ff0ef          	jal	80000468 <printint>
      i += 2;
    80000676:	0039849b          	addiw	s1,s3,3
    8000067a:	bded                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint32), 10, 0);
    8000067c:	f8843783          	ld	a5,-120(s0)
    80000680:	00878713          	addi	a4,a5,8
    80000684:	f8e43423          	sd	a4,-120(s0)
    80000688:	4601                	li	a2,0
    8000068a:	45a9                	li	a1,10
    8000068c:	0007e503          	lwu	a0,0(a5)
    80000690:	dd9ff0ef          	jal	80000468 <printint>
    80000694:	b5c5                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 10, 0);
    80000696:	f8843783          	ld	a5,-120(s0)
    8000069a:	00878713          	addi	a4,a5,8
    8000069e:	f8e43423          	sd	a4,-120(s0)
    800006a2:	4601                	li	a2,0
    800006a4:	45a9                	li	a1,10
    800006a6:	6388                	ld	a0,0(a5)
    800006a8:	dc1ff0ef          	jal	80000468 <printint>
      i += 1;
    800006ac:	0029849b          	addiw	s1,s3,2
    800006b0:	b5d1                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 10, 0);
    800006b2:	f8843783          	ld	a5,-120(s0)
    800006b6:	00878713          	addi	a4,a5,8
    800006ba:	f8e43423          	sd	a4,-120(s0)
    800006be:	4601                	li	a2,0
    800006c0:	45a9                	li	a1,10
    800006c2:	6388                	ld	a0,0(a5)
    800006c4:	da5ff0ef          	jal	80000468 <printint>
      i += 2;
    800006c8:	0039849b          	addiw	s1,s3,3
    800006cc:	b565                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint32), 16, 0);
    800006ce:	f8843783          	ld	a5,-120(s0)
    800006d2:	00878713          	addi	a4,a5,8
    800006d6:	f8e43423          	sd	a4,-120(s0)
    800006da:	4601                	li	a2,0
    800006dc:	45c1                	li	a1,16
    800006de:	0007e503          	lwu	a0,0(a5)
    800006e2:	d87ff0ef          	jal	80000468 <printint>
    800006e6:	b579                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 16, 0);
    800006e8:	f8843783          	ld	a5,-120(s0)
    800006ec:	00878713          	addi	a4,a5,8
    800006f0:	f8e43423          	sd	a4,-120(s0)
    800006f4:	4601                	li	a2,0
    800006f6:	45c1                	li	a1,16
    800006f8:	6388                	ld	a0,0(a5)
    800006fa:	d6fff0ef          	jal	80000468 <printint>
      i += 1;
    800006fe:	0029849b          	addiw	s1,s3,2
    80000702:	bd8d                	j	80000574 <printf+0x7a>
    80000704:	fc5e                	sd	s7,56(sp)
      printptr(va_arg(ap, uint64));
    80000706:	f8843783          	ld	a5,-120(s0)
    8000070a:	00878713          	addi	a4,a5,8
    8000070e:	f8e43423          	sd	a4,-120(s0)
    80000712:	0007b983          	ld	s3,0(a5)
  consputc('0');
    80000716:	03000513          	li	a0,48
    8000071a:	b5fff0ef          	jal	80000278 <consputc>
  consputc('x');
    8000071e:	07800513          	li	a0,120
    80000722:	b57ff0ef          	jal	80000278 <consputc>
    80000726:	4941                	li	s2,16
    consputc(digits[x >> (sizeof(uint64) * 8 - 4)]);
    80000728:	00007b97          	auipc	s7,0x7
    8000072c:	fe8b8b93          	addi	s7,s7,-24 # 80007710 <digits>
    80000730:	03c9d793          	srli	a5,s3,0x3c
    80000734:	97de                	add	a5,a5,s7
    80000736:	0007c503          	lbu	a0,0(a5)
    8000073a:	b3fff0ef          	jal	80000278 <consputc>
  for (i = 0; i < (sizeof(uint64) * 2); i++, x <<= 4)
    8000073e:	0992                	slli	s3,s3,0x4
    80000740:	397d                	addiw	s2,s2,-1
    80000742:	fe0917e3          	bnez	s2,80000730 <printf+0x236>
    80000746:	7be2                	ld	s7,56(sp)
    80000748:	b535                	j	80000574 <printf+0x7a>
      consputc(va_arg(ap, uint));
    8000074a:	f8843783          	ld	a5,-120(s0)
    8000074e:	00878713          	addi	a4,a5,8
    80000752:	f8e43423          	sd	a4,-120(s0)
    80000756:	4388                	lw	a0,0(a5)
    80000758:	b21ff0ef          	jal	80000278 <consputc>
    8000075c:	bd21                	j	80000574 <printf+0x7a>
      if((s = va_arg(ap, char*)) == 0)
    8000075e:	f8843783          	ld	a5,-120(s0)
    80000762:	00878713          	addi	a4,a5,8
    80000766:	f8e43423          	sd	a4,-120(s0)
    8000076a:	0007b903          	ld	s2,0(a5)
    8000076e:	00090d63          	beqz	s2,80000788 <printf+0x28e>
      for(; *s; s++)
    80000772:	00094503          	lbu	a0,0(s2)
    80000776:	de050fe3          	beqz	a0,80000574 <printf+0x7a>
        consputc(*s);
    8000077a:	affff0ef          	jal	80000278 <consputc>
      for(; *s; s++)
    8000077e:	0905                	addi	s2,s2,1
    80000780:	00094503          	lbu	a0,0(s2)
    80000784:	f97d                	bnez	a0,8000077a <printf+0x280>
    80000786:	b3fd                	j	80000574 <printf+0x7a>
        s = "(null)";
    80000788:	00007917          	auipc	s2,0x7
    8000078c:	88090913          	addi	s2,s2,-1920 # 80007008 <etext+0x8>
      for(; *s; s++)
    80000790:	02800513          	li	a0,40
    80000794:	b7dd                	j	8000077a <printf+0x280>
    80000796:	74a6                	ld	s1,104(sp)
    80000798:	7906                	ld	s2,96(sp)
    8000079a:	69e6                	ld	s3,88(sp)
    8000079c:	6aa6                	ld	s5,72(sp)
    8000079e:	6b06                	ld	s6,64(sp)
    800007a0:	7c42                	ld	s8,48(sp)
    800007a2:	7ca2                	ld	s9,40(sp)
    800007a4:	7d02                	ld	s10,32(sp)
    800007a6:	6de2                	ld	s11,24(sp)
    800007a8:	a811                	j	800007bc <printf+0x2c2>
    800007aa:	74a6                	ld	s1,104(sp)
    800007ac:	7906                	ld	s2,96(sp)
    800007ae:	69e6                	ld	s3,88(sp)
    800007b0:	6aa6                	ld	s5,72(sp)
    800007b2:	6b06                	ld	s6,64(sp)
    800007b4:	7c42                	ld	s8,48(sp)
    800007b6:	7ca2                	ld	s9,40(sp)
    800007b8:	7d02                	ld	s10,32(sp)
    800007ba:	6de2                	ld	s11,24(sp)
    }

  }
  va_end(ap);

  if(panicking == 0)
    800007bc:	00007797          	auipc	a5,0x7
    800007c0:	0887a783          	lw	a5,136(a5) # 80007844 <panicking>
    800007c4:	c799                	beqz	a5,800007d2 <printf+0x2d8>
    release(&pr.lock);

  return 0;
}
    800007c6:	4501                	li	a0,0
    800007c8:	70e6                	ld	ra,120(sp)
    800007ca:	7446                	ld	s0,112(sp)
    800007cc:	6a46                	ld	s4,80(sp)
    800007ce:	6129                	addi	sp,sp,192
    800007d0:	8082                	ret
    release(&pr.lock);
    800007d2:	0000f517          	auipc	a0,0xf
    800007d6:	14650513          	addi	a0,a0,326 # 8000f918 <pr>
    800007da:	48c000ef          	jal	80000c66 <release>
  return 0;
    800007de:	b7e5                	j	800007c6 <printf+0x2cc>

00000000800007e0 <panic>:

void
panic(char *s)
{
    800007e0:	1101                	addi	sp,sp,-32
    800007e2:	ec06                	sd	ra,24(sp)
    800007e4:	e822                	sd	s0,16(sp)
    800007e6:	e426                	sd	s1,8(sp)
    800007e8:	e04a                	sd	s2,0(sp)
    800007ea:	1000                	addi	s0,sp,32
    800007ec:	84aa                	mv	s1,a0
  panicking = 1;
    800007ee:	4905                	li	s2,1
    800007f0:	00007797          	auipc	a5,0x7
    800007f4:	0527aa23          	sw	s2,84(a5) # 80007844 <panicking>
  printf("panic: ");
    800007f8:	00007517          	auipc	a0,0x7
    800007fc:	82050513          	addi	a0,a0,-2016 # 80007018 <etext+0x18>
    80000800:	cfbff0ef          	jal	800004fa <printf>
  printf("%s\n", s);
    80000804:	85a6                	mv	a1,s1
    80000806:	00007517          	auipc	a0,0x7
    8000080a:	81a50513          	addi	a0,a0,-2022 # 80007020 <etext+0x20>
    8000080e:	cedff0ef          	jal	800004fa <printf>
  panicked = 1; // freeze uart output from other CPUs
    80000812:	00007797          	auipc	a5,0x7
    80000816:	0327a723          	sw	s2,46(a5) # 80007840 <panicked>
  for(;;)
    8000081a:	a001                	j	8000081a <panic+0x3a>

000000008000081c <printfinit>:
    ;
}

void
printfinit(void)
{
    8000081c:	1141                	addi	sp,sp,-16
    8000081e:	e406                	sd	ra,8(sp)
    80000820:	e022                	sd	s0,0(sp)
    80000822:	0800                	addi	s0,sp,16
  initlock(&pr.lock, "pr");
    80000824:	00007597          	auipc	a1,0x7
    80000828:	80458593          	addi	a1,a1,-2044 # 80007028 <etext+0x28>
    8000082c:	0000f517          	auipc	a0,0xf
    80000830:	0ec50513          	addi	a0,a0,236 # 8000f918 <pr>
    80000834:	31a000ef          	jal	80000b4e <initlock>
}
    80000838:	60a2                	ld	ra,8(sp)
    8000083a:	6402                	ld	s0,0(sp)
    8000083c:	0141                	addi	sp,sp,16
    8000083e:	8082                	ret

0000000080000840 <uartinit>:
extern volatile int panicking; // from printf.c
extern volatile int panicked; // from printf.c

void
uartinit(void)
{
    80000840:	1141                	addi	sp,sp,-16
    80000842:	e406                	sd	ra,8(sp)
    80000844:	e022                	sd	s0,0(sp)
    80000846:	0800                	addi	s0,sp,16
  // disable interrupts.
  WriteReg(IER, 0x00);
    80000848:	100007b7          	lui	a5,0x10000
    8000084c:	000780a3          	sb	zero,1(a5) # 10000001 <_entry-0x6fffffff>

  // special mode to set baud rate.
  WriteReg(LCR, LCR_BAUD_LATCH);
    80000850:	10000737          	lui	a4,0x10000
    80000854:	f8000693          	li	a3,-128
    80000858:	00d701a3          	sb	a3,3(a4) # 10000003 <_entry-0x6ffffffd>

  // LSB for baud rate of 38.4K.
  WriteReg(0, 0x03);
    8000085c:	468d                	li	a3,3
    8000085e:	10000637          	lui	a2,0x10000
    80000862:	00d60023          	sb	a3,0(a2) # 10000000 <_entry-0x70000000>

  // MSB for baud rate of 38.4K.
  WriteReg(1, 0x00);
    80000866:	000780a3          	sb	zero,1(a5)

  // leave set-baud mode,
  // and set word length to 8 bits, no parity.
  WriteReg(LCR, LCR_EIGHT_BITS);
    8000086a:	00d701a3          	sb	a3,3(a4)

  // reset and enable FIFOs.
  WriteReg(FCR, FCR_FIFO_ENABLE | FCR_FIFO_CLEAR);
    8000086e:	10000737          	lui	a4,0x10000
    80000872:	461d                	li	a2,7
    80000874:	00c70123          	sb	a2,2(a4) # 10000002 <_entry-0x6ffffffe>

  // enable transmit and receive interrupts.
  WriteReg(IER, IER_TX_ENABLE | IER_RX_ENABLE);
    80000878:	00d780a3          	sb	a3,1(a5)

  initlock(&tx_lock, "uart");
    8000087c:	00006597          	auipc	a1,0x6
    80000880:	7b458593          	addi	a1,a1,1972 # 80007030 <etext+0x30>
    80000884:	0000f517          	auipc	a0,0xf
    80000888:	0ac50513          	addi	a0,a0,172 # 8000f930 <tx_lock>
    8000088c:	2c2000ef          	jal	80000b4e <initlock>
}
    80000890:	60a2                	ld	ra,8(sp)
    80000892:	6402                	ld	s0,0(sp)
    80000894:	0141                	addi	sp,sp,16
    80000896:	8082                	ret

0000000080000898 <uartwrite>:
// transmit buf[] to the uart. it blocks if the
// uart is busy, so it cannot be called from
// interrupts, only from write() system calls.
void
uartwrite(char buf[], int n)
{
    80000898:	715d                	addi	sp,sp,-80
    8000089a:	e486                	sd	ra,72(sp)
    8000089c:	e0a2                	sd	s0,64(sp)
    8000089e:	fc26                	sd	s1,56(sp)
    800008a0:	ec56                	sd	s5,24(sp)
    800008a2:	0880                	addi	s0,sp,80
    800008a4:	8aaa                	mv	s5,a0
    800008a6:	84ae                	mv	s1,a1
  acquire(&tx_lock);
    800008a8:	0000f517          	auipc	a0,0xf
    800008ac:	08850513          	addi	a0,a0,136 # 8000f930 <tx_lock>
    800008b0:	31e000ef          	jal	80000bce <acquire>

  int i = 0;
  while(i < n){ 
    800008b4:	06905063          	blez	s1,80000914 <uartwrite+0x7c>
    800008b8:	f84a                	sd	s2,48(sp)
    800008ba:	f44e                	sd	s3,40(sp)
    800008bc:	f052                	sd	s4,32(sp)
    800008be:	e85a                	sd	s6,16(sp)
    800008c0:	e45e                	sd	s7,8(sp)
    800008c2:	8a56                	mv	s4,s5
    800008c4:	9aa6                	add	s5,s5,s1
    while(tx_busy != 0){
    800008c6:	00007497          	auipc	s1,0x7
    800008ca:	f8648493          	addi	s1,s1,-122 # 8000784c <tx_busy>
      // wait for a UART transmit-complete interrupt
      // to set tx_busy to 0.
      sleep(&tx_chan, &tx_lock);
    800008ce:	0000f997          	auipc	s3,0xf
    800008d2:	06298993          	addi	s3,s3,98 # 8000f930 <tx_lock>
    800008d6:	00007917          	auipc	s2,0x7
    800008da:	f7290913          	addi	s2,s2,-142 # 80007848 <tx_chan>
    }   
      
    WriteReg(THR, buf[i]);
    800008de:	10000bb7          	lui	s7,0x10000
    i += 1;
    tx_busy = 1;
    800008e2:	4b05                	li	s6,1
    800008e4:	a005                	j	80000904 <uartwrite+0x6c>
      sleep(&tx_chan, &tx_lock);
    800008e6:	85ce                	mv	a1,s3
    800008e8:	854a                	mv	a0,s2
    800008ea:	642010ef          	jal	80001f2c <sleep>
    while(tx_busy != 0){
    800008ee:	409c                	lw	a5,0(s1)
    800008f0:	fbfd                	bnez	a5,800008e6 <uartwrite+0x4e>
    WriteReg(THR, buf[i]);
    800008f2:	000a4783          	lbu	a5,0(s4)
    800008f6:	00fb8023          	sb	a5,0(s7) # 10000000 <_entry-0x70000000>
    tx_busy = 1;
    800008fa:	0164a023          	sw	s6,0(s1)
  while(i < n){ 
    800008fe:	0a05                	addi	s4,s4,1
    80000900:	015a0563          	beq	s4,s5,8000090a <uartwrite+0x72>
    while(tx_busy != 0){
    80000904:	409c                	lw	a5,0(s1)
    80000906:	f3e5                	bnez	a5,800008e6 <uartwrite+0x4e>
    80000908:	b7ed                	j	800008f2 <uartwrite+0x5a>
    8000090a:	7942                	ld	s2,48(sp)
    8000090c:	79a2                	ld	s3,40(sp)
    8000090e:	7a02                	ld	s4,32(sp)
    80000910:	6b42                	ld	s6,16(sp)
    80000912:	6ba2                	ld	s7,8(sp)
  }

  release(&tx_lock);
    80000914:	0000f517          	auipc	a0,0xf
    80000918:	01c50513          	addi	a0,a0,28 # 8000f930 <tx_lock>
    8000091c:	34a000ef          	jal	80000c66 <release>
}
    80000920:	60a6                	ld	ra,72(sp)
    80000922:	6406                	ld	s0,64(sp)
    80000924:	74e2                	ld	s1,56(sp)
    80000926:	6ae2                	ld	s5,24(sp)
    80000928:	6161                	addi	sp,sp,80
    8000092a:	8082                	ret

000000008000092c <uartputc_sync>:
// interrupts, for use by kernel printf() and
// to echo characters. it spins waiting for the uart's
// output register to be empty.
void
uartputc_sync(int c)
{
    8000092c:	1101                	addi	sp,sp,-32
    8000092e:	ec06                	sd	ra,24(sp)
    80000930:	e822                	sd	s0,16(sp)
    80000932:	e426                	sd	s1,8(sp)
    80000934:	1000                	addi	s0,sp,32
    80000936:	84aa                	mv	s1,a0
  if(panicking == 0)
    80000938:	00007797          	auipc	a5,0x7
    8000093c:	f0c7a783          	lw	a5,-244(a5) # 80007844 <panicking>
    80000940:	cf95                	beqz	a5,8000097c <uartputc_sync+0x50>
    push_off();

  if(panicked){
    80000942:	00007797          	auipc	a5,0x7
    80000946:	efe7a783          	lw	a5,-258(a5) # 80007840 <panicked>
    8000094a:	ef85                	bnez	a5,80000982 <uartputc_sync+0x56>
    for(;;)
      ;
  }

  // wait for Transmit Holding Empty to be set in LSR.
  while((ReadReg(LSR) & LSR_TX_IDLE) == 0)
    8000094c:	10000737          	lui	a4,0x10000
    80000950:	0715                	addi	a4,a4,5 # 10000005 <_entry-0x6ffffffb>
    80000952:	00074783          	lbu	a5,0(a4)
    80000956:	0207f793          	andi	a5,a5,32
    8000095a:	dfe5                	beqz	a5,80000952 <uartputc_sync+0x26>
    ;
  WriteReg(THR, c);
    8000095c:	0ff4f513          	zext.b	a0,s1
    80000960:	100007b7          	lui	a5,0x10000
    80000964:	00a78023          	sb	a0,0(a5) # 10000000 <_entry-0x70000000>

  if(panicking == 0)
    80000968:	00007797          	auipc	a5,0x7
    8000096c:	edc7a783          	lw	a5,-292(a5) # 80007844 <panicking>
    80000970:	cb91                	beqz	a5,80000984 <uartputc_sync+0x58>
    pop_off();
}
    80000972:	60e2                	ld	ra,24(sp)
    80000974:	6442                	ld	s0,16(sp)
    80000976:	64a2                	ld	s1,8(sp)
    80000978:	6105                	addi	sp,sp,32
    8000097a:	8082                	ret
    push_off();
    8000097c:	212000ef          	jal	80000b8e <push_off>
    80000980:	b7c9                	j	80000942 <uartputc_sync+0x16>
    for(;;)
    80000982:	a001                	j	80000982 <uartputc_sync+0x56>
    pop_off();
    80000984:	28e000ef          	jal	80000c12 <pop_off>
}
    80000988:	b7ed                	j	80000972 <uartputc_sync+0x46>

000000008000098a <uartgetc>:

// read one input character from the UART.
// return -1 if none is waiting.
int
uartgetc(void)
{
    8000098a:	1141                	addi	sp,sp,-16
    8000098c:	e422                	sd	s0,8(sp)
    8000098e:	0800                	addi	s0,sp,16
  if(ReadReg(LSR) & LSR_RX_READY){
    80000990:	100007b7          	lui	a5,0x10000
    80000994:	0795                	addi	a5,a5,5 # 10000005 <_entry-0x6ffffffb>
    80000996:	0007c783          	lbu	a5,0(a5)
    8000099a:	8b85                	andi	a5,a5,1
    8000099c:	cb81                	beqz	a5,800009ac <uartgetc+0x22>
    // input data is ready.
    return ReadReg(RHR);
    8000099e:	100007b7          	lui	a5,0x10000
    800009a2:	0007c503          	lbu	a0,0(a5) # 10000000 <_entry-0x70000000>
  } else {
    return -1;
  }
}
    800009a6:	6422                	ld	s0,8(sp)
    800009a8:	0141                	addi	sp,sp,16
    800009aa:	8082                	ret
    return -1;
    800009ac:	557d                	li	a0,-1
    800009ae:	bfe5                	j	800009a6 <uartgetc+0x1c>

00000000800009b0 <uartintr>:
// handle a uart interrupt, raised because input has
// arrived, or the uart is ready for more output, or
// both. called from devintr().
void
uartintr(void)
{
    800009b0:	1101                	addi	sp,sp,-32
    800009b2:	ec06                	sd	ra,24(sp)
    800009b4:	e822                	sd	s0,16(sp)
    800009b6:	e426                	sd	s1,8(sp)
    800009b8:	1000                	addi	s0,sp,32
  ReadReg(ISR); // acknowledge the interrupt
    800009ba:	100007b7          	lui	a5,0x10000
    800009be:	0789                	addi	a5,a5,2 # 10000002 <_entry-0x6ffffffe>
    800009c0:	0007c783          	lbu	a5,0(a5)

  acquire(&tx_lock);
    800009c4:	0000f517          	auipc	a0,0xf
    800009c8:	f6c50513          	addi	a0,a0,-148 # 8000f930 <tx_lock>
    800009cc:	202000ef          	jal	80000bce <acquire>
  if(ReadReg(LSR) & LSR_TX_IDLE){
    800009d0:	100007b7          	lui	a5,0x10000
    800009d4:	0795                	addi	a5,a5,5 # 10000005 <_entry-0x6ffffffb>
    800009d6:	0007c783          	lbu	a5,0(a5)
    800009da:	0207f793          	andi	a5,a5,32
    800009de:	eb89                	bnez	a5,800009f0 <uartintr+0x40>
    // UART finished transmitting; wake up sending thread.
    tx_busy = 0;
    wakeup(&tx_chan);
  }
  release(&tx_lock);
    800009e0:	0000f517          	auipc	a0,0xf
    800009e4:	f5050513          	addi	a0,a0,-176 # 8000f930 <tx_lock>
    800009e8:	27e000ef          	jal	80000c66 <release>

  // read and process incoming characters.
  while(1){
    int c = uartgetc();
    if(c == -1)
    800009ec:	54fd                	li	s1,-1
    800009ee:	a831                	j	80000a0a <uartintr+0x5a>
    tx_busy = 0;
    800009f0:	00007797          	auipc	a5,0x7
    800009f4:	e407ae23          	sw	zero,-420(a5) # 8000784c <tx_busy>
    wakeup(&tx_chan);
    800009f8:	00007517          	auipc	a0,0x7
    800009fc:	e5050513          	addi	a0,a0,-432 # 80007848 <tx_chan>
    80000a00:	578010ef          	jal	80001f78 <wakeup>
    80000a04:	bff1                	j	800009e0 <uartintr+0x30>
      break;
    consoleintr(c);
    80000a06:	8a5ff0ef          	jal	800002aa <consoleintr>
    int c = uartgetc();
    80000a0a:	f81ff0ef          	jal	8000098a <uartgetc>
    if(c == -1)
    80000a0e:	fe951ce3          	bne	a0,s1,80000a06 <uartintr+0x56>
  }
}
    80000a12:	60e2                	ld	ra,24(sp)
    80000a14:	6442                	ld	s0,16(sp)
    80000a16:	64a2                	ld	s1,8(sp)
    80000a18:	6105                	addi	sp,sp,32
    80000a1a:	8082                	ret

0000000080000a1c <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(void *pa)
{
    80000a1c:	1101                	addi	sp,sp,-32
    80000a1e:	ec06                	sd	ra,24(sp)
    80000a20:	e822                	sd	s0,16(sp)
    80000a22:	e426                	sd	s1,8(sp)
    80000a24:	e04a                	sd	s2,0(sp)
    80000a26:	1000                	addi	s0,sp,32
  struct run *r;

  if(((uint64)pa % PGSIZE) != 0 || (char*)pa < end || (uint64)pa >= PHYSTOP)
    80000a28:	03451793          	slli	a5,a0,0x34
    80000a2c:	e7a9                	bnez	a5,80000a76 <kfree+0x5a>
    80000a2e:	84aa                	mv	s1,a0
    80000a30:	00020797          	auipc	a5,0x20
    80000a34:	34878793          	addi	a5,a5,840 # 80020d78 <end>
    80000a38:	02f56f63          	bltu	a0,a5,80000a76 <kfree+0x5a>
    80000a3c:	47c5                	li	a5,17
    80000a3e:	07ee                	slli	a5,a5,0x1b
    80000a40:	02f57b63          	bgeu	a0,a5,80000a76 <kfree+0x5a>
    panic("kfree");

  // Fill with junk to catch dangling refs.
  memset(pa, 1, PGSIZE);
    80000a44:	6605                	lui	a2,0x1
    80000a46:	4585                	li	a1,1
    80000a48:	25a000ef          	jal	80000ca2 <memset>

  r = (struct run*)pa;

  acquire(&kmem.lock);
    80000a4c:	0000f917          	auipc	s2,0xf
    80000a50:	efc90913          	addi	s2,s2,-260 # 8000f948 <kmem>
    80000a54:	854a                	mv	a0,s2
    80000a56:	178000ef          	jal	80000bce <acquire>
  r->next = kmem.freelist;
    80000a5a:	01893783          	ld	a5,24(s2)
    80000a5e:	e09c                	sd	a5,0(s1)
  kmem.freelist = r;
    80000a60:	00993c23          	sd	s1,24(s2)
  release(&kmem.lock);
    80000a64:	854a                	mv	a0,s2
    80000a66:	200000ef          	jal	80000c66 <release>
}
    80000a6a:	60e2                	ld	ra,24(sp)
    80000a6c:	6442                	ld	s0,16(sp)
    80000a6e:	64a2                	ld	s1,8(sp)
    80000a70:	6902                	ld	s2,0(sp)
    80000a72:	6105                	addi	sp,sp,32
    80000a74:	8082                	ret
    panic("kfree");
    80000a76:	00006517          	auipc	a0,0x6
    80000a7a:	5c250513          	addi	a0,a0,1474 # 80007038 <etext+0x38>
    80000a7e:	d63ff0ef          	jal	800007e0 <panic>

0000000080000a82 <freerange>:
{
    80000a82:	7179                	addi	sp,sp,-48
    80000a84:	f406                	sd	ra,40(sp)
    80000a86:	f022                	sd	s0,32(sp)
    80000a88:	ec26                	sd	s1,24(sp)
    80000a8a:	1800                	addi	s0,sp,48
  p = (char*)PGROUNDUP((uint64)pa_start);
    80000a8c:	6785                	lui	a5,0x1
    80000a8e:	fff78713          	addi	a4,a5,-1 # fff <_entry-0x7ffff001>
    80000a92:	00e504b3          	add	s1,a0,a4
    80000a96:	777d                	lui	a4,0xfffff
    80000a98:	8cf9                	and	s1,s1,a4
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000a9a:	94be                	add	s1,s1,a5
    80000a9c:	0295e263          	bltu	a1,s1,80000ac0 <freerange+0x3e>
    80000aa0:	e84a                	sd	s2,16(sp)
    80000aa2:	e44e                	sd	s3,8(sp)
    80000aa4:	e052                	sd	s4,0(sp)
    80000aa6:	892e                	mv	s2,a1
    kfree(p);
    80000aa8:	7a7d                	lui	s4,0xfffff
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000aaa:	6985                	lui	s3,0x1
    kfree(p);
    80000aac:	01448533          	add	a0,s1,s4
    80000ab0:	f6dff0ef          	jal	80000a1c <kfree>
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000ab4:	94ce                	add	s1,s1,s3
    80000ab6:	fe997be3          	bgeu	s2,s1,80000aac <freerange+0x2a>
    80000aba:	6942                	ld	s2,16(sp)
    80000abc:	69a2                	ld	s3,8(sp)
    80000abe:	6a02                	ld	s4,0(sp)
}
    80000ac0:	70a2                	ld	ra,40(sp)
    80000ac2:	7402                	ld	s0,32(sp)
    80000ac4:	64e2                	ld	s1,24(sp)
    80000ac6:	6145                	addi	sp,sp,48
    80000ac8:	8082                	ret

0000000080000aca <kinit>:
{
    80000aca:	1141                	addi	sp,sp,-16
    80000acc:	e406                	sd	ra,8(sp)
    80000ace:	e022                	sd	s0,0(sp)
    80000ad0:	0800                	addi	s0,sp,16
  initlock(&kmem.lock, "kmem");
    80000ad2:	00006597          	auipc	a1,0x6
    80000ad6:	56e58593          	addi	a1,a1,1390 # 80007040 <etext+0x40>
    80000ada:	0000f517          	auipc	a0,0xf
    80000ade:	e6e50513          	addi	a0,a0,-402 # 8000f948 <kmem>
    80000ae2:	06c000ef          	jal	80000b4e <initlock>
  freerange(end, (void*)PHYSTOP);
    80000ae6:	45c5                	li	a1,17
    80000ae8:	05ee                	slli	a1,a1,0x1b
    80000aea:	00020517          	auipc	a0,0x20
    80000aee:	28e50513          	addi	a0,a0,654 # 80020d78 <end>
    80000af2:	f91ff0ef          	jal	80000a82 <freerange>
}
    80000af6:	60a2                	ld	ra,8(sp)
    80000af8:	6402                	ld	s0,0(sp)
    80000afa:	0141                	addi	sp,sp,16
    80000afc:	8082                	ret

0000000080000afe <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
void *
kalloc(void)
{
    80000afe:	1101                	addi	sp,sp,-32
    80000b00:	ec06                	sd	ra,24(sp)
    80000b02:	e822                	sd	s0,16(sp)
    80000b04:	e426                	sd	s1,8(sp)
    80000b06:	1000                	addi	s0,sp,32
  struct run *r;

  acquire(&kmem.lock);
    80000b08:	0000f497          	auipc	s1,0xf
    80000b0c:	e4048493          	addi	s1,s1,-448 # 8000f948 <kmem>
    80000b10:	8526                	mv	a0,s1
    80000b12:	0bc000ef          	jal	80000bce <acquire>
  r = kmem.freelist;
    80000b16:	6c84                	ld	s1,24(s1)
  if(r)
    80000b18:	c485                	beqz	s1,80000b40 <kalloc+0x42>
    kmem.freelist = r->next;
    80000b1a:	609c                	ld	a5,0(s1)
    80000b1c:	0000f517          	auipc	a0,0xf
    80000b20:	e2c50513          	addi	a0,a0,-468 # 8000f948 <kmem>
    80000b24:	ed1c                	sd	a5,24(a0)
  release(&kmem.lock);
    80000b26:	140000ef          	jal	80000c66 <release>

  if(r)
    memset((char*)r, 5, PGSIZE); // fill with junk
    80000b2a:	6605                	lui	a2,0x1
    80000b2c:	4595                	li	a1,5
    80000b2e:	8526                	mv	a0,s1
    80000b30:	172000ef          	jal	80000ca2 <memset>
  return (void*)r;
}
    80000b34:	8526                	mv	a0,s1
    80000b36:	60e2                	ld	ra,24(sp)
    80000b38:	6442                	ld	s0,16(sp)
    80000b3a:	64a2                	ld	s1,8(sp)
    80000b3c:	6105                	addi	sp,sp,32
    80000b3e:	8082                	ret
  release(&kmem.lock);
    80000b40:	0000f517          	auipc	a0,0xf
    80000b44:	e0850513          	addi	a0,a0,-504 # 8000f948 <kmem>
    80000b48:	11e000ef          	jal	80000c66 <release>
  if(r)
    80000b4c:	b7e5                	j	80000b34 <kalloc+0x36>

0000000080000b4e <initlock>:
#include "proc.h"
#include "defs.h"

void
initlock(struct spinlock *lk, char *name)
{
    80000b4e:	1141                	addi	sp,sp,-16
    80000b50:	e422                	sd	s0,8(sp)
    80000b52:	0800                	addi	s0,sp,16
  lk->name = name;
    80000b54:	e50c                	sd	a1,8(a0)
  lk->locked = 0;
    80000b56:	00052023          	sw	zero,0(a0)
  lk->cpu = 0;
    80000b5a:	00053823          	sd	zero,16(a0)
}
    80000b5e:	6422                	ld	s0,8(sp)
    80000b60:	0141                	addi	sp,sp,16
    80000b62:	8082                	ret

0000000080000b64 <holding>:
// Interrupts must be off.
int
holding(struct spinlock *lk)
{
  int r;
  r = (lk->locked && lk->cpu == mycpu());
    80000b64:	411c                	lw	a5,0(a0)
    80000b66:	e399                	bnez	a5,80000b6c <holding+0x8>
    80000b68:	4501                	li	a0,0
  return r;
}
    80000b6a:	8082                	ret
{
    80000b6c:	1101                	addi	sp,sp,-32
    80000b6e:	ec06                	sd	ra,24(sp)
    80000b70:	e822                	sd	s0,16(sp)
    80000b72:	e426                	sd	s1,8(sp)
    80000b74:	1000                	addi	s0,sp,32
  r = (lk->locked && lk->cpu == mycpu());
    80000b76:	6904                	ld	s1,16(a0)
    80000b78:	59d000ef          	jal	80001914 <mycpu>
    80000b7c:	40a48533          	sub	a0,s1,a0
    80000b80:	00153513          	seqz	a0,a0
}
    80000b84:	60e2                	ld	ra,24(sp)
    80000b86:	6442                	ld	s0,16(sp)
    80000b88:	64a2                	ld	s1,8(sp)
    80000b8a:	6105                	addi	sp,sp,32
    80000b8c:	8082                	ret

0000000080000b8e <push_off>:
// it takes two pop_off()s to undo two push_off()s.  Also, if interrupts
// are initially off, then push_off, pop_off leaves them off.

void
push_off(void)
{
    80000b8e:	1101                	addi	sp,sp,-32
    80000b90:	ec06                	sd	ra,24(sp)
    80000b92:	e822                	sd	s0,16(sp)
    80000b94:	e426                	sd	s1,8(sp)
    80000b96:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000b98:	100024f3          	csrr	s1,sstatus
    80000b9c:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    80000ba0:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80000ba2:	10079073          	csrw	sstatus,a5

  // disable interrupts to prevent an involuntary context
  // switch while using mycpu().
  intr_off();

  if(mycpu()->noff == 0)
    80000ba6:	56f000ef          	jal	80001914 <mycpu>
    80000baa:	5d3c                	lw	a5,120(a0)
    80000bac:	cb99                	beqz	a5,80000bc2 <push_off+0x34>
    mycpu()->intena = old;
  mycpu()->noff += 1;
    80000bae:	567000ef          	jal	80001914 <mycpu>
    80000bb2:	5d3c                	lw	a5,120(a0)
    80000bb4:	2785                	addiw	a5,a5,1
    80000bb6:	dd3c                	sw	a5,120(a0)
}
    80000bb8:	60e2                	ld	ra,24(sp)
    80000bba:	6442                	ld	s0,16(sp)
    80000bbc:	64a2                	ld	s1,8(sp)
    80000bbe:	6105                	addi	sp,sp,32
    80000bc0:	8082                	ret
    mycpu()->intena = old;
    80000bc2:	553000ef          	jal	80001914 <mycpu>
  return (x & SSTATUS_SIE) != 0;
    80000bc6:	8085                	srli	s1,s1,0x1
    80000bc8:	8885                	andi	s1,s1,1
    80000bca:	dd64                	sw	s1,124(a0)
    80000bcc:	b7cd                	j	80000bae <push_off+0x20>

0000000080000bce <acquire>:
{
    80000bce:	1101                	addi	sp,sp,-32
    80000bd0:	ec06                	sd	ra,24(sp)
    80000bd2:	e822                	sd	s0,16(sp)
    80000bd4:	e426                	sd	s1,8(sp)
    80000bd6:	1000                	addi	s0,sp,32
    80000bd8:	84aa                	mv	s1,a0
  push_off(); // disable interrupts to avoid deadlock.
    80000bda:	fb5ff0ef          	jal	80000b8e <push_off>
  if(holding(lk))
    80000bde:	8526                	mv	a0,s1
    80000be0:	f85ff0ef          	jal	80000b64 <holding>
  while(__sync_lock_test_and_set(&lk->locked, 1) != 0)
    80000be4:	4705                	li	a4,1
  if(holding(lk))
    80000be6:	e105                	bnez	a0,80000c06 <acquire+0x38>
  while(__sync_lock_test_and_set(&lk->locked, 1) != 0)
    80000be8:	87ba                	mv	a5,a4
    80000bea:	0cf4a7af          	amoswap.w.aq	a5,a5,(s1)
    80000bee:	2781                	sext.w	a5,a5
    80000bf0:	ffe5                	bnez	a5,80000be8 <acquire+0x1a>
  __sync_synchronize();
    80000bf2:	0ff0000f          	fence
  lk->cpu = mycpu();
    80000bf6:	51f000ef          	jal	80001914 <mycpu>
    80000bfa:	e888                	sd	a0,16(s1)
}
    80000bfc:	60e2                	ld	ra,24(sp)
    80000bfe:	6442                	ld	s0,16(sp)
    80000c00:	64a2                	ld	s1,8(sp)
    80000c02:	6105                	addi	sp,sp,32
    80000c04:	8082                	ret
    panic("acquire");
    80000c06:	00006517          	auipc	a0,0x6
    80000c0a:	44250513          	addi	a0,a0,1090 # 80007048 <etext+0x48>
    80000c0e:	bd3ff0ef          	jal	800007e0 <panic>

0000000080000c12 <pop_off>:

void
pop_off(void)
{
    80000c12:	1141                	addi	sp,sp,-16
    80000c14:	e406                	sd	ra,8(sp)
    80000c16:	e022                	sd	s0,0(sp)
    80000c18:	0800                	addi	s0,sp,16
  struct cpu *c = mycpu();
    80000c1a:	4fb000ef          	jal	80001914 <mycpu>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000c1e:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80000c22:	8b89                	andi	a5,a5,2
  if(intr_get())
    80000c24:	e78d                	bnez	a5,80000c4e <pop_off+0x3c>
    panic("pop_off - interruptible");
  if(c->noff < 1)
    80000c26:	5d3c                	lw	a5,120(a0)
    80000c28:	02f05963          	blez	a5,80000c5a <pop_off+0x48>
    panic("pop_off");
  c->noff -= 1;
    80000c2c:	37fd                	addiw	a5,a5,-1
    80000c2e:	0007871b          	sext.w	a4,a5
    80000c32:	dd3c                	sw	a5,120(a0)
  if(c->noff == 0 && c->intena)
    80000c34:	eb09                	bnez	a4,80000c46 <pop_off+0x34>
    80000c36:	5d7c                	lw	a5,124(a0)
    80000c38:	c799                	beqz	a5,80000c46 <pop_off+0x34>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000c3a:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80000c3e:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80000c42:	10079073          	csrw	sstatus,a5
    intr_on();
}
    80000c46:	60a2                	ld	ra,8(sp)
    80000c48:	6402                	ld	s0,0(sp)
    80000c4a:	0141                	addi	sp,sp,16
    80000c4c:	8082                	ret
    panic("pop_off - interruptible");
    80000c4e:	00006517          	auipc	a0,0x6
    80000c52:	40250513          	addi	a0,a0,1026 # 80007050 <etext+0x50>
    80000c56:	b8bff0ef          	jal	800007e0 <panic>
    panic("pop_off");
    80000c5a:	00006517          	auipc	a0,0x6
    80000c5e:	40e50513          	addi	a0,a0,1038 # 80007068 <etext+0x68>
    80000c62:	b7fff0ef          	jal	800007e0 <panic>

0000000080000c66 <release>:
{
    80000c66:	1101                	addi	sp,sp,-32
    80000c68:	ec06                	sd	ra,24(sp)
    80000c6a:	e822                	sd	s0,16(sp)
    80000c6c:	e426                	sd	s1,8(sp)
    80000c6e:	1000                	addi	s0,sp,32
    80000c70:	84aa                	mv	s1,a0
  if(!holding(lk))
    80000c72:	ef3ff0ef          	jal	80000b64 <holding>
    80000c76:	c105                	beqz	a0,80000c96 <release+0x30>
  lk->cpu = 0;
    80000c78:	0004b823          	sd	zero,16(s1)
  __sync_synchronize();
    80000c7c:	0ff0000f          	fence
  __sync_lock_release(&lk->locked);
    80000c80:	0f50000f          	fence	iorw,ow
    80000c84:	0804a02f          	amoswap.w	zero,zero,(s1)
  pop_off();
    80000c88:	f8bff0ef          	jal	80000c12 <pop_off>
}
    80000c8c:	60e2                	ld	ra,24(sp)
    80000c8e:	6442                	ld	s0,16(sp)
    80000c90:	64a2                	ld	s1,8(sp)
    80000c92:	6105                	addi	sp,sp,32
    80000c94:	8082                	ret
    panic("release");
    80000c96:	00006517          	auipc	a0,0x6
    80000c9a:	3da50513          	addi	a0,a0,986 # 80007070 <etext+0x70>
    80000c9e:	b43ff0ef          	jal	800007e0 <panic>

0000000080000ca2 <memset>:
#include "types.h"

void*
memset(void *dst, int c, uint n)
{
    80000ca2:	1141                	addi	sp,sp,-16
    80000ca4:	e422                	sd	s0,8(sp)
    80000ca6:	0800                	addi	s0,sp,16
  char *cdst = (char *) dst;
  int i;
  for(i = 0; i < n; i++){
    80000ca8:	ca19                	beqz	a2,80000cbe <memset+0x1c>
    80000caa:	87aa                	mv	a5,a0
    80000cac:	1602                	slli	a2,a2,0x20
    80000cae:	9201                	srli	a2,a2,0x20
    80000cb0:	00a60733          	add	a4,a2,a0
    cdst[i] = c;
    80000cb4:	00b78023          	sb	a1,0(a5)
  for(i = 0; i < n; i++){
    80000cb8:	0785                	addi	a5,a5,1
    80000cba:	fee79de3          	bne	a5,a4,80000cb4 <memset+0x12>
  }
  return dst;
}
    80000cbe:	6422                	ld	s0,8(sp)
    80000cc0:	0141                	addi	sp,sp,16
    80000cc2:	8082                	ret

0000000080000cc4 <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
    80000cc4:	1141                	addi	sp,sp,-16
    80000cc6:	e422                	sd	s0,8(sp)
    80000cc8:	0800                	addi	s0,sp,16
  const uchar *s1, *s2;

  s1 = v1;
  s2 = v2;
  while(n-- > 0){
    80000cca:	ca05                	beqz	a2,80000cfa <memcmp+0x36>
    80000ccc:	fff6069b          	addiw	a3,a2,-1 # fff <_entry-0x7ffff001>
    80000cd0:	1682                	slli	a3,a3,0x20
    80000cd2:	9281                	srli	a3,a3,0x20
    80000cd4:	0685                	addi	a3,a3,1
    80000cd6:	96aa                	add	a3,a3,a0
    if(*s1 != *s2)
    80000cd8:	00054783          	lbu	a5,0(a0)
    80000cdc:	0005c703          	lbu	a4,0(a1)
    80000ce0:	00e79863          	bne	a5,a4,80000cf0 <memcmp+0x2c>
      return *s1 - *s2;
    s1++, s2++;
    80000ce4:	0505                	addi	a0,a0,1
    80000ce6:	0585                	addi	a1,a1,1
  while(n-- > 0){
    80000ce8:	fed518e3          	bne	a0,a3,80000cd8 <memcmp+0x14>
  }

  return 0;
    80000cec:	4501                	li	a0,0
    80000cee:	a019                	j	80000cf4 <memcmp+0x30>
      return *s1 - *s2;
    80000cf0:	40e7853b          	subw	a0,a5,a4
}
    80000cf4:	6422                	ld	s0,8(sp)
    80000cf6:	0141                	addi	sp,sp,16
    80000cf8:	8082                	ret
  return 0;
    80000cfa:	4501                	li	a0,0
    80000cfc:	bfe5                	j	80000cf4 <memcmp+0x30>

0000000080000cfe <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
    80000cfe:	1141                	addi	sp,sp,-16
    80000d00:	e422                	sd	s0,8(sp)
    80000d02:	0800                	addi	s0,sp,16
  const char *s;
  char *d;

  if(n == 0)
    80000d04:	c205                	beqz	a2,80000d24 <memmove+0x26>
    return dst;
  
  s = src;
  d = dst;
  if(s < d && s + n > d){
    80000d06:	02a5e263          	bltu	a1,a0,80000d2a <memmove+0x2c>
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
    80000d0a:	1602                	slli	a2,a2,0x20
    80000d0c:	9201                	srli	a2,a2,0x20
    80000d0e:	00c587b3          	add	a5,a1,a2
{
    80000d12:	872a                	mv	a4,a0
      *d++ = *s++;
    80000d14:	0585                	addi	a1,a1,1
    80000d16:	0705                	addi	a4,a4,1 # fffffffffffff001 <end+0xffffffff7ffde289>
    80000d18:	fff5c683          	lbu	a3,-1(a1)
    80000d1c:	fed70fa3          	sb	a3,-1(a4)
    while(n-- > 0)
    80000d20:	feb79ae3          	bne	a5,a1,80000d14 <memmove+0x16>

  return dst;
}
    80000d24:	6422                	ld	s0,8(sp)
    80000d26:	0141                	addi	sp,sp,16
    80000d28:	8082                	ret
  if(s < d && s + n > d){
    80000d2a:	02061693          	slli	a3,a2,0x20
    80000d2e:	9281                	srli	a3,a3,0x20
    80000d30:	00d58733          	add	a4,a1,a3
    80000d34:	fce57be3          	bgeu	a0,a4,80000d0a <memmove+0xc>
    d += n;
    80000d38:	96aa                	add	a3,a3,a0
    while(n-- > 0)
    80000d3a:	fff6079b          	addiw	a5,a2,-1
    80000d3e:	1782                	slli	a5,a5,0x20
    80000d40:	9381                	srli	a5,a5,0x20
    80000d42:	fff7c793          	not	a5,a5
    80000d46:	97ba                	add	a5,a5,a4
      *--d = *--s;
    80000d48:	177d                	addi	a4,a4,-1
    80000d4a:	16fd                	addi	a3,a3,-1
    80000d4c:	00074603          	lbu	a2,0(a4)
    80000d50:	00c68023          	sb	a2,0(a3)
    while(n-- > 0)
    80000d54:	fef71ae3          	bne	a4,a5,80000d48 <memmove+0x4a>
    80000d58:	b7f1                	j	80000d24 <memmove+0x26>

0000000080000d5a <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
    80000d5a:	1141                	addi	sp,sp,-16
    80000d5c:	e406                	sd	ra,8(sp)
    80000d5e:	e022                	sd	s0,0(sp)
    80000d60:	0800                	addi	s0,sp,16
  return memmove(dst, src, n);
    80000d62:	f9dff0ef          	jal	80000cfe <memmove>
}
    80000d66:	60a2                	ld	ra,8(sp)
    80000d68:	6402                	ld	s0,0(sp)
    80000d6a:	0141                	addi	sp,sp,16
    80000d6c:	8082                	ret

0000000080000d6e <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
    80000d6e:	1141                	addi	sp,sp,-16
    80000d70:	e422                	sd	s0,8(sp)
    80000d72:	0800                	addi	s0,sp,16
  while(n > 0 && *p && *p == *q)
    80000d74:	ce11                	beqz	a2,80000d90 <strncmp+0x22>
    80000d76:	00054783          	lbu	a5,0(a0)
    80000d7a:	cf89                	beqz	a5,80000d94 <strncmp+0x26>
    80000d7c:	0005c703          	lbu	a4,0(a1)
    80000d80:	00f71a63          	bne	a4,a5,80000d94 <strncmp+0x26>
    n--, p++, q++;
    80000d84:	367d                	addiw	a2,a2,-1
    80000d86:	0505                	addi	a0,a0,1
    80000d88:	0585                	addi	a1,a1,1
  while(n > 0 && *p && *p == *q)
    80000d8a:	f675                	bnez	a2,80000d76 <strncmp+0x8>
  if(n == 0)
    return 0;
    80000d8c:	4501                	li	a0,0
    80000d8e:	a801                	j	80000d9e <strncmp+0x30>
    80000d90:	4501                	li	a0,0
    80000d92:	a031                	j	80000d9e <strncmp+0x30>
  return (uchar)*p - (uchar)*q;
    80000d94:	00054503          	lbu	a0,0(a0)
    80000d98:	0005c783          	lbu	a5,0(a1)
    80000d9c:	9d1d                	subw	a0,a0,a5
}
    80000d9e:	6422                	ld	s0,8(sp)
    80000da0:	0141                	addi	sp,sp,16
    80000da2:	8082                	ret

0000000080000da4 <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
    80000da4:	1141                	addi	sp,sp,-16
    80000da6:	e422                	sd	s0,8(sp)
    80000da8:	0800                	addi	s0,sp,16
  char *os;

  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
    80000daa:	87aa                	mv	a5,a0
    80000dac:	86b2                	mv	a3,a2
    80000dae:	367d                	addiw	a2,a2,-1
    80000db0:	02d05563          	blez	a3,80000dda <strncpy+0x36>
    80000db4:	0785                	addi	a5,a5,1
    80000db6:	0005c703          	lbu	a4,0(a1)
    80000dba:	fee78fa3          	sb	a4,-1(a5)
    80000dbe:	0585                	addi	a1,a1,1
    80000dc0:	f775                	bnez	a4,80000dac <strncpy+0x8>
    ;
  while(n-- > 0)
    80000dc2:	873e                	mv	a4,a5
    80000dc4:	9fb5                	addw	a5,a5,a3
    80000dc6:	37fd                	addiw	a5,a5,-1
    80000dc8:	00c05963          	blez	a2,80000dda <strncpy+0x36>
    *s++ = 0;
    80000dcc:	0705                	addi	a4,a4,1
    80000dce:	fe070fa3          	sb	zero,-1(a4)
  while(n-- > 0)
    80000dd2:	40e786bb          	subw	a3,a5,a4
    80000dd6:	fed04be3          	bgtz	a3,80000dcc <strncpy+0x28>
  return os;
}
    80000dda:	6422                	ld	s0,8(sp)
    80000ddc:	0141                	addi	sp,sp,16
    80000dde:	8082                	ret

0000000080000de0 <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
    80000de0:	1141                	addi	sp,sp,-16
    80000de2:	e422                	sd	s0,8(sp)
    80000de4:	0800                	addi	s0,sp,16
  char *os;

  os = s;
  if(n <= 0)
    80000de6:	02c05363          	blez	a2,80000e0c <safestrcpy+0x2c>
    80000dea:	fff6069b          	addiw	a3,a2,-1
    80000dee:	1682                	slli	a3,a3,0x20
    80000df0:	9281                	srli	a3,a3,0x20
    80000df2:	96ae                	add	a3,a3,a1
    80000df4:	87aa                	mv	a5,a0
    return os;
  while(--n > 0 && (*s++ = *t++) != 0)
    80000df6:	00d58963          	beq	a1,a3,80000e08 <safestrcpy+0x28>
    80000dfa:	0585                	addi	a1,a1,1
    80000dfc:	0785                	addi	a5,a5,1
    80000dfe:	fff5c703          	lbu	a4,-1(a1)
    80000e02:	fee78fa3          	sb	a4,-1(a5)
    80000e06:	fb65                	bnez	a4,80000df6 <safestrcpy+0x16>
    ;
  *s = 0;
    80000e08:	00078023          	sb	zero,0(a5)
  return os;
}
    80000e0c:	6422                	ld	s0,8(sp)
    80000e0e:	0141                	addi	sp,sp,16
    80000e10:	8082                	ret

0000000080000e12 <strlen>:

int
strlen(const char *s)
{
    80000e12:	1141                	addi	sp,sp,-16
    80000e14:	e422                	sd	s0,8(sp)
    80000e16:	0800                	addi	s0,sp,16
  int n;

  for(n = 0; s[n]; n++)
    80000e18:	00054783          	lbu	a5,0(a0)
    80000e1c:	cf91                	beqz	a5,80000e38 <strlen+0x26>
    80000e1e:	0505                	addi	a0,a0,1
    80000e20:	87aa                	mv	a5,a0
    80000e22:	86be                	mv	a3,a5
    80000e24:	0785                	addi	a5,a5,1
    80000e26:	fff7c703          	lbu	a4,-1(a5)
    80000e2a:	ff65                	bnez	a4,80000e22 <strlen+0x10>
    80000e2c:	40a6853b          	subw	a0,a3,a0
    80000e30:	2505                	addiw	a0,a0,1
    ;
  return n;
}
    80000e32:	6422                	ld	s0,8(sp)
    80000e34:	0141                	addi	sp,sp,16
    80000e36:	8082                	ret
  for(n = 0; s[n]; n++)
    80000e38:	4501                	li	a0,0
    80000e3a:	bfe5                	j	80000e32 <strlen+0x20>

0000000080000e3c <main>:
volatile static int started = 0;

// start() jumps here in supervisor mode on all CPUs.
void
main()
{
    80000e3c:	1141                	addi	sp,sp,-16
    80000e3e:	e406                	sd	ra,8(sp)
    80000e40:	e022                	sd	s0,0(sp)
    80000e42:	0800                	addi	s0,sp,16
  if(cpuid() == 0){
    80000e44:	2c1000ef          	jal	80001904 <cpuid>
    virtio_disk_init(); // emulated hard disk
    userinit();      // first user process
    __sync_synchronize();
    started = 1;
  } else {
    while(started == 0)
    80000e48:	00007717          	auipc	a4,0x7
    80000e4c:	a0870713          	addi	a4,a4,-1528 # 80007850 <started>
  if(cpuid() == 0){
    80000e50:	c51d                	beqz	a0,80000e7e <main+0x42>
    while(started == 0)
    80000e52:	431c                	lw	a5,0(a4)
    80000e54:	2781                	sext.w	a5,a5
    80000e56:	dff5                	beqz	a5,80000e52 <main+0x16>
      ;
    __sync_synchronize();
    80000e58:	0ff0000f          	fence
    printf("hart %d starting\n", cpuid());
    80000e5c:	2a9000ef          	jal	80001904 <cpuid>
    80000e60:	85aa                	mv	a1,a0
    80000e62:	00006517          	auipc	a0,0x6
    80000e66:	23650513          	addi	a0,a0,566 # 80007098 <etext+0x98>
    80000e6a:	e90ff0ef          	jal	800004fa <printf>
    kvminithart();    // turn on paging
    80000e6e:	080000ef          	jal	80000eee <kvminithart>
    trapinithart();   // install kernel trap vector
    80000e72:	5dc010ef          	jal	8000244e <trapinithart>
    plicinithart();   // ask PLIC for device interrupts
    80000e76:	5f2040ef          	jal	80005468 <plicinithart>
  }

  scheduler();        
    80000e7a:	71b000ef          	jal	80001d94 <scheduler>
    consoleinit();
    80000e7e:	da6ff0ef          	jal	80000424 <consoleinit>
    printfinit();
    80000e82:	99bff0ef          	jal	8000081c <printfinit>
    printf("\n");
    80000e86:	00006517          	auipc	a0,0x6
    80000e8a:	1f250513          	addi	a0,a0,498 # 80007078 <etext+0x78>
    80000e8e:	e6cff0ef          	jal	800004fa <printf>
    printf("xv6 kernel is booting\n");
    80000e92:	00006517          	auipc	a0,0x6
    80000e96:	1ee50513          	addi	a0,a0,494 # 80007080 <etext+0x80>
    80000e9a:	e60ff0ef          	jal	800004fa <printf>
    printf("\n");
    80000e9e:	00006517          	auipc	a0,0x6
    80000ea2:	1da50513          	addi	a0,a0,474 # 80007078 <etext+0x78>
    80000ea6:	e54ff0ef          	jal	800004fa <printf>
    kinit();         // physical page allocator
    80000eaa:	c21ff0ef          	jal	80000aca <kinit>
    kvminit();       // create kernel page table
    80000eae:	2ca000ef          	jal	80001178 <kvminit>
    kvminithart();   // turn on paging
    80000eb2:	03c000ef          	jal	80000eee <kvminithart>
    procinit();      // process table
    80000eb6:	199000ef          	jal	8000184e <procinit>
    trapinit();      // trap vectors
    80000eba:	570010ef          	jal	8000242a <trapinit>
    trapinithart();  // install kernel trap vector
    80000ebe:	590010ef          	jal	8000244e <trapinithart>
    plicinit();      // set up interrupt controller
    80000ec2:	58c040ef          	jal	8000544e <plicinit>
    plicinithart();  // ask PLIC for device interrupts
    80000ec6:	5a2040ef          	jal	80005468 <plicinithart>
    binit();         // buffer cache
    80000eca:	471010ef          	jal	80002b3a <binit>
    iinit();         // inode table
    80000ece:	1f6020ef          	jal	800030c4 <iinit>
    fileinit();      // file table
    80000ed2:	0e8030ef          	jal	80003fba <fileinit>
    virtio_disk_init(); // emulated hard disk
    80000ed6:	682040ef          	jal	80005558 <virtio_disk_init>
    userinit();      // first user process
    80000eda:	521000ef          	jal	80001bfa <userinit>
    __sync_synchronize();
    80000ede:	0ff0000f          	fence
    started = 1;
    80000ee2:	4785                	li	a5,1
    80000ee4:	00007717          	auipc	a4,0x7
    80000ee8:	96f72623          	sw	a5,-1684(a4) # 80007850 <started>
    80000eec:	b779                	j	80000e7a <main+0x3e>

0000000080000eee <kvminithart>:

// Switch the current CPU's h/w page table register to
// the kernel's page table, and enable paging.
void
kvminithart()
{
    80000eee:	1141                	addi	sp,sp,-16
    80000ef0:	e422                	sd	s0,8(sp)
    80000ef2:	0800                	addi	s0,sp,16
// flush the TLB.
static inline void
sfence_vma()
{
  // the zero, zero means flush all TLB entries.
  asm volatile("sfence.vma zero, zero");
    80000ef4:	12000073          	sfence.vma
  // wait for any previous writes to the page table memory to finish.
  sfence_vma();

  w_satp(MAKE_SATP(kernel_pagetable));
    80000ef8:	00007797          	auipc	a5,0x7
    80000efc:	9607b783          	ld	a5,-1696(a5) # 80007858 <kernel_pagetable>
    80000f00:	83b1                	srli	a5,a5,0xc
    80000f02:	577d                	li	a4,-1
    80000f04:	177e                	slli	a4,a4,0x3f
    80000f06:	8fd9                	or	a5,a5,a4
  asm volatile("csrw satp, %0" : : "r" (x));
    80000f08:	18079073          	csrw	satp,a5
  asm volatile("sfence.vma zero, zero");
    80000f0c:	12000073          	sfence.vma

  // flush stale entries from the TLB.
  sfence_vma();
}
    80000f10:	6422                	ld	s0,8(sp)
    80000f12:	0141                	addi	sp,sp,16
    80000f14:	8082                	ret

0000000080000f16 <walk>:
//   21..29 -- 9 bits of level-1 index.
//   12..20 -- 9 bits of level-0 index.
//    0..11 -- 12 bits of byte offset within the page.
pte_t *
walk(pagetable_t pagetable, uint64 va, int alloc)
{
    80000f16:	7139                	addi	sp,sp,-64
    80000f18:	fc06                	sd	ra,56(sp)
    80000f1a:	f822                	sd	s0,48(sp)
    80000f1c:	f426                	sd	s1,40(sp)
    80000f1e:	f04a                	sd	s2,32(sp)
    80000f20:	ec4e                	sd	s3,24(sp)
    80000f22:	e852                	sd	s4,16(sp)
    80000f24:	e456                	sd	s5,8(sp)
    80000f26:	e05a                	sd	s6,0(sp)
    80000f28:	0080                	addi	s0,sp,64
    80000f2a:	84aa                	mv	s1,a0
    80000f2c:	89ae                	mv	s3,a1
    80000f2e:	8ab2                	mv	s5,a2
  if(va >= MAXVA)
    80000f30:	57fd                	li	a5,-1
    80000f32:	83e9                	srli	a5,a5,0x1a
    80000f34:	4a79                	li	s4,30
    panic("walk");

  for(int level = 2; level > 0; level--) {
    80000f36:	4b31                	li	s6,12
  if(va >= MAXVA)
    80000f38:	02b7fc63          	bgeu	a5,a1,80000f70 <walk+0x5a>
    panic("walk");
    80000f3c:	00006517          	auipc	a0,0x6
    80000f40:	17450513          	addi	a0,a0,372 # 800070b0 <etext+0xb0>
    80000f44:	89dff0ef          	jal	800007e0 <panic>
    pte_t *pte = &pagetable[PX(level, va)];
    if(*pte & PTE_V) {
      pagetable = (pagetable_t)PTE2PA(*pte);
    } else {
      if(!alloc || (pagetable = (pde_t*)kalloc()) == 0)
    80000f48:	060a8263          	beqz	s5,80000fac <walk+0x96>
    80000f4c:	bb3ff0ef          	jal	80000afe <kalloc>
    80000f50:	84aa                	mv	s1,a0
    80000f52:	c139                	beqz	a0,80000f98 <walk+0x82>
        return 0;
      memset(pagetable, 0, PGSIZE);
    80000f54:	6605                	lui	a2,0x1
    80000f56:	4581                	li	a1,0
    80000f58:	d4bff0ef          	jal	80000ca2 <memset>
      *pte = PA2PTE(pagetable) | PTE_V;
    80000f5c:	00c4d793          	srli	a5,s1,0xc
    80000f60:	07aa                	slli	a5,a5,0xa
    80000f62:	0017e793          	ori	a5,a5,1
    80000f66:	00f93023          	sd	a5,0(s2)
  for(int level = 2; level > 0; level--) {
    80000f6a:	3a5d                	addiw	s4,s4,-9 # ffffffffffffeff7 <end+0xffffffff7ffde27f>
    80000f6c:	036a0063          	beq	s4,s6,80000f8c <walk+0x76>
    pte_t *pte = &pagetable[PX(level, va)];
    80000f70:	0149d933          	srl	s2,s3,s4
    80000f74:	1ff97913          	andi	s2,s2,511
    80000f78:	090e                	slli	s2,s2,0x3
    80000f7a:	9926                	add	s2,s2,s1
    if(*pte & PTE_V) {
    80000f7c:	00093483          	ld	s1,0(s2)
    80000f80:	0014f793          	andi	a5,s1,1
    80000f84:	d3f1                	beqz	a5,80000f48 <walk+0x32>
      pagetable = (pagetable_t)PTE2PA(*pte);
    80000f86:	80a9                	srli	s1,s1,0xa
    80000f88:	04b2                	slli	s1,s1,0xc
    80000f8a:	b7c5                	j	80000f6a <walk+0x54>
    }
  }
  return &pagetable[PX(0, va)];
    80000f8c:	00c9d513          	srli	a0,s3,0xc
    80000f90:	1ff57513          	andi	a0,a0,511
    80000f94:	050e                	slli	a0,a0,0x3
    80000f96:	9526                	add	a0,a0,s1
}
    80000f98:	70e2                	ld	ra,56(sp)
    80000f9a:	7442                	ld	s0,48(sp)
    80000f9c:	74a2                	ld	s1,40(sp)
    80000f9e:	7902                	ld	s2,32(sp)
    80000fa0:	69e2                	ld	s3,24(sp)
    80000fa2:	6a42                	ld	s4,16(sp)
    80000fa4:	6aa2                	ld	s5,8(sp)
    80000fa6:	6b02                	ld	s6,0(sp)
    80000fa8:	6121                	addi	sp,sp,64
    80000faa:	8082                	ret
        return 0;
    80000fac:	4501                	li	a0,0
    80000fae:	b7ed                	j	80000f98 <walk+0x82>

0000000080000fb0 <walkaddr>:
walkaddr(pagetable_t pagetable, uint64 va)
{
  pte_t *pte;
  uint64 pa;

  if(va >= MAXVA)
    80000fb0:	57fd                	li	a5,-1
    80000fb2:	83e9                	srli	a5,a5,0x1a
    80000fb4:	00b7f463          	bgeu	a5,a1,80000fbc <walkaddr+0xc>
    return 0;
    80000fb8:	4501                	li	a0,0
    return 0;
  if((*pte & PTE_U) == 0)
    return 0;
  pa = PTE2PA(*pte);
  return pa;
}
    80000fba:	8082                	ret
{
    80000fbc:	1141                	addi	sp,sp,-16
    80000fbe:	e406                	sd	ra,8(sp)
    80000fc0:	e022                	sd	s0,0(sp)
    80000fc2:	0800                	addi	s0,sp,16
  pte = walk(pagetable, va, 0);
    80000fc4:	4601                	li	a2,0
    80000fc6:	f51ff0ef          	jal	80000f16 <walk>
  if(pte == 0)
    80000fca:	c105                	beqz	a0,80000fea <walkaddr+0x3a>
  if((*pte & PTE_V) == 0)
    80000fcc:	611c                	ld	a5,0(a0)
  if((*pte & PTE_U) == 0)
    80000fce:	0117f693          	andi	a3,a5,17
    80000fd2:	4745                	li	a4,17
    return 0;
    80000fd4:	4501                	li	a0,0
  if((*pte & PTE_U) == 0)
    80000fd6:	00e68663          	beq	a3,a4,80000fe2 <walkaddr+0x32>
}
    80000fda:	60a2                	ld	ra,8(sp)
    80000fdc:	6402                	ld	s0,0(sp)
    80000fde:	0141                	addi	sp,sp,16
    80000fe0:	8082                	ret
  pa = PTE2PA(*pte);
    80000fe2:	83a9                	srli	a5,a5,0xa
    80000fe4:	00c79513          	slli	a0,a5,0xc
  return pa;
    80000fe8:	bfcd                	j	80000fda <walkaddr+0x2a>
    return 0;
    80000fea:	4501                	li	a0,0
    80000fec:	b7fd                	j	80000fda <walkaddr+0x2a>

0000000080000fee <mappages>:
// va and size MUST be page-aligned.
// Returns 0 on success, -1 if walk() couldn't
// allocate a needed page-table page.
int
mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
    80000fee:	715d                	addi	sp,sp,-80
    80000ff0:	e486                	sd	ra,72(sp)
    80000ff2:	e0a2                	sd	s0,64(sp)
    80000ff4:	fc26                	sd	s1,56(sp)
    80000ff6:	f84a                	sd	s2,48(sp)
    80000ff8:	f44e                	sd	s3,40(sp)
    80000ffa:	f052                	sd	s4,32(sp)
    80000ffc:	ec56                	sd	s5,24(sp)
    80000ffe:	e85a                	sd	s6,16(sp)
    80001000:	e45e                	sd	s7,8(sp)
    80001002:	0880                	addi	s0,sp,80
  uint64 a, last;
  pte_t *pte;

  if((va % PGSIZE) != 0)
    80001004:	03459793          	slli	a5,a1,0x34
    80001008:	e7a9                	bnez	a5,80001052 <mappages+0x64>
    8000100a:	8aaa                	mv	s5,a0
    8000100c:	8b3a                	mv	s6,a4
    panic("mappages: va not aligned");

  if((size % PGSIZE) != 0)
    8000100e:	03461793          	slli	a5,a2,0x34
    80001012:	e7b1                	bnez	a5,8000105e <mappages+0x70>
    panic("mappages: size not aligned");

  if(size == 0)
    80001014:	ca39                	beqz	a2,8000106a <mappages+0x7c>
    panic("mappages: size");
  
  a = va;
  last = va + size - PGSIZE;
    80001016:	77fd                	lui	a5,0xfffff
    80001018:	963e                	add	a2,a2,a5
    8000101a:	00b609b3          	add	s3,a2,a1
  a = va;
    8000101e:	892e                	mv	s2,a1
    80001020:	40b68a33          	sub	s4,a3,a1
    if(*pte & PTE_V)
      panic("mappages: remap");
    *pte = PA2PTE(pa) | perm | PTE_V;
    if(a == last)
      break;
    a += PGSIZE;
    80001024:	6b85                	lui	s7,0x1
    80001026:	014904b3          	add	s1,s2,s4
    if((pte = walk(pagetable, a, 1)) == 0)
    8000102a:	4605                	li	a2,1
    8000102c:	85ca                	mv	a1,s2
    8000102e:	8556                	mv	a0,s5
    80001030:	ee7ff0ef          	jal	80000f16 <walk>
    80001034:	c539                	beqz	a0,80001082 <mappages+0x94>
    if(*pte & PTE_V)
    80001036:	611c                	ld	a5,0(a0)
    80001038:	8b85                	andi	a5,a5,1
    8000103a:	ef95                	bnez	a5,80001076 <mappages+0x88>
    *pte = PA2PTE(pa) | perm | PTE_V;
    8000103c:	80b1                	srli	s1,s1,0xc
    8000103e:	04aa                	slli	s1,s1,0xa
    80001040:	0164e4b3          	or	s1,s1,s6
    80001044:	0014e493          	ori	s1,s1,1
    80001048:	e104                	sd	s1,0(a0)
    if(a == last)
    8000104a:	05390863          	beq	s2,s3,8000109a <mappages+0xac>
    a += PGSIZE;
    8000104e:	995e                	add	s2,s2,s7
    if((pte = walk(pagetable, a, 1)) == 0)
    80001050:	bfd9                	j	80001026 <mappages+0x38>
    panic("mappages: va not aligned");
    80001052:	00006517          	auipc	a0,0x6
    80001056:	06650513          	addi	a0,a0,102 # 800070b8 <etext+0xb8>
    8000105a:	f86ff0ef          	jal	800007e0 <panic>
    panic("mappages: size not aligned");
    8000105e:	00006517          	auipc	a0,0x6
    80001062:	07a50513          	addi	a0,a0,122 # 800070d8 <etext+0xd8>
    80001066:	f7aff0ef          	jal	800007e0 <panic>
    panic("mappages: size");
    8000106a:	00006517          	auipc	a0,0x6
    8000106e:	08e50513          	addi	a0,a0,142 # 800070f8 <etext+0xf8>
    80001072:	f6eff0ef          	jal	800007e0 <panic>
      panic("mappages: remap");
    80001076:	00006517          	auipc	a0,0x6
    8000107a:	09250513          	addi	a0,a0,146 # 80007108 <etext+0x108>
    8000107e:	f62ff0ef          	jal	800007e0 <panic>
      return -1;
    80001082:	557d                	li	a0,-1
    pa += PGSIZE;
  }
  return 0;
}
    80001084:	60a6                	ld	ra,72(sp)
    80001086:	6406                	ld	s0,64(sp)
    80001088:	74e2                	ld	s1,56(sp)
    8000108a:	7942                	ld	s2,48(sp)
    8000108c:	79a2                	ld	s3,40(sp)
    8000108e:	7a02                	ld	s4,32(sp)
    80001090:	6ae2                	ld	s5,24(sp)
    80001092:	6b42                	ld	s6,16(sp)
    80001094:	6ba2                	ld	s7,8(sp)
    80001096:	6161                	addi	sp,sp,80
    80001098:	8082                	ret
  return 0;
    8000109a:	4501                	li	a0,0
    8000109c:	b7e5                	j	80001084 <mappages+0x96>

000000008000109e <kvmmap>:
{
    8000109e:	1141                	addi	sp,sp,-16
    800010a0:	e406                	sd	ra,8(sp)
    800010a2:	e022                	sd	s0,0(sp)
    800010a4:	0800                	addi	s0,sp,16
    800010a6:	87b6                	mv	a5,a3
  if(mappages(kpgtbl, va, sz, pa, perm) != 0)
    800010a8:	86b2                	mv	a3,a2
    800010aa:	863e                	mv	a2,a5
    800010ac:	f43ff0ef          	jal	80000fee <mappages>
    800010b0:	e509                	bnez	a0,800010ba <kvmmap+0x1c>
}
    800010b2:	60a2                	ld	ra,8(sp)
    800010b4:	6402                	ld	s0,0(sp)
    800010b6:	0141                	addi	sp,sp,16
    800010b8:	8082                	ret
    panic("kvmmap");
    800010ba:	00006517          	auipc	a0,0x6
    800010be:	05e50513          	addi	a0,a0,94 # 80007118 <etext+0x118>
    800010c2:	f1eff0ef          	jal	800007e0 <panic>

00000000800010c6 <kvmmake>:
{
    800010c6:	1101                	addi	sp,sp,-32
    800010c8:	ec06                	sd	ra,24(sp)
    800010ca:	e822                	sd	s0,16(sp)
    800010cc:	e426                	sd	s1,8(sp)
    800010ce:	e04a                	sd	s2,0(sp)
    800010d0:	1000                	addi	s0,sp,32
  kpgtbl = (pagetable_t) kalloc();
    800010d2:	a2dff0ef          	jal	80000afe <kalloc>
    800010d6:	84aa                	mv	s1,a0
  memset(kpgtbl, 0, PGSIZE);
    800010d8:	6605                	lui	a2,0x1
    800010da:	4581                	li	a1,0
    800010dc:	bc7ff0ef          	jal	80000ca2 <memset>
  kvmmap(kpgtbl, UART0, UART0, PGSIZE, PTE_R | PTE_W);
    800010e0:	4719                	li	a4,6
    800010e2:	6685                	lui	a3,0x1
    800010e4:	10000637          	lui	a2,0x10000
    800010e8:	100005b7          	lui	a1,0x10000
    800010ec:	8526                	mv	a0,s1
    800010ee:	fb1ff0ef          	jal	8000109e <kvmmap>
  kvmmap(kpgtbl, VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);
    800010f2:	4719                	li	a4,6
    800010f4:	6685                	lui	a3,0x1
    800010f6:	10001637          	lui	a2,0x10001
    800010fa:	100015b7          	lui	a1,0x10001
    800010fe:	8526                	mv	a0,s1
    80001100:	f9fff0ef          	jal	8000109e <kvmmap>
  kvmmap(kpgtbl, PLIC, PLIC, 0x4000000, PTE_R | PTE_W);
    80001104:	4719                	li	a4,6
    80001106:	040006b7          	lui	a3,0x4000
    8000110a:	0c000637          	lui	a2,0xc000
    8000110e:	0c0005b7          	lui	a1,0xc000
    80001112:	8526                	mv	a0,s1
    80001114:	f8bff0ef          	jal	8000109e <kvmmap>
  kvmmap(kpgtbl, KERNBASE, KERNBASE, (uint64)etext-KERNBASE, PTE_R | PTE_X);
    80001118:	00006917          	auipc	s2,0x6
    8000111c:	ee890913          	addi	s2,s2,-280 # 80007000 <etext>
    80001120:	4729                	li	a4,10
    80001122:	80006697          	auipc	a3,0x80006
    80001126:	ede68693          	addi	a3,a3,-290 # 7000 <_entry-0x7fff9000>
    8000112a:	4605                	li	a2,1
    8000112c:	067e                	slli	a2,a2,0x1f
    8000112e:	85b2                	mv	a1,a2
    80001130:	8526                	mv	a0,s1
    80001132:	f6dff0ef          	jal	8000109e <kvmmap>
  kvmmap(kpgtbl, (uint64)etext, (uint64)etext, PHYSTOP-(uint64)etext, PTE_R | PTE_W);
    80001136:	46c5                	li	a3,17
    80001138:	06ee                	slli	a3,a3,0x1b
    8000113a:	4719                	li	a4,6
    8000113c:	412686b3          	sub	a3,a3,s2
    80001140:	864a                	mv	a2,s2
    80001142:	85ca                	mv	a1,s2
    80001144:	8526                	mv	a0,s1
    80001146:	f59ff0ef          	jal	8000109e <kvmmap>
  kvmmap(kpgtbl, TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
    8000114a:	4729                	li	a4,10
    8000114c:	6685                	lui	a3,0x1
    8000114e:	00005617          	auipc	a2,0x5
    80001152:	eb260613          	addi	a2,a2,-334 # 80006000 <_trampoline>
    80001156:	040005b7          	lui	a1,0x4000
    8000115a:	15fd                	addi	a1,a1,-1 # 3ffffff <_entry-0x7c000001>
    8000115c:	05b2                	slli	a1,a1,0xc
    8000115e:	8526                	mv	a0,s1
    80001160:	f3fff0ef          	jal	8000109e <kvmmap>
  proc_mapstacks(kpgtbl);
    80001164:	8526                	mv	a0,s1
    80001166:	650000ef          	jal	800017b6 <proc_mapstacks>
}
    8000116a:	8526                	mv	a0,s1
    8000116c:	60e2                	ld	ra,24(sp)
    8000116e:	6442                	ld	s0,16(sp)
    80001170:	64a2                	ld	s1,8(sp)
    80001172:	6902                	ld	s2,0(sp)
    80001174:	6105                	addi	sp,sp,32
    80001176:	8082                	ret

0000000080001178 <kvminit>:
{
    80001178:	1141                	addi	sp,sp,-16
    8000117a:	e406                	sd	ra,8(sp)
    8000117c:	e022                	sd	s0,0(sp)
    8000117e:	0800                	addi	s0,sp,16
  kernel_pagetable = kvmmake();
    80001180:	f47ff0ef          	jal	800010c6 <kvmmake>
    80001184:	00006797          	auipc	a5,0x6
    80001188:	6ca7ba23          	sd	a0,1748(a5) # 80007858 <kernel_pagetable>
}
    8000118c:	60a2                	ld	ra,8(sp)
    8000118e:	6402                	ld	s0,0(sp)
    80001190:	0141                	addi	sp,sp,16
    80001192:	8082                	ret

0000000080001194 <uvmcreate>:

// create an empty user page table.
// returns 0 if out of memory.
pagetable_t
uvmcreate()
{
    80001194:	1101                	addi	sp,sp,-32
    80001196:	ec06                	sd	ra,24(sp)
    80001198:	e822                	sd	s0,16(sp)
    8000119a:	e426                	sd	s1,8(sp)
    8000119c:	1000                	addi	s0,sp,32
  pagetable_t pagetable;
  pagetable = (pagetable_t) kalloc();
    8000119e:	961ff0ef          	jal	80000afe <kalloc>
    800011a2:	84aa                	mv	s1,a0
  if(pagetable == 0)
    800011a4:	c509                	beqz	a0,800011ae <uvmcreate+0x1a>
    return 0;
  memset(pagetable, 0, PGSIZE);
    800011a6:	6605                	lui	a2,0x1
    800011a8:	4581                	li	a1,0
    800011aa:	af9ff0ef          	jal	80000ca2 <memset>
  return pagetable;
}
    800011ae:	8526                	mv	a0,s1
    800011b0:	60e2                	ld	ra,24(sp)
    800011b2:	6442                	ld	s0,16(sp)
    800011b4:	64a2                	ld	s1,8(sp)
    800011b6:	6105                	addi	sp,sp,32
    800011b8:	8082                	ret

00000000800011ba <uvmunmap>:
// Remove npages of mappings starting from va. va must be
// page-aligned. It's OK if the mappings don't exist.
// Optionally free the physical memory.
void
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
    800011ba:	7139                	addi	sp,sp,-64
    800011bc:	fc06                	sd	ra,56(sp)
    800011be:	f822                	sd	s0,48(sp)
    800011c0:	0080                	addi	s0,sp,64
  uint64 a;
  pte_t *pte;

  if((va % PGSIZE) != 0)
    800011c2:	03459793          	slli	a5,a1,0x34
    800011c6:	e38d                	bnez	a5,800011e8 <uvmunmap+0x2e>
    800011c8:	f04a                	sd	s2,32(sp)
    800011ca:	ec4e                	sd	s3,24(sp)
    800011cc:	e852                	sd	s4,16(sp)
    800011ce:	e456                	sd	s5,8(sp)
    800011d0:	e05a                	sd	s6,0(sp)
    800011d2:	8a2a                	mv	s4,a0
    800011d4:	892e                	mv	s2,a1
    800011d6:	8ab6                	mv	s5,a3
    panic("uvmunmap: not aligned");

  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    800011d8:	0632                	slli	a2,a2,0xc
    800011da:	00b609b3          	add	s3,a2,a1
    800011de:	6b05                	lui	s6,0x1
    800011e0:	0535f963          	bgeu	a1,s3,80001232 <uvmunmap+0x78>
    800011e4:	f426                	sd	s1,40(sp)
    800011e6:	a015                	j	8000120a <uvmunmap+0x50>
    800011e8:	f426                	sd	s1,40(sp)
    800011ea:	f04a                	sd	s2,32(sp)
    800011ec:	ec4e                	sd	s3,24(sp)
    800011ee:	e852                	sd	s4,16(sp)
    800011f0:	e456                	sd	s5,8(sp)
    800011f2:	e05a                	sd	s6,0(sp)
    panic("uvmunmap: not aligned");
    800011f4:	00006517          	auipc	a0,0x6
    800011f8:	f2c50513          	addi	a0,a0,-212 # 80007120 <etext+0x120>
    800011fc:	de4ff0ef          	jal	800007e0 <panic>
      continue;
    if(do_free){
      uint64 pa = PTE2PA(*pte);
      kfree((void*)pa);
    }
    *pte = 0;
    80001200:	0004b023          	sd	zero,0(s1)
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    80001204:	995a                	add	s2,s2,s6
    80001206:	03397563          	bgeu	s2,s3,80001230 <uvmunmap+0x76>
    if((pte = walk(pagetable, a, 0)) == 0) // leaf page table entry allocated?
    8000120a:	4601                	li	a2,0
    8000120c:	85ca                	mv	a1,s2
    8000120e:	8552                	mv	a0,s4
    80001210:	d07ff0ef          	jal	80000f16 <walk>
    80001214:	84aa                	mv	s1,a0
    80001216:	d57d                	beqz	a0,80001204 <uvmunmap+0x4a>
    if((*pte & PTE_V) == 0)  // has physical page been allocated?
    80001218:	611c                	ld	a5,0(a0)
    8000121a:	0017f713          	andi	a4,a5,1
    8000121e:	d37d                	beqz	a4,80001204 <uvmunmap+0x4a>
    if(do_free){
    80001220:	fe0a80e3          	beqz	s5,80001200 <uvmunmap+0x46>
      uint64 pa = PTE2PA(*pte);
    80001224:	83a9                	srli	a5,a5,0xa
      kfree((void*)pa);
    80001226:	00c79513          	slli	a0,a5,0xc
    8000122a:	ff2ff0ef          	jal	80000a1c <kfree>
    8000122e:	bfc9                	j	80001200 <uvmunmap+0x46>
    80001230:	74a2                	ld	s1,40(sp)
    80001232:	7902                	ld	s2,32(sp)
    80001234:	69e2                	ld	s3,24(sp)
    80001236:	6a42                	ld	s4,16(sp)
    80001238:	6aa2                	ld	s5,8(sp)
    8000123a:	6b02                	ld	s6,0(sp)
  }
}
    8000123c:	70e2                	ld	ra,56(sp)
    8000123e:	7442                	ld	s0,48(sp)
    80001240:	6121                	addi	sp,sp,64
    80001242:	8082                	ret

0000000080001244 <uvmdealloc>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
uint64
uvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
    80001244:	1101                	addi	sp,sp,-32
    80001246:	ec06                	sd	ra,24(sp)
    80001248:	e822                	sd	s0,16(sp)
    8000124a:	e426                	sd	s1,8(sp)
    8000124c:	1000                	addi	s0,sp,32
  if(newsz >= oldsz)
    return oldsz;
    8000124e:	84ae                	mv	s1,a1
  if(newsz >= oldsz)
    80001250:	00b67d63          	bgeu	a2,a1,8000126a <uvmdealloc+0x26>
    80001254:	84b2                	mv	s1,a2

  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    80001256:	6785                	lui	a5,0x1
    80001258:	17fd                	addi	a5,a5,-1 # fff <_entry-0x7ffff001>
    8000125a:	00f60733          	add	a4,a2,a5
    8000125e:	76fd                	lui	a3,0xfffff
    80001260:	8f75                	and	a4,a4,a3
    80001262:	97ae                	add	a5,a5,a1
    80001264:	8ff5                	and	a5,a5,a3
    80001266:	00f76863          	bltu	a4,a5,80001276 <uvmdealloc+0x32>
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 1);
  }

  return newsz;
}
    8000126a:	8526                	mv	a0,s1
    8000126c:	60e2                	ld	ra,24(sp)
    8000126e:	6442                	ld	s0,16(sp)
    80001270:	64a2                	ld	s1,8(sp)
    80001272:	6105                	addi	sp,sp,32
    80001274:	8082                	ret
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    80001276:	8f99                	sub	a5,a5,a4
    80001278:	83b1                	srli	a5,a5,0xc
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 1);
    8000127a:	4685                	li	a3,1
    8000127c:	0007861b          	sext.w	a2,a5
    80001280:	85ba                	mv	a1,a4
    80001282:	f39ff0ef          	jal	800011ba <uvmunmap>
    80001286:	b7d5                	j	8000126a <uvmdealloc+0x26>

0000000080001288 <uvmalloc>:
  if(newsz < oldsz)
    80001288:	08b66f63          	bltu	a2,a1,80001326 <uvmalloc+0x9e>
{
    8000128c:	7139                	addi	sp,sp,-64
    8000128e:	fc06                	sd	ra,56(sp)
    80001290:	f822                	sd	s0,48(sp)
    80001292:	ec4e                	sd	s3,24(sp)
    80001294:	e852                	sd	s4,16(sp)
    80001296:	e456                	sd	s5,8(sp)
    80001298:	0080                	addi	s0,sp,64
    8000129a:	8aaa                	mv	s5,a0
    8000129c:	8a32                	mv	s4,a2
  oldsz = PGROUNDUP(oldsz);
    8000129e:	6785                	lui	a5,0x1
    800012a0:	17fd                	addi	a5,a5,-1 # fff <_entry-0x7ffff001>
    800012a2:	95be                	add	a1,a1,a5
    800012a4:	77fd                	lui	a5,0xfffff
    800012a6:	00f5f9b3          	and	s3,a1,a5
  for(a = oldsz; a < newsz; a += PGSIZE){
    800012aa:	08c9f063          	bgeu	s3,a2,8000132a <uvmalloc+0xa2>
    800012ae:	f426                	sd	s1,40(sp)
    800012b0:	f04a                	sd	s2,32(sp)
    800012b2:	e05a                	sd	s6,0(sp)
    800012b4:	894e                	mv	s2,s3
    if(mappages(pagetable, a, PGSIZE, (uint64)mem, PTE_R|PTE_U|xperm) != 0){
    800012b6:	0126eb13          	ori	s6,a3,18
    mem = kalloc();
    800012ba:	845ff0ef          	jal	80000afe <kalloc>
    800012be:	84aa                	mv	s1,a0
    if(mem == 0){
    800012c0:	c515                	beqz	a0,800012ec <uvmalloc+0x64>
    memset(mem, 0, PGSIZE);
    800012c2:	6605                	lui	a2,0x1
    800012c4:	4581                	li	a1,0
    800012c6:	9ddff0ef          	jal	80000ca2 <memset>
    if(mappages(pagetable, a, PGSIZE, (uint64)mem, PTE_R|PTE_U|xperm) != 0){
    800012ca:	875a                	mv	a4,s6
    800012cc:	86a6                	mv	a3,s1
    800012ce:	6605                	lui	a2,0x1
    800012d0:	85ca                	mv	a1,s2
    800012d2:	8556                	mv	a0,s5
    800012d4:	d1bff0ef          	jal	80000fee <mappages>
    800012d8:	e915                	bnez	a0,8000130c <uvmalloc+0x84>
  for(a = oldsz; a < newsz; a += PGSIZE){
    800012da:	6785                	lui	a5,0x1
    800012dc:	993e                	add	s2,s2,a5
    800012de:	fd496ee3          	bltu	s2,s4,800012ba <uvmalloc+0x32>
  return newsz;
    800012e2:	8552                	mv	a0,s4
    800012e4:	74a2                	ld	s1,40(sp)
    800012e6:	7902                	ld	s2,32(sp)
    800012e8:	6b02                	ld	s6,0(sp)
    800012ea:	a811                	j	800012fe <uvmalloc+0x76>
      uvmdealloc(pagetable, a, oldsz);
    800012ec:	864e                	mv	a2,s3
    800012ee:	85ca                	mv	a1,s2
    800012f0:	8556                	mv	a0,s5
    800012f2:	f53ff0ef          	jal	80001244 <uvmdealloc>
      return 0;
    800012f6:	4501                	li	a0,0
    800012f8:	74a2                	ld	s1,40(sp)
    800012fa:	7902                	ld	s2,32(sp)
    800012fc:	6b02                	ld	s6,0(sp)
}
    800012fe:	70e2                	ld	ra,56(sp)
    80001300:	7442                	ld	s0,48(sp)
    80001302:	69e2                	ld	s3,24(sp)
    80001304:	6a42                	ld	s4,16(sp)
    80001306:	6aa2                	ld	s5,8(sp)
    80001308:	6121                	addi	sp,sp,64
    8000130a:	8082                	ret
      kfree(mem);
    8000130c:	8526                	mv	a0,s1
    8000130e:	f0eff0ef          	jal	80000a1c <kfree>
      uvmdealloc(pagetable, a, oldsz);
    80001312:	864e                	mv	a2,s3
    80001314:	85ca                	mv	a1,s2
    80001316:	8556                	mv	a0,s5
    80001318:	f2dff0ef          	jal	80001244 <uvmdealloc>
      return 0;
    8000131c:	4501                	li	a0,0
    8000131e:	74a2                	ld	s1,40(sp)
    80001320:	7902                	ld	s2,32(sp)
    80001322:	6b02                	ld	s6,0(sp)
    80001324:	bfe9                	j	800012fe <uvmalloc+0x76>
    return oldsz;
    80001326:	852e                	mv	a0,a1
}
    80001328:	8082                	ret
  return newsz;
    8000132a:	8532                	mv	a0,a2
    8000132c:	bfc9                	j	800012fe <uvmalloc+0x76>

000000008000132e <freewalk>:

// Recursively free page-table pages.
// All leaf mappings must already have been removed.
void
freewalk(pagetable_t pagetable)
{
    8000132e:	7179                	addi	sp,sp,-48
    80001330:	f406                	sd	ra,40(sp)
    80001332:	f022                	sd	s0,32(sp)
    80001334:	ec26                	sd	s1,24(sp)
    80001336:	e84a                	sd	s2,16(sp)
    80001338:	e44e                	sd	s3,8(sp)
    8000133a:	e052                	sd	s4,0(sp)
    8000133c:	1800                	addi	s0,sp,48
    8000133e:	8a2a                	mv	s4,a0
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    80001340:	84aa                	mv	s1,a0
    80001342:	6905                	lui	s2,0x1
    80001344:	992a                	add	s2,s2,a0
    pte_t pte = pagetable[i];
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
    80001346:	4985                	li	s3,1
    80001348:	a819                	j	8000135e <freewalk+0x30>
      // this PTE points to a lower-level page table.
      uint64 child = PTE2PA(pte);
    8000134a:	83a9                	srli	a5,a5,0xa
      freewalk((pagetable_t)child);
    8000134c:	00c79513          	slli	a0,a5,0xc
    80001350:	fdfff0ef          	jal	8000132e <freewalk>
      pagetable[i] = 0;
    80001354:	0004b023          	sd	zero,0(s1)
  for(int i = 0; i < 512; i++){
    80001358:	04a1                	addi	s1,s1,8
    8000135a:	01248f63          	beq	s1,s2,80001378 <freewalk+0x4a>
    pte_t pte = pagetable[i];
    8000135e:	609c                	ld	a5,0(s1)
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
    80001360:	00f7f713          	andi	a4,a5,15
    80001364:	ff3703e3          	beq	a4,s3,8000134a <freewalk+0x1c>
    } else if(pte & PTE_V){
    80001368:	8b85                	andi	a5,a5,1
    8000136a:	d7fd                	beqz	a5,80001358 <freewalk+0x2a>
      panic("freewalk: leaf");
    8000136c:	00006517          	auipc	a0,0x6
    80001370:	dcc50513          	addi	a0,a0,-564 # 80007138 <etext+0x138>
    80001374:	c6cff0ef          	jal	800007e0 <panic>
    }
  }
  kfree((void*)pagetable);
    80001378:	8552                	mv	a0,s4
    8000137a:	ea2ff0ef          	jal	80000a1c <kfree>
}
    8000137e:	70a2                	ld	ra,40(sp)
    80001380:	7402                	ld	s0,32(sp)
    80001382:	64e2                	ld	s1,24(sp)
    80001384:	6942                	ld	s2,16(sp)
    80001386:	69a2                	ld	s3,8(sp)
    80001388:	6a02                	ld	s4,0(sp)
    8000138a:	6145                	addi	sp,sp,48
    8000138c:	8082                	ret

000000008000138e <uvmfree>:

// Free user memory pages,
// then free page-table pages.
void
uvmfree(pagetable_t pagetable, uint64 sz)
{
    8000138e:	1101                	addi	sp,sp,-32
    80001390:	ec06                	sd	ra,24(sp)
    80001392:	e822                	sd	s0,16(sp)
    80001394:	e426                	sd	s1,8(sp)
    80001396:	1000                	addi	s0,sp,32
    80001398:	84aa                	mv	s1,a0
  if(sz > 0)
    8000139a:	e989                	bnez	a1,800013ac <uvmfree+0x1e>
    uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1);
  freewalk(pagetable);
    8000139c:	8526                	mv	a0,s1
    8000139e:	f91ff0ef          	jal	8000132e <freewalk>
}
    800013a2:	60e2                	ld	ra,24(sp)
    800013a4:	6442                	ld	s0,16(sp)
    800013a6:	64a2                	ld	s1,8(sp)
    800013a8:	6105                	addi	sp,sp,32
    800013aa:	8082                	ret
    uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1);
    800013ac:	6785                	lui	a5,0x1
    800013ae:	17fd                	addi	a5,a5,-1 # fff <_entry-0x7ffff001>
    800013b0:	95be                	add	a1,a1,a5
    800013b2:	4685                	li	a3,1
    800013b4:	00c5d613          	srli	a2,a1,0xc
    800013b8:	4581                	li	a1,0
    800013ba:	e01ff0ef          	jal	800011ba <uvmunmap>
    800013be:	bff9                	j	8000139c <uvmfree+0xe>

00000000800013c0 <uvmcopy>:
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  char *mem;

  for(i = 0; i < sz; i += PGSIZE){
    800013c0:	ce49                	beqz	a2,8000145a <uvmcopy+0x9a>
{
    800013c2:	715d                	addi	sp,sp,-80
    800013c4:	e486                	sd	ra,72(sp)
    800013c6:	e0a2                	sd	s0,64(sp)
    800013c8:	fc26                	sd	s1,56(sp)
    800013ca:	f84a                	sd	s2,48(sp)
    800013cc:	f44e                	sd	s3,40(sp)
    800013ce:	f052                	sd	s4,32(sp)
    800013d0:	ec56                	sd	s5,24(sp)
    800013d2:	e85a                	sd	s6,16(sp)
    800013d4:	e45e                	sd	s7,8(sp)
    800013d6:	0880                	addi	s0,sp,80
    800013d8:	8aaa                	mv	s5,a0
    800013da:	8b2e                	mv	s6,a1
    800013dc:	8a32                	mv	s4,a2
  for(i = 0; i < sz; i += PGSIZE){
    800013de:	4481                	li	s1,0
    800013e0:	a029                	j	800013ea <uvmcopy+0x2a>
    800013e2:	6785                	lui	a5,0x1
    800013e4:	94be                	add	s1,s1,a5
    800013e6:	0544fe63          	bgeu	s1,s4,80001442 <uvmcopy+0x82>
    if((pte = walk(old, i, 0)) == 0)
    800013ea:	4601                	li	a2,0
    800013ec:	85a6                	mv	a1,s1
    800013ee:	8556                	mv	a0,s5
    800013f0:	b27ff0ef          	jal	80000f16 <walk>
    800013f4:	d57d                	beqz	a0,800013e2 <uvmcopy+0x22>
      continue;   // page table entry hasn't been allocated
    if((*pte & PTE_V) == 0)
    800013f6:	6118                	ld	a4,0(a0)
    800013f8:	00177793          	andi	a5,a4,1
    800013fc:	d3fd                	beqz	a5,800013e2 <uvmcopy+0x22>
      continue;   // physical page hasn't been allocated
    pa = PTE2PA(*pte);
    800013fe:	00a75593          	srli	a1,a4,0xa
    80001402:	00c59b93          	slli	s7,a1,0xc
    flags = PTE_FLAGS(*pte);
    80001406:	3ff77913          	andi	s2,a4,1023
    if((mem = kalloc()) == 0)
    8000140a:	ef4ff0ef          	jal	80000afe <kalloc>
    8000140e:	89aa                	mv	s3,a0
    80001410:	c105                	beqz	a0,80001430 <uvmcopy+0x70>
      goto err;
    memmove(mem, (char*)pa, PGSIZE);
    80001412:	6605                	lui	a2,0x1
    80001414:	85de                	mv	a1,s7
    80001416:	8e9ff0ef          	jal	80000cfe <memmove>
    if(mappages(new, i, PGSIZE, (uint64)mem, flags) != 0){
    8000141a:	874a                	mv	a4,s2
    8000141c:	86ce                	mv	a3,s3
    8000141e:	6605                	lui	a2,0x1
    80001420:	85a6                	mv	a1,s1
    80001422:	855a                	mv	a0,s6
    80001424:	bcbff0ef          	jal	80000fee <mappages>
    80001428:	dd4d                	beqz	a0,800013e2 <uvmcopy+0x22>
      kfree(mem);
    8000142a:	854e                	mv	a0,s3
    8000142c:	df0ff0ef          	jal	80000a1c <kfree>
    }
  }
  return 0;

 err:
  uvmunmap(new, 0, i / PGSIZE, 1);
    80001430:	4685                	li	a3,1
    80001432:	00c4d613          	srli	a2,s1,0xc
    80001436:	4581                	li	a1,0
    80001438:	855a                	mv	a0,s6
    8000143a:	d81ff0ef          	jal	800011ba <uvmunmap>
  return -1;
    8000143e:	557d                	li	a0,-1
    80001440:	a011                	j	80001444 <uvmcopy+0x84>
  return 0;
    80001442:	4501                	li	a0,0
}
    80001444:	60a6                	ld	ra,72(sp)
    80001446:	6406                	ld	s0,64(sp)
    80001448:	74e2                	ld	s1,56(sp)
    8000144a:	7942                	ld	s2,48(sp)
    8000144c:	79a2                	ld	s3,40(sp)
    8000144e:	7a02                	ld	s4,32(sp)
    80001450:	6ae2                	ld	s5,24(sp)
    80001452:	6b42                	ld	s6,16(sp)
    80001454:	6ba2                	ld	s7,8(sp)
    80001456:	6161                	addi	sp,sp,80
    80001458:	8082                	ret
  return 0;
    8000145a:	4501                	li	a0,0
}
    8000145c:	8082                	ret

000000008000145e <uvmclear>:

// mark a PTE invalid for user access.
// used by exec for the user stack guard page.
void
uvmclear(pagetable_t pagetable, uint64 va)
{
    8000145e:	1141                	addi	sp,sp,-16
    80001460:	e406                	sd	ra,8(sp)
    80001462:	e022                	sd	s0,0(sp)
    80001464:	0800                	addi	s0,sp,16
  pte_t *pte;
  
  pte = walk(pagetable, va, 0);
    80001466:	4601                	li	a2,0
    80001468:	aafff0ef          	jal	80000f16 <walk>
  if(pte == 0)
    8000146c:	c901                	beqz	a0,8000147c <uvmclear+0x1e>
    panic("uvmclear");
  *pte &= ~PTE_U;
    8000146e:	611c                	ld	a5,0(a0)
    80001470:	9bbd                	andi	a5,a5,-17
    80001472:	e11c                	sd	a5,0(a0)
}
    80001474:	60a2                	ld	ra,8(sp)
    80001476:	6402                	ld	s0,0(sp)
    80001478:	0141                	addi	sp,sp,16
    8000147a:	8082                	ret
    panic("uvmclear");
    8000147c:	00006517          	auipc	a0,0x6
    80001480:	ccc50513          	addi	a0,a0,-820 # 80007148 <etext+0x148>
    80001484:	b5cff0ef          	jal	800007e0 <panic>

0000000080001488 <copyinstr>:
copyinstr(pagetable_t pagetable, char *dst, uint64 srcva, uint64 max)
{
  uint64 n, va0, pa0;
  int got_null = 0;

  while(got_null == 0 && max > 0){
    80001488:	c6dd                	beqz	a3,80001536 <copyinstr+0xae>
{
    8000148a:	715d                	addi	sp,sp,-80
    8000148c:	e486                	sd	ra,72(sp)
    8000148e:	e0a2                	sd	s0,64(sp)
    80001490:	fc26                	sd	s1,56(sp)
    80001492:	f84a                	sd	s2,48(sp)
    80001494:	f44e                	sd	s3,40(sp)
    80001496:	f052                	sd	s4,32(sp)
    80001498:	ec56                	sd	s5,24(sp)
    8000149a:	e85a                	sd	s6,16(sp)
    8000149c:	e45e                	sd	s7,8(sp)
    8000149e:	0880                	addi	s0,sp,80
    800014a0:	8a2a                	mv	s4,a0
    800014a2:	8b2e                	mv	s6,a1
    800014a4:	8bb2                	mv	s7,a2
    800014a6:	8936                	mv	s2,a3
    va0 = PGROUNDDOWN(srcva);
    800014a8:	7afd                	lui	s5,0xfffff
    pa0 = walkaddr(pagetable, va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (srcva - va0);
    800014aa:	6985                	lui	s3,0x1
    800014ac:	a825                	j	800014e4 <copyinstr+0x5c>
      n = max;

    char *p = (char *) (pa0 + (srcva - va0));
    while(n > 0){
      if(*p == '\0'){
        *dst = '\0';
    800014ae:	00078023          	sb	zero,0(a5) # 1000 <_entry-0x7ffff000>
    800014b2:	4785                	li	a5,1
      dst++;
    }

    srcva = va0 + PGSIZE;
  }
  if(got_null){
    800014b4:	37fd                	addiw	a5,a5,-1
    800014b6:	0007851b          	sext.w	a0,a5
    return 0;
  } else {
    return -1;
  }
}
    800014ba:	60a6                	ld	ra,72(sp)
    800014bc:	6406                	ld	s0,64(sp)
    800014be:	74e2                	ld	s1,56(sp)
    800014c0:	7942                	ld	s2,48(sp)
    800014c2:	79a2                	ld	s3,40(sp)
    800014c4:	7a02                	ld	s4,32(sp)
    800014c6:	6ae2                	ld	s5,24(sp)
    800014c8:	6b42                	ld	s6,16(sp)
    800014ca:	6ba2                	ld	s7,8(sp)
    800014cc:	6161                	addi	sp,sp,80
    800014ce:	8082                	ret
    800014d0:	fff90713          	addi	a4,s2,-1 # fff <_entry-0x7ffff001>
    800014d4:	9742                	add	a4,a4,a6
      --max;
    800014d6:	40b70933          	sub	s2,a4,a1
    srcva = va0 + PGSIZE;
    800014da:	01348bb3          	add	s7,s1,s3
  while(got_null == 0 && max > 0){
    800014de:	04e58463          	beq	a1,a4,80001526 <copyinstr+0x9e>
{
    800014e2:	8b3e                	mv	s6,a5
    va0 = PGROUNDDOWN(srcva);
    800014e4:	015bf4b3          	and	s1,s7,s5
    pa0 = walkaddr(pagetable, va0);
    800014e8:	85a6                	mv	a1,s1
    800014ea:	8552                	mv	a0,s4
    800014ec:	ac5ff0ef          	jal	80000fb0 <walkaddr>
    if(pa0 == 0)
    800014f0:	cd0d                	beqz	a0,8000152a <copyinstr+0xa2>
    n = PGSIZE - (srcva - va0);
    800014f2:	417486b3          	sub	a3,s1,s7
    800014f6:	96ce                	add	a3,a3,s3
    if(n > max)
    800014f8:	00d97363          	bgeu	s2,a3,800014fe <copyinstr+0x76>
    800014fc:	86ca                	mv	a3,s2
    char *p = (char *) (pa0 + (srcva - va0));
    800014fe:	955e                	add	a0,a0,s7
    80001500:	8d05                	sub	a0,a0,s1
    while(n > 0){
    80001502:	c695                	beqz	a3,8000152e <copyinstr+0xa6>
    80001504:	87da                	mv	a5,s6
    80001506:	885a                	mv	a6,s6
      if(*p == '\0'){
    80001508:	41650633          	sub	a2,a0,s6
    while(n > 0){
    8000150c:	96da                	add	a3,a3,s6
    8000150e:	85be                	mv	a1,a5
      if(*p == '\0'){
    80001510:	00f60733          	add	a4,a2,a5
    80001514:	00074703          	lbu	a4,0(a4)
    80001518:	db59                	beqz	a4,800014ae <copyinstr+0x26>
        *dst = *p;
    8000151a:	00e78023          	sb	a4,0(a5)
      dst++;
    8000151e:	0785                	addi	a5,a5,1
    while(n > 0){
    80001520:	fed797e3          	bne	a5,a3,8000150e <copyinstr+0x86>
    80001524:	b775                	j	800014d0 <copyinstr+0x48>
    80001526:	4781                	li	a5,0
    80001528:	b771                	j	800014b4 <copyinstr+0x2c>
      return -1;
    8000152a:	557d                	li	a0,-1
    8000152c:	b779                	j	800014ba <copyinstr+0x32>
    srcva = va0 + PGSIZE;
    8000152e:	6b85                	lui	s7,0x1
    80001530:	9ba6                	add	s7,s7,s1
    80001532:	87da                	mv	a5,s6
    80001534:	b77d                	j	800014e2 <copyinstr+0x5a>
  int got_null = 0;
    80001536:	4781                	li	a5,0
  if(got_null){
    80001538:	37fd                	addiw	a5,a5,-1
    8000153a:	0007851b          	sext.w	a0,a5
}
    8000153e:	8082                	ret

0000000080001540 <ismapped>:
  return mem;
}

int
ismapped(pagetable_t pagetable, uint64 va)
{
    80001540:	1141                	addi	sp,sp,-16
    80001542:	e406                	sd	ra,8(sp)
    80001544:	e022                	sd	s0,0(sp)
    80001546:	0800                	addi	s0,sp,16
  pte_t *pte = walk(pagetable, va, 0);
    80001548:	4601                	li	a2,0
    8000154a:	9cdff0ef          	jal	80000f16 <walk>
  if (pte == 0) {
    8000154e:	c519                	beqz	a0,8000155c <ismapped+0x1c>
    return 0;
  }
  if (*pte & PTE_V){
    80001550:	6108                	ld	a0,0(a0)
    80001552:	8905                	andi	a0,a0,1
    return 1;
  }
  return 0;
}
    80001554:	60a2                	ld	ra,8(sp)
    80001556:	6402                	ld	s0,0(sp)
    80001558:	0141                	addi	sp,sp,16
    8000155a:	8082                	ret
    return 0;
    8000155c:	4501                	li	a0,0
    8000155e:	bfdd                	j	80001554 <ismapped+0x14>

0000000080001560 <vmfault>:
{
    80001560:	7179                	addi	sp,sp,-48
    80001562:	f406                	sd	ra,40(sp)
    80001564:	f022                	sd	s0,32(sp)
    80001566:	ec26                	sd	s1,24(sp)
    80001568:	e44e                	sd	s3,8(sp)
    8000156a:	1800                	addi	s0,sp,48
    8000156c:	89aa                	mv	s3,a0
    8000156e:	84ae                	mv	s1,a1
  struct proc *p = myproc();
    80001570:	3c0000ef          	jal	80001930 <myproc>
  if (va >= p->sz)
    80001574:	653c                	ld	a5,72(a0)
    80001576:	00f4ea63          	bltu	s1,a5,8000158a <vmfault+0x2a>
    return 0;
    8000157a:	4981                	li	s3,0
}
    8000157c:	854e                	mv	a0,s3
    8000157e:	70a2                	ld	ra,40(sp)
    80001580:	7402                	ld	s0,32(sp)
    80001582:	64e2                	ld	s1,24(sp)
    80001584:	69a2                	ld	s3,8(sp)
    80001586:	6145                	addi	sp,sp,48
    80001588:	8082                	ret
    8000158a:	e84a                	sd	s2,16(sp)
    8000158c:	892a                	mv	s2,a0
  va = PGROUNDDOWN(va);
    8000158e:	77fd                	lui	a5,0xfffff
    80001590:	8cfd                	and	s1,s1,a5
  if(ismapped(pagetable, va)) {
    80001592:	85a6                	mv	a1,s1
    80001594:	854e                	mv	a0,s3
    80001596:	fabff0ef          	jal	80001540 <ismapped>
    return 0;
    8000159a:	4981                	li	s3,0
  if(ismapped(pagetable, va)) {
    8000159c:	c119                	beqz	a0,800015a2 <vmfault+0x42>
    8000159e:	6942                	ld	s2,16(sp)
    800015a0:	bff1                	j	8000157c <vmfault+0x1c>
    800015a2:	e052                	sd	s4,0(sp)
  mem = (uint64) kalloc();
    800015a4:	d5aff0ef          	jal	80000afe <kalloc>
    800015a8:	8a2a                	mv	s4,a0
  if(mem == 0)
    800015aa:	c90d                	beqz	a0,800015dc <vmfault+0x7c>
  mem = (uint64) kalloc();
    800015ac:	89aa                	mv	s3,a0
  memset((void *) mem, 0, PGSIZE);
    800015ae:	6605                	lui	a2,0x1
    800015b0:	4581                	li	a1,0
    800015b2:	ef0ff0ef          	jal	80000ca2 <memset>
  if (mappages(p->pagetable, va, PGSIZE, mem, PTE_W|PTE_U|PTE_R) != 0) {
    800015b6:	4759                	li	a4,22
    800015b8:	86d2                	mv	a3,s4
    800015ba:	6605                	lui	a2,0x1
    800015bc:	85a6                	mv	a1,s1
    800015be:	05093503          	ld	a0,80(s2)
    800015c2:	a2dff0ef          	jal	80000fee <mappages>
    800015c6:	e501                	bnez	a0,800015ce <vmfault+0x6e>
    800015c8:	6942                	ld	s2,16(sp)
    800015ca:	6a02                	ld	s4,0(sp)
    800015cc:	bf45                	j	8000157c <vmfault+0x1c>
    kfree((void *)mem);
    800015ce:	8552                	mv	a0,s4
    800015d0:	c4cff0ef          	jal	80000a1c <kfree>
    return 0;
    800015d4:	4981                	li	s3,0
    800015d6:	6942                	ld	s2,16(sp)
    800015d8:	6a02                	ld	s4,0(sp)
    800015da:	b74d                	j	8000157c <vmfault+0x1c>
    800015dc:	6942                	ld	s2,16(sp)
    800015de:	6a02                	ld	s4,0(sp)
    800015e0:	bf71                	j	8000157c <vmfault+0x1c>

00000000800015e2 <copyout>:
  while(len > 0){
    800015e2:	c2cd                	beqz	a3,80001684 <copyout+0xa2>
{
    800015e4:	711d                	addi	sp,sp,-96
    800015e6:	ec86                	sd	ra,88(sp)
    800015e8:	e8a2                	sd	s0,80(sp)
    800015ea:	e4a6                	sd	s1,72(sp)
    800015ec:	f852                	sd	s4,48(sp)
    800015ee:	f05a                	sd	s6,32(sp)
    800015f0:	ec5e                	sd	s7,24(sp)
    800015f2:	e862                	sd	s8,16(sp)
    800015f4:	1080                	addi	s0,sp,96
    800015f6:	8c2a                	mv	s8,a0
    800015f8:	8b2e                	mv	s6,a1
    800015fa:	8bb2                	mv	s7,a2
    800015fc:	8a36                	mv	s4,a3
    va0 = PGROUNDDOWN(dstva);
    800015fe:	74fd                	lui	s1,0xfffff
    80001600:	8ced                	and	s1,s1,a1
    if(va0 >= MAXVA)
    80001602:	57fd                	li	a5,-1
    80001604:	83e9                	srli	a5,a5,0x1a
    80001606:	0897e163          	bltu	a5,s1,80001688 <copyout+0xa6>
    8000160a:	e0ca                	sd	s2,64(sp)
    8000160c:	fc4e                	sd	s3,56(sp)
    8000160e:	f456                	sd	s5,40(sp)
    80001610:	e466                	sd	s9,8(sp)
    80001612:	e06a                	sd	s10,0(sp)
    80001614:	6d05                	lui	s10,0x1
    80001616:	8cbe                	mv	s9,a5
    80001618:	a015                	j	8000163c <copyout+0x5a>
    memmove((void *)(pa0 + (dstva - va0)), src, n);
    8000161a:	409b0533          	sub	a0,s6,s1
    8000161e:	0009861b          	sext.w	a2,s3
    80001622:	85de                	mv	a1,s7
    80001624:	954a                	add	a0,a0,s2
    80001626:	ed8ff0ef          	jal	80000cfe <memmove>
    len -= n;
    8000162a:	413a0a33          	sub	s4,s4,s3
    src += n;
    8000162e:	9bce                	add	s7,s7,s3
  while(len > 0){
    80001630:	040a0363          	beqz	s4,80001676 <copyout+0x94>
    if(va0 >= MAXVA)
    80001634:	055cec63          	bltu	s9,s5,8000168c <copyout+0xaa>
    80001638:	84d6                	mv	s1,s5
    8000163a:	8b56                	mv	s6,s5
    pa0 = walkaddr(pagetable, va0);
    8000163c:	85a6                	mv	a1,s1
    8000163e:	8562                	mv	a0,s8
    80001640:	971ff0ef          	jal	80000fb0 <walkaddr>
    80001644:	892a                	mv	s2,a0
    if(pa0 == 0) {
    80001646:	e901                	bnez	a0,80001656 <copyout+0x74>
      if((pa0 = vmfault(pagetable, va0, 0)) == 0) {
    80001648:	4601                	li	a2,0
    8000164a:	85a6                	mv	a1,s1
    8000164c:	8562                	mv	a0,s8
    8000164e:	f13ff0ef          	jal	80001560 <vmfault>
    80001652:	892a                	mv	s2,a0
    80001654:	c139                	beqz	a0,8000169a <copyout+0xb8>
    pte = walk(pagetable, va0, 0);
    80001656:	4601                	li	a2,0
    80001658:	85a6                	mv	a1,s1
    8000165a:	8562                	mv	a0,s8
    8000165c:	8bbff0ef          	jal	80000f16 <walk>
    if((*pte & PTE_W) == 0)
    80001660:	611c                	ld	a5,0(a0)
    80001662:	8b91                	andi	a5,a5,4
    80001664:	c3b1                	beqz	a5,800016a8 <copyout+0xc6>
    n = PGSIZE - (dstva - va0);
    80001666:	01a48ab3          	add	s5,s1,s10
    8000166a:	416a89b3          	sub	s3,s5,s6
    if(n > len)
    8000166e:	fb3a76e3          	bgeu	s4,s3,8000161a <copyout+0x38>
    80001672:	89d2                	mv	s3,s4
    80001674:	b75d                	j	8000161a <copyout+0x38>
  return 0;
    80001676:	4501                	li	a0,0
    80001678:	6906                	ld	s2,64(sp)
    8000167a:	79e2                	ld	s3,56(sp)
    8000167c:	7aa2                	ld	s5,40(sp)
    8000167e:	6ca2                	ld	s9,8(sp)
    80001680:	6d02                	ld	s10,0(sp)
    80001682:	a80d                	j	800016b4 <copyout+0xd2>
    80001684:	4501                	li	a0,0
}
    80001686:	8082                	ret
      return -1;
    80001688:	557d                	li	a0,-1
    8000168a:	a02d                	j	800016b4 <copyout+0xd2>
    8000168c:	557d                	li	a0,-1
    8000168e:	6906                	ld	s2,64(sp)
    80001690:	79e2                	ld	s3,56(sp)
    80001692:	7aa2                	ld	s5,40(sp)
    80001694:	6ca2                	ld	s9,8(sp)
    80001696:	6d02                	ld	s10,0(sp)
    80001698:	a831                	j	800016b4 <copyout+0xd2>
        return -1;
    8000169a:	557d                	li	a0,-1
    8000169c:	6906                	ld	s2,64(sp)
    8000169e:	79e2                	ld	s3,56(sp)
    800016a0:	7aa2                	ld	s5,40(sp)
    800016a2:	6ca2                	ld	s9,8(sp)
    800016a4:	6d02                	ld	s10,0(sp)
    800016a6:	a039                	j	800016b4 <copyout+0xd2>
      return -1;
    800016a8:	557d                	li	a0,-1
    800016aa:	6906                	ld	s2,64(sp)
    800016ac:	79e2                	ld	s3,56(sp)
    800016ae:	7aa2                	ld	s5,40(sp)
    800016b0:	6ca2                	ld	s9,8(sp)
    800016b2:	6d02                	ld	s10,0(sp)
}
    800016b4:	60e6                	ld	ra,88(sp)
    800016b6:	6446                	ld	s0,80(sp)
    800016b8:	64a6                	ld	s1,72(sp)
    800016ba:	7a42                	ld	s4,48(sp)
    800016bc:	7b02                	ld	s6,32(sp)
    800016be:	6be2                	ld	s7,24(sp)
    800016c0:	6c42                	ld	s8,16(sp)
    800016c2:	6125                	addi	sp,sp,96
    800016c4:	8082                	ret

00000000800016c6 <copyin>:
  while(len > 0){
    800016c6:	c6c9                	beqz	a3,80001750 <copyin+0x8a>
{
    800016c8:	715d                	addi	sp,sp,-80
    800016ca:	e486                	sd	ra,72(sp)
    800016cc:	e0a2                	sd	s0,64(sp)
    800016ce:	fc26                	sd	s1,56(sp)
    800016d0:	f84a                	sd	s2,48(sp)
    800016d2:	f44e                	sd	s3,40(sp)
    800016d4:	f052                	sd	s4,32(sp)
    800016d6:	ec56                	sd	s5,24(sp)
    800016d8:	e85a                	sd	s6,16(sp)
    800016da:	e45e                	sd	s7,8(sp)
    800016dc:	e062                	sd	s8,0(sp)
    800016de:	0880                	addi	s0,sp,80
    800016e0:	8baa                	mv	s7,a0
    800016e2:	8aae                	mv	s5,a1
    800016e4:	8932                	mv	s2,a2
    800016e6:	8a36                	mv	s4,a3
    va0 = PGROUNDDOWN(srcva);
    800016e8:	7c7d                	lui	s8,0xfffff
    n = PGSIZE - (srcva - va0);
    800016ea:	6b05                	lui	s6,0x1
    800016ec:	a035                	j	80001718 <copyin+0x52>
    800016ee:	412984b3          	sub	s1,s3,s2
    800016f2:	94da                	add	s1,s1,s6
    if(n > len)
    800016f4:	009a7363          	bgeu	s4,s1,800016fa <copyin+0x34>
    800016f8:	84d2                	mv	s1,s4
    memmove(dst, (void *)(pa0 + (srcva - va0)), n);
    800016fa:	413905b3          	sub	a1,s2,s3
    800016fe:	0004861b          	sext.w	a2,s1
    80001702:	95aa                	add	a1,a1,a0
    80001704:	8556                	mv	a0,s5
    80001706:	df8ff0ef          	jal	80000cfe <memmove>
    len -= n;
    8000170a:	409a0a33          	sub	s4,s4,s1
    dst += n;
    8000170e:	9aa6                	add	s5,s5,s1
    srcva = va0 + PGSIZE;
    80001710:	01698933          	add	s2,s3,s6
  while(len > 0){
    80001714:	020a0163          	beqz	s4,80001736 <copyin+0x70>
    va0 = PGROUNDDOWN(srcva);
    80001718:	018979b3          	and	s3,s2,s8
    pa0 = walkaddr(pagetable, va0);
    8000171c:	85ce                	mv	a1,s3
    8000171e:	855e                	mv	a0,s7
    80001720:	891ff0ef          	jal	80000fb0 <walkaddr>
    if(pa0 == 0) {
    80001724:	f569                	bnez	a0,800016ee <copyin+0x28>
      if((pa0 = vmfault(pagetable, va0, 0)) == 0) {
    80001726:	4601                	li	a2,0
    80001728:	85ce                	mv	a1,s3
    8000172a:	855e                	mv	a0,s7
    8000172c:	e35ff0ef          	jal	80001560 <vmfault>
    80001730:	fd5d                	bnez	a0,800016ee <copyin+0x28>
        return -1;
    80001732:	557d                	li	a0,-1
    80001734:	a011                	j	80001738 <copyin+0x72>
  return 0;
    80001736:	4501                	li	a0,0
}
    80001738:	60a6                	ld	ra,72(sp)
    8000173a:	6406                	ld	s0,64(sp)
    8000173c:	74e2                	ld	s1,56(sp)
    8000173e:	7942                	ld	s2,48(sp)
    80001740:	79a2                	ld	s3,40(sp)
    80001742:	7a02                	ld	s4,32(sp)
    80001744:	6ae2                	ld	s5,24(sp)
    80001746:	6b42                	ld	s6,16(sp)
    80001748:	6ba2                	ld	s7,8(sp)
    8000174a:	6c02                	ld	s8,0(sp)
    8000174c:	6161                	addi	sp,sp,80
    8000174e:	8082                	ret
  return 0;
    80001750:	4501                	li	a0,0
}
    80001752:	8082                	ret

0000000080001754 <count_phys_pages>:

uint64
count_phys_pages(struct proc *p)
{
    80001754:	7139                	addi	sp,sp,-64
    80001756:	fc06                	sd	ra,56(sp)
    80001758:	f822                	sd	s0,48(sp)
    8000175a:	ec4e                	sd	s3,24(sp)
    8000175c:	0080                	addi	s0,sp,64
  uint64 count = 0;
  for(uint64 va = 0; va < p->sz; va += PGSIZE){
    8000175e:	04853983          	ld	s3,72(a0)
    80001762:	04098463          	beqz	s3,800017aa <count_phys_pages+0x56>
    80001766:	f426                	sd	s1,40(sp)
    80001768:	f04a                	sd	s2,32(sp)
    8000176a:	e852                	sd	s4,16(sp)
    8000176c:	e456                	sd	s5,8(sp)
    8000176e:	892a                	mv	s2,a0
    80001770:	4481                	li	s1,0
  uint64 count = 0;
    80001772:	4981                	li	s3,0
    pte_t *pte = walk(p->pagetable, va, 0);
    if(pte != 0 && (*pte & PTE_V) && (*pte & PTE_U)){
    80001774:	4ac5                	li	s5,17
  for(uint64 va = 0; va < p->sz; va += PGSIZE){
    80001776:	6a05                	lui	s4,0x1
    80001778:	a829                	j	80001792 <count_phys_pages+0x3e>
    if(pte != 0 && (*pte & PTE_V) && (*pte & PTE_U)){
    8000177a:	611c                	ld	a5,0(a0)
    8000177c:	8bc5                	andi	a5,a5,17
      count++;
    8000177e:	415787b3          	sub	a5,a5,s5
    80001782:	0017b793          	seqz	a5,a5
    80001786:	99be                	add	s3,s3,a5
  for(uint64 va = 0; va < p->sz; va += PGSIZE){
    80001788:	94d2                	add	s1,s1,s4
    8000178a:	04893783          	ld	a5,72(s2)
    8000178e:	00f4fa63          	bgeu	s1,a5,800017a2 <count_phys_pages+0x4e>
    pte_t *pte = walk(p->pagetable, va, 0);
    80001792:	4601                	li	a2,0
    80001794:	85a6                	mv	a1,s1
    80001796:	05093503          	ld	a0,80(s2)
    8000179a:	f7cff0ef          	jal	80000f16 <walk>
    if(pte != 0 && (*pte & PTE_V) && (*pte & PTE_U)){
    8000179e:	fd71                	bnez	a0,8000177a <count_phys_pages+0x26>
    800017a0:	b7e5                	j	80001788 <count_phys_pages+0x34>
    800017a2:	74a2                	ld	s1,40(sp)
    800017a4:	7902                	ld	s2,32(sp)
    800017a6:	6a42                	ld	s4,16(sp)
    800017a8:	6aa2                	ld	s5,8(sp)
    }
  }
  return count;
}
    800017aa:	854e                	mv	a0,s3
    800017ac:	70e2                	ld	ra,56(sp)
    800017ae:	7442                	ld	s0,48(sp)
    800017b0:	69e2                	ld	s3,24(sp)
    800017b2:	6121                	addi	sp,sp,64
    800017b4:	8082                	ret

00000000800017b6 <proc_mapstacks>:
// Allocate a page for each process's kernel stack.
// Map it high in memory, followed by an invalid
// guard page.
void
proc_mapstacks(pagetable_t kpgtbl)
{
    800017b6:	7139                	addi	sp,sp,-64
    800017b8:	fc06                	sd	ra,56(sp)
    800017ba:	f822                	sd	s0,48(sp)
    800017bc:	f426                	sd	s1,40(sp)
    800017be:	f04a                	sd	s2,32(sp)
    800017c0:	ec4e                	sd	s3,24(sp)
    800017c2:	e852                	sd	s4,16(sp)
    800017c4:	e456                	sd	s5,8(sp)
    800017c6:	e05a                	sd	s6,0(sp)
    800017c8:	0080                	addi	s0,sp,64
    800017ca:	8a2a                	mv	s4,a0
  struct proc *p;
  
  for(p = proc; p < &proc[NPROC]; p++) {
    800017cc:	0000e497          	auipc	s1,0xe
    800017d0:	5cc48493          	addi	s1,s1,1484 # 8000fd98 <proc>
    char *pa = kalloc();
    if(pa == 0)
      panic("kalloc");
    uint64 va = KSTACK((int) (p - proc));
    800017d4:	8b26                	mv	s6,s1
    800017d6:	ff4df937          	lui	s2,0xff4df
    800017da:	9bd90913          	addi	s2,s2,-1603 # ffffffffff4de9bd <end+0xffffffff7f4bdc45>
    800017de:	0936                	slli	s2,s2,0xd
    800017e0:	6f590913          	addi	s2,s2,1781
    800017e4:	0936                	slli	s2,s2,0xd
    800017e6:	bd390913          	addi	s2,s2,-1069
    800017ea:	0932                	slli	s2,s2,0xc
    800017ec:	7a790913          	addi	s2,s2,1959
    800017f0:	040009b7          	lui	s3,0x4000
    800017f4:	19fd                	addi	s3,s3,-1 # 3ffffff <_entry-0x7c000001>
    800017f6:	09b2                	slli	s3,s3,0xc
  for(p = proc; p < &proc[NPROC]; p++) {
    800017f8:	00014a97          	auipc	s5,0x14
    800017fc:	1a0a8a93          	addi	s5,s5,416 # 80015998 <tickslock>
    char *pa = kalloc();
    80001800:	afeff0ef          	jal	80000afe <kalloc>
    80001804:	862a                	mv	a2,a0
    if(pa == 0)
    80001806:	cd15                	beqz	a0,80001842 <proc_mapstacks+0x8c>
    uint64 va = KSTACK((int) (p - proc));
    80001808:	416485b3          	sub	a1,s1,s6
    8000180c:	8591                	srai	a1,a1,0x4
    8000180e:	032585b3          	mul	a1,a1,s2
    80001812:	2585                	addiw	a1,a1,1
    80001814:	00d5959b          	slliw	a1,a1,0xd
    kvmmap(kpgtbl, va, (uint64)pa, PGSIZE, PTE_R | PTE_W);
    80001818:	4719                	li	a4,6
    8000181a:	6685                	lui	a3,0x1
    8000181c:	40b985b3          	sub	a1,s3,a1
    80001820:	8552                	mv	a0,s4
    80001822:	87dff0ef          	jal	8000109e <kvmmap>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001826:	17048493          	addi	s1,s1,368
    8000182a:	fd549be3          	bne	s1,s5,80001800 <proc_mapstacks+0x4a>
  }
}
    8000182e:	70e2                	ld	ra,56(sp)
    80001830:	7442                	ld	s0,48(sp)
    80001832:	74a2                	ld	s1,40(sp)
    80001834:	7902                	ld	s2,32(sp)
    80001836:	69e2                	ld	s3,24(sp)
    80001838:	6a42                	ld	s4,16(sp)
    8000183a:	6aa2                	ld	s5,8(sp)
    8000183c:	6b02                	ld	s6,0(sp)
    8000183e:	6121                	addi	sp,sp,64
    80001840:	8082                	ret
      panic("kalloc");
    80001842:	00006517          	auipc	a0,0x6
    80001846:	91650513          	addi	a0,a0,-1770 # 80007158 <etext+0x158>
    8000184a:	f97fe0ef          	jal	800007e0 <panic>

000000008000184e <procinit>:

// initialize the proc table.
void
procinit(void)
{
    8000184e:	7139                	addi	sp,sp,-64
    80001850:	fc06                	sd	ra,56(sp)
    80001852:	f822                	sd	s0,48(sp)
    80001854:	f426                	sd	s1,40(sp)
    80001856:	f04a                	sd	s2,32(sp)
    80001858:	ec4e                	sd	s3,24(sp)
    8000185a:	e852                	sd	s4,16(sp)
    8000185c:	e456                	sd	s5,8(sp)
    8000185e:	e05a                	sd	s6,0(sp)
    80001860:	0080                	addi	s0,sp,64
  struct proc *p;
  
  initlock(&pid_lock, "nextpid");
    80001862:	00006597          	auipc	a1,0x6
    80001866:	8fe58593          	addi	a1,a1,-1794 # 80007160 <etext+0x160>
    8000186a:	0000e517          	auipc	a0,0xe
    8000186e:	0fe50513          	addi	a0,a0,254 # 8000f968 <pid_lock>
    80001872:	adcff0ef          	jal	80000b4e <initlock>
  initlock(&wait_lock, "wait_lock");
    80001876:	00006597          	auipc	a1,0x6
    8000187a:	8f258593          	addi	a1,a1,-1806 # 80007168 <etext+0x168>
    8000187e:	0000e517          	auipc	a0,0xe
    80001882:	10250513          	addi	a0,a0,258 # 8000f980 <wait_lock>
    80001886:	ac8ff0ef          	jal	80000b4e <initlock>
  for(p = proc; p < &proc[NPROC]; p++) {
    8000188a:	0000e497          	auipc	s1,0xe
    8000188e:	50e48493          	addi	s1,s1,1294 # 8000fd98 <proc>
      initlock(&p->lock, "proc");
    80001892:	00006b17          	auipc	s6,0x6
    80001896:	8e6b0b13          	addi	s6,s6,-1818 # 80007178 <etext+0x178>
      p->state = UNUSED;
      p->kstack = KSTACK((int) (p - proc));
    8000189a:	8aa6                	mv	s5,s1
    8000189c:	ff4df937          	lui	s2,0xff4df
    800018a0:	9bd90913          	addi	s2,s2,-1603 # ffffffffff4de9bd <end+0xffffffff7f4bdc45>
    800018a4:	0936                	slli	s2,s2,0xd
    800018a6:	6f590913          	addi	s2,s2,1781
    800018aa:	0936                	slli	s2,s2,0xd
    800018ac:	bd390913          	addi	s2,s2,-1069
    800018b0:	0932                	slli	s2,s2,0xc
    800018b2:	7a790913          	addi	s2,s2,1959
    800018b6:	040009b7          	lui	s3,0x4000
    800018ba:	19fd                	addi	s3,s3,-1 # 3ffffff <_entry-0x7c000001>
    800018bc:	09b2                	slli	s3,s3,0xc
  for(p = proc; p < &proc[NPROC]; p++) {
    800018be:	00014a17          	auipc	s4,0x14
    800018c2:	0daa0a13          	addi	s4,s4,218 # 80015998 <tickslock>
      initlock(&p->lock, "proc");
    800018c6:	85da                	mv	a1,s6
    800018c8:	8526                	mv	a0,s1
    800018ca:	a84ff0ef          	jal	80000b4e <initlock>
      p->state = UNUSED;
    800018ce:	0004ac23          	sw	zero,24(s1)
      p->kstack = KSTACK((int) (p - proc));
    800018d2:	415487b3          	sub	a5,s1,s5
    800018d6:	8791                	srai	a5,a5,0x4
    800018d8:	032787b3          	mul	a5,a5,s2
    800018dc:	2785                	addiw	a5,a5,1 # fffffffffffff001 <end+0xffffffff7ffde289>
    800018de:	00d7979b          	slliw	a5,a5,0xd
    800018e2:	40f987b3          	sub	a5,s3,a5
    800018e6:	e0bc                	sd	a5,64(s1)
  for(p = proc; p < &proc[NPROC]; p++) {
    800018e8:	17048493          	addi	s1,s1,368
    800018ec:	fd449de3          	bne	s1,s4,800018c6 <procinit+0x78>
  }
}
    800018f0:	70e2                	ld	ra,56(sp)
    800018f2:	7442                	ld	s0,48(sp)
    800018f4:	74a2                	ld	s1,40(sp)
    800018f6:	7902                	ld	s2,32(sp)
    800018f8:	69e2                	ld	s3,24(sp)
    800018fa:	6a42                	ld	s4,16(sp)
    800018fc:	6aa2                	ld	s5,8(sp)
    800018fe:	6b02                	ld	s6,0(sp)
    80001900:	6121                	addi	sp,sp,64
    80001902:	8082                	ret

0000000080001904 <cpuid>:
// Must be called with interrupts disabled,
// to prevent race with process being moved
// to a different CPU.
int
cpuid()
{
    80001904:	1141                	addi	sp,sp,-16
    80001906:	e422                	sd	s0,8(sp)
    80001908:	0800                	addi	s0,sp,16
  asm volatile("mv %0, tp" : "=r" (x) );
    8000190a:	8512                	mv	a0,tp
  int id = r_tp();
  return id;
}
    8000190c:	2501                	sext.w	a0,a0
    8000190e:	6422                	ld	s0,8(sp)
    80001910:	0141                	addi	sp,sp,16
    80001912:	8082                	ret

0000000080001914 <mycpu>:

// Return this CPU's cpu struct.
// Interrupts must be disabled.
struct cpu*
mycpu(void)
{
    80001914:	1141                	addi	sp,sp,-16
    80001916:	e422                	sd	s0,8(sp)
    80001918:	0800                	addi	s0,sp,16
    8000191a:	8792                	mv	a5,tp
  int id = cpuid();
  struct cpu *c = &cpus[id];
    8000191c:	2781                	sext.w	a5,a5
    8000191e:	079e                	slli	a5,a5,0x7
  return c;
}
    80001920:	0000e517          	auipc	a0,0xe
    80001924:	07850513          	addi	a0,a0,120 # 8000f998 <cpus>
    80001928:	953e                	add	a0,a0,a5
    8000192a:	6422                	ld	s0,8(sp)
    8000192c:	0141                	addi	sp,sp,16
    8000192e:	8082                	ret

0000000080001930 <myproc>:

// Return the current struct proc *, or zero if none.
struct proc*
myproc(void)
{
    80001930:	1101                	addi	sp,sp,-32
    80001932:	ec06                	sd	ra,24(sp)
    80001934:	e822                	sd	s0,16(sp)
    80001936:	e426                	sd	s1,8(sp)
    80001938:	1000                	addi	s0,sp,32
  push_off();
    8000193a:	a54ff0ef          	jal	80000b8e <push_off>
    8000193e:	8792                	mv	a5,tp
  struct cpu *c = mycpu();
  struct proc *p = c->proc;
    80001940:	2781                	sext.w	a5,a5
    80001942:	079e                	slli	a5,a5,0x7
    80001944:	0000e717          	auipc	a4,0xe
    80001948:	02470713          	addi	a4,a4,36 # 8000f968 <pid_lock>
    8000194c:	97ba                	add	a5,a5,a4
    8000194e:	7b84                	ld	s1,48(a5)
  pop_off();
    80001950:	ac2ff0ef          	jal	80000c12 <pop_off>
  return p;
}
    80001954:	8526                	mv	a0,s1
    80001956:	60e2                	ld	ra,24(sp)
    80001958:	6442                	ld	s0,16(sp)
    8000195a:	64a2                	ld	s1,8(sp)
    8000195c:	6105                	addi	sp,sp,32
    8000195e:	8082                	ret

0000000080001960 <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch to forkret.
void
forkret(void)
{
    80001960:	7179                	addi	sp,sp,-48
    80001962:	f406                	sd	ra,40(sp)
    80001964:	f022                	sd	s0,32(sp)
    80001966:	ec26                	sd	s1,24(sp)
    80001968:	1800                	addi	s0,sp,48
  extern char userret[];
  static int first = 1;
  struct proc *p = myproc();
    8000196a:	fc7ff0ef          	jal	80001930 <myproc>
    8000196e:	84aa                	mv	s1,a0

  // Still holding p->lock from scheduler.
  release(&p->lock);
    80001970:	af6ff0ef          	jal	80000c66 <release>

  if (first) {
    80001974:	00006797          	auipc	a5,0x6
    80001978:	ebc7a783          	lw	a5,-324(a5) # 80007830 <first.1>
    8000197c:	cf8d                	beqz	a5,800019b6 <forkret+0x56>
    // File system initialization must be run in the context of a
    // regular process (e.g., because it calls sleep), and thus cannot
    // be run from main().
    fsinit(ROOTDEV);
    8000197e:	4505                	li	a0,1
    80001980:	401010ef          	jal	80003580 <fsinit>

    first = 0;
    80001984:	00006797          	auipc	a5,0x6
    80001988:	ea07a623          	sw	zero,-340(a5) # 80007830 <first.1>
    // ensure other cores see first=0.
    __sync_synchronize();
    8000198c:	0ff0000f          	fence

    // We can invoke kexec() now that file system is initialized.
    // Put the return value (argc) of kexec into a0.
    p->trapframe->a0 = kexec("/init", (char *[]){ "/init", 0 });
    80001990:	00005517          	auipc	a0,0x5
    80001994:	7f050513          	addi	a0,a0,2032 # 80007180 <etext+0x180>
    80001998:	fca43823          	sd	a0,-48(s0)
    8000199c:	fc043c23          	sd	zero,-40(s0)
    800019a0:	fd040593          	addi	a1,s0,-48
    800019a4:	4dd020ef          	jal	80004680 <kexec>
    800019a8:	6cbc                	ld	a5,88(s1)
    800019aa:	fba8                	sd	a0,112(a5)
    if (p->trapframe->a0 == -1) {
    800019ac:	6cbc                	ld	a5,88(s1)
    800019ae:	7bb8                	ld	a4,112(a5)
    800019b0:	57fd                	li	a5,-1
    800019b2:	02f70d63          	beq	a4,a5,800019ec <forkret+0x8c>
      panic("exec");
    }
  }

  // return to user space, mimicing usertrap()'s return.
  prepare_return();
    800019b6:	2b1000ef          	jal	80002466 <prepare_return>
  uint64 satp = MAKE_SATP(p->pagetable);
    800019ba:	68a8                	ld	a0,80(s1)
    800019bc:	8131                	srli	a0,a0,0xc
  uint64 trampoline_userret = TRAMPOLINE + (userret - trampoline);
    800019be:	04000737          	lui	a4,0x4000
    800019c2:	177d                	addi	a4,a4,-1 # 3ffffff <_entry-0x7c000001>
    800019c4:	0732                	slli	a4,a4,0xc
    800019c6:	00004797          	auipc	a5,0x4
    800019ca:	6d678793          	addi	a5,a5,1750 # 8000609c <userret>
    800019ce:	00004697          	auipc	a3,0x4
    800019d2:	63268693          	addi	a3,a3,1586 # 80006000 <_trampoline>
    800019d6:	8f95                	sub	a5,a5,a3
    800019d8:	97ba                	add	a5,a5,a4
  ((void (*)(uint64))trampoline_userret)(satp);
    800019da:	577d                	li	a4,-1
    800019dc:	177e                	slli	a4,a4,0x3f
    800019de:	8d59                	or	a0,a0,a4
    800019e0:	9782                	jalr	a5
}
    800019e2:	70a2                	ld	ra,40(sp)
    800019e4:	7402                	ld	s0,32(sp)
    800019e6:	64e2                	ld	s1,24(sp)
    800019e8:	6145                	addi	sp,sp,48
    800019ea:	8082                	ret
      panic("exec");
    800019ec:	00005517          	auipc	a0,0x5
    800019f0:	79c50513          	addi	a0,a0,1948 # 80007188 <etext+0x188>
    800019f4:	dedfe0ef          	jal	800007e0 <panic>

00000000800019f8 <allocpid>:
{
    800019f8:	1101                	addi	sp,sp,-32
    800019fa:	ec06                	sd	ra,24(sp)
    800019fc:	e822                	sd	s0,16(sp)
    800019fe:	e426                	sd	s1,8(sp)
    80001a00:	e04a                	sd	s2,0(sp)
    80001a02:	1000                	addi	s0,sp,32
  acquire(&pid_lock);
    80001a04:	0000e917          	auipc	s2,0xe
    80001a08:	f6490913          	addi	s2,s2,-156 # 8000f968 <pid_lock>
    80001a0c:	854a                	mv	a0,s2
    80001a0e:	9c0ff0ef          	jal	80000bce <acquire>
  pid = nextpid;
    80001a12:	00006797          	auipc	a5,0x6
    80001a16:	e2278793          	addi	a5,a5,-478 # 80007834 <nextpid>
    80001a1a:	4384                	lw	s1,0(a5)
  nextpid = nextpid + 1;
    80001a1c:	0014871b          	addiw	a4,s1,1
    80001a20:	c398                	sw	a4,0(a5)
  release(&pid_lock);
    80001a22:	854a                	mv	a0,s2
    80001a24:	a42ff0ef          	jal	80000c66 <release>
}
    80001a28:	8526                	mv	a0,s1
    80001a2a:	60e2                	ld	ra,24(sp)
    80001a2c:	6442                	ld	s0,16(sp)
    80001a2e:	64a2                	ld	s1,8(sp)
    80001a30:	6902                	ld	s2,0(sp)
    80001a32:	6105                	addi	sp,sp,32
    80001a34:	8082                	ret

0000000080001a36 <proc_pagetable>:
{
    80001a36:	1101                	addi	sp,sp,-32
    80001a38:	ec06                	sd	ra,24(sp)
    80001a3a:	e822                	sd	s0,16(sp)
    80001a3c:	e426                	sd	s1,8(sp)
    80001a3e:	e04a                	sd	s2,0(sp)
    80001a40:	1000                	addi	s0,sp,32
    80001a42:	892a                	mv	s2,a0
  pagetable = uvmcreate();
    80001a44:	f50ff0ef          	jal	80001194 <uvmcreate>
    80001a48:	84aa                	mv	s1,a0
  if(pagetable == 0)
    80001a4a:	cd05                	beqz	a0,80001a82 <proc_pagetable+0x4c>
  if(mappages(pagetable, TRAMPOLINE, PGSIZE,
    80001a4c:	4729                	li	a4,10
    80001a4e:	00004697          	auipc	a3,0x4
    80001a52:	5b268693          	addi	a3,a3,1458 # 80006000 <_trampoline>
    80001a56:	6605                	lui	a2,0x1
    80001a58:	040005b7          	lui	a1,0x4000
    80001a5c:	15fd                	addi	a1,a1,-1 # 3ffffff <_entry-0x7c000001>
    80001a5e:	05b2                	slli	a1,a1,0xc
    80001a60:	d8eff0ef          	jal	80000fee <mappages>
    80001a64:	02054663          	bltz	a0,80001a90 <proc_pagetable+0x5a>
  if(mappages(pagetable, TRAPFRAME, PGSIZE,
    80001a68:	4719                	li	a4,6
    80001a6a:	05893683          	ld	a3,88(s2)
    80001a6e:	6605                	lui	a2,0x1
    80001a70:	020005b7          	lui	a1,0x2000
    80001a74:	15fd                	addi	a1,a1,-1 # 1ffffff <_entry-0x7e000001>
    80001a76:	05b6                	slli	a1,a1,0xd
    80001a78:	8526                	mv	a0,s1
    80001a7a:	d74ff0ef          	jal	80000fee <mappages>
    80001a7e:	00054f63          	bltz	a0,80001a9c <proc_pagetable+0x66>
}
    80001a82:	8526                	mv	a0,s1
    80001a84:	60e2                	ld	ra,24(sp)
    80001a86:	6442                	ld	s0,16(sp)
    80001a88:	64a2                	ld	s1,8(sp)
    80001a8a:	6902                	ld	s2,0(sp)
    80001a8c:	6105                	addi	sp,sp,32
    80001a8e:	8082                	ret
    uvmfree(pagetable, 0);
    80001a90:	4581                	li	a1,0
    80001a92:	8526                	mv	a0,s1
    80001a94:	8fbff0ef          	jal	8000138e <uvmfree>
    return 0;
    80001a98:	4481                	li	s1,0
    80001a9a:	b7e5                	j	80001a82 <proc_pagetable+0x4c>
    uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    80001a9c:	4681                	li	a3,0
    80001a9e:	4605                	li	a2,1
    80001aa0:	040005b7          	lui	a1,0x4000
    80001aa4:	15fd                	addi	a1,a1,-1 # 3ffffff <_entry-0x7c000001>
    80001aa6:	05b2                	slli	a1,a1,0xc
    80001aa8:	8526                	mv	a0,s1
    80001aaa:	f10ff0ef          	jal	800011ba <uvmunmap>
    uvmfree(pagetable, 0);
    80001aae:	4581                	li	a1,0
    80001ab0:	8526                	mv	a0,s1
    80001ab2:	8ddff0ef          	jal	8000138e <uvmfree>
    return 0;
    80001ab6:	4481                	li	s1,0
    80001ab8:	b7e9                	j	80001a82 <proc_pagetable+0x4c>

0000000080001aba <proc_freepagetable>:
{
    80001aba:	1101                	addi	sp,sp,-32
    80001abc:	ec06                	sd	ra,24(sp)
    80001abe:	e822                	sd	s0,16(sp)
    80001ac0:	e426                	sd	s1,8(sp)
    80001ac2:	e04a                	sd	s2,0(sp)
    80001ac4:	1000                	addi	s0,sp,32
    80001ac6:	84aa                	mv	s1,a0
    80001ac8:	892e                	mv	s2,a1
  uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    80001aca:	4681                	li	a3,0
    80001acc:	4605                	li	a2,1
    80001ace:	040005b7          	lui	a1,0x4000
    80001ad2:	15fd                	addi	a1,a1,-1 # 3ffffff <_entry-0x7c000001>
    80001ad4:	05b2                	slli	a1,a1,0xc
    80001ad6:	ee4ff0ef          	jal	800011ba <uvmunmap>
  uvmunmap(pagetable, TRAPFRAME, 1, 0);
    80001ada:	4681                	li	a3,0
    80001adc:	4605                	li	a2,1
    80001ade:	020005b7          	lui	a1,0x2000
    80001ae2:	15fd                	addi	a1,a1,-1 # 1ffffff <_entry-0x7e000001>
    80001ae4:	05b6                	slli	a1,a1,0xd
    80001ae6:	8526                	mv	a0,s1
    80001ae8:	ed2ff0ef          	jal	800011ba <uvmunmap>
  uvmfree(pagetable, sz);
    80001aec:	85ca                	mv	a1,s2
    80001aee:	8526                	mv	a0,s1
    80001af0:	89fff0ef          	jal	8000138e <uvmfree>
}
    80001af4:	60e2                	ld	ra,24(sp)
    80001af6:	6442                	ld	s0,16(sp)
    80001af8:	64a2                	ld	s1,8(sp)
    80001afa:	6902                	ld	s2,0(sp)
    80001afc:	6105                	addi	sp,sp,32
    80001afe:	8082                	ret

0000000080001b00 <freeproc>:
{
    80001b00:	1101                	addi	sp,sp,-32
    80001b02:	ec06                	sd	ra,24(sp)
    80001b04:	e822                	sd	s0,16(sp)
    80001b06:	e426                	sd	s1,8(sp)
    80001b08:	1000                	addi	s0,sp,32
    80001b0a:	84aa                	mv	s1,a0
  if(p->trapframe)
    80001b0c:	6d28                	ld	a0,88(a0)
    80001b0e:	c119                	beqz	a0,80001b14 <freeproc+0x14>
    kfree((void*)p->trapframe);
    80001b10:	f0dfe0ef          	jal	80000a1c <kfree>
  p->trapframe = 0;
    80001b14:	0404bc23          	sd	zero,88(s1)
  if(p->pagetable)
    80001b18:	68a8                	ld	a0,80(s1)
    80001b1a:	c501                	beqz	a0,80001b22 <freeproc+0x22>
    proc_freepagetable(p->pagetable, p->sz);
    80001b1c:	64ac                	ld	a1,72(s1)
    80001b1e:	f9dff0ef          	jal	80001aba <proc_freepagetable>
  p->pagetable = 0;
    80001b22:	0404b823          	sd	zero,80(s1)
  p->sz = 0;
    80001b26:	0404b423          	sd	zero,72(s1)
  p->pid = 0;
    80001b2a:	0204a823          	sw	zero,48(s1)
  p->parent = 0;
    80001b2e:	0204bc23          	sd	zero,56(s1)
  p->name[0] = 0;
    80001b32:	14048c23          	sb	zero,344(s1)
  p->chan = 0;
    80001b36:	0204b023          	sd	zero,32(s1)
  p->killed = 0;
    80001b3a:	0204a423          	sw	zero,40(s1)
  p->xstate = 0;
    80001b3e:	0204a623          	sw	zero,44(s1)
  p->mem_reqs = 0;
    80001b42:	1604b423          	sd	zero,360(s1)
  p->state = UNUSED;
    80001b46:	0004ac23          	sw	zero,24(s1)
}
    80001b4a:	60e2                	ld	ra,24(sp)
    80001b4c:	6442                	ld	s0,16(sp)
    80001b4e:	64a2                	ld	s1,8(sp)
    80001b50:	6105                	addi	sp,sp,32
    80001b52:	8082                	ret

0000000080001b54 <allocproc>:
{
    80001b54:	1101                	addi	sp,sp,-32
    80001b56:	ec06                	sd	ra,24(sp)
    80001b58:	e822                	sd	s0,16(sp)
    80001b5a:	e426                	sd	s1,8(sp)
    80001b5c:	e04a                	sd	s2,0(sp)
    80001b5e:	1000                	addi	s0,sp,32
  for(p = proc; p < &proc[NPROC]; p++) {
    80001b60:	0000e497          	auipc	s1,0xe
    80001b64:	23848493          	addi	s1,s1,568 # 8000fd98 <proc>
    80001b68:	00014917          	auipc	s2,0x14
    80001b6c:	e3090913          	addi	s2,s2,-464 # 80015998 <tickslock>
    acquire(&p->lock);
    80001b70:	8526                	mv	a0,s1
    80001b72:	85cff0ef          	jal	80000bce <acquire>
    if(p->state == UNUSED) {
    80001b76:	4c9c                	lw	a5,24(s1)
    80001b78:	cb91                	beqz	a5,80001b8c <allocproc+0x38>
      release(&p->lock);
    80001b7a:	8526                	mv	a0,s1
    80001b7c:	8eaff0ef          	jal	80000c66 <release>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001b80:	17048493          	addi	s1,s1,368
    80001b84:	ff2496e3          	bne	s1,s2,80001b70 <allocproc+0x1c>
  return 0;
    80001b88:	4481                	li	s1,0
    80001b8a:	a089                	j	80001bcc <allocproc+0x78>
  p->pid = allocpid();
    80001b8c:	e6dff0ef          	jal	800019f8 <allocpid>
    80001b90:	d888                	sw	a0,48(s1)
  p->state = USED;
    80001b92:	4785                	li	a5,1
    80001b94:	cc9c                	sw	a5,24(s1)
  if((p->trapframe = (struct trapframe *)kalloc()) == 0){
    80001b96:	f69fe0ef          	jal	80000afe <kalloc>
    80001b9a:	892a                	mv	s2,a0
    80001b9c:	eca8                	sd	a0,88(s1)
    80001b9e:	cd15                	beqz	a0,80001bda <allocproc+0x86>
  p->pagetable = proc_pagetable(p);
    80001ba0:	8526                	mv	a0,s1
    80001ba2:	e95ff0ef          	jal	80001a36 <proc_pagetable>
    80001ba6:	892a                	mv	s2,a0
    80001ba8:	e8a8                	sd	a0,80(s1)
  if(p->pagetable == 0){
    80001baa:	c121                	beqz	a0,80001bea <allocproc+0x96>
  memset(&p->context, 0, sizeof(p->context));
    80001bac:	07000613          	li	a2,112
    80001bb0:	4581                	li	a1,0
    80001bb2:	06048513          	addi	a0,s1,96
    80001bb6:	8ecff0ef          	jal	80000ca2 <memset>
  p->context.ra = (uint64)forkret;
    80001bba:	00000797          	auipc	a5,0x0
    80001bbe:	da678793          	addi	a5,a5,-602 # 80001960 <forkret>
    80001bc2:	f0bc                	sd	a5,96(s1)
  p->context.sp = p->kstack + PGSIZE;
    80001bc4:	60bc                	ld	a5,64(s1)
    80001bc6:	6705                	lui	a4,0x1
    80001bc8:	97ba                	add	a5,a5,a4
    80001bca:	f4bc                	sd	a5,104(s1)
}
    80001bcc:	8526                	mv	a0,s1
    80001bce:	60e2                	ld	ra,24(sp)
    80001bd0:	6442                	ld	s0,16(sp)
    80001bd2:	64a2                	ld	s1,8(sp)
    80001bd4:	6902                	ld	s2,0(sp)
    80001bd6:	6105                	addi	sp,sp,32
    80001bd8:	8082                	ret
    freeproc(p);
    80001bda:	8526                	mv	a0,s1
    80001bdc:	f25ff0ef          	jal	80001b00 <freeproc>
    release(&p->lock);
    80001be0:	8526                	mv	a0,s1
    80001be2:	884ff0ef          	jal	80000c66 <release>
    return 0;
    80001be6:	84ca                	mv	s1,s2
    80001be8:	b7d5                	j	80001bcc <allocproc+0x78>
    freeproc(p);
    80001bea:	8526                	mv	a0,s1
    80001bec:	f15ff0ef          	jal	80001b00 <freeproc>
    release(&p->lock);
    80001bf0:	8526                	mv	a0,s1
    80001bf2:	874ff0ef          	jal	80000c66 <release>
    return 0;
    80001bf6:	84ca                	mv	s1,s2
    80001bf8:	bfd1                	j	80001bcc <allocproc+0x78>

0000000080001bfa <userinit>:
{
    80001bfa:	1101                	addi	sp,sp,-32
    80001bfc:	ec06                	sd	ra,24(sp)
    80001bfe:	e822                	sd	s0,16(sp)
    80001c00:	e426                	sd	s1,8(sp)
    80001c02:	1000                	addi	s0,sp,32
  p = allocproc();
    80001c04:	f51ff0ef          	jal	80001b54 <allocproc>
    80001c08:	84aa                	mv	s1,a0
  initproc = p;
    80001c0a:	00006797          	auipc	a5,0x6
    80001c0e:	c4a7bb23          	sd	a0,-938(a5) # 80007860 <initproc>
  p->cwd = namei("/");
    80001c12:	00005517          	auipc	a0,0x5
    80001c16:	57e50513          	addi	a0,a0,1406 # 80007190 <etext+0x190>
    80001c1a:	689010ef          	jal	80003aa2 <namei>
    80001c1e:	14a4b823          	sd	a0,336(s1)
  p->state = RUNNABLE;
    80001c22:	478d                	li	a5,3
    80001c24:	cc9c                	sw	a5,24(s1)
  release(&p->lock);
    80001c26:	8526                	mv	a0,s1
    80001c28:	83eff0ef          	jal	80000c66 <release>
}
    80001c2c:	60e2                	ld	ra,24(sp)
    80001c2e:	6442                	ld	s0,16(sp)
    80001c30:	64a2                	ld	s1,8(sp)
    80001c32:	6105                	addi	sp,sp,32
    80001c34:	8082                	ret

0000000080001c36 <growproc>:
{
    80001c36:	1101                	addi	sp,sp,-32
    80001c38:	ec06                	sd	ra,24(sp)
    80001c3a:	e822                	sd	s0,16(sp)
    80001c3c:	e426                	sd	s1,8(sp)
    80001c3e:	e04a                	sd	s2,0(sp)
    80001c40:	1000                	addi	s0,sp,32
    80001c42:	892a                	mv	s2,a0
  struct proc *p = myproc();
    80001c44:	cedff0ef          	jal	80001930 <myproc>
    80001c48:	84aa                	mv	s1,a0
  sz = p->sz;
    80001c4a:	652c                	ld	a1,72(a0)
  if(n > 0){
    80001c4c:	01204c63          	bgtz	s2,80001c64 <growproc+0x2e>
  } else if(n < 0){
    80001c50:	02094463          	bltz	s2,80001c78 <growproc+0x42>
  p->sz = sz;
    80001c54:	e4ac                	sd	a1,72(s1)
  return 0;
    80001c56:	4501                	li	a0,0
}
    80001c58:	60e2                	ld	ra,24(sp)
    80001c5a:	6442                	ld	s0,16(sp)
    80001c5c:	64a2                	ld	s1,8(sp)
    80001c5e:	6902                	ld	s2,0(sp)
    80001c60:	6105                	addi	sp,sp,32
    80001c62:	8082                	ret
    if((sz = uvmalloc(p->pagetable, sz, sz + n, PTE_W)) == 0) {
    80001c64:	4691                	li	a3,4
    80001c66:	00b90633          	add	a2,s2,a1
    80001c6a:	6928                	ld	a0,80(a0)
    80001c6c:	e1cff0ef          	jal	80001288 <uvmalloc>
    80001c70:	85aa                	mv	a1,a0
    80001c72:	f16d                	bnez	a0,80001c54 <growproc+0x1e>
      return -1;
    80001c74:	557d                	li	a0,-1
    80001c76:	b7cd                	j	80001c58 <growproc+0x22>
    sz = uvmdealloc(p->pagetable, sz, sz + n);
    80001c78:	00b90633          	add	a2,s2,a1
    80001c7c:	6928                	ld	a0,80(a0)
    80001c7e:	dc6ff0ef          	jal	80001244 <uvmdealloc>
    80001c82:	85aa                	mv	a1,a0
    80001c84:	bfc1                	j	80001c54 <growproc+0x1e>

0000000080001c86 <kfork>:
{
    80001c86:	7139                	addi	sp,sp,-64
    80001c88:	fc06                	sd	ra,56(sp)
    80001c8a:	f822                	sd	s0,48(sp)
    80001c8c:	f04a                	sd	s2,32(sp)
    80001c8e:	e456                	sd	s5,8(sp)
    80001c90:	0080                	addi	s0,sp,64
  struct proc *p = myproc();
    80001c92:	c9fff0ef          	jal	80001930 <myproc>
    80001c96:	8aaa                	mv	s5,a0
  if((np = allocproc()) == 0){
    80001c98:	ebdff0ef          	jal	80001b54 <allocproc>
    80001c9c:	0e050a63          	beqz	a0,80001d90 <kfork+0x10a>
    80001ca0:	e852                	sd	s4,16(sp)
    80001ca2:	8a2a                	mv	s4,a0
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0){
    80001ca4:	048ab603          	ld	a2,72(s5)
    80001ca8:	692c                	ld	a1,80(a0)
    80001caa:	050ab503          	ld	a0,80(s5)
    80001cae:	f12ff0ef          	jal	800013c0 <uvmcopy>
    80001cb2:	04054a63          	bltz	a0,80001d06 <kfork+0x80>
    80001cb6:	f426                	sd	s1,40(sp)
    80001cb8:	ec4e                	sd	s3,24(sp)
  np->sz = p->sz;
    80001cba:	048ab783          	ld	a5,72(s5)
    80001cbe:	04fa3423          	sd	a5,72(s4)
  *(np->trapframe) = *(p->trapframe);
    80001cc2:	058ab683          	ld	a3,88(s5)
    80001cc6:	87b6                	mv	a5,a3
    80001cc8:	058a3703          	ld	a4,88(s4)
    80001ccc:	12068693          	addi	a3,a3,288
    80001cd0:	0007b803          	ld	a6,0(a5)
    80001cd4:	6788                	ld	a0,8(a5)
    80001cd6:	6b8c                	ld	a1,16(a5)
    80001cd8:	6f90                	ld	a2,24(a5)
    80001cda:	01073023          	sd	a6,0(a4) # 1000 <_entry-0x7ffff000>
    80001cde:	e708                	sd	a0,8(a4)
    80001ce0:	eb0c                	sd	a1,16(a4)
    80001ce2:	ef10                	sd	a2,24(a4)
    80001ce4:	02078793          	addi	a5,a5,32
    80001ce8:	02070713          	addi	a4,a4,32
    80001cec:	fed792e3          	bne	a5,a3,80001cd0 <kfork+0x4a>
  np->trapframe->a0 = 0;
    80001cf0:	058a3783          	ld	a5,88(s4)
    80001cf4:	0607b823          	sd	zero,112(a5)
  for(i = 0; i < NOFILE; i++)
    80001cf8:	0d0a8493          	addi	s1,s5,208
    80001cfc:	0d0a0913          	addi	s2,s4,208
    80001d00:	150a8993          	addi	s3,s5,336
    80001d04:	a831                	j	80001d20 <kfork+0x9a>
    freeproc(np);
    80001d06:	8552                	mv	a0,s4
    80001d08:	df9ff0ef          	jal	80001b00 <freeproc>
    release(&np->lock);
    80001d0c:	8552                	mv	a0,s4
    80001d0e:	f59fe0ef          	jal	80000c66 <release>
    return -1;
    80001d12:	597d                	li	s2,-1
    80001d14:	6a42                	ld	s4,16(sp)
    80001d16:	a0b5                	j	80001d82 <kfork+0xfc>
  for(i = 0; i < NOFILE; i++)
    80001d18:	04a1                	addi	s1,s1,8
    80001d1a:	0921                	addi	s2,s2,8
    80001d1c:	01348963          	beq	s1,s3,80001d2e <kfork+0xa8>
    if(p->ofile[i])
    80001d20:	6088                	ld	a0,0(s1)
    80001d22:	d97d                	beqz	a0,80001d18 <kfork+0x92>
      np->ofile[i] = filedup(p->ofile[i]);
    80001d24:	318020ef          	jal	8000403c <filedup>
    80001d28:	00a93023          	sd	a0,0(s2)
    80001d2c:	b7f5                	j	80001d18 <kfork+0x92>
  np->cwd = idup(p->cwd);
    80001d2e:	150ab503          	ld	a0,336(s5)
    80001d32:	524010ef          	jal	80003256 <idup>
    80001d36:	14aa3823          	sd	a0,336(s4)
  safestrcpy(np->name, p->name, sizeof(p->name));
    80001d3a:	4641                	li	a2,16
    80001d3c:	158a8593          	addi	a1,s5,344
    80001d40:	158a0513          	addi	a0,s4,344
    80001d44:	89cff0ef          	jal	80000de0 <safestrcpy>
  pid = np->pid;
    80001d48:	030a2903          	lw	s2,48(s4)
  release(&np->lock);
    80001d4c:	8552                	mv	a0,s4
    80001d4e:	f19fe0ef          	jal	80000c66 <release>
  acquire(&wait_lock);
    80001d52:	0000e497          	auipc	s1,0xe
    80001d56:	c2e48493          	addi	s1,s1,-978 # 8000f980 <wait_lock>
    80001d5a:	8526                	mv	a0,s1
    80001d5c:	e73fe0ef          	jal	80000bce <acquire>
  np->parent = p;
    80001d60:	035a3c23          	sd	s5,56(s4)
  release(&wait_lock);
    80001d64:	8526                	mv	a0,s1
    80001d66:	f01fe0ef          	jal	80000c66 <release>
  acquire(&np->lock);
    80001d6a:	8552                	mv	a0,s4
    80001d6c:	e63fe0ef          	jal	80000bce <acquire>
  np->state = RUNNABLE;
    80001d70:	478d                	li	a5,3
    80001d72:	00fa2c23          	sw	a5,24(s4)
  release(&np->lock);
    80001d76:	8552                	mv	a0,s4
    80001d78:	eeffe0ef          	jal	80000c66 <release>
  return pid;
    80001d7c:	74a2                	ld	s1,40(sp)
    80001d7e:	69e2                	ld	s3,24(sp)
    80001d80:	6a42                	ld	s4,16(sp)
}
    80001d82:	854a                	mv	a0,s2
    80001d84:	70e2                	ld	ra,56(sp)
    80001d86:	7442                	ld	s0,48(sp)
    80001d88:	7902                	ld	s2,32(sp)
    80001d8a:	6aa2                	ld	s5,8(sp)
    80001d8c:	6121                	addi	sp,sp,64
    80001d8e:	8082                	ret
    return -1;
    80001d90:	597d                	li	s2,-1
    80001d92:	bfc5                	j	80001d82 <kfork+0xfc>

0000000080001d94 <scheduler>:
{
    80001d94:	715d                	addi	sp,sp,-80
    80001d96:	e486                	sd	ra,72(sp)
    80001d98:	e0a2                	sd	s0,64(sp)
    80001d9a:	fc26                	sd	s1,56(sp)
    80001d9c:	f84a                	sd	s2,48(sp)
    80001d9e:	f44e                	sd	s3,40(sp)
    80001da0:	f052                	sd	s4,32(sp)
    80001da2:	ec56                	sd	s5,24(sp)
    80001da4:	e85a                	sd	s6,16(sp)
    80001da6:	e45e                	sd	s7,8(sp)
    80001da8:	e062                	sd	s8,0(sp)
    80001daa:	0880                	addi	s0,sp,80
    80001dac:	8792                	mv	a5,tp
  int id = r_tp();
    80001dae:	2781                	sext.w	a5,a5
  c->proc = 0;
    80001db0:	00779b13          	slli	s6,a5,0x7
    80001db4:	0000e717          	auipc	a4,0xe
    80001db8:	bb470713          	addi	a4,a4,-1100 # 8000f968 <pid_lock>
    80001dbc:	975a                	add	a4,a4,s6
    80001dbe:	02073823          	sd	zero,48(a4)
        swtch(&c->context, &p->context);
    80001dc2:	0000e717          	auipc	a4,0xe
    80001dc6:	bde70713          	addi	a4,a4,-1058 # 8000f9a0 <cpus+0x8>
    80001dca:	9b3a                	add	s6,s6,a4
        p->state = RUNNING;
    80001dcc:	4c11                	li	s8,4
        c->proc = p;
    80001dce:	079e                	slli	a5,a5,0x7
    80001dd0:	0000ea17          	auipc	s4,0xe
    80001dd4:	b98a0a13          	addi	s4,s4,-1128 # 8000f968 <pid_lock>
    80001dd8:	9a3e                	add	s4,s4,a5
        found = 1;
    80001dda:	4b85                	li	s7,1
    for(p = proc; p < &proc[NPROC]; p++) {
    80001ddc:	00014997          	auipc	s3,0x14
    80001de0:	bbc98993          	addi	s3,s3,-1092 # 80015998 <tickslock>
    80001de4:	a83d                	j	80001e22 <scheduler+0x8e>
      release(&p->lock);
    80001de6:	8526                	mv	a0,s1
    80001de8:	e7ffe0ef          	jal	80000c66 <release>
    for(p = proc; p < &proc[NPROC]; p++) {
    80001dec:	17048493          	addi	s1,s1,368
    80001df0:	03348563          	beq	s1,s3,80001e1a <scheduler+0x86>
      acquire(&p->lock);
    80001df4:	8526                	mv	a0,s1
    80001df6:	dd9fe0ef          	jal	80000bce <acquire>
      if(p->state == RUNNABLE) {
    80001dfa:	4c9c                	lw	a5,24(s1)
    80001dfc:	ff2795e3          	bne	a5,s2,80001de6 <scheduler+0x52>
        p->state = RUNNING;
    80001e00:	0184ac23          	sw	s8,24(s1)
        c->proc = p;
    80001e04:	029a3823          	sd	s1,48(s4)
        swtch(&c->context, &p->context);
    80001e08:	06048593          	addi	a1,s1,96
    80001e0c:	855a                	mv	a0,s6
    80001e0e:	5b2000ef          	jal	800023c0 <swtch>
        c->proc = 0;
    80001e12:	020a3823          	sd	zero,48(s4)
        found = 1;
    80001e16:	8ade                	mv	s5,s7
    80001e18:	b7f9                	j	80001de6 <scheduler+0x52>
    if(found == 0) {
    80001e1a:	000a9463          	bnez	s5,80001e22 <scheduler+0x8e>
      asm volatile("wfi");
    80001e1e:	10500073          	wfi
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80001e22:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80001e26:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80001e2a:	10079073          	csrw	sstatus,a5
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80001e2e:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    80001e32:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80001e34:	10079073          	csrw	sstatus,a5
    int found = 0;
    80001e38:	4a81                	li	s5,0
    for(p = proc; p < &proc[NPROC]; p++) {
    80001e3a:	0000e497          	auipc	s1,0xe
    80001e3e:	f5e48493          	addi	s1,s1,-162 # 8000fd98 <proc>
      if(p->state == RUNNABLE) {
    80001e42:	490d                	li	s2,3
    80001e44:	bf45                	j	80001df4 <scheduler+0x60>

0000000080001e46 <sched>:
{
    80001e46:	7179                	addi	sp,sp,-48
    80001e48:	f406                	sd	ra,40(sp)
    80001e4a:	f022                	sd	s0,32(sp)
    80001e4c:	ec26                	sd	s1,24(sp)
    80001e4e:	e84a                	sd	s2,16(sp)
    80001e50:	e44e                	sd	s3,8(sp)
    80001e52:	1800                	addi	s0,sp,48
  struct proc *p = myproc();
    80001e54:	addff0ef          	jal	80001930 <myproc>
    80001e58:	84aa                	mv	s1,a0
  if(!holding(&p->lock))
    80001e5a:	d0bfe0ef          	jal	80000b64 <holding>
    80001e5e:	c92d                	beqz	a0,80001ed0 <sched+0x8a>
  asm volatile("mv %0, tp" : "=r" (x) );
    80001e60:	8792                	mv	a5,tp
  if(mycpu()->noff != 1)
    80001e62:	2781                	sext.w	a5,a5
    80001e64:	079e                	slli	a5,a5,0x7
    80001e66:	0000e717          	auipc	a4,0xe
    80001e6a:	b0270713          	addi	a4,a4,-1278 # 8000f968 <pid_lock>
    80001e6e:	97ba                	add	a5,a5,a4
    80001e70:	0a87a703          	lw	a4,168(a5)
    80001e74:	4785                	li	a5,1
    80001e76:	06f71363          	bne	a4,a5,80001edc <sched+0x96>
  if(p->state == RUNNING)
    80001e7a:	4c98                	lw	a4,24(s1)
    80001e7c:	4791                	li	a5,4
    80001e7e:	06f70563          	beq	a4,a5,80001ee8 <sched+0xa2>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80001e82:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80001e86:	8b89                	andi	a5,a5,2
  if(intr_get())
    80001e88:	e7b5                	bnez	a5,80001ef4 <sched+0xae>
  asm volatile("mv %0, tp" : "=r" (x) );
    80001e8a:	8792                	mv	a5,tp
  intena = mycpu()->intena;
    80001e8c:	0000e917          	auipc	s2,0xe
    80001e90:	adc90913          	addi	s2,s2,-1316 # 8000f968 <pid_lock>
    80001e94:	2781                	sext.w	a5,a5
    80001e96:	079e                	slli	a5,a5,0x7
    80001e98:	97ca                	add	a5,a5,s2
    80001e9a:	0ac7a983          	lw	s3,172(a5)
    80001e9e:	8792                	mv	a5,tp
  swtch(&p->context, &mycpu()->context);
    80001ea0:	2781                	sext.w	a5,a5
    80001ea2:	079e                	slli	a5,a5,0x7
    80001ea4:	0000e597          	auipc	a1,0xe
    80001ea8:	afc58593          	addi	a1,a1,-1284 # 8000f9a0 <cpus+0x8>
    80001eac:	95be                	add	a1,a1,a5
    80001eae:	06048513          	addi	a0,s1,96
    80001eb2:	50e000ef          	jal	800023c0 <swtch>
    80001eb6:	8792                	mv	a5,tp
  mycpu()->intena = intena;
    80001eb8:	2781                	sext.w	a5,a5
    80001eba:	079e                	slli	a5,a5,0x7
    80001ebc:	993e                	add	s2,s2,a5
    80001ebe:	0b392623          	sw	s3,172(s2)
}
    80001ec2:	70a2                	ld	ra,40(sp)
    80001ec4:	7402                	ld	s0,32(sp)
    80001ec6:	64e2                	ld	s1,24(sp)
    80001ec8:	6942                	ld	s2,16(sp)
    80001eca:	69a2                	ld	s3,8(sp)
    80001ecc:	6145                	addi	sp,sp,48
    80001ece:	8082                	ret
    panic("sched p->lock");
    80001ed0:	00005517          	auipc	a0,0x5
    80001ed4:	2c850513          	addi	a0,a0,712 # 80007198 <etext+0x198>
    80001ed8:	909fe0ef          	jal	800007e0 <panic>
    panic("sched locks");
    80001edc:	00005517          	auipc	a0,0x5
    80001ee0:	2cc50513          	addi	a0,a0,716 # 800071a8 <etext+0x1a8>
    80001ee4:	8fdfe0ef          	jal	800007e0 <panic>
    panic("sched RUNNING");
    80001ee8:	00005517          	auipc	a0,0x5
    80001eec:	2d050513          	addi	a0,a0,720 # 800071b8 <etext+0x1b8>
    80001ef0:	8f1fe0ef          	jal	800007e0 <panic>
    panic("sched interruptible");
    80001ef4:	00005517          	auipc	a0,0x5
    80001ef8:	2d450513          	addi	a0,a0,724 # 800071c8 <etext+0x1c8>
    80001efc:	8e5fe0ef          	jal	800007e0 <panic>

0000000080001f00 <yield>:
{
    80001f00:	1101                	addi	sp,sp,-32
    80001f02:	ec06                	sd	ra,24(sp)
    80001f04:	e822                	sd	s0,16(sp)
    80001f06:	e426                	sd	s1,8(sp)
    80001f08:	1000                	addi	s0,sp,32
  struct proc *p = myproc();
    80001f0a:	a27ff0ef          	jal	80001930 <myproc>
    80001f0e:	84aa                	mv	s1,a0
  acquire(&p->lock);
    80001f10:	cbffe0ef          	jal	80000bce <acquire>
  p->state = RUNNABLE;
    80001f14:	478d                	li	a5,3
    80001f16:	cc9c                	sw	a5,24(s1)
  sched();
    80001f18:	f2fff0ef          	jal	80001e46 <sched>
  release(&p->lock);
    80001f1c:	8526                	mv	a0,s1
    80001f1e:	d49fe0ef          	jal	80000c66 <release>
}
    80001f22:	60e2                	ld	ra,24(sp)
    80001f24:	6442                	ld	s0,16(sp)
    80001f26:	64a2                	ld	s1,8(sp)
    80001f28:	6105                	addi	sp,sp,32
    80001f2a:	8082                	ret

0000000080001f2c <sleep>:

// Sleep on channel chan, releasing condition lock lk.
// Re-acquires lk when awakened.
void
sleep(void *chan, struct spinlock *lk)
{
    80001f2c:	7179                	addi	sp,sp,-48
    80001f2e:	f406                	sd	ra,40(sp)
    80001f30:	f022                	sd	s0,32(sp)
    80001f32:	ec26                	sd	s1,24(sp)
    80001f34:	e84a                	sd	s2,16(sp)
    80001f36:	e44e                	sd	s3,8(sp)
    80001f38:	1800                	addi	s0,sp,48
    80001f3a:	89aa                	mv	s3,a0
    80001f3c:	892e                	mv	s2,a1
  struct proc *p = myproc();
    80001f3e:	9f3ff0ef          	jal	80001930 <myproc>
    80001f42:	84aa                	mv	s1,a0
  // Once we hold p->lock, we can be
  // guaranteed that we won't miss any wakeup
  // (wakeup locks p->lock),
  // so it's okay to release lk.

  acquire(&p->lock);  //DOC: sleeplock1
    80001f44:	c8bfe0ef          	jal	80000bce <acquire>
  release(lk);
    80001f48:	854a                	mv	a0,s2
    80001f4a:	d1dfe0ef          	jal	80000c66 <release>

  // Go to sleep.
  p->chan = chan;
    80001f4e:	0334b023          	sd	s3,32(s1)
  p->state = SLEEPING;
    80001f52:	4789                	li	a5,2
    80001f54:	cc9c                	sw	a5,24(s1)

  sched();
    80001f56:	ef1ff0ef          	jal	80001e46 <sched>

  // Tidy up.
  p->chan = 0;
    80001f5a:	0204b023          	sd	zero,32(s1)

  // Reacquire original lock.
  release(&p->lock);
    80001f5e:	8526                	mv	a0,s1
    80001f60:	d07fe0ef          	jal	80000c66 <release>
  acquire(lk);
    80001f64:	854a                	mv	a0,s2
    80001f66:	c69fe0ef          	jal	80000bce <acquire>
}
    80001f6a:	70a2                	ld	ra,40(sp)
    80001f6c:	7402                	ld	s0,32(sp)
    80001f6e:	64e2                	ld	s1,24(sp)
    80001f70:	6942                	ld	s2,16(sp)
    80001f72:	69a2                	ld	s3,8(sp)
    80001f74:	6145                	addi	sp,sp,48
    80001f76:	8082                	ret

0000000080001f78 <wakeup>:

// Wake up all processes sleeping on channel chan.
// Caller should hold the condition lock.
void
wakeup(void *chan)
{
    80001f78:	7139                	addi	sp,sp,-64
    80001f7a:	fc06                	sd	ra,56(sp)
    80001f7c:	f822                	sd	s0,48(sp)
    80001f7e:	f426                	sd	s1,40(sp)
    80001f80:	f04a                	sd	s2,32(sp)
    80001f82:	ec4e                	sd	s3,24(sp)
    80001f84:	e852                	sd	s4,16(sp)
    80001f86:	e456                	sd	s5,8(sp)
    80001f88:	0080                	addi	s0,sp,64
    80001f8a:	8a2a                	mv	s4,a0
  struct proc *p;

  for(p = proc; p < &proc[NPROC]; p++) {
    80001f8c:	0000e497          	auipc	s1,0xe
    80001f90:	e0c48493          	addi	s1,s1,-500 # 8000fd98 <proc>
    if(p != myproc()){
      acquire(&p->lock);
      if(p->state == SLEEPING && p->chan == chan) {
    80001f94:	4989                	li	s3,2
        p->state = RUNNABLE;
    80001f96:	4a8d                	li	s5,3
  for(p = proc; p < &proc[NPROC]; p++) {
    80001f98:	00014917          	auipc	s2,0x14
    80001f9c:	a0090913          	addi	s2,s2,-1536 # 80015998 <tickslock>
    80001fa0:	a801                	j	80001fb0 <wakeup+0x38>
      }
      release(&p->lock);
    80001fa2:	8526                	mv	a0,s1
    80001fa4:	cc3fe0ef          	jal	80000c66 <release>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001fa8:	17048493          	addi	s1,s1,368
    80001fac:	03248263          	beq	s1,s2,80001fd0 <wakeup+0x58>
    if(p != myproc()){
    80001fb0:	981ff0ef          	jal	80001930 <myproc>
    80001fb4:	fea48ae3          	beq	s1,a0,80001fa8 <wakeup+0x30>
      acquire(&p->lock);
    80001fb8:	8526                	mv	a0,s1
    80001fba:	c15fe0ef          	jal	80000bce <acquire>
      if(p->state == SLEEPING && p->chan == chan) {
    80001fbe:	4c9c                	lw	a5,24(s1)
    80001fc0:	ff3791e3          	bne	a5,s3,80001fa2 <wakeup+0x2a>
    80001fc4:	709c                	ld	a5,32(s1)
    80001fc6:	fd479ee3          	bne	a5,s4,80001fa2 <wakeup+0x2a>
        p->state = RUNNABLE;
    80001fca:	0154ac23          	sw	s5,24(s1)
    80001fce:	bfd1                	j	80001fa2 <wakeup+0x2a>
    }
  }
}
    80001fd0:	70e2                	ld	ra,56(sp)
    80001fd2:	7442                	ld	s0,48(sp)
    80001fd4:	74a2                	ld	s1,40(sp)
    80001fd6:	7902                	ld	s2,32(sp)
    80001fd8:	69e2                	ld	s3,24(sp)
    80001fda:	6a42                	ld	s4,16(sp)
    80001fdc:	6aa2                	ld	s5,8(sp)
    80001fde:	6121                	addi	sp,sp,64
    80001fe0:	8082                	ret

0000000080001fe2 <reparent>:
{
    80001fe2:	7179                	addi	sp,sp,-48
    80001fe4:	f406                	sd	ra,40(sp)
    80001fe6:	f022                	sd	s0,32(sp)
    80001fe8:	ec26                	sd	s1,24(sp)
    80001fea:	e84a                	sd	s2,16(sp)
    80001fec:	e44e                	sd	s3,8(sp)
    80001fee:	e052                	sd	s4,0(sp)
    80001ff0:	1800                	addi	s0,sp,48
    80001ff2:	892a                	mv	s2,a0
  for(pp = proc; pp < &proc[NPROC]; pp++){
    80001ff4:	0000e497          	auipc	s1,0xe
    80001ff8:	da448493          	addi	s1,s1,-604 # 8000fd98 <proc>
      pp->parent = initproc;
    80001ffc:	00006a17          	auipc	s4,0x6
    80002000:	864a0a13          	addi	s4,s4,-1948 # 80007860 <initproc>
  for(pp = proc; pp < &proc[NPROC]; pp++){
    80002004:	00014997          	auipc	s3,0x14
    80002008:	99498993          	addi	s3,s3,-1644 # 80015998 <tickslock>
    8000200c:	a029                	j	80002016 <reparent+0x34>
    8000200e:	17048493          	addi	s1,s1,368
    80002012:	01348b63          	beq	s1,s3,80002028 <reparent+0x46>
    if(pp->parent == p){
    80002016:	7c9c                	ld	a5,56(s1)
    80002018:	ff279be3          	bne	a5,s2,8000200e <reparent+0x2c>
      pp->parent = initproc;
    8000201c:	000a3503          	ld	a0,0(s4)
    80002020:	fc88                	sd	a0,56(s1)
      wakeup(initproc);
    80002022:	f57ff0ef          	jal	80001f78 <wakeup>
    80002026:	b7e5                	j	8000200e <reparent+0x2c>
}
    80002028:	70a2                	ld	ra,40(sp)
    8000202a:	7402                	ld	s0,32(sp)
    8000202c:	64e2                	ld	s1,24(sp)
    8000202e:	6942                	ld	s2,16(sp)
    80002030:	69a2                	ld	s3,8(sp)
    80002032:	6a02                	ld	s4,0(sp)
    80002034:	6145                	addi	sp,sp,48
    80002036:	8082                	ret

0000000080002038 <kexit>:
{
    80002038:	7179                	addi	sp,sp,-48
    8000203a:	f406                	sd	ra,40(sp)
    8000203c:	f022                	sd	s0,32(sp)
    8000203e:	ec26                	sd	s1,24(sp)
    80002040:	e84a                	sd	s2,16(sp)
    80002042:	e44e                	sd	s3,8(sp)
    80002044:	e052                	sd	s4,0(sp)
    80002046:	1800                	addi	s0,sp,48
    80002048:	8a2a                	mv	s4,a0
  struct proc *p = myproc();
    8000204a:	8e7ff0ef          	jal	80001930 <myproc>
    8000204e:	89aa                	mv	s3,a0
  if(p == initproc)
    80002050:	00006797          	auipc	a5,0x6
    80002054:	8107b783          	ld	a5,-2032(a5) # 80007860 <initproc>
    80002058:	0d050493          	addi	s1,a0,208
    8000205c:	15050913          	addi	s2,a0,336
    80002060:	00a79f63          	bne	a5,a0,8000207e <kexit+0x46>
    panic("init exiting");
    80002064:	00005517          	auipc	a0,0x5
    80002068:	17c50513          	addi	a0,a0,380 # 800071e0 <etext+0x1e0>
    8000206c:	f74fe0ef          	jal	800007e0 <panic>
      fileclose(f);
    80002070:	012020ef          	jal	80004082 <fileclose>
      p->ofile[fd] = 0;
    80002074:	0004b023          	sd	zero,0(s1)
  for(int fd = 0; fd < NOFILE; fd++){
    80002078:	04a1                	addi	s1,s1,8
    8000207a:	01248563          	beq	s1,s2,80002084 <kexit+0x4c>
    if(p->ofile[fd]){
    8000207e:	6088                	ld	a0,0(s1)
    80002080:	f965                	bnez	a0,80002070 <kexit+0x38>
    80002082:	bfdd                	j	80002078 <kexit+0x40>
  begin_op();
    80002084:	3f3010ef          	jal	80003c76 <begin_op>
  iput(p->cwd);
    80002088:	1509b503          	ld	a0,336(s3)
    8000208c:	382010ef          	jal	8000340e <iput>
  end_op();
    80002090:	451010ef          	jal	80003ce0 <end_op>
  p->cwd = 0;
    80002094:	1409b823          	sd	zero,336(s3)
  acquire(&wait_lock);
    80002098:	0000e497          	auipc	s1,0xe
    8000209c:	8e848493          	addi	s1,s1,-1816 # 8000f980 <wait_lock>
    800020a0:	8526                	mv	a0,s1
    800020a2:	b2dfe0ef          	jal	80000bce <acquire>
  reparent(p);
    800020a6:	854e                	mv	a0,s3
    800020a8:	f3bff0ef          	jal	80001fe2 <reparent>
  wakeup(p->parent);
    800020ac:	0389b503          	ld	a0,56(s3)
    800020b0:	ec9ff0ef          	jal	80001f78 <wakeup>
  acquire(&p->lock);
    800020b4:	854e                	mv	a0,s3
    800020b6:	b19fe0ef          	jal	80000bce <acquire>
  p->xstate = status;
    800020ba:	0349a623          	sw	s4,44(s3)
  p->state = ZOMBIE;
    800020be:	4795                	li	a5,5
    800020c0:	00f9ac23          	sw	a5,24(s3)
  release(&wait_lock);
    800020c4:	8526                	mv	a0,s1
    800020c6:	ba1fe0ef          	jal	80000c66 <release>
  sched();
    800020ca:	d7dff0ef          	jal	80001e46 <sched>
  panic("zombie exit");
    800020ce:	00005517          	auipc	a0,0x5
    800020d2:	12250513          	addi	a0,a0,290 # 800071f0 <etext+0x1f0>
    800020d6:	f0afe0ef          	jal	800007e0 <panic>

00000000800020da <kkill>:
// Kill the process with the given pid.
// The victim won't exit until it tries to return
// to user space (see usertrap() in trap.c).
int
kkill(int pid)
{
    800020da:	7179                	addi	sp,sp,-48
    800020dc:	f406                	sd	ra,40(sp)
    800020de:	f022                	sd	s0,32(sp)
    800020e0:	ec26                	sd	s1,24(sp)
    800020e2:	e84a                	sd	s2,16(sp)
    800020e4:	e44e                	sd	s3,8(sp)
    800020e6:	1800                	addi	s0,sp,48
    800020e8:	892a                	mv	s2,a0
  struct proc *p;

  for(p = proc; p < &proc[NPROC]; p++){
    800020ea:	0000e497          	auipc	s1,0xe
    800020ee:	cae48493          	addi	s1,s1,-850 # 8000fd98 <proc>
    800020f2:	00014997          	auipc	s3,0x14
    800020f6:	8a698993          	addi	s3,s3,-1882 # 80015998 <tickslock>
    acquire(&p->lock);
    800020fa:	8526                	mv	a0,s1
    800020fc:	ad3fe0ef          	jal	80000bce <acquire>
    if(p->pid == pid){
    80002100:	589c                	lw	a5,48(s1)
    80002102:	01278b63          	beq	a5,s2,80002118 <kkill+0x3e>
        p->state = RUNNABLE;
      }
      release(&p->lock);
      return 0;
    }
    release(&p->lock);
    80002106:	8526                	mv	a0,s1
    80002108:	b5ffe0ef          	jal	80000c66 <release>
  for(p = proc; p < &proc[NPROC]; p++){
    8000210c:	17048493          	addi	s1,s1,368
    80002110:	ff3495e3          	bne	s1,s3,800020fa <kkill+0x20>
  }
  return -1;
    80002114:	557d                	li	a0,-1
    80002116:	a819                	j	8000212c <kkill+0x52>
      p->killed = 1;
    80002118:	4785                	li	a5,1
    8000211a:	d49c                	sw	a5,40(s1)
      if(p->state == SLEEPING){
    8000211c:	4c98                	lw	a4,24(s1)
    8000211e:	4789                	li	a5,2
    80002120:	00f70d63          	beq	a4,a5,8000213a <kkill+0x60>
      release(&p->lock);
    80002124:	8526                	mv	a0,s1
    80002126:	b41fe0ef          	jal	80000c66 <release>
      return 0;
    8000212a:	4501                	li	a0,0
}
    8000212c:	70a2                	ld	ra,40(sp)
    8000212e:	7402                	ld	s0,32(sp)
    80002130:	64e2                	ld	s1,24(sp)
    80002132:	6942                	ld	s2,16(sp)
    80002134:	69a2                	ld	s3,8(sp)
    80002136:	6145                	addi	sp,sp,48
    80002138:	8082                	ret
        p->state = RUNNABLE;
    8000213a:	478d                	li	a5,3
    8000213c:	cc9c                	sw	a5,24(s1)
    8000213e:	b7dd                	j	80002124 <kkill+0x4a>

0000000080002140 <setkilled>:

void
setkilled(struct proc *p)
{
    80002140:	1101                	addi	sp,sp,-32
    80002142:	ec06                	sd	ra,24(sp)
    80002144:	e822                	sd	s0,16(sp)
    80002146:	e426                	sd	s1,8(sp)
    80002148:	1000                	addi	s0,sp,32
    8000214a:	84aa                	mv	s1,a0
  acquire(&p->lock);
    8000214c:	a83fe0ef          	jal	80000bce <acquire>
  p->killed = 1;
    80002150:	4785                	li	a5,1
    80002152:	d49c                	sw	a5,40(s1)
  release(&p->lock);
    80002154:	8526                	mv	a0,s1
    80002156:	b11fe0ef          	jal	80000c66 <release>
}
    8000215a:	60e2                	ld	ra,24(sp)
    8000215c:	6442                	ld	s0,16(sp)
    8000215e:	64a2                	ld	s1,8(sp)
    80002160:	6105                	addi	sp,sp,32
    80002162:	8082                	ret

0000000080002164 <killed>:

int
killed(struct proc *p)
{
    80002164:	1101                	addi	sp,sp,-32
    80002166:	ec06                	sd	ra,24(sp)
    80002168:	e822                	sd	s0,16(sp)
    8000216a:	e426                	sd	s1,8(sp)
    8000216c:	e04a                	sd	s2,0(sp)
    8000216e:	1000                	addi	s0,sp,32
    80002170:	84aa                	mv	s1,a0
  int k;
  
  acquire(&p->lock);
    80002172:	a5dfe0ef          	jal	80000bce <acquire>
  k = p->killed;
    80002176:	0284a903          	lw	s2,40(s1)
  release(&p->lock);
    8000217a:	8526                	mv	a0,s1
    8000217c:	aebfe0ef          	jal	80000c66 <release>
  return k;
}
    80002180:	854a                	mv	a0,s2
    80002182:	60e2                	ld	ra,24(sp)
    80002184:	6442                	ld	s0,16(sp)
    80002186:	64a2                	ld	s1,8(sp)
    80002188:	6902                	ld	s2,0(sp)
    8000218a:	6105                	addi	sp,sp,32
    8000218c:	8082                	ret

000000008000218e <kwait>:
{
    8000218e:	715d                	addi	sp,sp,-80
    80002190:	e486                	sd	ra,72(sp)
    80002192:	e0a2                	sd	s0,64(sp)
    80002194:	fc26                	sd	s1,56(sp)
    80002196:	f84a                	sd	s2,48(sp)
    80002198:	f44e                	sd	s3,40(sp)
    8000219a:	f052                	sd	s4,32(sp)
    8000219c:	ec56                	sd	s5,24(sp)
    8000219e:	e85a                	sd	s6,16(sp)
    800021a0:	e45e                	sd	s7,8(sp)
    800021a2:	e062                	sd	s8,0(sp)
    800021a4:	0880                	addi	s0,sp,80
    800021a6:	8b2a                	mv	s6,a0
  struct proc *p = myproc();
    800021a8:	f88ff0ef          	jal	80001930 <myproc>
    800021ac:	892a                	mv	s2,a0
  acquire(&wait_lock);
    800021ae:	0000d517          	auipc	a0,0xd
    800021b2:	7d250513          	addi	a0,a0,2002 # 8000f980 <wait_lock>
    800021b6:	a19fe0ef          	jal	80000bce <acquire>
    havekids = 0;
    800021ba:	4b81                	li	s7,0
        if(pp->state == ZOMBIE){
    800021bc:	4a15                	li	s4,5
        havekids = 1;
    800021be:	4a85                	li	s5,1
    for(pp = proc; pp < &proc[NPROC]; pp++){
    800021c0:	00013997          	auipc	s3,0x13
    800021c4:	7d898993          	addi	s3,s3,2008 # 80015998 <tickslock>
    sleep(p, &wait_lock);  //DOC: wait-sleep
    800021c8:	0000dc17          	auipc	s8,0xd
    800021cc:	7b8c0c13          	addi	s8,s8,1976 # 8000f980 <wait_lock>
    800021d0:	a871                	j	8000226c <kwait+0xde>
          pid = pp->pid;
    800021d2:	0304a983          	lw	s3,48(s1)
          if(addr != 0 && copyout(p->pagetable, addr, (char *)&pp->xstate,
    800021d6:	000b0c63          	beqz	s6,800021ee <kwait+0x60>
    800021da:	4691                	li	a3,4
    800021dc:	02c48613          	addi	a2,s1,44
    800021e0:	85da                	mv	a1,s6
    800021e2:	05093503          	ld	a0,80(s2)
    800021e6:	bfcff0ef          	jal	800015e2 <copyout>
    800021ea:	02054b63          	bltz	a0,80002220 <kwait+0x92>
          freeproc(pp);
    800021ee:	8526                	mv	a0,s1
    800021f0:	911ff0ef          	jal	80001b00 <freeproc>
          release(&pp->lock);
    800021f4:	8526                	mv	a0,s1
    800021f6:	a71fe0ef          	jal	80000c66 <release>
          release(&wait_lock);
    800021fa:	0000d517          	auipc	a0,0xd
    800021fe:	78650513          	addi	a0,a0,1926 # 8000f980 <wait_lock>
    80002202:	a65fe0ef          	jal	80000c66 <release>
}
    80002206:	854e                	mv	a0,s3
    80002208:	60a6                	ld	ra,72(sp)
    8000220a:	6406                	ld	s0,64(sp)
    8000220c:	74e2                	ld	s1,56(sp)
    8000220e:	7942                	ld	s2,48(sp)
    80002210:	79a2                	ld	s3,40(sp)
    80002212:	7a02                	ld	s4,32(sp)
    80002214:	6ae2                	ld	s5,24(sp)
    80002216:	6b42                	ld	s6,16(sp)
    80002218:	6ba2                	ld	s7,8(sp)
    8000221a:	6c02                	ld	s8,0(sp)
    8000221c:	6161                	addi	sp,sp,80
    8000221e:	8082                	ret
            release(&pp->lock);
    80002220:	8526                	mv	a0,s1
    80002222:	a45fe0ef          	jal	80000c66 <release>
            release(&wait_lock);
    80002226:	0000d517          	auipc	a0,0xd
    8000222a:	75a50513          	addi	a0,a0,1882 # 8000f980 <wait_lock>
    8000222e:	a39fe0ef          	jal	80000c66 <release>
            return -1;
    80002232:	59fd                	li	s3,-1
    80002234:	bfc9                	j	80002206 <kwait+0x78>
    for(pp = proc; pp < &proc[NPROC]; pp++){
    80002236:	17048493          	addi	s1,s1,368
    8000223a:	03348063          	beq	s1,s3,8000225a <kwait+0xcc>
      if(pp->parent == p){
    8000223e:	7c9c                	ld	a5,56(s1)
    80002240:	ff279be3          	bne	a5,s2,80002236 <kwait+0xa8>
        acquire(&pp->lock);
    80002244:	8526                	mv	a0,s1
    80002246:	989fe0ef          	jal	80000bce <acquire>
        if(pp->state == ZOMBIE){
    8000224a:	4c9c                	lw	a5,24(s1)
    8000224c:	f94783e3          	beq	a5,s4,800021d2 <kwait+0x44>
        release(&pp->lock);
    80002250:	8526                	mv	a0,s1
    80002252:	a15fe0ef          	jal	80000c66 <release>
        havekids = 1;
    80002256:	8756                	mv	a4,s5
    80002258:	bff9                	j	80002236 <kwait+0xa8>
    if(!havekids || killed(p)){
    8000225a:	cf19                	beqz	a4,80002278 <kwait+0xea>
    8000225c:	854a                	mv	a0,s2
    8000225e:	f07ff0ef          	jal	80002164 <killed>
    80002262:	e919                	bnez	a0,80002278 <kwait+0xea>
    sleep(p, &wait_lock);  //DOC: wait-sleep
    80002264:	85e2                	mv	a1,s8
    80002266:	854a                	mv	a0,s2
    80002268:	cc5ff0ef          	jal	80001f2c <sleep>
    havekids = 0;
    8000226c:	875e                	mv	a4,s7
    for(pp = proc; pp < &proc[NPROC]; pp++){
    8000226e:	0000e497          	auipc	s1,0xe
    80002272:	b2a48493          	addi	s1,s1,-1238 # 8000fd98 <proc>
    80002276:	b7e1                	j	8000223e <kwait+0xb0>
      release(&wait_lock);
    80002278:	0000d517          	auipc	a0,0xd
    8000227c:	70850513          	addi	a0,a0,1800 # 8000f980 <wait_lock>
    80002280:	9e7fe0ef          	jal	80000c66 <release>
      return -1;
    80002284:	59fd                	li	s3,-1
    80002286:	b741                	j	80002206 <kwait+0x78>

0000000080002288 <either_copyout>:
// Copy to either a user address, or kernel address,
// depending on usr_dst.
// Returns 0 on success, -1 on error.
int
either_copyout(int user_dst, uint64 dst, void *src, uint64 len)
{
    80002288:	7179                	addi	sp,sp,-48
    8000228a:	f406                	sd	ra,40(sp)
    8000228c:	f022                	sd	s0,32(sp)
    8000228e:	ec26                	sd	s1,24(sp)
    80002290:	e84a                	sd	s2,16(sp)
    80002292:	e44e                	sd	s3,8(sp)
    80002294:	e052                	sd	s4,0(sp)
    80002296:	1800                	addi	s0,sp,48
    80002298:	84aa                	mv	s1,a0
    8000229a:	892e                	mv	s2,a1
    8000229c:	89b2                	mv	s3,a2
    8000229e:	8a36                	mv	s4,a3
  struct proc *p = myproc();
    800022a0:	e90ff0ef          	jal	80001930 <myproc>
  if(user_dst){
    800022a4:	cc99                	beqz	s1,800022c2 <either_copyout+0x3a>
    return copyout(p->pagetable, dst, src, len);
    800022a6:	86d2                	mv	a3,s4
    800022a8:	864e                	mv	a2,s3
    800022aa:	85ca                	mv	a1,s2
    800022ac:	6928                	ld	a0,80(a0)
    800022ae:	b34ff0ef          	jal	800015e2 <copyout>
  } else {
    memmove((char *)dst, src, len);
    return 0;
  }
}
    800022b2:	70a2                	ld	ra,40(sp)
    800022b4:	7402                	ld	s0,32(sp)
    800022b6:	64e2                	ld	s1,24(sp)
    800022b8:	6942                	ld	s2,16(sp)
    800022ba:	69a2                	ld	s3,8(sp)
    800022bc:	6a02                	ld	s4,0(sp)
    800022be:	6145                	addi	sp,sp,48
    800022c0:	8082                	ret
    memmove((char *)dst, src, len);
    800022c2:	000a061b          	sext.w	a2,s4
    800022c6:	85ce                	mv	a1,s3
    800022c8:	854a                	mv	a0,s2
    800022ca:	a35fe0ef          	jal	80000cfe <memmove>
    return 0;
    800022ce:	8526                	mv	a0,s1
    800022d0:	b7cd                	j	800022b2 <either_copyout+0x2a>

00000000800022d2 <either_copyin>:
// Copy from either a user address, or kernel address,
// depending on usr_src.
// Returns 0 on success, -1 on error.
int
either_copyin(void *dst, int user_src, uint64 src, uint64 len)
{
    800022d2:	7179                	addi	sp,sp,-48
    800022d4:	f406                	sd	ra,40(sp)
    800022d6:	f022                	sd	s0,32(sp)
    800022d8:	ec26                	sd	s1,24(sp)
    800022da:	e84a                	sd	s2,16(sp)
    800022dc:	e44e                	sd	s3,8(sp)
    800022de:	e052                	sd	s4,0(sp)
    800022e0:	1800                	addi	s0,sp,48
    800022e2:	892a                	mv	s2,a0
    800022e4:	84ae                	mv	s1,a1
    800022e6:	89b2                	mv	s3,a2
    800022e8:	8a36                	mv	s4,a3
  struct proc *p = myproc();
    800022ea:	e46ff0ef          	jal	80001930 <myproc>
  if(user_src){
    800022ee:	cc99                	beqz	s1,8000230c <either_copyin+0x3a>
    return copyin(p->pagetable, dst, src, len);
    800022f0:	86d2                	mv	a3,s4
    800022f2:	864e                	mv	a2,s3
    800022f4:	85ca                	mv	a1,s2
    800022f6:	6928                	ld	a0,80(a0)
    800022f8:	bceff0ef          	jal	800016c6 <copyin>
  } else {
    memmove(dst, (char*)src, len);
    return 0;
  }
}
    800022fc:	70a2                	ld	ra,40(sp)
    800022fe:	7402                	ld	s0,32(sp)
    80002300:	64e2                	ld	s1,24(sp)
    80002302:	6942                	ld	s2,16(sp)
    80002304:	69a2                	ld	s3,8(sp)
    80002306:	6a02                	ld	s4,0(sp)
    80002308:	6145                	addi	sp,sp,48
    8000230a:	8082                	ret
    memmove(dst, (char*)src, len);
    8000230c:	000a061b          	sext.w	a2,s4
    80002310:	85ce                	mv	a1,s3
    80002312:	854a                	mv	a0,s2
    80002314:	9ebfe0ef          	jal	80000cfe <memmove>
    return 0;
    80002318:	8526                	mv	a0,s1
    8000231a:	b7cd                	j	800022fc <either_copyin+0x2a>

000000008000231c <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
    8000231c:	715d                	addi	sp,sp,-80
    8000231e:	e486                	sd	ra,72(sp)
    80002320:	e0a2                	sd	s0,64(sp)
    80002322:	fc26                	sd	s1,56(sp)
    80002324:	f84a                	sd	s2,48(sp)
    80002326:	f44e                	sd	s3,40(sp)
    80002328:	f052                	sd	s4,32(sp)
    8000232a:	ec56                	sd	s5,24(sp)
    8000232c:	e85a                	sd	s6,16(sp)
    8000232e:	e45e                	sd	s7,8(sp)
    80002330:	0880                	addi	s0,sp,80
  [ZOMBIE]    "zombie"
  };
  struct proc *p;
  char *state;

  printf("\n");
    80002332:	00005517          	auipc	a0,0x5
    80002336:	d4650513          	addi	a0,a0,-698 # 80007078 <etext+0x78>
    8000233a:	9c0fe0ef          	jal	800004fa <printf>
  for(p = proc; p < &proc[NPROC]; p++){
    8000233e:	0000e497          	auipc	s1,0xe
    80002342:	bb248493          	addi	s1,s1,-1102 # 8000fef0 <proc+0x158>
    80002346:	00013917          	auipc	s2,0x13
    8000234a:	7aa90913          	addi	s2,s2,1962 # 80015af0 <bcache+0x140>
    if(p->state == UNUSED)
      continue;
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    8000234e:	4b15                	li	s6,5
      state = states[p->state];
    else
      state = "???";
    80002350:	00005997          	auipc	s3,0x5
    80002354:	eb098993          	addi	s3,s3,-336 # 80007200 <etext+0x200>
    printf("%d %s %s", p->pid, state, p->name);
    80002358:	00005a97          	auipc	s5,0x5
    8000235c:	eb0a8a93          	addi	s5,s5,-336 # 80007208 <etext+0x208>
    printf("\n");
    80002360:	00005a17          	auipc	s4,0x5
    80002364:	d18a0a13          	addi	s4,s4,-744 # 80007078 <etext+0x78>
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    80002368:	00005b97          	auipc	s7,0x5
    8000236c:	3c0b8b93          	addi	s7,s7,960 # 80007728 <states.0>
    80002370:	a829                	j	8000238a <procdump+0x6e>
    printf("%d %s %s", p->pid, state, p->name);
    80002372:	ed86a583          	lw	a1,-296(a3)
    80002376:	8556                	mv	a0,s5
    80002378:	982fe0ef          	jal	800004fa <printf>
    printf("\n");
    8000237c:	8552                	mv	a0,s4
    8000237e:	97cfe0ef          	jal	800004fa <printf>
  for(p = proc; p < &proc[NPROC]; p++){
    80002382:	17048493          	addi	s1,s1,368
    80002386:	03248263          	beq	s1,s2,800023aa <procdump+0x8e>
    if(p->state == UNUSED)
    8000238a:	86a6                	mv	a3,s1
    8000238c:	ec04a783          	lw	a5,-320(s1)
    80002390:	dbed                	beqz	a5,80002382 <procdump+0x66>
      state = "???";
    80002392:	864e                	mv	a2,s3
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    80002394:	fcfb6fe3          	bltu	s6,a5,80002372 <procdump+0x56>
    80002398:	02079713          	slli	a4,a5,0x20
    8000239c:	01d75793          	srli	a5,a4,0x1d
    800023a0:	97de                	add	a5,a5,s7
    800023a2:	6390                	ld	a2,0(a5)
    800023a4:	f679                	bnez	a2,80002372 <procdump+0x56>
      state = "???";
    800023a6:	864e                	mv	a2,s3
    800023a8:	b7e9                	j	80002372 <procdump+0x56>
  }
}
    800023aa:	60a6                	ld	ra,72(sp)
    800023ac:	6406                	ld	s0,64(sp)
    800023ae:	74e2                	ld	s1,56(sp)
    800023b0:	7942                	ld	s2,48(sp)
    800023b2:	79a2                	ld	s3,40(sp)
    800023b4:	7a02                	ld	s4,32(sp)
    800023b6:	6ae2                	ld	s5,24(sp)
    800023b8:	6b42                	ld	s6,16(sp)
    800023ba:	6ba2                	ld	s7,8(sp)
    800023bc:	6161                	addi	sp,sp,80
    800023be:	8082                	ret

00000000800023c0 <swtch>:
# Save current registers in old. Load from new.	


.globl swtch
swtch:
        sd ra, 0(a0)
    800023c0:	00153023          	sd	ra,0(a0)
        sd sp, 8(a0)
    800023c4:	00253423          	sd	sp,8(a0)
        sd s0, 16(a0)
    800023c8:	e900                	sd	s0,16(a0)
        sd s1, 24(a0)
    800023ca:	ed04                	sd	s1,24(a0)
        sd s2, 32(a0)
    800023cc:	03253023          	sd	s2,32(a0)
        sd s3, 40(a0)
    800023d0:	03353423          	sd	s3,40(a0)
        sd s4, 48(a0)
    800023d4:	03453823          	sd	s4,48(a0)
        sd s5, 56(a0)
    800023d8:	03553c23          	sd	s5,56(a0)
        sd s6, 64(a0)
    800023dc:	05653023          	sd	s6,64(a0)
        sd s7, 72(a0)
    800023e0:	05753423          	sd	s7,72(a0)
        sd s8, 80(a0)
    800023e4:	05853823          	sd	s8,80(a0)
        sd s9, 88(a0)
    800023e8:	05953c23          	sd	s9,88(a0)
        sd s10, 96(a0)
    800023ec:	07a53023          	sd	s10,96(a0)
        sd s11, 104(a0)
    800023f0:	07b53423          	sd	s11,104(a0)

        ld ra, 0(a1)
    800023f4:	0005b083          	ld	ra,0(a1)
        ld sp, 8(a1)
    800023f8:	0085b103          	ld	sp,8(a1)
        ld s0, 16(a1)
    800023fc:	6980                	ld	s0,16(a1)
        ld s1, 24(a1)
    800023fe:	6d84                	ld	s1,24(a1)
        ld s2, 32(a1)
    80002400:	0205b903          	ld	s2,32(a1)
        ld s3, 40(a1)
    80002404:	0285b983          	ld	s3,40(a1)
        ld s4, 48(a1)
    80002408:	0305ba03          	ld	s4,48(a1)
        ld s5, 56(a1)
    8000240c:	0385ba83          	ld	s5,56(a1)
        ld s6, 64(a1)
    80002410:	0405bb03          	ld	s6,64(a1)
        ld s7, 72(a1)
    80002414:	0485bb83          	ld	s7,72(a1)
        ld s8, 80(a1)
    80002418:	0505bc03          	ld	s8,80(a1)
        ld s9, 88(a1)
    8000241c:	0585bc83          	ld	s9,88(a1)
        ld s10, 96(a1)
    80002420:	0605bd03          	ld	s10,96(a1)
        ld s11, 104(a1)
    80002424:	0685bd83          	ld	s11,104(a1)
        
        ret
    80002428:	8082                	ret

000000008000242a <trapinit>:

extern int devintr();

void
trapinit(void)
{
    8000242a:	1141                	addi	sp,sp,-16
    8000242c:	e406                	sd	ra,8(sp)
    8000242e:	e022                	sd	s0,0(sp)
    80002430:	0800                	addi	s0,sp,16
  initlock(&tickslock, "time");
    80002432:	00005597          	auipc	a1,0x5
    80002436:	e1658593          	addi	a1,a1,-490 # 80007248 <etext+0x248>
    8000243a:	00013517          	auipc	a0,0x13
    8000243e:	55e50513          	addi	a0,a0,1374 # 80015998 <tickslock>
    80002442:	f0cfe0ef          	jal	80000b4e <initlock>
}
    80002446:	60a2                	ld	ra,8(sp)
    80002448:	6402                	ld	s0,0(sp)
    8000244a:	0141                	addi	sp,sp,16
    8000244c:	8082                	ret

000000008000244e <trapinithart>:

// set up to take exceptions and traps while in the kernel.
void
trapinithart(void)
{
    8000244e:	1141                	addi	sp,sp,-16
    80002450:	e422                	sd	s0,8(sp)
    80002452:	0800                	addi	s0,sp,16
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002454:	00003797          	auipc	a5,0x3
    80002458:	f9c78793          	addi	a5,a5,-100 # 800053f0 <kernelvec>
    8000245c:	10579073          	csrw	stvec,a5
  w_stvec((uint64)kernelvec);
}
    80002460:	6422                	ld	s0,8(sp)
    80002462:	0141                	addi	sp,sp,16
    80002464:	8082                	ret

0000000080002466 <prepare_return>:
//
// set up trapframe and control registers for a return to user space
//
void
prepare_return(void)
{
    80002466:	1141                	addi	sp,sp,-16
    80002468:	e406                	sd	ra,8(sp)
    8000246a:	e022                	sd	s0,0(sp)
    8000246c:	0800                	addi	s0,sp,16
  struct proc *p = myproc();
    8000246e:	cc2ff0ef          	jal	80001930 <myproc>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002472:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    80002476:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002478:	10079073          	csrw	sstatus,a5
  // kerneltrap() to usertrap(). because a trap from kernel
  // code to usertrap would be a disaster, turn off interrupts.
  intr_off();

  // send syscalls, interrupts, and exceptions to uservec in trampoline.S
  uint64 trampoline_uservec = TRAMPOLINE + (uservec - trampoline);
    8000247c:	04000737          	lui	a4,0x4000
    80002480:	177d                	addi	a4,a4,-1 # 3ffffff <_entry-0x7c000001>
    80002482:	0732                	slli	a4,a4,0xc
    80002484:	00004797          	auipc	a5,0x4
    80002488:	b7c78793          	addi	a5,a5,-1156 # 80006000 <_trampoline>
    8000248c:	00004697          	auipc	a3,0x4
    80002490:	b7468693          	addi	a3,a3,-1164 # 80006000 <_trampoline>
    80002494:	8f95                	sub	a5,a5,a3
    80002496:	97ba                	add	a5,a5,a4
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002498:	10579073          	csrw	stvec,a5
  w_stvec(trampoline_uservec);

  // set up trapframe values that uservec will need when
  // the process next traps into the kernel.
  p->trapframe->kernel_satp = r_satp();         // kernel page table
    8000249c:	6d3c                	ld	a5,88(a0)
  asm volatile("csrr %0, satp" : "=r" (x) );
    8000249e:	18002773          	csrr	a4,satp
    800024a2:	e398                	sd	a4,0(a5)
  p->trapframe->kernel_sp = p->kstack + PGSIZE; // process's kernel stack
    800024a4:	6d38                	ld	a4,88(a0)
    800024a6:	613c                	ld	a5,64(a0)
    800024a8:	6685                	lui	a3,0x1
    800024aa:	97b6                	add	a5,a5,a3
    800024ac:	e71c                	sd	a5,8(a4)
  p->trapframe->kernel_trap = (uint64)usertrap;
    800024ae:	6d3c                	ld	a5,88(a0)
    800024b0:	00000717          	auipc	a4,0x0
    800024b4:	0f870713          	addi	a4,a4,248 # 800025a8 <usertrap>
    800024b8:	eb98                	sd	a4,16(a5)
  p->trapframe->kernel_hartid = r_tp();         // hartid for cpuid()
    800024ba:	6d3c                	ld	a5,88(a0)
  asm volatile("mv %0, tp" : "=r" (x) );
    800024bc:	8712                	mv	a4,tp
    800024be:	f398                	sd	a4,32(a5)
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    800024c0:	100027f3          	csrr	a5,sstatus
  // set up the registers that trampoline.S's sret will use
  // to get to user space.
  
  // set S Previous Privilege mode to User.
  unsigned long x = r_sstatus();
  x &= ~SSTATUS_SPP; // clear SPP to 0 for user mode
    800024c4:	eff7f793          	andi	a5,a5,-257
  x |= SSTATUS_SPIE; // enable interrupts in user mode
    800024c8:	0207e793          	ori	a5,a5,32
  asm volatile("csrw sstatus, %0" : : "r" (x));
    800024cc:	10079073          	csrw	sstatus,a5
  w_sstatus(x);

  // set S Exception Program Counter to the saved user pc.
  w_sepc(p->trapframe->epc);
    800024d0:	6d3c                	ld	a5,88(a0)
  asm volatile("csrw sepc, %0" : : "r" (x));
    800024d2:	6f9c                	ld	a5,24(a5)
    800024d4:	14179073          	csrw	sepc,a5
}
    800024d8:	60a2                	ld	ra,8(sp)
    800024da:	6402                	ld	s0,0(sp)
    800024dc:	0141                	addi	sp,sp,16
    800024de:	8082                	ret

00000000800024e0 <clockintr>:
  w_sstatus(sstatus);
}

void
clockintr()
{
    800024e0:	1101                	addi	sp,sp,-32
    800024e2:	ec06                	sd	ra,24(sp)
    800024e4:	e822                	sd	s0,16(sp)
    800024e6:	1000                	addi	s0,sp,32
  if(cpuid() == 0){
    800024e8:	c1cff0ef          	jal	80001904 <cpuid>
    800024ec:	cd11                	beqz	a0,80002508 <clockintr+0x28>
  asm volatile("csrr %0, time" : "=r" (x) );
    800024ee:	c01027f3          	rdtime	a5
  }

  // ask for the next timer interrupt. this also clears
  // the interrupt request. 1000000 is about a tenth
  // of a second.
  w_stimecmp(r_time() + 1000000);
    800024f2:	000f4737          	lui	a4,0xf4
    800024f6:	24070713          	addi	a4,a4,576 # f4240 <_entry-0x7ff0bdc0>
    800024fa:	97ba                	add	a5,a5,a4
  asm volatile("csrw 0x14d, %0" : : "r" (x));
    800024fc:	14d79073          	csrw	stimecmp,a5
}
    80002500:	60e2                	ld	ra,24(sp)
    80002502:	6442                	ld	s0,16(sp)
    80002504:	6105                	addi	sp,sp,32
    80002506:	8082                	ret
    80002508:	e426                	sd	s1,8(sp)
    acquire(&tickslock);
    8000250a:	00013497          	auipc	s1,0x13
    8000250e:	48e48493          	addi	s1,s1,1166 # 80015998 <tickslock>
    80002512:	8526                	mv	a0,s1
    80002514:	ebafe0ef          	jal	80000bce <acquire>
    ticks++;
    80002518:	00005517          	auipc	a0,0x5
    8000251c:	35050513          	addi	a0,a0,848 # 80007868 <ticks>
    80002520:	411c                	lw	a5,0(a0)
    80002522:	2785                	addiw	a5,a5,1
    80002524:	c11c                	sw	a5,0(a0)
    wakeup(&ticks);
    80002526:	a53ff0ef          	jal	80001f78 <wakeup>
    release(&tickslock);
    8000252a:	8526                	mv	a0,s1
    8000252c:	f3afe0ef          	jal	80000c66 <release>
    80002530:	64a2                	ld	s1,8(sp)
    80002532:	bf75                	j	800024ee <clockintr+0xe>

0000000080002534 <devintr>:
// returns 2 if timer interrupt,
// 1 if other device,
// 0 if not recognized.
int
devintr()
{
    80002534:	1101                	addi	sp,sp,-32
    80002536:	ec06                	sd	ra,24(sp)
    80002538:	e822                	sd	s0,16(sp)
    8000253a:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, scause" : "=r" (x) );
    8000253c:	14202773          	csrr	a4,scause
  uint64 scause = r_scause();

  if(scause == 0x8000000000000009L){
    80002540:	57fd                	li	a5,-1
    80002542:	17fe                	slli	a5,a5,0x3f
    80002544:	07a5                	addi	a5,a5,9
    80002546:	00f70c63          	beq	a4,a5,8000255e <devintr+0x2a>
    // now allowed to interrupt again.
    if(irq)
      plic_complete(irq);

    return 1;
  } else if(scause == 0x8000000000000005L){
    8000254a:	57fd                	li	a5,-1
    8000254c:	17fe                	slli	a5,a5,0x3f
    8000254e:	0795                	addi	a5,a5,5
    // timer interrupt.
    clockintr();
    return 2;
  } else {
    return 0;
    80002550:	4501                	li	a0,0
  } else if(scause == 0x8000000000000005L){
    80002552:	04f70763          	beq	a4,a5,800025a0 <devintr+0x6c>
  }
}
    80002556:	60e2                	ld	ra,24(sp)
    80002558:	6442                	ld	s0,16(sp)
    8000255a:	6105                	addi	sp,sp,32
    8000255c:	8082                	ret
    8000255e:	e426                	sd	s1,8(sp)
    int irq = plic_claim();
    80002560:	73d020ef          	jal	8000549c <plic_claim>
    80002564:	84aa                	mv	s1,a0
    if(irq == UART0_IRQ){
    80002566:	47a9                	li	a5,10
    80002568:	00f50963          	beq	a0,a5,8000257a <devintr+0x46>
    } else if(irq == VIRTIO0_IRQ){
    8000256c:	4785                	li	a5,1
    8000256e:	00f50963          	beq	a0,a5,80002580 <devintr+0x4c>
    return 1;
    80002572:	4505                	li	a0,1
    } else if(irq){
    80002574:	e889                	bnez	s1,80002586 <devintr+0x52>
    80002576:	64a2                	ld	s1,8(sp)
    80002578:	bff9                	j	80002556 <devintr+0x22>
      uartintr();
    8000257a:	c36fe0ef          	jal	800009b0 <uartintr>
    if(irq)
    8000257e:	a819                	j	80002594 <devintr+0x60>
      virtio_disk_intr();
    80002580:	3e2030ef          	jal	80005962 <virtio_disk_intr>
    if(irq)
    80002584:	a801                	j	80002594 <devintr+0x60>
      printf("unexpected interrupt irq=%d\n", irq);
    80002586:	85a6                	mv	a1,s1
    80002588:	00005517          	auipc	a0,0x5
    8000258c:	cc850513          	addi	a0,a0,-824 # 80007250 <etext+0x250>
    80002590:	f6bfd0ef          	jal	800004fa <printf>
      plic_complete(irq);
    80002594:	8526                	mv	a0,s1
    80002596:	727020ef          	jal	800054bc <plic_complete>
    return 1;
    8000259a:	4505                	li	a0,1
    8000259c:	64a2                	ld	s1,8(sp)
    8000259e:	bf65                	j	80002556 <devintr+0x22>
    clockintr();
    800025a0:	f41ff0ef          	jal	800024e0 <clockintr>
    return 2;
    800025a4:	4509                	li	a0,2
    800025a6:	bf45                	j	80002556 <devintr+0x22>

00000000800025a8 <usertrap>:
{
    800025a8:	1101                	addi	sp,sp,-32
    800025aa:	ec06                	sd	ra,24(sp)
    800025ac:	e822                	sd	s0,16(sp)
    800025ae:	e426                	sd	s1,8(sp)
    800025b0:	e04a                	sd	s2,0(sp)
    800025b2:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    800025b4:	100027f3          	csrr	a5,sstatus
  if((r_sstatus() & SSTATUS_SPP) != 0)
    800025b8:	1007f793          	andi	a5,a5,256
    800025bc:	eba5                	bnez	a5,8000262c <usertrap+0x84>
  asm volatile("csrw stvec, %0" : : "r" (x));
    800025be:	00003797          	auipc	a5,0x3
    800025c2:	e3278793          	addi	a5,a5,-462 # 800053f0 <kernelvec>
    800025c6:	10579073          	csrw	stvec,a5
  struct proc *p = myproc();
    800025ca:	b66ff0ef          	jal	80001930 <myproc>
    800025ce:	84aa                	mv	s1,a0
  p->trapframe->epc = r_sepc();
    800025d0:	6d3c                	ld	a5,88(a0)
  asm volatile("csrr %0, sepc" : "=r" (x) );
    800025d2:	14102773          	csrr	a4,sepc
    800025d6:	ef98                	sd	a4,24(a5)
  asm volatile("csrr %0, scause" : "=r" (x) );
    800025d8:	14202773          	csrr	a4,scause
  if(r_scause() == 8){
    800025dc:	47a1                	li	a5,8
    800025de:	04f70d63          	beq	a4,a5,80002638 <usertrap+0x90>
  } else if((which_dev = devintr()) != 0){
    800025e2:	f53ff0ef          	jal	80002534 <devintr>
    800025e6:	892a                	mv	s2,a0
    800025e8:	e945                	bnez	a0,80002698 <usertrap+0xf0>
    800025ea:	14202773          	csrr	a4,scause
  } else if((r_scause() == 15 || r_scause() == 13) &&
    800025ee:	47bd                	li	a5,15
    800025f0:	08f70863          	beq	a4,a5,80002680 <usertrap+0xd8>
    800025f4:	14202773          	csrr	a4,scause
    800025f8:	47b5                	li	a5,13
    800025fa:	08f70363          	beq	a4,a5,80002680 <usertrap+0xd8>
    800025fe:	142025f3          	csrr	a1,scause
    printf("usertrap(): unexpected scause 0x%lx pid=%d\n", r_scause(), p->pid);
    80002602:	5890                	lw	a2,48(s1)
    80002604:	00005517          	auipc	a0,0x5
    80002608:	c8c50513          	addi	a0,a0,-884 # 80007290 <etext+0x290>
    8000260c:	eeffd0ef          	jal	800004fa <printf>
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002610:	141025f3          	csrr	a1,sepc
  asm volatile("csrr %0, stval" : "=r" (x) );
    80002614:	14302673          	csrr	a2,stval
    printf("            sepc=0x%lx stval=0x%lx\n", r_sepc(), r_stval());
    80002618:	00005517          	auipc	a0,0x5
    8000261c:	ca850513          	addi	a0,a0,-856 # 800072c0 <etext+0x2c0>
    80002620:	edbfd0ef          	jal	800004fa <printf>
    setkilled(p);
    80002624:	8526                	mv	a0,s1
    80002626:	b1bff0ef          	jal	80002140 <setkilled>
    8000262a:	a035                	j	80002656 <usertrap+0xae>
    panic("usertrap: not from user mode");
    8000262c:	00005517          	auipc	a0,0x5
    80002630:	c4450513          	addi	a0,a0,-956 # 80007270 <etext+0x270>
    80002634:	9acfe0ef          	jal	800007e0 <panic>
    if(killed(p))
    80002638:	b2dff0ef          	jal	80002164 <killed>
    8000263c:	ed15                	bnez	a0,80002678 <usertrap+0xd0>
    p->trapframe->epc += 4;
    8000263e:	6cb8                	ld	a4,88(s1)
    80002640:	6f1c                	ld	a5,24(a4)
    80002642:	0791                	addi	a5,a5,4
    80002644:	ef1c                	sd	a5,24(a4)
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002646:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    8000264a:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    8000264e:	10079073          	csrw	sstatus,a5
    syscall();
    80002652:	246000ef          	jal	80002898 <syscall>
  if(killed(p))
    80002656:	8526                	mv	a0,s1
    80002658:	b0dff0ef          	jal	80002164 <killed>
    8000265c:	e139                	bnez	a0,800026a2 <usertrap+0xfa>
  prepare_return();
    8000265e:	e09ff0ef          	jal	80002466 <prepare_return>
  uint64 satp = MAKE_SATP(p->pagetable);
    80002662:	68a8                	ld	a0,80(s1)
    80002664:	8131                	srli	a0,a0,0xc
    80002666:	57fd                	li	a5,-1
    80002668:	17fe                	slli	a5,a5,0x3f
    8000266a:	8d5d                	or	a0,a0,a5
}
    8000266c:	60e2                	ld	ra,24(sp)
    8000266e:	6442                	ld	s0,16(sp)
    80002670:	64a2                	ld	s1,8(sp)
    80002672:	6902                	ld	s2,0(sp)
    80002674:	6105                	addi	sp,sp,32
    80002676:	8082                	ret
      kexit(-1);
    80002678:	557d                	li	a0,-1
    8000267a:	9bfff0ef          	jal	80002038 <kexit>
    8000267e:	b7c1                	j	8000263e <usertrap+0x96>
  asm volatile("csrr %0, stval" : "=r" (x) );
    80002680:	143025f3          	csrr	a1,stval
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002684:	14202673          	csrr	a2,scause
            vmfault(p->pagetable, r_stval(), (r_scause() == 13)? 1 : 0) != 0) {
    80002688:	164d                	addi	a2,a2,-13 # ff3 <_entry-0x7ffff00d>
    8000268a:	00163613          	seqz	a2,a2
    8000268e:	68a8                	ld	a0,80(s1)
    80002690:	ed1fe0ef          	jal	80001560 <vmfault>
  } else if((r_scause() == 15 || r_scause() == 13) &&
    80002694:	f169                	bnez	a0,80002656 <usertrap+0xae>
    80002696:	b7a5                	j	800025fe <usertrap+0x56>
  if(killed(p))
    80002698:	8526                	mv	a0,s1
    8000269a:	acbff0ef          	jal	80002164 <killed>
    8000269e:	c511                	beqz	a0,800026aa <usertrap+0x102>
    800026a0:	a011                	j	800026a4 <usertrap+0xfc>
    800026a2:	4901                	li	s2,0
    kexit(-1);
    800026a4:	557d                	li	a0,-1
    800026a6:	993ff0ef          	jal	80002038 <kexit>
  if(which_dev == 2)
    800026aa:	4789                	li	a5,2
    800026ac:	faf919e3          	bne	s2,a5,8000265e <usertrap+0xb6>
    yield();
    800026b0:	851ff0ef          	jal	80001f00 <yield>
    800026b4:	b76d                	j	8000265e <usertrap+0xb6>

00000000800026b6 <kerneltrap>:
{
    800026b6:	7179                	addi	sp,sp,-48
    800026b8:	f406                	sd	ra,40(sp)
    800026ba:	f022                	sd	s0,32(sp)
    800026bc:	ec26                	sd	s1,24(sp)
    800026be:	e84a                	sd	s2,16(sp)
    800026c0:	e44e                	sd	s3,8(sp)
    800026c2:	1800                	addi	s0,sp,48
  asm volatile("csrr %0, sepc" : "=r" (x) );
    800026c4:	14102973          	csrr	s2,sepc
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    800026c8:	100024f3          	csrr	s1,sstatus
  asm volatile("csrr %0, scause" : "=r" (x) );
    800026cc:	142029f3          	csrr	s3,scause
  if((sstatus & SSTATUS_SPP) == 0)
    800026d0:	1004f793          	andi	a5,s1,256
    800026d4:	c795                	beqz	a5,80002700 <kerneltrap+0x4a>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    800026d6:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    800026da:	8b89                	andi	a5,a5,2
  if(intr_get() != 0)
    800026dc:	eb85                	bnez	a5,8000270c <kerneltrap+0x56>
  if((which_dev = devintr()) == 0){
    800026de:	e57ff0ef          	jal	80002534 <devintr>
    800026e2:	c91d                	beqz	a0,80002718 <kerneltrap+0x62>
  if(which_dev == 2 && myproc() != 0)
    800026e4:	4789                	li	a5,2
    800026e6:	04f50a63          	beq	a0,a5,8000273a <kerneltrap+0x84>
  asm volatile("csrw sepc, %0" : : "r" (x));
    800026ea:	14191073          	csrw	sepc,s2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    800026ee:	10049073          	csrw	sstatus,s1
}
    800026f2:	70a2                	ld	ra,40(sp)
    800026f4:	7402                	ld	s0,32(sp)
    800026f6:	64e2                	ld	s1,24(sp)
    800026f8:	6942                	ld	s2,16(sp)
    800026fa:	69a2                	ld	s3,8(sp)
    800026fc:	6145                	addi	sp,sp,48
    800026fe:	8082                	ret
    panic("kerneltrap: not from supervisor mode");
    80002700:	00005517          	auipc	a0,0x5
    80002704:	be850513          	addi	a0,a0,-1048 # 800072e8 <etext+0x2e8>
    80002708:	8d8fe0ef          	jal	800007e0 <panic>
    panic("kerneltrap: interrupts enabled");
    8000270c:	00005517          	auipc	a0,0x5
    80002710:	c0450513          	addi	a0,a0,-1020 # 80007310 <etext+0x310>
    80002714:	8ccfe0ef          	jal	800007e0 <panic>
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002718:	14102673          	csrr	a2,sepc
  asm volatile("csrr %0, stval" : "=r" (x) );
    8000271c:	143026f3          	csrr	a3,stval
    printf("scause=0x%lx sepc=0x%lx stval=0x%lx\n", scause, r_sepc(), r_stval());
    80002720:	85ce                	mv	a1,s3
    80002722:	00005517          	auipc	a0,0x5
    80002726:	c0e50513          	addi	a0,a0,-1010 # 80007330 <etext+0x330>
    8000272a:	dd1fd0ef          	jal	800004fa <printf>
    panic("kerneltrap");
    8000272e:	00005517          	auipc	a0,0x5
    80002732:	c2a50513          	addi	a0,a0,-982 # 80007358 <etext+0x358>
    80002736:	8aafe0ef          	jal	800007e0 <panic>
  if(which_dev == 2 && myproc() != 0)
    8000273a:	9f6ff0ef          	jal	80001930 <myproc>
    8000273e:	d555                	beqz	a0,800026ea <kerneltrap+0x34>
    yield();
    80002740:	fc0ff0ef          	jal	80001f00 <yield>
    80002744:	b75d                	j	800026ea <kerneltrap+0x34>

0000000080002746 <argraw>:
  return strlen(buf);
}

static uint64
argraw(int n)
{
    80002746:	1101                	addi	sp,sp,-32
    80002748:	ec06                	sd	ra,24(sp)
    8000274a:	e822                	sd	s0,16(sp)
    8000274c:	e426                	sd	s1,8(sp)
    8000274e:	1000                	addi	s0,sp,32
    80002750:	84aa                	mv	s1,a0
  struct proc *p = myproc();
    80002752:	9deff0ef          	jal	80001930 <myproc>
  switch (n) {
    80002756:	4795                	li	a5,5
    80002758:	0497e163          	bltu	a5,s1,8000279a <argraw+0x54>
    8000275c:	048a                	slli	s1,s1,0x2
    8000275e:	00005717          	auipc	a4,0x5
    80002762:	ffa70713          	addi	a4,a4,-6 # 80007758 <states.0+0x30>
    80002766:	94ba                	add	s1,s1,a4
    80002768:	409c                	lw	a5,0(s1)
    8000276a:	97ba                	add	a5,a5,a4
    8000276c:	8782                	jr	a5
  case 0:
    return p->trapframe->a0;
    8000276e:	6d3c                	ld	a5,88(a0)
    80002770:	7ba8                	ld	a0,112(a5)
  case 5:
    return p->trapframe->a5;
  }
  panic("argraw");
  return -1;
}
    80002772:	60e2                	ld	ra,24(sp)
    80002774:	6442                	ld	s0,16(sp)
    80002776:	64a2                	ld	s1,8(sp)
    80002778:	6105                	addi	sp,sp,32
    8000277a:	8082                	ret
    return p->trapframe->a1;
    8000277c:	6d3c                	ld	a5,88(a0)
    8000277e:	7fa8                	ld	a0,120(a5)
    80002780:	bfcd                	j	80002772 <argraw+0x2c>
    return p->trapframe->a2;
    80002782:	6d3c                	ld	a5,88(a0)
    80002784:	63c8                	ld	a0,128(a5)
    80002786:	b7f5                	j	80002772 <argraw+0x2c>
    return p->trapframe->a3;
    80002788:	6d3c                	ld	a5,88(a0)
    8000278a:	67c8                	ld	a0,136(a5)
    8000278c:	b7dd                	j	80002772 <argraw+0x2c>
    return p->trapframe->a4;
    8000278e:	6d3c                	ld	a5,88(a0)
    80002790:	6bc8                	ld	a0,144(a5)
    80002792:	b7c5                	j	80002772 <argraw+0x2c>
    return p->trapframe->a5;
    80002794:	6d3c                	ld	a5,88(a0)
    80002796:	6fc8                	ld	a0,152(a5)
    80002798:	bfe9                	j	80002772 <argraw+0x2c>
  panic("argraw");
    8000279a:	00005517          	auipc	a0,0x5
    8000279e:	bce50513          	addi	a0,a0,-1074 # 80007368 <etext+0x368>
    800027a2:	83efe0ef          	jal	800007e0 <panic>

00000000800027a6 <fetchaddr>:
{
    800027a6:	1101                	addi	sp,sp,-32
    800027a8:	ec06                	sd	ra,24(sp)
    800027aa:	e822                	sd	s0,16(sp)
    800027ac:	e426                	sd	s1,8(sp)
    800027ae:	e04a                	sd	s2,0(sp)
    800027b0:	1000                	addi	s0,sp,32
    800027b2:	84aa                	mv	s1,a0
    800027b4:	892e                	mv	s2,a1
  struct proc *p = myproc();
    800027b6:	97aff0ef          	jal	80001930 <myproc>
  if(addr >= p->sz || addr+sizeof(uint64) > p->sz) // both tests needed, in case of overflow
    800027ba:	653c                	ld	a5,72(a0)
    800027bc:	02f4f663          	bgeu	s1,a5,800027e8 <fetchaddr+0x42>
    800027c0:	00848713          	addi	a4,s1,8
    800027c4:	02e7e463          	bltu	a5,a4,800027ec <fetchaddr+0x46>
  if(copyin(p->pagetable, (char *)ip, addr, sizeof(*ip)) != 0)
    800027c8:	46a1                	li	a3,8
    800027ca:	8626                	mv	a2,s1
    800027cc:	85ca                	mv	a1,s2
    800027ce:	6928                	ld	a0,80(a0)
    800027d0:	ef7fe0ef          	jal	800016c6 <copyin>
    800027d4:	00a03533          	snez	a0,a0
    800027d8:	40a00533          	neg	a0,a0
}
    800027dc:	60e2                	ld	ra,24(sp)
    800027de:	6442                	ld	s0,16(sp)
    800027e0:	64a2                	ld	s1,8(sp)
    800027e2:	6902                	ld	s2,0(sp)
    800027e4:	6105                	addi	sp,sp,32
    800027e6:	8082                	ret
    return -1;
    800027e8:	557d                	li	a0,-1
    800027ea:	bfcd                	j	800027dc <fetchaddr+0x36>
    800027ec:	557d                	li	a0,-1
    800027ee:	b7fd                	j	800027dc <fetchaddr+0x36>

00000000800027f0 <fetchstr>:
{
    800027f0:	7179                	addi	sp,sp,-48
    800027f2:	f406                	sd	ra,40(sp)
    800027f4:	f022                	sd	s0,32(sp)
    800027f6:	ec26                	sd	s1,24(sp)
    800027f8:	e84a                	sd	s2,16(sp)
    800027fa:	e44e                	sd	s3,8(sp)
    800027fc:	1800                	addi	s0,sp,48
    800027fe:	892a                	mv	s2,a0
    80002800:	84ae                	mv	s1,a1
    80002802:	89b2                	mv	s3,a2
  struct proc *p = myproc();
    80002804:	92cff0ef          	jal	80001930 <myproc>
  if(copyinstr(p->pagetable, buf, addr, max) < 0)
    80002808:	86ce                	mv	a3,s3
    8000280a:	864a                	mv	a2,s2
    8000280c:	85a6                	mv	a1,s1
    8000280e:	6928                	ld	a0,80(a0)
    80002810:	c79fe0ef          	jal	80001488 <copyinstr>
    80002814:	00054c63          	bltz	a0,8000282c <fetchstr+0x3c>
  return strlen(buf);
    80002818:	8526                	mv	a0,s1
    8000281a:	df8fe0ef          	jal	80000e12 <strlen>
}
    8000281e:	70a2                	ld	ra,40(sp)
    80002820:	7402                	ld	s0,32(sp)
    80002822:	64e2                	ld	s1,24(sp)
    80002824:	6942                	ld	s2,16(sp)
    80002826:	69a2                	ld	s3,8(sp)
    80002828:	6145                	addi	sp,sp,48
    8000282a:	8082                	ret
    return -1;
    8000282c:	557d                	li	a0,-1
    8000282e:	bfc5                	j	8000281e <fetchstr+0x2e>

0000000080002830 <argint>:

// Fetch the nth 32-bit system call argument.
void
argint(int n, int *ip)
{
    80002830:	1101                	addi	sp,sp,-32
    80002832:	ec06                	sd	ra,24(sp)
    80002834:	e822                	sd	s0,16(sp)
    80002836:	e426                	sd	s1,8(sp)
    80002838:	1000                	addi	s0,sp,32
    8000283a:	84ae                	mv	s1,a1
  *ip = argraw(n);
    8000283c:	f0bff0ef          	jal	80002746 <argraw>
    80002840:	c088                	sw	a0,0(s1)
}
    80002842:	60e2                	ld	ra,24(sp)
    80002844:	6442                	ld	s0,16(sp)
    80002846:	64a2                	ld	s1,8(sp)
    80002848:	6105                	addi	sp,sp,32
    8000284a:	8082                	ret

000000008000284c <argaddr>:
// Retrieve an argument as a pointer.
// Doesn't check for legality, since
// copyin/copyout will do that.
void
argaddr(int n, uint64 *ip)
{
    8000284c:	1101                	addi	sp,sp,-32
    8000284e:	ec06                	sd	ra,24(sp)
    80002850:	e822                	sd	s0,16(sp)
    80002852:	e426                	sd	s1,8(sp)
    80002854:	1000                	addi	s0,sp,32
    80002856:	84ae                	mv	s1,a1
  *ip = argraw(n);
    80002858:	eefff0ef          	jal	80002746 <argraw>
    8000285c:	e088                	sd	a0,0(s1)
}
    8000285e:	60e2                	ld	ra,24(sp)
    80002860:	6442                	ld	s0,16(sp)
    80002862:	64a2                	ld	s1,8(sp)
    80002864:	6105                	addi	sp,sp,32
    80002866:	8082                	ret

0000000080002868 <argstr>:
// Fetch the nth word-sized system call argument as a null-terminated string.
// Copies into buf, at most max.
// Returns string length if OK (including nul), -1 if error.
int
argstr(int n, char *buf, int max)
{
    80002868:	7179                	addi	sp,sp,-48
    8000286a:	f406                	sd	ra,40(sp)
    8000286c:	f022                	sd	s0,32(sp)
    8000286e:	ec26                	sd	s1,24(sp)
    80002870:	e84a                	sd	s2,16(sp)
    80002872:	1800                	addi	s0,sp,48
    80002874:	84ae                	mv	s1,a1
    80002876:	8932                	mv	s2,a2
  uint64 addr;
  argaddr(n, &addr);
    80002878:	fd840593          	addi	a1,s0,-40
    8000287c:	fd1ff0ef          	jal	8000284c <argaddr>
  return fetchstr(addr, buf, max);
    80002880:	864a                	mv	a2,s2
    80002882:	85a6                	mv	a1,s1
    80002884:	fd843503          	ld	a0,-40(s0)
    80002888:	f69ff0ef          	jal	800027f0 <fetchstr>
}
    8000288c:	70a2                	ld	ra,40(sp)
    8000288e:	7402                	ld	s0,32(sp)
    80002890:	64e2                	ld	s1,24(sp)
    80002892:	6942                	ld	s2,16(sp)
    80002894:	6145                	addi	sp,sp,48
    80002896:	8082                	ret

0000000080002898 <syscall>:
[SYS_memtrack] sys_memtrack,
};

void
syscall(void)
{
    80002898:	1101                	addi	sp,sp,-32
    8000289a:	ec06                	sd	ra,24(sp)
    8000289c:	e822                	sd	s0,16(sp)
    8000289e:	e426                	sd	s1,8(sp)
    800028a0:	e04a                	sd	s2,0(sp)
    800028a2:	1000                	addi	s0,sp,32
  int num;
  struct proc *p = myproc();
    800028a4:	88cff0ef          	jal	80001930 <myproc>
    800028a8:	84aa                	mv	s1,a0

  num = p->trapframe->a7;
    800028aa:	05853903          	ld	s2,88(a0)
    800028ae:	0a893783          	ld	a5,168(s2)
    800028b2:	0007869b          	sext.w	a3,a5
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    800028b6:	37fd                	addiw	a5,a5,-1
    800028b8:	4755                	li	a4,21
    800028ba:	00f76f63          	bltu	a4,a5,800028d8 <syscall+0x40>
    800028be:	00369713          	slli	a4,a3,0x3
    800028c2:	00005797          	auipc	a5,0x5
    800028c6:	eae78793          	addi	a5,a5,-338 # 80007770 <syscalls>
    800028ca:	97ba                	add	a5,a5,a4
    800028cc:	639c                	ld	a5,0(a5)
    800028ce:	c789                	beqz	a5,800028d8 <syscall+0x40>
    // Use num to lookup the system call function for num, call it,
    // and store its return value in p->trapframe->a0
    p->trapframe->a0 = syscalls[num]();
    800028d0:	9782                	jalr	a5
    800028d2:	06a93823          	sd	a0,112(s2)
    800028d6:	a829                	j	800028f0 <syscall+0x58>
  } else {
    printf("%d %s: unknown sys call %d\n",
    800028d8:	15848613          	addi	a2,s1,344
    800028dc:	588c                	lw	a1,48(s1)
    800028de:	00005517          	auipc	a0,0x5
    800028e2:	a9250513          	addi	a0,a0,-1390 # 80007370 <etext+0x370>
    800028e6:	c15fd0ef          	jal	800004fa <printf>
            p->pid, p->name, num);
    p->trapframe->a0 = -1;
    800028ea:	6cbc                	ld	a5,88(s1)
    800028ec:	577d                	li	a4,-1
    800028ee:	fbb8                	sd	a4,112(a5)
  }
}
    800028f0:	60e2                	ld	ra,24(sp)
    800028f2:	6442                	ld	s0,16(sp)
    800028f4:	64a2                	ld	s1,8(sp)
    800028f6:	6902                	ld	s2,0(sp)
    800028f8:	6105                	addi	sp,sp,32
    800028fa:	8082                	ret

00000000800028fc <sys_exit>:
#include "proc.h"
#include "vm.h"

uint64
sys_exit(void)
{
    800028fc:	1101                	addi	sp,sp,-32
    800028fe:	ec06                	sd	ra,24(sp)
    80002900:	e822                	sd	s0,16(sp)
    80002902:	1000                	addi	s0,sp,32
  int n;
  argint(0, &n);
    80002904:	fec40593          	addi	a1,s0,-20
    80002908:	4501                	li	a0,0
    8000290a:	f27ff0ef          	jal	80002830 <argint>
  kexit(n);
    8000290e:	fec42503          	lw	a0,-20(s0)
    80002912:	f26ff0ef          	jal	80002038 <kexit>
  return 0;  // not reached
}
    80002916:	4501                	li	a0,0
    80002918:	60e2                	ld	ra,24(sp)
    8000291a:	6442                	ld	s0,16(sp)
    8000291c:	6105                	addi	sp,sp,32
    8000291e:	8082                	ret

0000000080002920 <sys_getpid>:

uint64
sys_getpid(void)
{
    80002920:	1141                	addi	sp,sp,-16
    80002922:	e406                	sd	ra,8(sp)
    80002924:	e022                	sd	s0,0(sp)
    80002926:	0800                	addi	s0,sp,16
  return myproc()->pid;
    80002928:	808ff0ef          	jal	80001930 <myproc>
}
    8000292c:	5908                	lw	a0,48(a0)
    8000292e:	60a2                	ld	ra,8(sp)
    80002930:	6402                	ld	s0,0(sp)
    80002932:	0141                	addi	sp,sp,16
    80002934:	8082                	ret

0000000080002936 <sys_fork>:

uint64
sys_fork(void)
{
    80002936:	1141                	addi	sp,sp,-16
    80002938:	e406                	sd	ra,8(sp)
    8000293a:	e022                	sd	s0,0(sp)
    8000293c:	0800                	addi	s0,sp,16
  return kfork();
    8000293e:	b48ff0ef          	jal	80001c86 <kfork>
}
    80002942:	60a2                	ld	ra,8(sp)
    80002944:	6402                	ld	s0,0(sp)
    80002946:	0141                	addi	sp,sp,16
    80002948:	8082                	ret

000000008000294a <sys_wait>:

uint64
sys_wait(void)
{
    8000294a:	1101                	addi	sp,sp,-32
    8000294c:	ec06                	sd	ra,24(sp)
    8000294e:	e822                	sd	s0,16(sp)
    80002950:	1000                	addi	s0,sp,32
  uint64 p;
  argaddr(0, &p);
    80002952:	fe840593          	addi	a1,s0,-24
    80002956:	4501                	li	a0,0
    80002958:	ef5ff0ef          	jal	8000284c <argaddr>
  return kwait(p);
    8000295c:	fe843503          	ld	a0,-24(s0)
    80002960:	82fff0ef          	jal	8000218e <kwait>
}
    80002964:	60e2                	ld	ra,24(sp)
    80002966:	6442                	ld	s0,16(sp)
    80002968:	6105                	addi	sp,sp,32
    8000296a:	8082                	ret

000000008000296c <sys_sbrk>:

uint64
sys_sbrk(void)
{
    8000296c:	7179                	addi	sp,sp,-48
    8000296e:	f406                	sd	ra,40(sp)
    80002970:	f022                	sd	s0,32(sp)
    80002972:	ec26                	sd	s1,24(sp)
    80002974:	1800                	addi	s0,sp,48
  uint64 addr;
  int t;
  int n;

  argint(0, &n);
    80002976:	fd840593          	addi	a1,s0,-40
    8000297a:	4501                	li	a0,0
    8000297c:	eb5ff0ef          	jal	80002830 <argint>
  argint(1, &t);
    80002980:	fdc40593          	addi	a1,s0,-36
    80002984:	4505                	li	a0,1
    80002986:	eabff0ef          	jal	80002830 <argint>
  addr = myproc()->sz;
    8000298a:	fa7fe0ef          	jal	80001930 <myproc>
    8000298e:	6524                	ld	s1,72(a0)

  if(t == SBRK_EAGER || n < 0) {
    80002990:	fdc42703          	lw	a4,-36(s0)
    80002994:	4785                	li	a5,1
    80002996:	02f70163          	beq	a4,a5,800029b8 <sys_sbrk+0x4c>
    8000299a:	fd842783          	lw	a5,-40(s0)
    8000299e:	0007cd63          	bltz	a5,800029b8 <sys_sbrk+0x4c>
    }
  } else {
    // Lazily allocate memory for this process: increase its memory
    // size but don't allocate memory. If the processes uses the
    // memory, vmfault() will allocate it.
    if(addr + n < addr)
    800029a2:	97a6                	add	a5,a5,s1
    800029a4:	0497e463          	bltu	a5,s1,800029ec <sys_sbrk+0x80>
      return -1;
    myproc()->sz += n;
    800029a8:	f89fe0ef          	jal	80001930 <myproc>
    800029ac:	fd842703          	lw	a4,-40(s0)
    800029b0:	653c                	ld	a5,72(a0)
    800029b2:	97ba                	add	a5,a5,a4
    800029b4:	e53c                	sd	a5,72(a0)
    800029b6:	a039                	j	800029c4 <sys_sbrk+0x58>
    if(growproc(n) < 0) {
    800029b8:	fd842503          	lw	a0,-40(s0)
    800029bc:	a7aff0ef          	jal	80001c36 <growproc>
    800029c0:	02054463          	bltz	a0,800029e8 <sys_sbrk+0x7c>
  }
  if(n > 0) {
    800029c4:	fd842783          	lw	a5,-40(s0)
    800029c8:	00f04863          	bgtz	a5,800029d8 <sys_sbrk+0x6c>
    myproc()->mem_reqs++;
  }
  return addr;
}
    800029cc:	8526                	mv	a0,s1
    800029ce:	70a2                	ld	ra,40(sp)
    800029d0:	7402                	ld	s0,32(sp)
    800029d2:	64e2                	ld	s1,24(sp)
    800029d4:	6145                	addi	sp,sp,48
    800029d6:	8082                	ret
    myproc()->mem_reqs++;
    800029d8:	f59fe0ef          	jal	80001930 <myproc>
    800029dc:	16853783          	ld	a5,360(a0)
    800029e0:	0785                	addi	a5,a5,1
    800029e2:	16f53423          	sd	a5,360(a0)
    800029e6:	b7dd                	j	800029cc <sys_sbrk+0x60>
      return -1;
    800029e8:	54fd                	li	s1,-1
    800029ea:	b7cd                	j	800029cc <sys_sbrk+0x60>
      return -1;
    800029ec:	54fd                	li	s1,-1
    800029ee:	bff9                	j	800029cc <sys_sbrk+0x60>

00000000800029f0 <sys_pause>:

uint64
sys_pause(void)
{
    800029f0:	7139                	addi	sp,sp,-64
    800029f2:	fc06                	sd	ra,56(sp)
    800029f4:	f822                	sd	s0,48(sp)
    800029f6:	f04a                	sd	s2,32(sp)
    800029f8:	0080                	addi	s0,sp,64
  int n;
  uint ticks0;

  argint(0, &n);
    800029fa:	fcc40593          	addi	a1,s0,-52
    800029fe:	4501                	li	a0,0
    80002a00:	e31ff0ef          	jal	80002830 <argint>
  if(n < 0)
    80002a04:	fcc42783          	lw	a5,-52(s0)
    80002a08:	0607c763          	bltz	a5,80002a76 <sys_pause+0x86>
    n = 0;
  acquire(&tickslock);
    80002a0c:	00013517          	auipc	a0,0x13
    80002a10:	f8c50513          	addi	a0,a0,-116 # 80015998 <tickslock>
    80002a14:	9bafe0ef          	jal	80000bce <acquire>
  ticks0 = ticks;
    80002a18:	00005917          	auipc	s2,0x5
    80002a1c:	e5092903          	lw	s2,-432(s2) # 80007868 <ticks>
  while(ticks - ticks0 < n){
    80002a20:	fcc42783          	lw	a5,-52(s0)
    80002a24:	cf8d                	beqz	a5,80002a5e <sys_pause+0x6e>
    80002a26:	f426                	sd	s1,40(sp)
    80002a28:	ec4e                	sd	s3,24(sp)
    if(killed(myproc())){
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
    80002a2a:	00013997          	auipc	s3,0x13
    80002a2e:	f6e98993          	addi	s3,s3,-146 # 80015998 <tickslock>
    80002a32:	00005497          	auipc	s1,0x5
    80002a36:	e3648493          	addi	s1,s1,-458 # 80007868 <ticks>
    if(killed(myproc())){
    80002a3a:	ef7fe0ef          	jal	80001930 <myproc>
    80002a3e:	f26ff0ef          	jal	80002164 <killed>
    80002a42:	ed0d                	bnez	a0,80002a7c <sys_pause+0x8c>
    sleep(&ticks, &tickslock);
    80002a44:	85ce                	mv	a1,s3
    80002a46:	8526                	mv	a0,s1
    80002a48:	ce4ff0ef          	jal	80001f2c <sleep>
  while(ticks - ticks0 < n){
    80002a4c:	409c                	lw	a5,0(s1)
    80002a4e:	412787bb          	subw	a5,a5,s2
    80002a52:	fcc42703          	lw	a4,-52(s0)
    80002a56:	fee7e2e3          	bltu	a5,a4,80002a3a <sys_pause+0x4a>
    80002a5a:	74a2                	ld	s1,40(sp)
    80002a5c:	69e2                	ld	s3,24(sp)
  }
  release(&tickslock);
    80002a5e:	00013517          	auipc	a0,0x13
    80002a62:	f3a50513          	addi	a0,a0,-198 # 80015998 <tickslock>
    80002a66:	a00fe0ef          	jal	80000c66 <release>
  return 0;
    80002a6a:	4501                	li	a0,0
}
    80002a6c:	70e2                	ld	ra,56(sp)
    80002a6e:	7442                	ld	s0,48(sp)
    80002a70:	7902                	ld	s2,32(sp)
    80002a72:	6121                	addi	sp,sp,64
    80002a74:	8082                	ret
    n = 0;
    80002a76:	fc042623          	sw	zero,-52(s0)
    80002a7a:	bf49                	j	80002a0c <sys_pause+0x1c>
      release(&tickslock);
    80002a7c:	00013517          	auipc	a0,0x13
    80002a80:	f1c50513          	addi	a0,a0,-228 # 80015998 <tickslock>
    80002a84:	9e2fe0ef          	jal	80000c66 <release>
      return -1;
    80002a88:	557d                	li	a0,-1
    80002a8a:	74a2                	ld	s1,40(sp)
    80002a8c:	69e2                	ld	s3,24(sp)
    80002a8e:	bff9                	j	80002a6c <sys_pause+0x7c>

0000000080002a90 <sys_kill>:

uint64
sys_kill(void)
{
    80002a90:	1101                	addi	sp,sp,-32
    80002a92:	ec06                	sd	ra,24(sp)
    80002a94:	e822                	sd	s0,16(sp)
    80002a96:	1000                	addi	s0,sp,32
  int pid;

  argint(0, &pid);
    80002a98:	fec40593          	addi	a1,s0,-20
    80002a9c:	4501                	li	a0,0
    80002a9e:	d93ff0ef          	jal	80002830 <argint>
  return kkill(pid);
    80002aa2:	fec42503          	lw	a0,-20(s0)
    80002aa6:	e34ff0ef          	jal	800020da <kkill>
}
    80002aaa:	60e2                	ld	ra,24(sp)
    80002aac:	6442                	ld	s0,16(sp)
    80002aae:	6105                	addi	sp,sp,32
    80002ab0:	8082                	ret

0000000080002ab2 <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
uint64
sys_uptime(void)
{
    80002ab2:	1101                	addi	sp,sp,-32
    80002ab4:	ec06                	sd	ra,24(sp)
    80002ab6:	e822                	sd	s0,16(sp)
    80002ab8:	e426                	sd	s1,8(sp)
    80002aba:	1000                	addi	s0,sp,32
  uint xticks;

  acquire(&tickslock);
    80002abc:	00013517          	auipc	a0,0x13
    80002ac0:	edc50513          	addi	a0,a0,-292 # 80015998 <tickslock>
    80002ac4:	90afe0ef          	jal	80000bce <acquire>
  xticks = ticks;
    80002ac8:	00005497          	auipc	s1,0x5
    80002acc:	da04a483          	lw	s1,-608(s1) # 80007868 <ticks>
  release(&tickslock);
    80002ad0:	00013517          	auipc	a0,0x13
    80002ad4:	ec850513          	addi	a0,a0,-312 # 80015998 <tickslock>
    80002ad8:	98efe0ef          	jal	80000c66 <release>
  return xticks;
}
    80002adc:	02049513          	slli	a0,s1,0x20
    80002ae0:	9101                	srli	a0,a0,0x20
    80002ae2:	60e2                	ld	ra,24(sp)
    80002ae4:	6442                	ld	s0,16(sp)
    80002ae6:	64a2                	ld	s1,8(sp)
    80002ae8:	6105                	addi	sp,sp,32
    80002aea:	8082                	ret

0000000080002aec <sys_memtrack>:

uint64
sys_memtrack(void)
{
    80002aec:	7139                	addi	sp,sp,-64
    80002aee:	fc06                	sd	ra,56(sp)
    80002af0:	f822                	sd	s0,48(sp)
    80002af2:	0080                	addi	s0,sp,64
  uint64 addr;
  argaddr(0, &addr);
    80002af4:	fd840593          	addi	a1,s0,-40
    80002af8:	4501                	li	a0,0
    80002afa:	d53ff0ef          	jal	8000284c <argaddr>
  if(addr == 0)
    80002afe:	fd843783          	ld	a5,-40(s0)
    return -1;
    80002b02:	557d                	li	a0,-1
  if(addr == 0)
    80002b04:	c79d                	beqz	a5,80002b32 <sys_memtrack+0x46>
    80002b06:	f426                	sd	s1,40(sp)

  struct proc *p = myproc();
    80002b08:	e29fe0ef          	jal	80001930 <myproc>
    80002b0c:	84aa                	mv	s1,a0
  struct proc_mem pm;

  pm.phys_pages = count_phys_pages(p);
    80002b0e:	c47fe0ef          	jal	80001754 <count_phys_pages>
    80002b12:	fca43423          	sd	a0,-56(s0)
  pm.mem_reqs = p->mem_reqs;
    80002b16:	1684b783          	ld	a5,360(s1)
    80002b1a:	fcf43823          	sd	a5,-48(s0)

  if(copyout(p->pagetable, addr, (char *)&pm, sizeof(pm)) < 0)
    80002b1e:	46c1                	li	a3,16
    80002b20:	fc840613          	addi	a2,s0,-56
    80002b24:	fd843583          	ld	a1,-40(s0)
    80002b28:	68a8                	ld	a0,80(s1)
    80002b2a:	ab9fe0ef          	jal	800015e2 <copyout>
    80002b2e:	957d                	srai	a0,a0,0x3f
    80002b30:	74a2                	ld	s1,40(sp)
    return -1;

  return 0;
}
    80002b32:	70e2                	ld	ra,56(sp)
    80002b34:	7442                	ld	s0,48(sp)
    80002b36:	6121                	addi	sp,sp,64
    80002b38:	8082                	ret

0000000080002b3a <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
    80002b3a:	7179                	addi	sp,sp,-48
    80002b3c:	f406                	sd	ra,40(sp)
    80002b3e:	f022                	sd	s0,32(sp)
    80002b40:	ec26                	sd	s1,24(sp)
    80002b42:	e84a                	sd	s2,16(sp)
    80002b44:	e44e                	sd	s3,8(sp)
    80002b46:	e052                	sd	s4,0(sp)
    80002b48:	1800                	addi	s0,sp,48
  struct buf *b;

  initlock(&bcache.lock, "bcache");
    80002b4a:	00005597          	auipc	a1,0x5
    80002b4e:	84658593          	addi	a1,a1,-1978 # 80007390 <etext+0x390>
    80002b52:	00013517          	auipc	a0,0x13
    80002b56:	e5e50513          	addi	a0,a0,-418 # 800159b0 <bcache>
    80002b5a:	ff5fd0ef          	jal	80000b4e <initlock>

  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
    80002b5e:	0001b797          	auipc	a5,0x1b
    80002b62:	e5278793          	addi	a5,a5,-430 # 8001d9b0 <bcache+0x8000>
    80002b66:	0001b717          	auipc	a4,0x1b
    80002b6a:	0b270713          	addi	a4,a4,178 # 8001dc18 <bcache+0x8268>
    80002b6e:	2ae7b823          	sd	a4,688(a5)
  bcache.head.next = &bcache.head;
    80002b72:	2ae7bc23          	sd	a4,696(a5)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    80002b76:	00013497          	auipc	s1,0x13
    80002b7a:	e5248493          	addi	s1,s1,-430 # 800159c8 <bcache+0x18>
    b->next = bcache.head.next;
    80002b7e:	893e                	mv	s2,a5
    b->prev = &bcache.head;
    80002b80:	89ba                	mv	s3,a4
    initsleeplock(&b->lock, "buffer");
    80002b82:	00005a17          	auipc	s4,0x5
    80002b86:	816a0a13          	addi	s4,s4,-2026 # 80007398 <etext+0x398>
    b->next = bcache.head.next;
    80002b8a:	2b893783          	ld	a5,696(s2)
    80002b8e:	e8bc                	sd	a5,80(s1)
    b->prev = &bcache.head;
    80002b90:	0534b423          	sd	s3,72(s1)
    initsleeplock(&b->lock, "buffer");
    80002b94:	85d2                	mv	a1,s4
    80002b96:	01048513          	addi	a0,s1,16
    80002b9a:	322010ef          	jal	80003ebc <initsleeplock>
    bcache.head.next->prev = b;
    80002b9e:	2b893783          	ld	a5,696(s2)
    80002ba2:	e7a4                	sd	s1,72(a5)
    bcache.head.next = b;
    80002ba4:	2a993c23          	sd	s1,696(s2)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    80002ba8:	45848493          	addi	s1,s1,1112
    80002bac:	fd349fe3          	bne	s1,s3,80002b8a <binit+0x50>
  }
}
    80002bb0:	70a2                	ld	ra,40(sp)
    80002bb2:	7402                	ld	s0,32(sp)
    80002bb4:	64e2                	ld	s1,24(sp)
    80002bb6:	6942                	ld	s2,16(sp)
    80002bb8:	69a2                	ld	s3,8(sp)
    80002bba:	6a02                	ld	s4,0(sp)
    80002bbc:	6145                	addi	sp,sp,48
    80002bbe:	8082                	ret

0000000080002bc0 <bread>:
}

// Return a locked buf with the contents of the indicated block.
struct buf*
bread(uint dev, uint blockno)
{
    80002bc0:	7179                	addi	sp,sp,-48
    80002bc2:	f406                	sd	ra,40(sp)
    80002bc4:	f022                	sd	s0,32(sp)
    80002bc6:	ec26                	sd	s1,24(sp)
    80002bc8:	e84a                	sd	s2,16(sp)
    80002bca:	e44e                	sd	s3,8(sp)
    80002bcc:	1800                	addi	s0,sp,48
    80002bce:	892a                	mv	s2,a0
    80002bd0:	89ae                	mv	s3,a1
  acquire(&bcache.lock);
    80002bd2:	00013517          	auipc	a0,0x13
    80002bd6:	dde50513          	addi	a0,a0,-546 # 800159b0 <bcache>
    80002bda:	ff5fd0ef          	jal	80000bce <acquire>
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
    80002bde:	0001b497          	auipc	s1,0x1b
    80002be2:	08a4b483          	ld	s1,138(s1) # 8001dc68 <bcache+0x82b8>
    80002be6:	0001b797          	auipc	a5,0x1b
    80002bea:	03278793          	addi	a5,a5,50 # 8001dc18 <bcache+0x8268>
    80002bee:	02f48b63          	beq	s1,a5,80002c24 <bread+0x64>
    80002bf2:	873e                	mv	a4,a5
    80002bf4:	a021                	j	80002bfc <bread+0x3c>
    80002bf6:	68a4                	ld	s1,80(s1)
    80002bf8:	02e48663          	beq	s1,a4,80002c24 <bread+0x64>
    if(b->dev == dev && b->blockno == blockno){
    80002bfc:	449c                	lw	a5,8(s1)
    80002bfe:	ff279ce3          	bne	a5,s2,80002bf6 <bread+0x36>
    80002c02:	44dc                	lw	a5,12(s1)
    80002c04:	ff3799e3          	bne	a5,s3,80002bf6 <bread+0x36>
      b->refcnt++;
    80002c08:	40bc                	lw	a5,64(s1)
    80002c0a:	2785                	addiw	a5,a5,1
    80002c0c:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    80002c0e:	00013517          	auipc	a0,0x13
    80002c12:	da250513          	addi	a0,a0,-606 # 800159b0 <bcache>
    80002c16:	850fe0ef          	jal	80000c66 <release>
      acquiresleep(&b->lock);
    80002c1a:	01048513          	addi	a0,s1,16
    80002c1e:	2d4010ef          	jal	80003ef2 <acquiresleep>
      return b;
    80002c22:	a889                	j	80002c74 <bread+0xb4>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    80002c24:	0001b497          	auipc	s1,0x1b
    80002c28:	03c4b483          	ld	s1,60(s1) # 8001dc60 <bcache+0x82b0>
    80002c2c:	0001b797          	auipc	a5,0x1b
    80002c30:	fec78793          	addi	a5,a5,-20 # 8001dc18 <bcache+0x8268>
    80002c34:	00f48863          	beq	s1,a5,80002c44 <bread+0x84>
    80002c38:	873e                	mv	a4,a5
    if(b->refcnt == 0) {
    80002c3a:	40bc                	lw	a5,64(s1)
    80002c3c:	cb91                	beqz	a5,80002c50 <bread+0x90>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    80002c3e:	64a4                	ld	s1,72(s1)
    80002c40:	fee49de3          	bne	s1,a4,80002c3a <bread+0x7a>
  panic("bget: no buffers");
    80002c44:	00004517          	auipc	a0,0x4
    80002c48:	75c50513          	addi	a0,a0,1884 # 800073a0 <etext+0x3a0>
    80002c4c:	b95fd0ef          	jal	800007e0 <panic>
      b->dev = dev;
    80002c50:	0124a423          	sw	s2,8(s1)
      b->blockno = blockno;
    80002c54:	0134a623          	sw	s3,12(s1)
      b->valid = 0;
    80002c58:	0004a023          	sw	zero,0(s1)
      b->refcnt = 1;
    80002c5c:	4785                	li	a5,1
    80002c5e:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    80002c60:	00013517          	auipc	a0,0x13
    80002c64:	d5050513          	addi	a0,a0,-688 # 800159b0 <bcache>
    80002c68:	ffffd0ef          	jal	80000c66 <release>
      acquiresleep(&b->lock);
    80002c6c:	01048513          	addi	a0,s1,16
    80002c70:	282010ef          	jal	80003ef2 <acquiresleep>
  struct buf *b;

  b = bget(dev, blockno);
  if(!b->valid) {
    80002c74:	409c                	lw	a5,0(s1)
    80002c76:	cb89                	beqz	a5,80002c88 <bread+0xc8>
    virtio_disk_rw(b, 0);
    b->valid = 1;
  }
  return b;
}
    80002c78:	8526                	mv	a0,s1
    80002c7a:	70a2                	ld	ra,40(sp)
    80002c7c:	7402                	ld	s0,32(sp)
    80002c7e:	64e2                	ld	s1,24(sp)
    80002c80:	6942                	ld	s2,16(sp)
    80002c82:	69a2                	ld	s3,8(sp)
    80002c84:	6145                	addi	sp,sp,48
    80002c86:	8082                	ret
    virtio_disk_rw(b, 0);
    80002c88:	4581                	li	a1,0
    80002c8a:	8526                	mv	a0,s1
    80002c8c:	2c5020ef          	jal	80005750 <virtio_disk_rw>
    b->valid = 1;
    80002c90:	4785                	li	a5,1
    80002c92:	c09c                	sw	a5,0(s1)
  return b;
    80002c94:	b7d5                	j	80002c78 <bread+0xb8>

0000000080002c96 <bwrite>:

// Write b's contents to disk.  Must be locked.
void
bwrite(struct buf *b)
{
    80002c96:	1101                	addi	sp,sp,-32
    80002c98:	ec06                	sd	ra,24(sp)
    80002c9a:	e822                	sd	s0,16(sp)
    80002c9c:	e426                	sd	s1,8(sp)
    80002c9e:	1000                	addi	s0,sp,32
    80002ca0:	84aa                	mv	s1,a0
  if(!holdingsleep(&b->lock))
    80002ca2:	0541                	addi	a0,a0,16
    80002ca4:	2cc010ef          	jal	80003f70 <holdingsleep>
    80002ca8:	c911                	beqz	a0,80002cbc <bwrite+0x26>
    panic("bwrite");
  virtio_disk_rw(b, 1);
    80002caa:	4585                	li	a1,1
    80002cac:	8526                	mv	a0,s1
    80002cae:	2a3020ef          	jal	80005750 <virtio_disk_rw>
}
    80002cb2:	60e2                	ld	ra,24(sp)
    80002cb4:	6442                	ld	s0,16(sp)
    80002cb6:	64a2                	ld	s1,8(sp)
    80002cb8:	6105                	addi	sp,sp,32
    80002cba:	8082                	ret
    panic("bwrite");
    80002cbc:	00004517          	auipc	a0,0x4
    80002cc0:	6fc50513          	addi	a0,a0,1788 # 800073b8 <etext+0x3b8>
    80002cc4:	b1dfd0ef          	jal	800007e0 <panic>

0000000080002cc8 <brelse>:

// Release a locked buffer.
// Move to the head of the most-recently-used list.
void
brelse(struct buf *b)
{
    80002cc8:	1101                	addi	sp,sp,-32
    80002cca:	ec06                	sd	ra,24(sp)
    80002ccc:	e822                	sd	s0,16(sp)
    80002cce:	e426                	sd	s1,8(sp)
    80002cd0:	e04a                	sd	s2,0(sp)
    80002cd2:	1000                	addi	s0,sp,32
    80002cd4:	84aa                	mv	s1,a0
  if(!holdingsleep(&b->lock))
    80002cd6:	01050913          	addi	s2,a0,16
    80002cda:	854a                	mv	a0,s2
    80002cdc:	294010ef          	jal	80003f70 <holdingsleep>
    80002ce0:	c135                	beqz	a0,80002d44 <brelse+0x7c>
    panic("brelse");

  releasesleep(&b->lock);
    80002ce2:	854a                	mv	a0,s2
    80002ce4:	254010ef          	jal	80003f38 <releasesleep>

  acquire(&bcache.lock);
    80002ce8:	00013517          	auipc	a0,0x13
    80002cec:	cc850513          	addi	a0,a0,-824 # 800159b0 <bcache>
    80002cf0:	edffd0ef          	jal	80000bce <acquire>
  b->refcnt--;
    80002cf4:	40bc                	lw	a5,64(s1)
    80002cf6:	37fd                	addiw	a5,a5,-1
    80002cf8:	0007871b          	sext.w	a4,a5
    80002cfc:	c0bc                	sw	a5,64(s1)
  if (b->refcnt == 0) {
    80002cfe:	e71d                	bnez	a4,80002d2c <brelse+0x64>
    // no one is waiting for it.
    b->next->prev = b->prev;
    80002d00:	68b8                	ld	a4,80(s1)
    80002d02:	64bc                	ld	a5,72(s1)
    80002d04:	e73c                	sd	a5,72(a4)
    b->prev->next = b->next;
    80002d06:	68b8                	ld	a4,80(s1)
    80002d08:	ebb8                	sd	a4,80(a5)
    b->next = bcache.head.next;
    80002d0a:	0001b797          	auipc	a5,0x1b
    80002d0e:	ca678793          	addi	a5,a5,-858 # 8001d9b0 <bcache+0x8000>
    80002d12:	2b87b703          	ld	a4,696(a5)
    80002d16:	e8b8                	sd	a4,80(s1)
    b->prev = &bcache.head;
    80002d18:	0001b717          	auipc	a4,0x1b
    80002d1c:	f0070713          	addi	a4,a4,-256 # 8001dc18 <bcache+0x8268>
    80002d20:	e4b8                	sd	a4,72(s1)
    bcache.head.next->prev = b;
    80002d22:	2b87b703          	ld	a4,696(a5)
    80002d26:	e724                	sd	s1,72(a4)
    bcache.head.next = b;
    80002d28:	2a97bc23          	sd	s1,696(a5)
  }
  
  release(&bcache.lock);
    80002d2c:	00013517          	auipc	a0,0x13
    80002d30:	c8450513          	addi	a0,a0,-892 # 800159b0 <bcache>
    80002d34:	f33fd0ef          	jal	80000c66 <release>
}
    80002d38:	60e2                	ld	ra,24(sp)
    80002d3a:	6442                	ld	s0,16(sp)
    80002d3c:	64a2                	ld	s1,8(sp)
    80002d3e:	6902                	ld	s2,0(sp)
    80002d40:	6105                	addi	sp,sp,32
    80002d42:	8082                	ret
    panic("brelse");
    80002d44:	00004517          	auipc	a0,0x4
    80002d48:	67c50513          	addi	a0,a0,1660 # 800073c0 <etext+0x3c0>
    80002d4c:	a95fd0ef          	jal	800007e0 <panic>

0000000080002d50 <bpin>:

void
bpin(struct buf *b) {
    80002d50:	1101                	addi	sp,sp,-32
    80002d52:	ec06                	sd	ra,24(sp)
    80002d54:	e822                	sd	s0,16(sp)
    80002d56:	e426                	sd	s1,8(sp)
    80002d58:	1000                	addi	s0,sp,32
    80002d5a:	84aa                	mv	s1,a0
  acquire(&bcache.lock);
    80002d5c:	00013517          	auipc	a0,0x13
    80002d60:	c5450513          	addi	a0,a0,-940 # 800159b0 <bcache>
    80002d64:	e6bfd0ef          	jal	80000bce <acquire>
  b->refcnt++;
    80002d68:	40bc                	lw	a5,64(s1)
    80002d6a:	2785                	addiw	a5,a5,1
    80002d6c:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    80002d6e:	00013517          	auipc	a0,0x13
    80002d72:	c4250513          	addi	a0,a0,-958 # 800159b0 <bcache>
    80002d76:	ef1fd0ef          	jal	80000c66 <release>
}
    80002d7a:	60e2                	ld	ra,24(sp)
    80002d7c:	6442                	ld	s0,16(sp)
    80002d7e:	64a2                	ld	s1,8(sp)
    80002d80:	6105                	addi	sp,sp,32
    80002d82:	8082                	ret

0000000080002d84 <bunpin>:

void
bunpin(struct buf *b) {
    80002d84:	1101                	addi	sp,sp,-32
    80002d86:	ec06                	sd	ra,24(sp)
    80002d88:	e822                	sd	s0,16(sp)
    80002d8a:	e426                	sd	s1,8(sp)
    80002d8c:	1000                	addi	s0,sp,32
    80002d8e:	84aa                	mv	s1,a0
  acquire(&bcache.lock);
    80002d90:	00013517          	auipc	a0,0x13
    80002d94:	c2050513          	addi	a0,a0,-992 # 800159b0 <bcache>
    80002d98:	e37fd0ef          	jal	80000bce <acquire>
  b->refcnt--;
    80002d9c:	40bc                	lw	a5,64(s1)
    80002d9e:	37fd                	addiw	a5,a5,-1
    80002da0:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    80002da2:	00013517          	auipc	a0,0x13
    80002da6:	c0e50513          	addi	a0,a0,-1010 # 800159b0 <bcache>
    80002daa:	ebdfd0ef          	jal	80000c66 <release>
}
    80002dae:	60e2                	ld	ra,24(sp)
    80002db0:	6442                	ld	s0,16(sp)
    80002db2:	64a2                	ld	s1,8(sp)
    80002db4:	6105                	addi	sp,sp,32
    80002db6:	8082                	ret

0000000080002db8 <bfree>:
}

// Free a disk block.
static void
bfree(int dev, uint b)
{
    80002db8:	1101                	addi	sp,sp,-32
    80002dba:	ec06                	sd	ra,24(sp)
    80002dbc:	e822                	sd	s0,16(sp)
    80002dbe:	e426                	sd	s1,8(sp)
    80002dc0:	e04a                	sd	s2,0(sp)
    80002dc2:	1000                	addi	s0,sp,32
    80002dc4:	84ae                	mv	s1,a1
  struct buf *bp;
  int bi, m;

  bp = bread(dev, BBLOCK(b, sb));
    80002dc6:	00d5d59b          	srliw	a1,a1,0xd
    80002dca:	0001b797          	auipc	a5,0x1b
    80002dce:	2c27a783          	lw	a5,706(a5) # 8001e08c <sb+0x1c>
    80002dd2:	9dbd                	addw	a1,a1,a5
    80002dd4:	dedff0ef          	jal	80002bc0 <bread>
  bi = b % BPB;
  m = 1 << (bi % 8);
    80002dd8:	0074f713          	andi	a4,s1,7
    80002ddc:	4785                	li	a5,1
    80002dde:	00e797bb          	sllw	a5,a5,a4
  if((bp->data[bi/8] & m) == 0)
    80002de2:	14ce                	slli	s1,s1,0x33
    80002de4:	90d9                	srli	s1,s1,0x36
    80002de6:	00950733          	add	a4,a0,s1
    80002dea:	05874703          	lbu	a4,88(a4)
    80002dee:	00e7f6b3          	and	a3,a5,a4
    80002df2:	c29d                	beqz	a3,80002e18 <bfree+0x60>
    80002df4:	892a                	mv	s2,a0
    panic("freeing free block");
  bp->data[bi/8] &= ~m;
    80002df6:	94aa                	add	s1,s1,a0
    80002df8:	fff7c793          	not	a5,a5
    80002dfc:	8f7d                	and	a4,a4,a5
    80002dfe:	04e48c23          	sb	a4,88(s1)
  log_write(bp);
    80002e02:	7f9000ef          	jal	80003dfa <log_write>
  brelse(bp);
    80002e06:	854a                	mv	a0,s2
    80002e08:	ec1ff0ef          	jal	80002cc8 <brelse>
}
    80002e0c:	60e2                	ld	ra,24(sp)
    80002e0e:	6442                	ld	s0,16(sp)
    80002e10:	64a2                	ld	s1,8(sp)
    80002e12:	6902                	ld	s2,0(sp)
    80002e14:	6105                	addi	sp,sp,32
    80002e16:	8082                	ret
    panic("freeing free block");
    80002e18:	00004517          	auipc	a0,0x4
    80002e1c:	5b050513          	addi	a0,a0,1456 # 800073c8 <etext+0x3c8>
    80002e20:	9c1fd0ef          	jal	800007e0 <panic>

0000000080002e24 <balloc>:
{
    80002e24:	711d                	addi	sp,sp,-96
    80002e26:	ec86                	sd	ra,88(sp)
    80002e28:	e8a2                	sd	s0,80(sp)
    80002e2a:	e4a6                	sd	s1,72(sp)
    80002e2c:	1080                	addi	s0,sp,96
  for(b = 0; b < sb.size; b += BPB){
    80002e2e:	0001b797          	auipc	a5,0x1b
    80002e32:	2467a783          	lw	a5,582(a5) # 8001e074 <sb+0x4>
    80002e36:	0e078f63          	beqz	a5,80002f34 <balloc+0x110>
    80002e3a:	e0ca                	sd	s2,64(sp)
    80002e3c:	fc4e                	sd	s3,56(sp)
    80002e3e:	f852                	sd	s4,48(sp)
    80002e40:	f456                	sd	s5,40(sp)
    80002e42:	f05a                	sd	s6,32(sp)
    80002e44:	ec5e                	sd	s7,24(sp)
    80002e46:	e862                	sd	s8,16(sp)
    80002e48:	e466                	sd	s9,8(sp)
    80002e4a:	8baa                	mv	s7,a0
    80002e4c:	4a81                	li	s5,0
    bp = bread(dev, BBLOCK(b, sb));
    80002e4e:	0001bb17          	auipc	s6,0x1b
    80002e52:	222b0b13          	addi	s6,s6,546 # 8001e070 <sb>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002e56:	4c01                	li	s8,0
      m = 1 << (bi % 8);
    80002e58:	4985                	li	s3,1
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002e5a:	6a09                	lui	s4,0x2
  for(b = 0; b < sb.size; b += BPB){
    80002e5c:	6c89                	lui	s9,0x2
    80002e5e:	a0b5                	j	80002eca <balloc+0xa6>
        bp->data[bi/8] |= m;  // Mark block in use.
    80002e60:	97ca                	add	a5,a5,s2
    80002e62:	8e55                	or	a2,a2,a3
    80002e64:	04c78c23          	sb	a2,88(a5)
        log_write(bp);
    80002e68:	854a                	mv	a0,s2
    80002e6a:	791000ef          	jal	80003dfa <log_write>
        brelse(bp);
    80002e6e:	854a                	mv	a0,s2
    80002e70:	e59ff0ef          	jal	80002cc8 <brelse>
  bp = bread(dev, bno);
    80002e74:	85a6                	mv	a1,s1
    80002e76:	855e                	mv	a0,s7
    80002e78:	d49ff0ef          	jal	80002bc0 <bread>
    80002e7c:	892a                	mv	s2,a0
  memset(bp->data, 0, BSIZE);
    80002e7e:	40000613          	li	a2,1024
    80002e82:	4581                	li	a1,0
    80002e84:	05850513          	addi	a0,a0,88
    80002e88:	e1bfd0ef          	jal	80000ca2 <memset>
  log_write(bp);
    80002e8c:	854a                	mv	a0,s2
    80002e8e:	76d000ef          	jal	80003dfa <log_write>
  brelse(bp);
    80002e92:	854a                	mv	a0,s2
    80002e94:	e35ff0ef          	jal	80002cc8 <brelse>
}
    80002e98:	6906                	ld	s2,64(sp)
    80002e9a:	79e2                	ld	s3,56(sp)
    80002e9c:	7a42                	ld	s4,48(sp)
    80002e9e:	7aa2                	ld	s5,40(sp)
    80002ea0:	7b02                	ld	s6,32(sp)
    80002ea2:	6be2                	ld	s7,24(sp)
    80002ea4:	6c42                	ld	s8,16(sp)
    80002ea6:	6ca2                	ld	s9,8(sp)
}
    80002ea8:	8526                	mv	a0,s1
    80002eaa:	60e6                	ld	ra,88(sp)
    80002eac:	6446                	ld	s0,80(sp)
    80002eae:	64a6                	ld	s1,72(sp)
    80002eb0:	6125                	addi	sp,sp,96
    80002eb2:	8082                	ret
    brelse(bp);
    80002eb4:	854a                	mv	a0,s2
    80002eb6:	e13ff0ef          	jal	80002cc8 <brelse>
  for(b = 0; b < sb.size; b += BPB){
    80002eba:	015c87bb          	addw	a5,s9,s5
    80002ebe:	00078a9b          	sext.w	s5,a5
    80002ec2:	004b2703          	lw	a4,4(s6)
    80002ec6:	04eaff63          	bgeu	s5,a4,80002f24 <balloc+0x100>
    bp = bread(dev, BBLOCK(b, sb));
    80002eca:	41fad79b          	sraiw	a5,s5,0x1f
    80002ece:	0137d79b          	srliw	a5,a5,0x13
    80002ed2:	015787bb          	addw	a5,a5,s5
    80002ed6:	40d7d79b          	sraiw	a5,a5,0xd
    80002eda:	01cb2583          	lw	a1,28(s6)
    80002ede:	9dbd                	addw	a1,a1,a5
    80002ee0:	855e                	mv	a0,s7
    80002ee2:	cdfff0ef          	jal	80002bc0 <bread>
    80002ee6:	892a                	mv	s2,a0
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002ee8:	004b2503          	lw	a0,4(s6)
    80002eec:	000a849b          	sext.w	s1,s5
    80002ef0:	8762                	mv	a4,s8
    80002ef2:	fca4f1e3          	bgeu	s1,a0,80002eb4 <balloc+0x90>
      m = 1 << (bi % 8);
    80002ef6:	00777693          	andi	a3,a4,7
    80002efa:	00d996bb          	sllw	a3,s3,a3
      if((bp->data[bi/8] & m) == 0){  // Is block free?
    80002efe:	41f7579b          	sraiw	a5,a4,0x1f
    80002f02:	01d7d79b          	srliw	a5,a5,0x1d
    80002f06:	9fb9                	addw	a5,a5,a4
    80002f08:	4037d79b          	sraiw	a5,a5,0x3
    80002f0c:	00f90633          	add	a2,s2,a5
    80002f10:	05864603          	lbu	a2,88(a2)
    80002f14:	00c6f5b3          	and	a1,a3,a2
    80002f18:	d5a1                	beqz	a1,80002e60 <balloc+0x3c>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002f1a:	2705                	addiw	a4,a4,1
    80002f1c:	2485                	addiw	s1,s1,1
    80002f1e:	fd471ae3          	bne	a4,s4,80002ef2 <balloc+0xce>
    80002f22:	bf49                	j	80002eb4 <balloc+0x90>
    80002f24:	6906                	ld	s2,64(sp)
    80002f26:	79e2                	ld	s3,56(sp)
    80002f28:	7a42                	ld	s4,48(sp)
    80002f2a:	7aa2                	ld	s5,40(sp)
    80002f2c:	7b02                	ld	s6,32(sp)
    80002f2e:	6be2                	ld	s7,24(sp)
    80002f30:	6c42                	ld	s8,16(sp)
    80002f32:	6ca2                	ld	s9,8(sp)
  printf("balloc: out of blocks\n");
    80002f34:	00004517          	auipc	a0,0x4
    80002f38:	4ac50513          	addi	a0,a0,1196 # 800073e0 <etext+0x3e0>
    80002f3c:	dbefd0ef          	jal	800004fa <printf>
  return 0;
    80002f40:	4481                	li	s1,0
    80002f42:	b79d                	j	80002ea8 <balloc+0x84>

0000000080002f44 <bmap>:
// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
// returns 0 if out of disk space.
static uint
bmap(struct inode *ip, uint bn)
{
    80002f44:	7179                	addi	sp,sp,-48
    80002f46:	f406                	sd	ra,40(sp)
    80002f48:	f022                	sd	s0,32(sp)
    80002f4a:	ec26                	sd	s1,24(sp)
    80002f4c:	e84a                	sd	s2,16(sp)
    80002f4e:	e44e                	sd	s3,8(sp)
    80002f50:	1800                	addi	s0,sp,48
    80002f52:	89aa                	mv	s3,a0
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
    80002f54:	47ad                	li	a5,11
    80002f56:	02b7e663          	bltu	a5,a1,80002f82 <bmap+0x3e>
    if((addr = ip->addrs[bn]) == 0){
    80002f5a:	02059793          	slli	a5,a1,0x20
    80002f5e:	01e7d593          	srli	a1,a5,0x1e
    80002f62:	00b504b3          	add	s1,a0,a1
    80002f66:	0504a903          	lw	s2,80(s1)
    80002f6a:	06091a63          	bnez	s2,80002fde <bmap+0x9a>
      addr = balloc(ip->dev);
    80002f6e:	4108                	lw	a0,0(a0)
    80002f70:	eb5ff0ef          	jal	80002e24 <balloc>
    80002f74:	0005091b          	sext.w	s2,a0
      if(addr == 0)
    80002f78:	06090363          	beqz	s2,80002fde <bmap+0x9a>
        return 0;
      ip->addrs[bn] = addr;
    80002f7c:	0524a823          	sw	s2,80(s1)
    80002f80:	a8b9                	j	80002fde <bmap+0x9a>
    }
    return addr;
  }
  bn -= NDIRECT;
    80002f82:	ff45849b          	addiw	s1,a1,-12
    80002f86:	0004871b          	sext.w	a4,s1

  if(bn < NINDIRECT){
    80002f8a:	0ff00793          	li	a5,255
    80002f8e:	06e7ee63          	bltu	a5,a4,8000300a <bmap+0xc6>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0){
    80002f92:	08052903          	lw	s2,128(a0)
    80002f96:	00091d63          	bnez	s2,80002fb0 <bmap+0x6c>
      addr = balloc(ip->dev);
    80002f9a:	4108                	lw	a0,0(a0)
    80002f9c:	e89ff0ef          	jal	80002e24 <balloc>
    80002fa0:	0005091b          	sext.w	s2,a0
      if(addr == 0)
    80002fa4:	02090d63          	beqz	s2,80002fde <bmap+0x9a>
    80002fa8:	e052                	sd	s4,0(sp)
        return 0;
      ip->addrs[NDIRECT] = addr;
    80002faa:	0929a023          	sw	s2,128(s3)
    80002fae:	a011                	j	80002fb2 <bmap+0x6e>
    80002fb0:	e052                	sd	s4,0(sp)
    }
    bp = bread(ip->dev, addr);
    80002fb2:	85ca                	mv	a1,s2
    80002fb4:	0009a503          	lw	a0,0(s3)
    80002fb8:	c09ff0ef          	jal	80002bc0 <bread>
    80002fbc:	8a2a                	mv	s4,a0
    a = (uint*)bp->data;
    80002fbe:	05850793          	addi	a5,a0,88
    if((addr = a[bn]) == 0){
    80002fc2:	02049713          	slli	a4,s1,0x20
    80002fc6:	01e75593          	srli	a1,a4,0x1e
    80002fca:	00b784b3          	add	s1,a5,a1
    80002fce:	0004a903          	lw	s2,0(s1)
    80002fd2:	00090e63          	beqz	s2,80002fee <bmap+0xaa>
      if(addr){
        a[bn] = addr;
        log_write(bp);
      }
    }
    brelse(bp);
    80002fd6:	8552                	mv	a0,s4
    80002fd8:	cf1ff0ef          	jal	80002cc8 <brelse>
    return addr;
    80002fdc:	6a02                	ld	s4,0(sp)
  }

  panic("bmap: out of range");
}
    80002fde:	854a                	mv	a0,s2
    80002fe0:	70a2                	ld	ra,40(sp)
    80002fe2:	7402                	ld	s0,32(sp)
    80002fe4:	64e2                	ld	s1,24(sp)
    80002fe6:	6942                	ld	s2,16(sp)
    80002fe8:	69a2                	ld	s3,8(sp)
    80002fea:	6145                	addi	sp,sp,48
    80002fec:	8082                	ret
      addr = balloc(ip->dev);
    80002fee:	0009a503          	lw	a0,0(s3)
    80002ff2:	e33ff0ef          	jal	80002e24 <balloc>
    80002ff6:	0005091b          	sext.w	s2,a0
      if(addr){
    80002ffa:	fc090ee3          	beqz	s2,80002fd6 <bmap+0x92>
        a[bn] = addr;
    80002ffe:	0124a023          	sw	s2,0(s1)
        log_write(bp);
    80003002:	8552                	mv	a0,s4
    80003004:	5f7000ef          	jal	80003dfa <log_write>
    80003008:	b7f9                	j	80002fd6 <bmap+0x92>
    8000300a:	e052                	sd	s4,0(sp)
  panic("bmap: out of range");
    8000300c:	00004517          	auipc	a0,0x4
    80003010:	3ec50513          	addi	a0,a0,1004 # 800073f8 <etext+0x3f8>
    80003014:	fccfd0ef          	jal	800007e0 <panic>

0000000080003018 <iget>:
{
    80003018:	7179                	addi	sp,sp,-48
    8000301a:	f406                	sd	ra,40(sp)
    8000301c:	f022                	sd	s0,32(sp)
    8000301e:	ec26                	sd	s1,24(sp)
    80003020:	e84a                	sd	s2,16(sp)
    80003022:	e44e                	sd	s3,8(sp)
    80003024:	e052                	sd	s4,0(sp)
    80003026:	1800                	addi	s0,sp,48
    80003028:	89aa                	mv	s3,a0
    8000302a:	8a2e                	mv	s4,a1
  acquire(&itable.lock);
    8000302c:	0001b517          	auipc	a0,0x1b
    80003030:	06450513          	addi	a0,a0,100 # 8001e090 <itable>
    80003034:	b9bfd0ef          	jal	80000bce <acquire>
  empty = 0;
    80003038:	4901                	li	s2,0
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    8000303a:	0001b497          	auipc	s1,0x1b
    8000303e:	06e48493          	addi	s1,s1,110 # 8001e0a8 <itable+0x18>
    80003042:	0001d697          	auipc	a3,0x1d
    80003046:	af668693          	addi	a3,a3,-1290 # 8001fb38 <log>
    8000304a:	a039                	j	80003058 <iget+0x40>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    8000304c:	02090963          	beqz	s2,8000307e <iget+0x66>
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    80003050:	08848493          	addi	s1,s1,136
    80003054:	02d48863          	beq	s1,a3,80003084 <iget+0x6c>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
    80003058:	449c                	lw	a5,8(s1)
    8000305a:	fef059e3          	blez	a5,8000304c <iget+0x34>
    8000305e:	4098                	lw	a4,0(s1)
    80003060:	ff3716e3          	bne	a4,s3,8000304c <iget+0x34>
    80003064:	40d8                	lw	a4,4(s1)
    80003066:	ff4713e3          	bne	a4,s4,8000304c <iget+0x34>
      ip->ref++;
    8000306a:	2785                	addiw	a5,a5,1
    8000306c:	c49c                	sw	a5,8(s1)
      release(&itable.lock);
    8000306e:	0001b517          	auipc	a0,0x1b
    80003072:	02250513          	addi	a0,a0,34 # 8001e090 <itable>
    80003076:	bf1fd0ef          	jal	80000c66 <release>
      return ip;
    8000307a:	8926                	mv	s2,s1
    8000307c:	a02d                	j	800030a6 <iget+0x8e>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    8000307e:	fbe9                	bnez	a5,80003050 <iget+0x38>
      empty = ip;
    80003080:	8926                	mv	s2,s1
    80003082:	b7f9                	j	80003050 <iget+0x38>
  if(empty == 0)
    80003084:	02090a63          	beqz	s2,800030b8 <iget+0xa0>
  ip->dev = dev;
    80003088:	01392023          	sw	s3,0(s2)
  ip->inum = inum;
    8000308c:	01492223          	sw	s4,4(s2)
  ip->ref = 1;
    80003090:	4785                	li	a5,1
    80003092:	00f92423          	sw	a5,8(s2)
  ip->valid = 0;
    80003096:	04092023          	sw	zero,64(s2)
  release(&itable.lock);
    8000309a:	0001b517          	auipc	a0,0x1b
    8000309e:	ff650513          	addi	a0,a0,-10 # 8001e090 <itable>
    800030a2:	bc5fd0ef          	jal	80000c66 <release>
}
    800030a6:	854a                	mv	a0,s2
    800030a8:	70a2                	ld	ra,40(sp)
    800030aa:	7402                	ld	s0,32(sp)
    800030ac:	64e2                	ld	s1,24(sp)
    800030ae:	6942                	ld	s2,16(sp)
    800030b0:	69a2                	ld	s3,8(sp)
    800030b2:	6a02                	ld	s4,0(sp)
    800030b4:	6145                	addi	sp,sp,48
    800030b6:	8082                	ret
    panic("iget: no inodes");
    800030b8:	00004517          	auipc	a0,0x4
    800030bc:	35850513          	addi	a0,a0,856 # 80007410 <etext+0x410>
    800030c0:	f20fd0ef          	jal	800007e0 <panic>

00000000800030c4 <iinit>:
{
    800030c4:	7179                	addi	sp,sp,-48
    800030c6:	f406                	sd	ra,40(sp)
    800030c8:	f022                	sd	s0,32(sp)
    800030ca:	ec26                	sd	s1,24(sp)
    800030cc:	e84a                	sd	s2,16(sp)
    800030ce:	e44e                	sd	s3,8(sp)
    800030d0:	1800                	addi	s0,sp,48
  initlock(&itable.lock, "itable");
    800030d2:	00004597          	auipc	a1,0x4
    800030d6:	34e58593          	addi	a1,a1,846 # 80007420 <etext+0x420>
    800030da:	0001b517          	auipc	a0,0x1b
    800030de:	fb650513          	addi	a0,a0,-74 # 8001e090 <itable>
    800030e2:	a6dfd0ef          	jal	80000b4e <initlock>
  for(i = 0; i < NINODE; i++) {
    800030e6:	0001b497          	auipc	s1,0x1b
    800030ea:	fd248493          	addi	s1,s1,-46 # 8001e0b8 <itable+0x28>
    800030ee:	0001d997          	auipc	s3,0x1d
    800030f2:	a5a98993          	addi	s3,s3,-1446 # 8001fb48 <log+0x10>
    initsleeplock(&itable.inode[i].lock, "inode");
    800030f6:	00004917          	auipc	s2,0x4
    800030fa:	33290913          	addi	s2,s2,818 # 80007428 <etext+0x428>
    800030fe:	85ca                	mv	a1,s2
    80003100:	8526                	mv	a0,s1
    80003102:	5bb000ef          	jal	80003ebc <initsleeplock>
  for(i = 0; i < NINODE; i++) {
    80003106:	08848493          	addi	s1,s1,136
    8000310a:	ff349ae3          	bne	s1,s3,800030fe <iinit+0x3a>
}
    8000310e:	70a2                	ld	ra,40(sp)
    80003110:	7402                	ld	s0,32(sp)
    80003112:	64e2                	ld	s1,24(sp)
    80003114:	6942                	ld	s2,16(sp)
    80003116:	69a2                	ld	s3,8(sp)
    80003118:	6145                	addi	sp,sp,48
    8000311a:	8082                	ret

000000008000311c <ialloc>:
{
    8000311c:	7139                	addi	sp,sp,-64
    8000311e:	fc06                	sd	ra,56(sp)
    80003120:	f822                	sd	s0,48(sp)
    80003122:	0080                	addi	s0,sp,64
  for(inum = 1; inum < sb.ninodes; inum++){
    80003124:	0001b717          	auipc	a4,0x1b
    80003128:	f5872703          	lw	a4,-168(a4) # 8001e07c <sb+0xc>
    8000312c:	4785                	li	a5,1
    8000312e:	06e7f063          	bgeu	a5,a4,8000318e <ialloc+0x72>
    80003132:	f426                	sd	s1,40(sp)
    80003134:	f04a                	sd	s2,32(sp)
    80003136:	ec4e                	sd	s3,24(sp)
    80003138:	e852                	sd	s4,16(sp)
    8000313a:	e456                	sd	s5,8(sp)
    8000313c:	e05a                	sd	s6,0(sp)
    8000313e:	8aaa                	mv	s5,a0
    80003140:	8b2e                	mv	s6,a1
    80003142:	4905                	li	s2,1
    bp = bread(dev, IBLOCK(inum, sb));
    80003144:	0001ba17          	auipc	s4,0x1b
    80003148:	f2ca0a13          	addi	s4,s4,-212 # 8001e070 <sb>
    8000314c:	00495593          	srli	a1,s2,0x4
    80003150:	018a2783          	lw	a5,24(s4)
    80003154:	9dbd                	addw	a1,a1,a5
    80003156:	8556                	mv	a0,s5
    80003158:	a69ff0ef          	jal	80002bc0 <bread>
    8000315c:	84aa                	mv	s1,a0
    dip = (struct dinode*)bp->data + inum%IPB;
    8000315e:	05850993          	addi	s3,a0,88
    80003162:	00f97793          	andi	a5,s2,15
    80003166:	079a                	slli	a5,a5,0x6
    80003168:	99be                	add	s3,s3,a5
    if(dip->type == 0){  // a free inode
    8000316a:	00099783          	lh	a5,0(s3)
    8000316e:	cb9d                	beqz	a5,800031a4 <ialloc+0x88>
    brelse(bp);
    80003170:	b59ff0ef          	jal	80002cc8 <brelse>
  for(inum = 1; inum < sb.ninodes; inum++){
    80003174:	0905                	addi	s2,s2,1
    80003176:	00ca2703          	lw	a4,12(s4)
    8000317a:	0009079b          	sext.w	a5,s2
    8000317e:	fce7e7e3          	bltu	a5,a4,8000314c <ialloc+0x30>
    80003182:	74a2                	ld	s1,40(sp)
    80003184:	7902                	ld	s2,32(sp)
    80003186:	69e2                	ld	s3,24(sp)
    80003188:	6a42                	ld	s4,16(sp)
    8000318a:	6aa2                	ld	s5,8(sp)
    8000318c:	6b02                	ld	s6,0(sp)
  printf("ialloc: no inodes\n");
    8000318e:	00004517          	auipc	a0,0x4
    80003192:	2a250513          	addi	a0,a0,674 # 80007430 <etext+0x430>
    80003196:	b64fd0ef          	jal	800004fa <printf>
  return 0;
    8000319a:	4501                	li	a0,0
}
    8000319c:	70e2                	ld	ra,56(sp)
    8000319e:	7442                	ld	s0,48(sp)
    800031a0:	6121                	addi	sp,sp,64
    800031a2:	8082                	ret
      memset(dip, 0, sizeof(*dip));
    800031a4:	04000613          	li	a2,64
    800031a8:	4581                	li	a1,0
    800031aa:	854e                	mv	a0,s3
    800031ac:	af7fd0ef          	jal	80000ca2 <memset>
      dip->type = type;
    800031b0:	01699023          	sh	s6,0(s3)
      log_write(bp);   // mark it allocated on the disk
    800031b4:	8526                	mv	a0,s1
    800031b6:	445000ef          	jal	80003dfa <log_write>
      brelse(bp);
    800031ba:	8526                	mv	a0,s1
    800031bc:	b0dff0ef          	jal	80002cc8 <brelse>
      return iget(dev, inum);
    800031c0:	0009059b          	sext.w	a1,s2
    800031c4:	8556                	mv	a0,s5
    800031c6:	e53ff0ef          	jal	80003018 <iget>
    800031ca:	74a2                	ld	s1,40(sp)
    800031cc:	7902                	ld	s2,32(sp)
    800031ce:	69e2                	ld	s3,24(sp)
    800031d0:	6a42                	ld	s4,16(sp)
    800031d2:	6aa2                	ld	s5,8(sp)
    800031d4:	6b02                	ld	s6,0(sp)
    800031d6:	b7d9                	j	8000319c <ialloc+0x80>

00000000800031d8 <iupdate>:
{
    800031d8:	1101                	addi	sp,sp,-32
    800031da:	ec06                	sd	ra,24(sp)
    800031dc:	e822                	sd	s0,16(sp)
    800031de:	e426                	sd	s1,8(sp)
    800031e0:	e04a                	sd	s2,0(sp)
    800031e2:	1000                	addi	s0,sp,32
    800031e4:	84aa                	mv	s1,a0
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    800031e6:	415c                	lw	a5,4(a0)
    800031e8:	0047d79b          	srliw	a5,a5,0x4
    800031ec:	0001b597          	auipc	a1,0x1b
    800031f0:	e9c5a583          	lw	a1,-356(a1) # 8001e088 <sb+0x18>
    800031f4:	9dbd                	addw	a1,a1,a5
    800031f6:	4108                	lw	a0,0(a0)
    800031f8:	9c9ff0ef          	jal	80002bc0 <bread>
    800031fc:	892a                	mv	s2,a0
  dip = (struct dinode*)bp->data + ip->inum%IPB;
    800031fe:	05850793          	addi	a5,a0,88
    80003202:	40d8                	lw	a4,4(s1)
    80003204:	8b3d                	andi	a4,a4,15
    80003206:	071a                	slli	a4,a4,0x6
    80003208:	97ba                	add	a5,a5,a4
  dip->type = ip->type;
    8000320a:	04449703          	lh	a4,68(s1)
    8000320e:	00e79023          	sh	a4,0(a5)
  dip->major = ip->major;
    80003212:	04649703          	lh	a4,70(s1)
    80003216:	00e79123          	sh	a4,2(a5)
  dip->minor = ip->minor;
    8000321a:	04849703          	lh	a4,72(s1)
    8000321e:	00e79223          	sh	a4,4(a5)
  dip->nlink = ip->nlink;
    80003222:	04a49703          	lh	a4,74(s1)
    80003226:	00e79323          	sh	a4,6(a5)
  dip->size = ip->size;
    8000322a:	44f8                	lw	a4,76(s1)
    8000322c:	c798                	sw	a4,8(a5)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
    8000322e:	03400613          	li	a2,52
    80003232:	05048593          	addi	a1,s1,80
    80003236:	00c78513          	addi	a0,a5,12
    8000323a:	ac5fd0ef          	jal	80000cfe <memmove>
  log_write(bp);
    8000323e:	854a                	mv	a0,s2
    80003240:	3bb000ef          	jal	80003dfa <log_write>
  brelse(bp);
    80003244:	854a                	mv	a0,s2
    80003246:	a83ff0ef          	jal	80002cc8 <brelse>
}
    8000324a:	60e2                	ld	ra,24(sp)
    8000324c:	6442                	ld	s0,16(sp)
    8000324e:	64a2                	ld	s1,8(sp)
    80003250:	6902                	ld	s2,0(sp)
    80003252:	6105                	addi	sp,sp,32
    80003254:	8082                	ret

0000000080003256 <idup>:
{
    80003256:	1101                	addi	sp,sp,-32
    80003258:	ec06                	sd	ra,24(sp)
    8000325a:	e822                	sd	s0,16(sp)
    8000325c:	e426                	sd	s1,8(sp)
    8000325e:	1000                	addi	s0,sp,32
    80003260:	84aa                	mv	s1,a0
  acquire(&itable.lock);
    80003262:	0001b517          	auipc	a0,0x1b
    80003266:	e2e50513          	addi	a0,a0,-466 # 8001e090 <itable>
    8000326a:	965fd0ef          	jal	80000bce <acquire>
  ip->ref++;
    8000326e:	449c                	lw	a5,8(s1)
    80003270:	2785                	addiw	a5,a5,1
    80003272:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    80003274:	0001b517          	auipc	a0,0x1b
    80003278:	e1c50513          	addi	a0,a0,-484 # 8001e090 <itable>
    8000327c:	9ebfd0ef          	jal	80000c66 <release>
}
    80003280:	8526                	mv	a0,s1
    80003282:	60e2                	ld	ra,24(sp)
    80003284:	6442                	ld	s0,16(sp)
    80003286:	64a2                	ld	s1,8(sp)
    80003288:	6105                	addi	sp,sp,32
    8000328a:	8082                	ret

000000008000328c <ilock>:
{
    8000328c:	1101                	addi	sp,sp,-32
    8000328e:	ec06                	sd	ra,24(sp)
    80003290:	e822                	sd	s0,16(sp)
    80003292:	e426                	sd	s1,8(sp)
    80003294:	1000                	addi	s0,sp,32
  if(ip == 0 || ip->ref < 1)
    80003296:	cd19                	beqz	a0,800032b4 <ilock+0x28>
    80003298:	84aa                	mv	s1,a0
    8000329a:	451c                	lw	a5,8(a0)
    8000329c:	00f05c63          	blez	a5,800032b4 <ilock+0x28>
  acquiresleep(&ip->lock);
    800032a0:	0541                	addi	a0,a0,16
    800032a2:	451000ef          	jal	80003ef2 <acquiresleep>
  if(ip->valid == 0){
    800032a6:	40bc                	lw	a5,64(s1)
    800032a8:	cf89                	beqz	a5,800032c2 <ilock+0x36>
}
    800032aa:	60e2                	ld	ra,24(sp)
    800032ac:	6442                	ld	s0,16(sp)
    800032ae:	64a2                	ld	s1,8(sp)
    800032b0:	6105                	addi	sp,sp,32
    800032b2:	8082                	ret
    800032b4:	e04a                	sd	s2,0(sp)
    panic("ilock");
    800032b6:	00004517          	auipc	a0,0x4
    800032ba:	19250513          	addi	a0,a0,402 # 80007448 <etext+0x448>
    800032be:	d22fd0ef          	jal	800007e0 <panic>
    800032c2:	e04a                	sd	s2,0(sp)
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    800032c4:	40dc                	lw	a5,4(s1)
    800032c6:	0047d79b          	srliw	a5,a5,0x4
    800032ca:	0001b597          	auipc	a1,0x1b
    800032ce:	dbe5a583          	lw	a1,-578(a1) # 8001e088 <sb+0x18>
    800032d2:	9dbd                	addw	a1,a1,a5
    800032d4:	4088                	lw	a0,0(s1)
    800032d6:	8ebff0ef          	jal	80002bc0 <bread>
    800032da:	892a                	mv	s2,a0
    dip = (struct dinode*)bp->data + ip->inum%IPB;
    800032dc:	05850593          	addi	a1,a0,88
    800032e0:	40dc                	lw	a5,4(s1)
    800032e2:	8bbd                	andi	a5,a5,15
    800032e4:	079a                	slli	a5,a5,0x6
    800032e6:	95be                	add	a1,a1,a5
    ip->type = dip->type;
    800032e8:	00059783          	lh	a5,0(a1)
    800032ec:	04f49223          	sh	a5,68(s1)
    ip->major = dip->major;
    800032f0:	00259783          	lh	a5,2(a1)
    800032f4:	04f49323          	sh	a5,70(s1)
    ip->minor = dip->minor;
    800032f8:	00459783          	lh	a5,4(a1)
    800032fc:	04f49423          	sh	a5,72(s1)
    ip->nlink = dip->nlink;
    80003300:	00659783          	lh	a5,6(a1)
    80003304:	04f49523          	sh	a5,74(s1)
    ip->size = dip->size;
    80003308:	459c                	lw	a5,8(a1)
    8000330a:	c4fc                	sw	a5,76(s1)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
    8000330c:	03400613          	li	a2,52
    80003310:	05b1                	addi	a1,a1,12
    80003312:	05048513          	addi	a0,s1,80
    80003316:	9e9fd0ef          	jal	80000cfe <memmove>
    brelse(bp);
    8000331a:	854a                	mv	a0,s2
    8000331c:	9adff0ef          	jal	80002cc8 <brelse>
    ip->valid = 1;
    80003320:	4785                	li	a5,1
    80003322:	c0bc                	sw	a5,64(s1)
    if(ip->type == 0)
    80003324:	04449783          	lh	a5,68(s1)
    80003328:	c399                	beqz	a5,8000332e <ilock+0xa2>
    8000332a:	6902                	ld	s2,0(sp)
    8000332c:	bfbd                	j	800032aa <ilock+0x1e>
      panic("ilock: no type");
    8000332e:	00004517          	auipc	a0,0x4
    80003332:	12250513          	addi	a0,a0,290 # 80007450 <etext+0x450>
    80003336:	caafd0ef          	jal	800007e0 <panic>

000000008000333a <iunlock>:
{
    8000333a:	1101                	addi	sp,sp,-32
    8000333c:	ec06                	sd	ra,24(sp)
    8000333e:	e822                	sd	s0,16(sp)
    80003340:	e426                	sd	s1,8(sp)
    80003342:	e04a                	sd	s2,0(sp)
    80003344:	1000                	addi	s0,sp,32
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
    80003346:	c505                	beqz	a0,8000336e <iunlock+0x34>
    80003348:	84aa                	mv	s1,a0
    8000334a:	01050913          	addi	s2,a0,16
    8000334e:	854a                	mv	a0,s2
    80003350:	421000ef          	jal	80003f70 <holdingsleep>
    80003354:	cd09                	beqz	a0,8000336e <iunlock+0x34>
    80003356:	449c                	lw	a5,8(s1)
    80003358:	00f05b63          	blez	a5,8000336e <iunlock+0x34>
  releasesleep(&ip->lock);
    8000335c:	854a                	mv	a0,s2
    8000335e:	3db000ef          	jal	80003f38 <releasesleep>
}
    80003362:	60e2                	ld	ra,24(sp)
    80003364:	6442                	ld	s0,16(sp)
    80003366:	64a2                	ld	s1,8(sp)
    80003368:	6902                	ld	s2,0(sp)
    8000336a:	6105                	addi	sp,sp,32
    8000336c:	8082                	ret
    panic("iunlock");
    8000336e:	00004517          	auipc	a0,0x4
    80003372:	0f250513          	addi	a0,a0,242 # 80007460 <etext+0x460>
    80003376:	c6afd0ef          	jal	800007e0 <panic>

000000008000337a <itrunc>:

// Truncate inode (discard contents).
// Caller must hold ip->lock.
void
itrunc(struct inode *ip)
{
    8000337a:	7179                	addi	sp,sp,-48
    8000337c:	f406                	sd	ra,40(sp)
    8000337e:	f022                	sd	s0,32(sp)
    80003380:	ec26                	sd	s1,24(sp)
    80003382:	e84a                	sd	s2,16(sp)
    80003384:	e44e                	sd	s3,8(sp)
    80003386:	1800                	addi	s0,sp,48
    80003388:	89aa                	mv	s3,a0
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
    8000338a:	05050493          	addi	s1,a0,80
    8000338e:	08050913          	addi	s2,a0,128
    80003392:	a021                	j	8000339a <itrunc+0x20>
    80003394:	0491                	addi	s1,s1,4
    80003396:	01248b63          	beq	s1,s2,800033ac <itrunc+0x32>
    if(ip->addrs[i]){
    8000339a:	408c                	lw	a1,0(s1)
    8000339c:	dde5                	beqz	a1,80003394 <itrunc+0x1a>
      bfree(ip->dev, ip->addrs[i]);
    8000339e:	0009a503          	lw	a0,0(s3)
    800033a2:	a17ff0ef          	jal	80002db8 <bfree>
      ip->addrs[i] = 0;
    800033a6:	0004a023          	sw	zero,0(s1)
    800033aa:	b7ed                	j	80003394 <itrunc+0x1a>
    }
  }

  if(ip->addrs[NDIRECT]){
    800033ac:	0809a583          	lw	a1,128(s3)
    800033b0:	ed89                	bnez	a1,800033ca <itrunc+0x50>
    brelse(bp);
    bfree(ip->dev, ip->addrs[NDIRECT]);
    ip->addrs[NDIRECT] = 0;
  }

  ip->size = 0;
    800033b2:	0409a623          	sw	zero,76(s3)
  iupdate(ip);
    800033b6:	854e                	mv	a0,s3
    800033b8:	e21ff0ef          	jal	800031d8 <iupdate>
}
    800033bc:	70a2                	ld	ra,40(sp)
    800033be:	7402                	ld	s0,32(sp)
    800033c0:	64e2                	ld	s1,24(sp)
    800033c2:	6942                	ld	s2,16(sp)
    800033c4:	69a2                	ld	s3,8(sp)
    800033c6:	6145                	addi	sp,sp,48
    800033c8:	8082                	ret
    800033ca:	e052                	sd	s4,0(sp)
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    800033cc:	0009a503          	lw	a0,0(s3)
    800033d0:	ff0ff0ef          	jal	80002bc0 <bread>
    800033d4:	8a2a                	mv	s4,a0
    for(j = 0; j < NINDIRECT; j++){
    800033d6:	05850493          	addi	s1,a0,88
    800033da:	45850913          	addi	s2,a0,1112
    800033de:	a021                	j	800033e6 <itrunc+0x6c>
    800033e0:	0491                	addi	s1,s1,4
    800033e2:	01248963          	beq	s1,s2,800033f4 <itrunc+0x7a>
      if(a[j])
    800033e6:	408c                	lw	a1,0(s1)
    800033e8:	dde5                	beqz	a1,800033e0 <itrunc+0x66>
        bfree(ip->dev, a[j]);
    800033ea:	0009a503          	lw	a0,0(s3)
    800033ee:	9cbff0ef          	jal	80002db8 <bfree>
    800033f2:	b7fd                	j	800033e0 <itrunc+0x66>
    brelse(bp);
    800033f4:	8552                	mv	a0,s4
    800033f6:	8d3ff0ef          	jal	80002cc8 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
    800033fa:	0809a583          	lw	a1,128(s3)
    800033fe:	0009a503          	lw	a0,0(s3)
    80003402:	9b7ff0ef          	jal	80002db8 <bfree>
    ip->addrs[NDIRECT] = 0;
    80003406:	0809a023          	sw	zero,128(s3)
    8000340a:	6a02                	ld	s4,0(sp)
    8000340c:	b75d                	j	800033b2 <itrunc+0x38>

000000008000340e <iput>:
{
    8000340e:	1101                	addi	sp,sp,-32
    80003410:	ec06                	sd	ra,24(sp)
    80003412:	e822                	sd	s0,16(sp)
    80003414:	e426                	sd	s1,8(sp)
    80003416:	1000                	addi	s0,sp,32
    80003418:	84aa                	mv	s1,a0
  acquire(&itable.lock);
    8000341a:	0001b517          	auipc	a0,0x1b
    8000341e:	c7650513          	addi	a0,a0,-906 # 8001e090 <itable>
    80003422:	facfd0ef          	jal	80000bce <acquire>
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    80003426:	4498                	lw	a4,8(s1)
    80003428:	4785                	li	a5,1
    8000342a:	02f70063          	beq	a4,a5,8000344a <iput+0x3c>
  ip->ref--;
    8000342e:	449c                	lw	a5,8(s1)
    80003430:	37fd                	addiw	a5,a5,-1
    80003432:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    80003434:	0001b517          	auipc	a0,0x1b
    80003438:	c5c50513          	addi	a0,a0,-932 # 8001e090 <itable>
    8000343c:	82bfd0ef          	jal	80000c66 <release>
}
    80003440:	60e2                	ld	ra,24(sp)
    80003442:	6442                	ld	s0,16(sp)
    80003444:	64a2                	ld	s1,8(sp)
    80003446:	6105                	addi	sp,sp,32
    80003448:	8082                	ret
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    8000344a:	40bc                	lw	a5,64(s1)
    8000344c:	d3ed                	beqz	a5,8000342e <iput+0x20>
    8000344e:	04a49783          	lh	a5,74(s1)
    80003452:	fff1                	bnez	a5,8000342e <iput+0x20>
    80003454:	e04a                	sd	s2,0(sp)
    acquiresleep(&ip->lock);
    80003456:	01048913          	addi	s2,s1,16
    8000345a:	854a                	mv	a0,s2
    8000345c:	297000ef          	jal	80003ef2 <acquiresleep>
    release(&itable.lock);
    80003460:	0001b517          	auipc	a0,0x1b
    80003464:	c3050513          	addi	a0,a0,-976 # 8001e090 <itable>
    80003468:	ffefd0ef          	jal	80000c66 <release>
    itrunc(ip);
    8000346c:	8526                	mv	a0,s1
    8000346e:	f0dff0ef          	jal	8000337a <itrunc>
    ip->type = 0;
    80003472:	04049223          	sh	zero,68(s1)
    iupdate(ip);
    80003476:	8526                	mv	a0,s1
    80003478:	d61ff0ef          	jal	800031d8 <iupdate>
    ip->valid = 0;
    8000347c:	0404a023          	sw	zero,64(s1)
    releasesleep(&ip->lock);
    80003480:	854a                	mv	a0,s2
    80003482:	2b7000ef          	jal	80003f38 <releasesleep>
    acquire(&itable.lock);
    80003486:	0001b517          	auipc	a0,0x1b
    8000348a:	c0a50513          	addi	a0,a0,-1014 # 8001e090 <itable>
    8000348e:	f40fd0ef          	jal	80000bce <acquire>
    80003492:	6902                	ld	s2,0(sp)
    80003494:	bf69                	j	8000342e <iput+0x20>

0000000080003496 <iunlockput>:
{
    80003496:	1101                	addi	sp,sp,-32
    80003498:	ec06                	sd	ra,24(sp)
    8000349a:	e822                	sd	s0,16(sp)
    8000349c:	e426                	sd	s1,8(sp)
    8000349e:	1000                	addi	s0,sp,32
    800034a0:	84aa                	mv	s1,a0
  iunlock(ip);
    800034a2:	e99ff0ef          	jal	8000333a <iunlock>
  iput(ip);
    800034a6:	8526                	mv	a0,s1
    800034a8:	f67ff0ef          	jal	8000340e <iput>
}
    800034ac:	60e2                	ld	ra,24(sp)
    800034ae:	6442                	ld	s0,16(sp)
    800034b0:	64a2                	ld	s1,8(sp)
    800034b2:	6105                	addi	sp,sp,32
    800034b4:	8082                	ret

00000000800034b6 <ireclaim>:
  for (int inum = 1; inum < sb.ninodes; inum++) {
    800034b6:	0001b717          	auipc	a4,0x1b
    800034ba:	bc672703          	lw	a4,-1082(a4) # 8001e07c <sb+0xc>
    800034be:	4785                	li	a5,1
    800034c0:	0ae7ff63          	bgeu	a5,a4,8000357e <ireclaim+0xc8>
{
    800034c4:	7139                	addi	sp,sp,-64
    800034c6:	fc06                	sd	ra,56(sp)
    800034c8:	f822                	sd	s0,48(sp)
    800034ca:	f426                	sd	s1,40(sp)
    800034cc:	f04a                	sd	s2,32(sp)
    800034ce:	ec4e                	sd	s3,24(sp)
    800034d0:	e852                	sd	s4,16(sp)
    800034d2:	e456                	sd	s5,8(sp)
    800034d4:	e05a                	sd	s6,0(sp)
    800034d6:	0080                	addi	s0,sp,64
  for (int inum = 1; inum < sb.ninodes; inum++) {
    800034d8:	4485                	li	s1,1
    struct buf *bp = bread(dev, IBLOCK(inum, sb));
    800034da:	00050a1b          	sext.w	s4,a0
    800034de:	0001ba97          	auipc	s5,0x1b
    800034e2:	b92a8a93          	addi	s5,s5,-1134 # 8001e070 <sb>
      printf("ireclaim: orphaned inode %d\n", inum);
    800034e6:	00004b17          	auipc	s6,0x4
    800034ea:	f82b0b13          	addi	s6,s6,-126 # 80007468 <etext+0x468>
    800034ee:	a099                	j	80003534 <ireclaim+0x7e>
    800034f0:	85ce                	mv	a1,s3
    800034f2:	855a                	mv	a0,s6
    800034f4:	806fd0ef          	jal	800004fa <printf>
      ip = iget(dev, inum);
    800034f8:	85ce                	mv	a1,s3
    800034fa:	8552                	mv	a0,s4
    800034fc:	b1dff0ef          	jal	80003018 <iget>
    80003500:	89aa                	mv	s3,a0
    brelse(bp);
    80003502:	854a                	mv	a0,s2
    80003504:	fc4ff0ef          	jal	80002cc8 <brelse>
    if (ip) {
    80003508:	00098f63          	beqz	s3,80003526 <ireclaim+0x70>
      begin_op();
    8000350c:	76a000ef          	jal	80003c76 <begin_op>
      ilock(ip);
    80003510:	854e                	mv	a0,s3
    80003512:	d7bff0ef          	jal	8000328c <ilock>
      iunlock(ip);
    80003516:	854e                	mv	a0,s3
    80003518:	e23ff0ef          	jal	8000333a <iunlock>
      iput(ip);
    8000351c:	854e                	mv	a0,s3
    8000351e:	ef1ff0ef          	jal	8000340e <iput>
      end_op();
    80003522:	7be000ef          	jal	80003ce0 <end_op>
  for (int inum = 1; inum < sb.ninodes; inum++) {
    80003526:	0485                	addi	s1,s1,1
    80003528:	00caa703          	lw	a4,12(s5)
    8000352c:	0004879b          	sext.w	a5,s1
    80003530:	02e7fd63          	bgeu	a5,a4,8000356a <ireclaim+0xb4>
    80003534:	0004899b          	sext.w	s3,s1
    struct buf *bp = bread(dev, IBLOCK(inum, sb));
    80003538:	0044d593          	srli	a1,s1,0x4
    8000353c:	018aa783          	lw	a5,24(s5)
    80003540:	9dbd                	addw	a1,a1,a5
    80003542:	8552                	mv	a0,s4
    80003544:	e7cff0ef          	jal	80002bc0 <bread>
    80003548:	892a                	mv	s2,a0
    struct dinode *dip = (struct dinode *)bp->data + inum % IPB;
    8000354a:	05850793          	addi	a5,a0,88
    8000354e:	00f9f713          	andi	a4,s3,15
    80003552:	071a                	slli	a4,a4,0x6
    80003554:	97ba                	add	a5,a5,a4
    if (dip->type != 0 && dip->nlink == 0) {  // is an orphaned inode
    80003556:	00079703          	lh	a4,0(a5)
    8000355a:	c701                	beqz	a4,80003562 <ireclaim+0xac>
    8000355c:	00679783          	lh	a5,6(a5)
    80003560:	dbc1                	beqz	a5,800034f0 <ireclaim+0x3a>
    brelse(bp);
    80003562:	854a                	mv	a0,s2
    80003564:	f64ff0ef          	jal	80002cc8 <brelse>
    if (ip) {
    80003568:	bf7d                	j	80003526 <ireclaim+0x70>
}
    8000356a:	70e2                	ld	ra,56(sp)
    8000356c:	7442                	ld	s0,48(sp)
    8000356e:	74a2                	ld	s1,40(sp)
    80003570:	7902                	ld	s2,32(sp)
    80003572:	69e2                	ld	s3,24(sp)
    80003574:	6a42                	ld	s4,16(sp)
    80003576:	6aa2                	ld	s5,8(sp)
    80003578:	6b02                	ld	s6,0(sp)
    8000357a:	6121                	addi	sp,sp,64
    8000357c:	8082                	ret
    8000357e:	8082                	ret

0000000080003580 <fsinit>:
fsinit(int dev) {
    80003580:	7179                	addi	sp,sp,-48
    80003582:	f406                	sd	ra,40(sp)
    80003584:	f022                	sd	s0,32(sp)
    80003586:	ec26                	sd	s1,24(sp)
    80003588:	e84a                	sd	s2,16(sp)
    8000358a:	e44e                	sd	s3,8(sp)
    8000358c:	1800                	addi	s0,sp,48
    8000358e:	84aa                	mv	s1,a0
  bp = bread(dev, 1);
    80003590:	4585                	li	a1,1
    80003592:	e2eff0ef          	jal	80002bc0 <bread>
    80003596:	892a                	mv	s2,a0
  memmove(sb, bp->data, sizeof(*sb));
    80003598:	0001b997          	auipc	s3,0x1b
    8000359c:	ad898993          	addi	s3,s3,-1320 # 8001e070 <sb>
    800035a0:	02000613          	li	a2,32
    800035a4:	05850593          	addi	a1,a0,88
    800035a8:	854e                	mv	a0,s3
    800035aa:	f54fd0ef          	jal	80000cfe <memmove>
  brelse(bp);
    800035ae:	854a                	mv	a0,s2
    800035b0:	f18ff0ef          	jal	80002cc8 <brelse>
  if(sb.magic != FSMAGIC)
    800035b4:	0009a703          	lw	a4,0(s3)
    800035b8:	102037b7          	lui	a5,0x10203
    800035bc:	04078793          	addi	a5,a5,64 # 10203040 <_entry-0x6fdfcfc0>
    800035c0:	02f71363          	bne	a4,a5,800035e6 <fsinit+0x66>
  initlog(dev, &sb);
    800035c4:	0001b597          	auipc	a1,0x1b
    800035c8:	aac58593          	addi	a1,a1,-1364 # 8001e070 <sb>
    800035cc:	8526                	mv	a0,s1
    800035ce:	62a000ef          	jal	80003bf8 <initlog>
  ireclaim(dev);
    800035d2:	8526                	mv	a0,s1
    800035d4:	ee3ff0ef          	jal	800034b6 <ireclaim>
}
    800035d8:	70a2                	ld	ra,40(sp)
    800035da:	7402                	ld	s0,32(sp)
    800035dc:	64e2                	ld	s1,24(sp)
    800035de:	6942                	ld	s2,16(sp)
    800035e0:	69a2                	ld	s3,8(sp)
    800035e2:	6145                	addi	sp,sp,48
    800035e4:	8082                	ret
    panic("invalid file system");
    800035e6:	00004517          	auipc	a0,0x4
    800035ea:	ea250513          	addi	a0,a0,-350 # 80007488 <etext+0x488>
    800035ee:	9f2fd0ef          	jal	800007e0 <panic>

00000000800035f2 <stati>:

// Copy stat information from inode.
// Caller must hold ip->lock.
void
stati(struct inode *ip, struct stat *st)
{
    800035f2:	1141                	addi	sp,sp,-16
    800035f4:	e422                	sd	s0,8(sp)
    800035f6:	0800                	addi	s0,sp,16
  st->dev = ip->dev;
    800035f8:	411c                	lw	a5,0(a0)
    800035fa:	c19c                	sw	a5,0(a1)
  st->ino = ip->inum;
    800035fc:	415c                	lw	a5,4(a0)
    800035fe:	c1dc                	sw	a5,4(a1)
  st->type = ip->type;
    80003600:	04451783          	lh	a5,68(a0)
    80003604:	00f59423          	sh	a5,8(a1)
  st->nlink = ip->nlink;
    80003608:	04a51783          	lh	a5,74(a0)
    8000360c:	00f59523          	sh	a5,10(a1)
  st->size = ip->size;
    80003610:	04c56783          	lwu	a5,76(a0)
    80003614:	e99c                	sd	a5,16(a1)
}
    80003616:	6422                	ld	s0,8(sp)
    80003618:	0141                	addi	sp,sp,16
    8000361a:	8082                	ret

000000008000361c <readi>:
readi(struct inode *ip, int user_dst, uint64 dst, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;

  if(off > ip->size || off + n < off)
    8000361c:	457c                	lw	a5,76(a0)
    8000361e:	0ed7eb63          	bltu	a5,a3,80003714 <readi+0xf8>
{
    80003622:	7159                	addi	sp,sp,-112
    80003624:	f486                	sd	ra,104(sp)
    80003626:	f0a2                	sd	s0,96(sp)
    80003628:	eca6                	sd	s1,88(sp)
    8000362a:	e0d2                	sd	s4,64(sp)
    8000362c:	fc56                	sd	s5,56(sp)
    8000362e:	f85a                	sd	s6,48(sp)
    80003630:	f45e                	sd	s7,40(sp)
    80003632:	1880                	addi	s0,sp,112
    80003634:	8b2a                	mv	s6,a0
    80003636:	8bae                	mv	s7,a1
    80003638:	8a32                	mv	s4,a2
    8000363a:	84b6                	mv	s1,a3
    8000363c:	8aba                	mv	s5,a4
  if(off > ip->size || off + n < off)
    8000363e:	9f35                	addw	a4,a4,a3
    return 0;
    80003640:	4501                	li	a0,0
  if(off > ip->size || off + n < off)
    80003642:	0cd76063          	bltu	a4,a3,80003702 <readi+0xe6>
    80003646:	e4ce                	sd	s3,72(sp)
  if(off + n > ip->size)
    80003648:	00e7f463          	bgeu	a5,a4,80003650 <readi+0x34>
    n = ip->size - off;
    8000364c:	40d78abb          	subw	s5,a5,a3

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003650:	080a8f63          	beqz	s5,800036ee <readi+0xd2>
    80003654:	e8ca                	sd	s2,80(sp)
    80003656:	f062                	sd	s8,32(sp)
    80003658:	ec66                	sd	s9,24(sp)
    8000365a:	e86a                	sd	s10,16(sp)
    8000365c:	e46e                	sd	s11,8(sp)
    8000365e:	4981                	li	s3,0
    uint addr = bmap(ip, off/BSIZE);
    if(addr == 0)
      break;
    bp = bread(ip->dev, addr);
    m = min(n - tot, BSIZE - off%BSIZE);
    80003660:	40000c93          	li	s9,1024
    if(either_copyout(user_dst, dst, bp->data + (off % BSIZE), m) == -1) {
    80003664:	5c7d                	li	s8,-1
    80003666:	a80d                	j	80003698 <readi+0x7c>
    80003668:	020d1d93          	slli	s11,s10,0x20
    8000366c:	020ddd93          	srli	s11,s11,0x20
    80003670:	05890613          	addi	a2,s2,88
    80003674:	86ee                	mv	a3,s11
    80003676:	963a                	add	a2,a2,a4
    80003678:	85d2                	mv	a1,s4
    8000367a:	855e                	mv	a0,s7
    8000367c:	c0dfe0ef          	jal	80002288 <either_copyout>
    80003680:	05850763          	beq	a0,s8,800036ce <readi+0xb2>
      brelse(bp);
      tot = -1;
      break;
    }
    brelse(bp);
    80003684:	854a                	mv	a0,s2
    80003686:	e42ff0ef          	jal	80002cc8 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    8000368a:	013d09bb          	addw	s3,s10,s3
    8000368e:	009d04bb          	addw	s1,s10,s1
    80003692:	9a6e                	add	s4,s4,s11
    80003694:	0559f763          	bgeu	s3,s5,800036e2 <readi+0xc6>
    uint addr = bmap(ip, off/BSIZE);
    80003698:	00a4d59b          	srliw	a1,s1,0xa
    8000369c:	855a                	mv	a0,s6
    8000369e:	8a7ff0ef          	jal	80002f44 <bmap>
    800036a2:	0005059b          	sext.w	a1,a0
    if(addr == 0)
    800036a6:	c5b1                	beqz	a1,800036f2 <readi+0xd6>
    bp = bread(ip->dev, addr);
    800036a8:	000b2503          	lw	a0,0(s6)
    800036ac:	d14ff0ef          	jal	80002bc0 <bread>
    800036b0:	892a                	mv	s2,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    800036b2:	3ff4f713          	andi	a4,s1,1023
    800036b6:	40ec87bb          	subw	a5,s9,a4
    800036ba:	413a86bb          	subw	a3,s5,s3
    800036be:	8d3e                	mv	s10,a5
    800036c0:	2781                	sext.w	a5,a5
    800036c2:	0006861b          	sext.w	a2,a3
    800036c6:	faf671e3          	bgeu	a2,a5,80003668 <readi+0x4c>
    800036ca:	8d36                	mv	s10,a3
    800036cc:	bf71                	j	80003668 <readi+0x4c>
      brelse(bp);
    800036ce:	854a                	mv	a0,s2
    800036d0:	df8ff0ef          	jal	80002cc8 <brelse>
      tot = -1;
    800036d4:	59fd                	li	s3,-1
      break;
    800036d6:	6946                	ld	s2,80(sp)
    800036d8:	7c02                	ld	s8,32(sp)
    800036da:	6ce2                	ld	s9,24(sp)
    800036dc:	6d42                	ld	s10,16(sp)
    800036de:	6da2                	ld	s11,8(sp)
    800036e0:	a831                	j	800036fc <readi+0xe0>
    800036e2:	6946                	ld	s2,80(sp)
    800036e4:	7c02                	ld	s8,32(sp)
    800036e6:	6ce2                	ld	s9,24(sp)
    800036e8:	6d42                	ld	s10,16(sp)
    800036ea:	6da2                	ld	s11,8(sp)
    800036ec:	a801                	j	800036fc <readi+0xe0>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    800036ee:	89d6                	mv	s3,s5
    800036f0:	a031                	j	800036fc <readi+0xe0>
    800036f2:	6946                	ld	s2,80(sp)
    800036f4:	7c02                	ld	s8,32(sp)
    800036f6:	6ce2                	ld	s9,24(sp)
    800036f8:	6d42                	ld	s10,16(sp)
    800036fa:	6da2                	ld	s11,8(sp)
  }
  return tot;
    800036fc:	0009851b          	sext.w	a0,s3
    80003700:	69a6                	ld	s3,72(sp)
}
    80003702:	70a6                	ld	ra,104(sp)
    80003704:	7406                	ld	s0,96(sp)
    80003706:	64e6                	ld	s1,88(sp)
    80003708:	6a06                	ld	s4,64(sp)
    8000370a:	7ae2                	ld	s5,56(sp)
    8000370c:	7b42                	ld	s6,48(sp)
    8000370e:	7ba2                	ld	s7,40(sp)
    80003710:	6165                	addi	sp,sp,112
    80003712:	8082                	ret
    return 0;
    80003714:	4501                	li	a0,0
}
    80003716:	8082                	ret

0000000080003718 <writei>:
writei(struct inode *ip, int user_src, uint64 src, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;

  if(off > ip->size || off + n < off)
    80003718:	457c                	lw	a5,76(a0)
    8000371a:	10d7e063          	bltu	a5,a3,8000381a <writei+0x102>
{
    8000371e:	7159                	addi	sp,sp,-112
    80003720:	f486                	sd	ra,104(sp)
    80003722:	f0a2                	sd	s0,96(sp)
    80003724:	e8ca                	sd	s2,80(sp)
    80003726:	e0d2                	sd	s4,64(sp)
    80003728:	fc56                	sd	s5,56(sp)
    8000372a:	f85a                	sd	s6,48(sp)
    8000372c:	f45e                	sd	s7,40(sp)
    8000372e:	1880                	addi	s0,sp,112
    80003730:	8aaa                	mv	s5,a0
    80003732:	8bae                	mv	s7,a1
    80003734:	8a32                	mv	s4,a2
    80003736:	8936                	mv	s2,a3
    80003738:	8b3a                	mv	s6,a4
  if(off > ip->size || off + n < off)
    8000373a:	00e687bb          	addw	a5,a3,a4
    8000373e:	0ed7e063          	bltu	a5,a3,8000381e <writei+0x106>
    return -1;
  if(off + n > MAXFILE*BSIZE)
    80003742:	00043737          	lui	a4,0x43
    80003746:	0cf76e63          	bltu	a4,a5,80003822 <writei+0x10a>
    8000374a:	e4ce                	sd	s3,72(sp)
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    8000374c:	0a0b0f63          	beqz	s6,8000380a <writei+0xf2>
    80003750:	eca6                	sd	s1,88(sp)
    80003752:	f062                	sd	s8,32(sp)
    80003754:	ec66                	sd	s9,24(sp)
    80003756:	e86a                	sd	s10,16(sp)
    80003758:	e46e                	sd	s11,8(sp)
    8000375a:	4981                	li	s3,0
    uint addr = bmap(ip, off/BSIZE);
    if(addr == 0)
      break;
    bp = bread(ip->dev, addr);
    m = min(n - tot, BSIZE - off%BSIZE);
    8000375c:	40000c93          	li	s9,1024
    if(either_copyin(bp->data + (off % BSIZE), user_src, src, m) == -1) {
    80003760:	5c7d                	li	s8,-1
    80003762:	a825                	j	8000379a <writei+0x82>
    80003764:	020d1d93          	slli	s11,s10,0x20
    80003768:	020ddd93          	srli	s11,s11,0x20
    8000376c:	05848513          	addi	a0,s1,88
    80003770:	86ee                	mv	a3,s11
    80003772:	8652                	mv	a2,s4
    80003774:	85de                	mv	a1,s7
    80003776:	953a                	add	a0,a0,a4
    80003778:	b5bfe0ef          	jal	800022d2 <either_copyin>
    8000377c:	05850a63          	beq	a0,s8,800037d0 <writei+0xb8>
      brelse(bp);
      break;
    }
    log_write(bp);
    80003780:	8526                	mv	a0,s1
    80003782:	678000ef          	jal	80003dfa <log_write>
    brelse(bp);
    80003786:	8526                	mv	a0,s1
    80003788:	d40ff0ef          	jal	80002cc8 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    8000378c:	013d09bb          	addw	s3,s10,s3
    80003790:	012d093b          	addw	s2,s10,s2
    80003794:	9a6e                	add	s4,s4,s11
    80003796:	0569f063          	bgeu	s3,s6,800037d6 <writei+0xbe>
    uint addr = bmap(ip, off/BSIZE);
    8000379a:	00a9559b          	srliw	a1,s2,0xa
    8000379e:	8556                	mv	a0,s5
    800037a0:	fa4ff0ef          	jal	80002f44 <bmap>
    800037a4:	0005059b          	sext.w	a1,a0
    if(addr == 0)
    800037a8:	c59d                	beqz	a1,800037d6 <writei+0xbe>
    bp = bread(ip->dev, addr);
    800037aa:	000aa503          	lw	a0,0(s5)
    800037ae:	c12ff0ef          	jal	80002bc0 <bread>
    800037b2:	84aa                	mv	s1,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    800037b4:	3ff97713          	andi	a4,s2,1023
    800037b8:	40ec87bb          	subw	a5,s9,a4
    800037bc:	413b06bb          	subw	a3,s6,s3
    800037c0:	8d3e                	mv	s10,a5
    800037c2:	2781                	sext.w	a5,a5
    800037c4:	0006861b          	sext.w	a2,a3
    800037c8:	f8f67ee3          	bgeu	a2,a5,80003764 <writei+0x4c>
    800037cc:	8d36                	mv	s10,a3
    800037ce:	bf59                	j	80003764 <writei+0x4c>
      brelse(bp);
    800037d0:	8526                	mv	a0,s1
    800037d2:	cf6ff0ef          	jal	80002cc8 <brelse>
  }

  if(off > ip->size)
    800037d6:	04caa783          	lw	a5,76(s5)
    800037da:	0327fa63          	bgeu	a5,s2,8000380e <writei+0xf6>
    ip->size = off;
    800037de:	052aa623          	sw	s2,76(s5)
    800037e2:	64e6                	ld	s1,88(sp)
    800037e4:	7c02                	ld	s8,32(sp)
    800037e6:	6ce2                	ld	s9,24(sp)
    800037e8:	6d42                	ld	s10,16(sp)
    800037ea:	6da2                	ld	s11,8(sp)

  // write the i-node back to disk even if the size didn't change
  // because the loop above might have called bmap() and added a new
  // block to ip->addrs[].
  iupdate(ip);
    800037ec:	8556                	mv	a0,s5
    800037ee:	9ebff0ef          	jal	800031d8 <iupdate>

  return tot;
    800037f2:	0009851b          	sext.w	a0,s3
    800037f6:	69a6                	ld	s3,72(sp)
}
    800037f8:	70a6                	ld	ra,104(sp)
    800037fa:	7406                	ld	s0,96(sp)
    800037fc:	6946                	ld	s2,80(sp)
    800037fe:	6a06                	ld	s4,64(sp)
    80003800:	7ae2                	ld	s5,56(sp)
    80003802:	7b42                	ld	s6,48(sp)
    80003804:	7ba2                	ld	s7,40(sp)
    80003806:	6165                	addi	sp,sp,112
    80003808:	8082                	ret
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    8000380a:	89da                	mv	s3,s6
    8000380c:	b7c5                	j	800037ec <writei+0xd4>
    8000380e:	64e6                	ld	s1,88(sp)
    80003810:	7c02                	ld	s8,32(sp)
    80003812:	6ce2                	ld	s9,24(sp)
    80003814:	6d42                	ld	s10,16(sp)
    80003816:	6da2                	ld	s11,8(sp)
    80003818:	bfd1                	j	800037ec <writei+0xd4>
    return -1;
    8000381a:	557d                	li	a0,-1
}
    8000381c:	8082                	ret
    return -1;
    8000381e:	557d                	li	a0,-1
    80003820:	bfe1                	j	800037f8 <writei+0xe0>
    return -1;
    80003822:	557d                	li	a0,-1
    80003824:	bfd1                	j	800037f8 <writei+0xe0>

0000000080003826 <namecmp>:

// Directories

int
namecmp(const char *s, const char *t)
{
    80003826:	1141                	addi	sp,sp,-16
    80003828:	e406                	sd	ra,8(sp)
    8000382a:	e022                	sd	s0,0(sp)
    8000382c:	0800                	addi	s0,sp,16
  return strncmp(s, t, DIRSIZ);
    8000382e:	4639                	li	a2,14
    80003830:	d3efd0ef          	jal	80000d6e <strncmp>
}
    80003834:	60a2                	ld	ra,8(sp)
    80003836:	6402                	ld	s0,0(sp)
    80003838:	0141                	addi	sp,sp,16
    8000383a:	8082                	ret

000000008000383c <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
    8000383c:	7139                	addi	sp,sp,-64
    8000383e:	fc06                	sd	ra,56(sp)
    80003840:	f822                	sd	s0,48(sp)
    80003842:	f426                	sd	s1,40(sp)
    80003844:	f04a                	sd	s2,32(sp)
    80003846:	ec4e                	sd	s3,24(sp)
    80003848:	e852                	sd	s4,16(sp)
    8000384a:	0080                	addi	s0,sp,64
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
    8000384c:	04451703          	lh	a4,68(a0)
    80003850:	4785                	li	a5,1
    80003852:	00f71a63          	bne	a4,a5,80003866 <dirlookup+0x2a>
    80003856:	892a                	mv	s2,a0
    80003858:	89ae                	mv	s3,a1
    8000385a:	8a32                	mv	s4,a2
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
    8000385c:	457c                	lw	a5,76(a0)
    8000385e:	4481                	li	s1,0
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
    80003860:	4501                	li	a0,0
  for(off = 0; off < dp->size; off += sizeof(de)){
    80003862:	e39d                	bnez	a5,80003888 <dirlookup+0x4c>
    80003864:	a095                	j	800038c8 <dirlookup+0x8c>
    panic("dirlookup not DIR");
    80003866:	00004517          	auipc	a0,0x4
    8000386a:	c3a50513          	addi	a0,a0,-966 # 800074a0 <etext+0x4a0>
    8000386e:	f73fc0ef          	jal	800007e0 <panic>
      panic("dirlookup read");
    80003872:	00004517          	auipc	a0,0x4
    80003876:	c4650513          	addi	a0,a0,-954 # 800074b8 <etext+0x4b8>
    8000387a:	f67fc0ef          	jal	800007e0 <panic>
  for(off = 0; off < dp->size; off += sizeof(de)){
    8000387e:	24c1                	addiw	s1,s1,16
    80003880:	04c92783          	lw	a5,76(s2)
    80003884:	04f4f163          	bgeu	s1,a5,800038c6 <dirlookup+0x8a>
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80003888:	4741                	li	a4,16
    8000388a:	86a6                	mv	a3,s1
    8000388c:	fc040613          	addi	a2,s0,-64
    80003890:	4581                	li	a1,0
    80003892:	854a                	mv	a0,s2
    80003894:	d89ff0ef          	jal	8000361c <readi>
    80003898:	47c1                	li	a5,16
    8000389a:	fcf51ce3          	bne	a0,a5,80003872 <dirlookup+0x36>
    if(de.inum == 0)
    8000389e:	fc045783          	lhu	a5,-64(s0)
    800038a2:	dff1                	beqz	a5,8000387e <dirlookup+0x42>
    if(namecmp(name, de.name) == 0){
    800038a4:	fc240593          	addi	a1,s0,-62
    800038a8:	854e                	mv	a0,s3
    800038aa:	f7dff0ef          	jal	80003826 <namecmp>
    800038ae:	f961                	bnez	a0,8000387e <dirlookup+0x42>
      if(poff)
    800038b0:	000a0463          	beqz	s4,800038b8 <dirlookup+0x7c>
        *poff = off;
    800038b4:	009a2023          	sw	s1,0(s4)
      return iget(dp->dev, inum);
    800038b8:	fc045583          	lhu	a1,-64(s0)
    800038bc:	00092503          	lw	a0,0(s2)
    800038c0:	f58ff0ef          	jal	80003018 <iget>
    800038c4:	a011                	j	800038c8 <dirlookup+0x8c>
  return 0;
    800038c6:	4501                	li	a0,0
}
    800038c8:	70e2                	ld	ra,56(sp)
    800038ca:	7442                	ld	s0,48(sp)
    800038cc:	74a2                	ld	s1,40(sp)
    800038ce:	7902                	ld	s2,32(sp)
    800038d0:	69e2                	ld	s3,24(sp)
    800038d2:	6a42                	ld	s4,16(sp)
    800038d4:	6121                	addi	sp,sp,64
    800038d6:	8082                	ret

00000000800038d8 <namex>:
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
// Must be called inside a transaction since it calls iput().
static struct inode*
namex(char *path, int nameiparent, char *name)
{
    800038d8:	711d                	addi	sp,sp,-96
    800038da:	ec86                	sd	ra,88(sp)
    800038dc:	e8a2                	sd	s0,80(sp)
    800038de:	e4a6                	sd	s1,72(sp)
    800038e0:	e0ca                	sd	s2,64(sp)
    800038e2:	fc4e                	sd	s3,56(sp)
    800038e4:	f852                	sd	s4,48(sp)
    800038e6:	f456                	sd	s5,40(sp)
    800038e8:	f05a                	sd	s6,32(sp)
    800038ea:	ec5e                	sd	s7,24(sp)
    800038ec:	e862                	sd	s8,16(sp)
    800038ee:	e466                	sd	s9,8(sp)
    800038f0:	1080                	addi	s0,sp,96
    800038f2:	84aa                	mv	s1,a0
    800038f4:	8b2e                	mv	s6,a1
    800038f6:	8ab2                	mv	s5,a2
  struct inode *ip, *next;

  if(*path == '/')
    800038f8:	00054703          	lbu	a4,0(a0)
    800038fc:	02f00793          	li	a5,47
    80003900:	00f70e63          	beq	a4,a5,8000391c <namex+0x44>
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(myproc()->cwd);
    80003904:	82cfe0ef          	jal	80001930 <myproc>
    80003908:	15053503          	ld	a0,336(a0)
    8000390c:	94bff0ef          	jal	80003256 <idup>
    80003910:	8a2a                	mv	s4,a0
  while(*path == '/')
    80003912:	02f00913          	li	s2,47
  if(len >= DIRSIZ)
    80003916:	4c35                	li	s8,13

  while((path = skipelem(path, name)) != 0){
    ilock(ip);
    if(ip->type != T_DIR){
    80003918:	4b85                	li	s7,1
    8000391a:	a871                	j	800039b6 <namex+0xde>
    ip = iget(ROOTDEV, ROOTINO);
    8000391c:	4585                	li	a1,1
    8000391e:	4505                	li	a0,1
    80003920:	ef8ff0ef          	jal	80003018 <iget>
    80003924:	8a2a                	mv	s4,a0
    80003926:	b7f5                	j	80003912 <namex+0x3a>
      iunlockput(ip);
    80003928:	8552                	mv	a0,s4
    8000392a:	b6dff0ef          	jal	80003496 <iunlockput>
      return 0;
    8000392e:	4a01                	li	s4,0
  if(nameiparent){
    iput(ip);
    return 0;
  }
  return ip;
}
    80003930:	8552                	mv	a0,s4
    80003932:	60e6                	ld	ra,88(sp)
    80003934:	6446                	ld	s0,80(sp)
    80003936:	64a6                	ld	s1,72(sp)
    80003938:	6906                	ld	s2,64(sp)
    8000393a:	79e2                	ld	s3,56(sp)
    8000393c:	7a42                	ld	s4,48(sp)
    8000393e:	7aa2                	ld	s5,40(sp)
    80003940:	7b02                	ld	s6,32(sp)
    80003942:	6be2                	ld	s7,24(sp)
    80003944:	6c42                	ld	s8,16(sp)
    80003946:	6ca2                	ld	s9,8(sp)
    80003948:	6125                	addi	sp,sp,96
    8000394a:	8082                	ret
      iunlock(ip);
    8000394c:	8552                	mv	a0,s4
    8000394e:	9edff0ef          	jal	8000333a <iunlock>
      return ip;
    80003952:	bff9                	j	80003930 <namex+0x58>
      iunlockput(ip);
    80003954:	8552                	mv	a0,s4
    80003956:	b41ff0ef          	jal	80003496 <iunlockput>
      return 0;
    8000395a:	8a4e                	mv	s4,s3
    8000395c:	bfd1                	j	80003930 <namex+0x58>
  len = path - s;
    8000395e:	40998633          	sub	a2,s3,s1
    80003962:	00060c9b          	sext.w	s9,a2
  if(len >= DIRSIZ)
    80003966:	099c5063          	bge	s8,s9,800039e6 <namex+0x10e>
    memmove(name, s, DIRSIZ);
    8000396a:	4639                	li	a2,14
    8000396c:	85a6                	mv	a1,s1
    8000396e:	8556                	mv	a0,s5
    80003970:	b8efd0ef          	jal	80000cfe <memmove>
    80003974:	84ce                	mv	s1,s3
  while(*path == '/')
    80003976:	0004c783          	lbu	a5,0(s1)
    8000397a:	01279763          	bne	a5,s2,80003988 <namex+0xb0>
    path++;
    8000397e:	0485                	addi	s1,s1,1
  while(*path == '/')
    80003980:	0004c783          	lbu	a5,0(s1)
    80003984:	ff278de3          	beq	a5,s2,8000397e <namex+0xa6>
    ilock(ip);
    80003988:	8552                	mv	a0,s4
    8000398a:	903ff0ef          	jal	8000328c <ilock>
    if(ip->type != T_DIR){
    8000398e:	044a1783          	lh	a5,68(s4)
    80003992:	f9779be3          	bne	a5,s7,80003928 <namex+0x50>
    if(nameiparent && *path == '\0'){
    80003996:	000b0563          	beqz	s6,800039a0 <namex+0xc8>
    8000399a:	0004c783          	lbu	a5,0(s1)
    8000399e:	d7dd                	beqz	a5,8000394c <namex+0x74>
    if((next = dirlookup(ip, name, 0)) == 0){
    800039a0:	4601                	li	a2,0
    800039a2:	85d6                	mv	a1,s5
    800039a4:	8552                	mv	a0,s4
    800039a6:	e97ff0ef          	jal	8000383c <dirlookup>
    800039aa:	89aa                	mv	s3,a0
    800039ac:	d545                	beqz	a0,80003954 <namex+0x7c>
    iunlockput(ip);
    800039ae:	8552                	mv	a0,s4
    800039b0:	ae7ff0ef          	jal	80003496 <iunlockput>
    ip = next;
    800039b4:	8a4e                	mv	s4,s3
  while(*path == '/')
    800039b6:	0004c783          	lbu	a5,0(s1)
    800039ba:	01279763          	bne	a5,s2,800039c8 <namex+0xf0>
    path++;
    800039be:	0485                	addi	s1,s1,1
  while(*path == '/')
    800039c0:	0004c783          	lbu	a5,0(s1)
    800039c4:	ff278de3          	beq	a5,s2,800039be <namex+0xe6>
  if(*path == 0)
    800039c8:	cb8d                	beqz	a5,800039fa <namex+0x122>
  while(*path != '/' && *path != 0)
    800039ca:	0004c783          	lbu	a5,0(s1)
    800039ce:	89a6                	mv	s3,s1
  len = path - s;
    800039d0:	4c81                	li	s9,0
    800039d2:	4601                	li	a2,0
  while(*path != '/' && *path != 0)
    800039d4:	01278963          	beq	a5,s2,800039e6 <namex+0x10e>
    800039d8:	d3d9                	beqz	a5,8000395e <namex+0x86>
    path++;
    800039da:	0985                	addi	s3,s3,1
  while(*path != '/' && *path != 0)
    800039dc:	0009c783          	lbu	a5,0(s3)
    800039e0:	ff279ce3          	bne	a5,s2,800039d8 <namex+0x100>
    800039e4:	bfad                	j	8000395e <namex+0x86>
    memmove(name, s, len);
    800039e6:	2601                	sext.w	a2,a2
    800039e8:	85a6                	mv	a1,s1
    800039ea:	8556                	mv	a0,s5
    800039ec:	b12fd0ef          	jal	80000cfe <memmove>
    name[len] = 0;
    800039f0:	9cd6                	add	s9,s9,s5
    800039f2:	000c8023          	sb	zero,0(s9) # 2000 <_entry-0x7fffe000>
    800039f6:	84ce                	mv	s1,s3
    800039f8:	bfbd                	j	80003976 <namex+0x9e>
  if(nameiparent){
    800039fa:	f20b0be3          	beqz	s6,80003930 <namex+0x58>
    iput(ip);
    800039fe:	8552                	mv	a0,s4
    80003a00:	a0fff0ef          	jal	8000340e <iput>
    return 0;
    80003a04:	4a01                	li	s4,0
    80003a06:	b72d                	j	80003930 <namex+0x58>

0000000080003a08 <dirlink>:
{
    80003a08:	7139                	addi	sp,sp,-64
    80003a0a:	fc06                	sd	ra,56(sp)
    80003a0c:	f822                	sd	s0,48(sp)
    80003a0e:	f04a                	sd	s2,32(sp)
    80003a10:	ec4e                	sd	s3,24(sp)
    80003a12:	e852                	sd	s4,16(sp)
    80003a14:	0080                	addi	s0,sp,64
    80003a16:	892a                	mv	s2,a0
    80003a18:	8a2e                	mv	s4,a1
    80003a1a:	89b2                	mv	s3,a2
  if((ip = dirlookup(dp, name, 0)) != 0){
    80003a1c:	4601                	li	a2,0
    80003a1e:	e1fff0ef          	jal	8000383c <dirlookup>
    80003a22:	e535                	bnez	a0,80003a8e <dirlink+0x86>
    80003a24:	f426                	sd	s1,40(sp)
  for(off = 0; off < dp->size; off += sizeof(de)){
    80003a26:	04c92483          	lw	s1,76(s2)
    80003a2a:	c48d                	beqz	s1,80003a54 <dirlink+0x4c>
    80003a2c:	4481                	li	s1,0
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80003a2e:	4741                	li	a4,16
    80003a30:	86a6                	mv	a3,s1
    80003a32:	fc040613          	addi	a2,s0,-64
    80003a36:	4581                	li	a1,0
    80003a38:	854a                	mv	a0,s2
    80003a3a:	be3ff0ef          	jal	8000361c <readi>
    80003a3e:	47c1                	li	a5,16
    80003a40:	04f51b63          	bne	a0,a5,80003a96 <dirlink+0x8e>
    if(de.inum == 0)
    80003a44:	fc045783          	lhu	a5,-64(s0)
    80003a48:	c791                	beqz	a5,80003a54 <dirlink+0x4c>
  for(off = 0; off < dp->size; off += sizeof(de)){
    80003a4a:	24c1                	addiw	s1,s1,16
    80003a4c:	04c92783          	lw	a5,76(s2)
    80003a50:	fcf4efe3          	bltu	s1,a5,80003a2e <dirlink+0x26>
  strncpy(de.name, name, DIRSIZ);
    80003a54:	4639                	li	a2,14
    80003a56:	85d2                	mv	a1,s4
    80003a58:	fc240513          	addi	a0,s0,-62
    80003a5c:	b48fd0ef          	jal	80000da4 <strncpy>
  de.inum = inum;
    80003a60:	fd341023          	sh	s3,-64(s0)
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80003a64:	4741                	li	a4,16
    80003a66:	86a6                	mv	a3,s1
    80003a68:	fc040613          	addi	a2,s0,-64
    80003a6c:	4581                	li	a1,0
    80003a6e:	854a                	mv	a0,s2
    80003a70:	ca9ff0ef          	jal	80003718 <writei>
    80003a74:	1541                	addi	a0,a0,-16
    80003a76:	00a03533          	snez	a0,a0
    80003a7a:	40a00533          	neg	a0,a0
    80003a7e:	74a2                	ld	s1,40(sp)
}
    80003a80:	70e2                	ld	ra,56(sp)
    80003a82:	7442                	ld	s0,48(sp)
    80003a84:	7902                	ld	s2,32(sp)
    80003a86:	69e2                	ld	s3,24(sp)
    80003a88:	6a42                	ld	s4,16(sp)
    80003a8a:	6121                	addi	sp,sp,64
    80003a8c:	8082                	ret
    iput(ip);
    80003a8e:	981ff0ef          	jal	8000340e <iput>
    return -1;
    80003a92:	557d                	li	a0,-1
    80003a94:	b7f5                	j	80003a80 <dirlink+0x78>
      panic("dirlink read");
    80003a96:	00004517          	auipc	a0,0x4
    80003a9a:	a3250513          	addi	a0,a0,-1486 # 800074c8 <etext+0x4c8>
    80003a9e:	d43fc0ef          	jal	800007e0 <panic>

0000000080003aa2 <namei>:

struct inode*
namei(char *path)
{
    80003aa2:	1101                	addi	sp,sp,-32
    80003aa4:	ec06                	sd	ra,24(sp)
    80003aa6:	e822                	sd	s0,16(sp)
    80003aa8:	1000                	addi	s0,sp,32
  char name[DIRSIZ];
  return namex(path, 0, name);
    80003aaa:	fe040613          	addi	a2,s0,-32
    80003aae:	4581                	li	a1,0
    80003ab0:	e29ff0ef          	jal	800038d8 <namex>
}
    80003ab4:	60e2                	ld	ra,24(sp)
    80003ab6:	6442                	ld	s0,16(sp)
    80003ab8:	6105                	addi	sp,sp,32
    80003aba:	8082                	ret

0000000080003abc <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
    80003abc:	1141                	addi	sp,sp,-16
    80003abe:	e406                	sd	ra,8(sp)
    80003ac0:	e022                	sd	s0,0(sp)
    80003ac2:	0800                	addi	s0,sp,16
    80003ac4:	862e                	mv	a2,a1
  return namex(path, 1, name);
    80003ac6:	4585                	li	a1,1
    80003ac8:	e11ff0ef          	jal	800038d8 <namex>
}
    80003acc:	60a2                	ld	ra,8(sp)
    80003ace:	6402                	ld	s0,0(sp)
    80003ad0:	0141                	addi	sp,sp,16
    80003ad2:	8082                	ret

0000000080003ad4 <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
    80003ad4:	1101                	addi	sp,sp,-32
    80003ad6:	ec06                	sd	ra,24(sp)
    80003ad8:	e822                	sd	s0,16(sp)
    80003ada:	e426                	sd	s1,8(sp)
    80003adc:	e04a                	sd	s2,0(sp)
    80003ade:	1000                	addi	s0,sp,32
  struct buf *buf = bread(log.dev, log.start);
    80003ae0:	0001c917          	auipc	s2,0x1c
    80003ae4:	05890913          	addi	s2,s2,88 # 8001fb38 <log>
    80003ae8:	01892583          	lw	a1,24(s2)
    80003aec:	02492503          	lw	a0,36(s2)
    80003af0:	8d0ff0ef          	jal	80002bc0 <bread>
    80003af4:	84aa                	mv	s1,a0
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
    80003af6:	02892603          	lw	a2,40(s2)
    80003afa:	cd30                	sw	a2,88(a0)
  for (i = 0; i < log.lh.n; i++) {
    80003afc:	00c05f63          	blez	a2,80003b1a <write_head+0x46>
    80003b00:	0001c717          	auipc	a4,0x1c
    80003b04:	06470713          	addi	a4,a4,100 # 8001fb64 <log+0x2c>
    80003b08:	87aa                	mv	a5,a0
    80003b0a:	060a                	slli	a2,a2,0x2
    80003b0c:	962a                	add	a2,a2,a0
    hb->block[i] = log.lh.block[i];
    80003b0e:	4314                	lw	a3,0(a4)
    80003b10:	cff4                	sw	a3,92(a5)
  for (i = 0; i < log.lh.n; i++) {
    80003b12:	0711                	addi	a4,a4,4
    80003b14:	0791                	addi	a5,a5,4
    80003b16:	fec79ce3          	bne	a5,a2,80003b0e <write_head+0x3a>
  }
  bwrite(buf);
    80003b1a:	8526                	mv	a0,s1
    80003b1c:	97aff0ef          	jal	80002c96 <bwrite>
  brelse(buf);
    80003b20:	8526                	mv	a0,s1
    80003b22:	9a6ff0ef          	jal	80002cc8 <brelse>
}
    80003b26:	60e2                	ld	ra,24(sp)
    80003b28:	6442                	ld	s0,16(sp)
    80003b2a:	64a2                	ld	s1,8(sp)
    80003b2c:	6902                	ld	s2,0(sp)
    80003b2e:	6105                	addi	sp,sp,32
    80003b30:	8082                	ret

0000000080003b32 <install_trans>:
  for (tail = 0; tail < log.lh.n; tail++) {
    80003b32:	0001c797          	auipc	a5,0x1c
    80003b36:	02e7a783          	lw	a5,46(a5) # 8001fb60 <log+0x28>
    80003b3a:	0af05e63          	blez	a5,80003bf6 <install_trans+0xc4>
{
    80003b3e:	715d                	addi	sp,sp,-80
    80003b40:	e486                	sd	ra,72(sp)
    80003b42:	e0a2                	sd	s0,64(sp)
    80003b44:	fc26                	sd	s1,56(sp)
    80003b46:	f84a                	sd	s2,48(sp)
    80003b48:	f44e                	sd	s3,40(sp)
    80003b4a:	f052                	sd	s4,32(sp)
    80003b4c:	ec56                	sd	s5,24(sp)
    80003b4e:	e85a                	sd	s6,16(sp)
    80003b50:	e45e                	sd	s7,8(sp)
    80003b52:	0880                	addi	s0,sp,80
    80003b54:	8b2a                	mv	s6,a0
    80003b56:	0001ca97          	auipc	s5,0x1c
    80003b5a:	00ea8a93          	addi	s5,s5,14 # 8001fb64 <log+0x2c>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003b5e:	4981                	li	s3,0
      printf("recovering tail %d dst %d\n", tail, log.lh.block[tail]);
    80003b60:	00004b97          	auipc	s7,0x4
    80003b64:	978b8b93          	addi	s7,s7,-1672 # 800074d8 <etext+0x4d8>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80003b68:	0001ca17          	auipc	s4,0x1c
    80003b6c:	fd0a0a13          	addi	s4,s4,-48 # 8001fb38 <log>
    80003b70:	a025                	j	80003b98 <install_trans+0x66>
      printf("recovering tail %d dst %d\n", tail, log.lh.block[tail]);
    80003b72:	000aa603          	lw	a2,0(s5)
    80003b76:	85ce                	mv	a1,s3
    80003b78:	855e                	mv	a0,s7
    80003b7a:	981fc0ef          	jal	800004fa <printf>
    80003b7e:	a839                	j	80003b9c <install_trans+0x6a>
    brelse(lbuf);
    80003b80:	854a                	mv	a0,s2
    80003b82:	946ff0ef          	jal	80002cc8 <brelse>
    brelse(dbuf);
    80003b86:	8526                	mv	a0,s1
    80003b88:	940ff0ef          	jal	80002cc8 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003b8c:	2985                	addiw	s3,s3,1
    80003b8e:	0a91                	addi	s5,s5,4
    80003b90:	028a2783          	lw	a5,40(s4)
    80003b94:	04f9d663          	bge	s3,a5,80003be0 <install_trans+0xae>
    if(recovering) {
    80003b98:	fc0b1de3          	bnez	s6,80003b72 <install_trans+0x40>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80003b9c:	018a2583          	lw	a1,24(s4)
    80003ba0:	013585bb          	addw	a1,a1,s3
    80003ba4:	2585                	addiw	a1,a1,1
    80003ba6:	024a2503          	lw	a0,36(s4)
    80003baa:	816ff0ef          	jal	80002bc0 <bread>
    80003bae:	892a                	mv	s2,a0
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
    80003bb0:	000aa583          	lw	a1,0(s5)
    80003bb4:	024a2503          	lw	a0,36(s4)
    80003bb8:	808ff0ef          	jal	80002bc0 <bread>
    80003bbc:	84aa                	mv	s1,a0
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    80003bbe:	40000613          	li	a2,1024
    80003bc2:	05890593          	addi	a1,s2,88
    80003bc6:	05850513          	addi	a0,a0,88
    80003bca:	934fd0ef          	jal	80000cfe <memmove>
    bwrite(dbuf);  // write dst to disk
    80003bce:	8526                	mv	a0,s1
    80003bd0:	8c6ff0ef          	jal	80002c96 <bwrite>
    if(recovering == 0)
    80003bd4:	fa0b16e3          	bnez	s6,80003b80 <install_trans+0x4e>
      bunpin(dbuf);
    80003bd8:	8526                	mv	a0,s1
    80003bda:	9aaff0ef          	jal	80002d84 <bunpin>
    80003bde:	b74d                	j	80003b80 <install_trans+0x4e>
}
    80003be0:	60a6                	ld	ra,72(sp)
    80003be2:	6406                	ld	s0,64(sp)
    80003be4:	74e2                	ld	s1,56(sp)
    80003be6:	7942                	ld	s2,48(sp)
    80003be8:	79a2                	ld	s3,40(sp)
    80003bea:	7a02                	ld	s4,32(sp)
    80003bec:	6ae2                	ld	s5,24(sp)
    80003bee:	6b42                	ld	s6,16(sp)
    80003bf0:	6ba2                	ld	s7,8(sp)
    80003bf2:	6161                	addi	sp,sp,80
    80003bf4:	8082                	ret
    80003bf6:	8082                	ret

0000000080003bf8 <initlog>:
{
    80003bf8:	7179                	addi	sp,sp,-48
    80003bfa:	f406                	sd	ra,40(sp)
    80003bfc:	f022                	sd	s0,32(sp)
    80003bfe:	ec26                	sd	s1,24(sp)
    80003c00:	e84a                	sd	s2,16(sp)
    80003c02:	e44e                	sd	s3,8(sp)
    80003c04:	1800                	addi	s0,sp,48
    80003c06:	892a                	mv	s2,a0
    80003c08:	89ae                	mv	s3,a1
  initlock(&log.lock, "log");
    80003c0a:	0001c497          	auipc	s1,0x1c
    80003c0e:	f2e48493          	addi	s1,s1,-210 # 8001fb38 <log>
    80003c12:	00004597          	auipc	a1,0x4
    80003c16:	8e658593          	addi	a1,a1,-1818 # 800074f8 <etext+0x4f8>
    80003c1a:	8526                	mv	a0,s1
    80003c1c:	f33fc0ef          	jal	80000b4e <initlock>
  log.start = sb->logstart;
    80003c20:	0149a583          	lw	a1,20(s3)
    80003c24:	cc8c                	sw	a1,24(s1)
  log.dev = dev;
    80003c26:	0324a223          	sw	s2,36(s1)
  struct buf *buf = bread(log.dev, log.start);
    80003c2a:	854a                	mv	a0,s2
    80003c2c:	f95fe0ef          	jal	80002bc0 <bread>
  log.lh.n = lh->n;
    80003c30:	4d30                	lw	a2,88(a0)
    80003c32:	d490                	sw	a2,40(s1)
  for (i = 0; i < log.lh.n; i++) {
    80003c34:	00c05f63          	blez	a2,80003c52 <initlog+0x5a>
    80003c38:	87aa                	mv	a5,a0
    80003c3a:	0001c717          	auipc	a4,0x1c
    80003c3e:	f2a70713          	addi	a4,a4,-214 # 8001fb64 <log+0x2c>
    80003c42:	060a                	slli	a2,a2,0x2
    80003c44:	962a                	add	a2,a2,a0
    log.lh.block[i] = lh->block[i];
    80003c46:	4ff4                	lw	a3,92(a5)
    80003c48:	c314                	sw	a3,0(a4)
  for (i = 0; i < log.lh.n; i++) {
    80003c4a:	0791                	addi	a5,a5,4
    80003c4c:	0711                	addi	a4,a4,4
    80003c4e:	fec79ce3          	bne	a5,a2,80003c46 <initlog+0x4e>
  brelse(buf);
    80003c52:	876ff0ef          	jal	80002cc8 <brelse>

static void
recover_from_log(void)
{
  read_head();
  install_trans(1); // if committed, copy from log to disk
    80003c56:	4505                	li	a0,1
    80003c58:	edbff0ef          	jal	80003b32 <install_trans>
  log.lh.n = 0;
    80003c5c:	0001c797          	auipc	a5,0x1c
    80003c60:	f007a223          	sw	zero,-252(a5) # 8001fb60 <log+0x28>
  write_head(); // clear the log
    80003c64:	e71ff0ef          	jal	80003ad4 <write_head>
}
    80003c68:	70a2                	ld	ra,40(sp)
    80003c6a:	7402                	ld	s0,32(sp)
    80003c6c:	64e2                	ld	s1,24(sp)
    80003c6e:	6942                	ld	s2,16(sp)
    80003c70:	69a2                	ld	s3,8(sp)
    80003c72:	6145                	addi	sp,sp,48
    80003c74:	8082                	ret

0000000080003c76 <begin_op>:
}

// called at the start of each FS system call.
void
begin_op(void)
{
    80003c76:	1101                	addi	sp,sp,-32
    80003c78:	ec06                	sd	ra,24(sp)
    80003c7a:	e822                	sd	s0,16(sp)
    80003c7c:	e426                	sd	s1,8(sp)
    80003c7e:	e04a                	sd	s2,0(sp)
    80003c80:	1000                	addi	s0,sp,32
  acquire(&log.lock);
    80003c82:	0001c517          	auipc	a0,0x1c
    80003c86:	eb650513          	addi	a0,a0,-330 # 8001fb38 <log>
    80003c8a:	f45fc0ef          	jal	80000bce <acquire>
  while(1){
    if(log.committing){
    80003c8e:	0001c497          	auipc	s1,0x1c
    80003c92:	eaa48493          	addi	s1,s1,-342 # 8001fb38 <log>
      sleep(&log, &log.lock);
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGBLOCKS){
    80003c96:	4979                	li	s2,30
    80003c98:	a029                	j	80003ca2 <begin_op+0x2c>
      sleep(&log, &log.lock);
    80003c9a:	85a6                	mv	a1,s1
    80003c9c:	8526                	mv	a0,s1
    80003c9e:	a8efe0ef          	jal	80001f2c <sleep>
    if(log.committing){
    80003ca2:	509c                	lw	a5,32(s1)
    80003ca4:	fbfd                	bnez	a5,80003c9a <begin_op+0x24>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGBLOCKS){
    80003ca6:	4cd8                	lw	a4,28(s1)
    80003ca8:	2705                	addiw	a4,a4,1
    80003caa:	0027179b          	slliw	a5,a4,0x2
    80003cae:	9fb9                	addw	a5,a5,a4
    80003cb0:	0017979b          	slliw	a5,a5,0x1
    80003cb4:	5494                	lw	a3,40(s1)
    80003cb6:	9fb5                	addw	a5,a5,a3
    80003cb8:	00f95763          	bge	s2,a5,80003cc6 <begin_op+0x50>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
    80003cbc:	85a6                	mv	a1,s1
    80003cbe:	8526                	mv	a0,s1
    80003cc0:	a6cfe0ef          	jal	80001f2c <sleep>
    80003cc4:	bff9                	j	80003ca2 <begin_op+0x2c>
    } else {
      log.outstanding += 1;
    80003cc6:	0001c517          	auipc	a0,0x1c
    80003cca:	e7250513          	addi	a0,a0,-398 # 8001fb38 <log>
    80003cce:	cd58                	sw	a4,28(a0)
      release(&log.lock);
    80003cd0:	f97fc0ef          	jal	80000c66 <release>
      break;
    }
  }
}
    80003cd4:	60e2                	ld	ra,24(sp)
    80003cd6:	6442                	ld	s0,16(sp)
    80003cd8:	64a2                	ld	s1,8(sp)
    80003cda:	6902                	ld	s2,0(sp)
    80003cdc:	6105                	addi	sp,sp,32
    80003cde:	8082                	ret

0000000080003ce0 <end_op>:

// called at the end of each FS system call.
// commits if this was the last outstanding operation.
void
end_op(void)
{
    80003ce0:	7139                	addi	sp,sp,-64
    80003ce2:	fc06                	sd	ra,56(sp)
    80003ce4:	f822                	sd	s0,48(sp)
    80003ce6:	f426                	sd	s1,40(sp)
    80003ce8:	f04a                	sd	s2,32(sp)
    80003cea:	0080                	addi	s0,sp,64
  int do_commit = 0;

  acquire(&log.lock);
    80003cec:	0001c497          	auipc	s1,0x1c
    80003cf0:	e4c48493          	addi	s1,s1,-436 # 8001fb38 <log>
    80003cf4:	8526                	mv	a0,s1
    80003cf6:	ed9fc0ef          	jal	80000bce <acquire>
  log.outstanding -= 1;
    80003cfa:	4cdc                	lw	a5,28(s1)
    80003cfc:	37fd                	addiw	a5,a5,-1
    80003cfe:	0007891b          	sext.w	s2,a5
    80003d02:	ccdc                	sw	a5,28(s1)
  if(log.committing)
    80003d04:	509c                	lw	a5,32(s1)
    80003d06:	ef9d                	bnez	a5,80003d44 <end_op+0x64>
    panic("log.committing");
  if(log.outstanding == 0){
    80003d08:	04091763          	bnez	s2,80003d56 <end_op+0x76>
    do_commit = 1;
    log.committing = 1;
    80003d0c:	0001c497          	auipc	s1,0x1c
    80003d10:	e2c48493          	addi	s1,s1,-468 # 8001fb38 <log>
    80003d14:	4785                	li	a5,1
    80003d16:	d09c                	sw	a5,32(s1)
    // begin_op() may be waiting for log space,
    // and decrementing log.outstanding has decreased
    // the amount of reserved space.
    wakeup(&log);
  }
  release(&log.lock);
    80003d18:	8526                	mv	a0,s1
    80003d1a:	f4dfc0ef          	jal	80000c66 <release>
}

static void
commit()
{
  if (log.lh.n > 0) {
    80003d1e:	549c                	lw	a5,40(s1)
    80003d20:	04f04b63          	bgtz	a5,80003d76 <end_op+0x96>
    acquire(&log.lock);
    80003d24:	0001c497          	auipc	s1,0x1c
    80003d28:	e1448493          	addi	s1,s1,-492 # 8001fb38 <log>
    80003d2c:	8526                	mv	a0,s1
    80003d2e:	ea1fc0ef          	jal	80000bce <acquire>
    log.committing = 0;
    80003d32:	0204a023          	sw	zero,32(s1)
    wakeup(&log);
    80003d36:	8526                	mv	a0,s1
    80003d38:	a40fe0ef          	jal	80001f78 <wakeup>
    release(&log.lock);
    80003d3c:	8526                	mv	a0,s1
    80003d3e:	f29fc0ef          	jal	80000c66 <release>
}
    80003d42:	a025                	j	80003d6a <end_op+0x8a>
    80003d44:	ec4e                	sd	s3,24(sp)
    80003d46:	e852                	sd	s4,16(sp)
    80003d48:	e456                	sd	s5,8(sp)
    panic("log.committing");
    80003d4a:	00003517          	auipc	a0,0x3
    80003d4e:	7b650513          	addi	a0,a0,1974 # 80007500 <etext+0x500>
    80003d52:	a8ffc0ef          	jal	800007e0 <panic>
    wakeup(&log);
    80003d56:	0001c497          	auipc	s1,0x1c
    80003d5a:	de248493          	addi	s1,s1,-542 # 8001fb38 <log>
    80003d5e:	8526                	mv	a0,s1
    80003d60:	a18fe0ef          	jal	80001f78 <wakeup>
  release(&log.lock);
    80003d64:	8526                	mv	a0,s1
    80003d66:	f01fc0ef          	jal	80000c66 <release>
}
    80003d6a:	70e2                	ld	ra,56(sp)
    80003d6c:	7442                	ld	s0,48(sp)
    80003d6e:	74a2                	ld	s1,40(sp)
    80003d70:	7902                	ld	s2,32(sp)
    80003d72:	6121                	addi	sp,sp,64
    80003d74:	8082                	ret
    80003d76:	ec4e                	sd	s3,24(sp)
    80003d78:	e852                	sd	s4,16(sp)
    80003d7a:	e456                	sd	s5,8(sp)
  for (tail = 0; tail < log.lh.n; tail++) {
    80003d7c:	0001ca97          	auipc	s5,0x1c
    80003d80:	de8a8a93          	addi	s5,s5,-536 # 8001fb64 <log+0x2c>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
    80003d84:	0001ca17          	auipc	s4,0x1c
    80003d88:	db4a0a13          	addi	s4,s4,-588 # 8001fb38 <log>
    80003d8c:	018a2583          	lw	a1,24(s4)
    80003d90:	012585bb          	addw	a1,a1,s2
    80003d94:	2585                	addiw	a1,a1,1
    80003d96:	024a2503          	lw	a0,36(s4)
    80003d9a:	e27fe0ef          	jal	80002bc0 <bread>
    80003d9e:	84aa                	mv	s1,a0
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
    80003da0:	000aa583          	lw	a1,0(s5)
    80003da4:	024a2503          	lw	a0,36(s4)
    80003da8:	e19fe0ef          	jal	80002bc0 <bread>
    80003dac:	89aa                	mv	s3,a0
    memmove(to->data, from->data, BSIZE);
    80003dae:	40000613          	li	a2,1024
    80003db2:	05850593          	addi	a1,a0,88
    80003db6:	05848513          	addi	a0,s1,88
    80003dba:	f45fc0ef          	jal	80000cfe <memmove>
    bwrite(to);  // write the log
    80003dbe:	8526                	mv	a0,s1
    80003dc0:	ed7fe0ef          	jal	80002c96 <bwrite>
    brelse(from);
    80003dc4:	854e                	mv	a0,s3
    80003dc6:	f03fe0ef          	jal	80002cc8 <brelse>
    brelse(to);
    80003dca:	8526                	mv	a0,s1
    80003dcc:	efdfe0ef          	jal	80002cc8 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003dd0:	2905                	addiw	s2,s2,1
    80003dd2:	0a91                	addi	s5,s5,4
    80003dd4:	028a2783          	lw	a5,40(s4)
    80003dd8:	faf94ae3          	blt	s2,a5,80003d8c <end_op+0xac>
    write_log();     // Write modified blocks from cache to log
    write_head();    // Write header to disk -- the real commit
    80003ddc:	cf9ff0ef          	jal	80003ad4 <write_head>
    install_trans(0); // Now install writes to home locations
    80003de0:	4501                	li	a0,0
    80003de2:	d51ff0ef          	jal	80003b32 <install_trans>
    log.lh.n = 0;
    80003de6:	0001c797          	auipc	a5,0x1c
    80003dea:	d607ad23          	sw	zero,-646(a5) # 8001fb60 <log+0x28>
    write_head();    // Erase the transaction from the log
    80003dee:	ce7ff0ef          	jal	80003ad4 <write_head>
    80003df2:	69e2                	ld	s3,24(sp)
    80003df4:	6a42                	ld	s4,16(sp)
    80003df6:	6aa2                	ld	s5,8(sp)
    80003df8:	b735                	j	80003d24 <end_op+0x44>

0000000080003dfa <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
    80003dfa:	1101                	addi	sp,sp,-32
    80003dfc:	ec06                	sd	ra,24(sp)
    80003dfe:	e822                	sd	s0,16(sp)
    80003e00:	e426                	sd	s1,8(sp)
    80003e02:	e04a                	sd	s2,0(sp)
    80003e04:	1000                	addi	s0,sp,32
    80003e06:	84aa                	mv	s1,a0
  int i;

  acquire(&log.lock);
    80003e08:	0001c917          	auipc	s2,0x1c
    80003e0c:	d3090913          	addi	s2,s2,-720 # 8001fb38 <log>
    80003e10:	854a                	mv	a0,s2
    80003e12:	dbdfc0ef          	jal	80000bce <acquire>
  if (log.lh.n >= LOGBLOCKS)
    80003e16:	02892603          	lw	a2,40(s2)
    80003e1a:	47f5                	li	a5,29
    80003e1c:	04c7cc63          	blt	a5,a2,80003e74 <log_write+0x7a>
    panic("too big a transaction");
  if (log.outstanding < 1)
    80003e20:	0001c797          	auipc	a5,0x1c
    80003e24:	d347a783          	lw	a5,-716(a5) # 8001fb54 <log+0x1c>
    80003e28:	04f05c63          	blez	a5,80003e80 <log_write+0x86>
    panic("log_write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
    80003e2c:	4781                	li	a5,0
    80003e2e:	04c05f63          	blez	a2,80003e8c <log_write+0x92>
    if (log.lh.block[i] == b->blockno)   // log absorption
    80003e32:	44cc                	lw	a1,12(s1)
    80003e34:	0001c717          	auipc	a4,0x1c
    80003e38:	d3070713          	addi	a4,a4,-720 # 8001fb64 <log+0x2c>
  for (i = 0; i < log.lh.n; i++) {
    80003e3c:	4781                	li	a5,0
    if (log.lh.block[i] == b->blockno)   // log absorption
    80003e3e:	4314                	lw	a3,0(a4)
    80003e40:	04b68663          	beq	a3,a1,80003e8c <log_write+0x92>
  for (i = 0; i < log.lh.n; i++) {
    80003e44:	2785                	addiw	a5,a5,1
    80003e46:	0711                	addi	a4,a4,4
    80003e48:	fef61be3          	bne	a2,a5,80003e3e <log_write+0x44>
      break;
  }
  log.lh.block[i] = b->blockno;
    80003e4c:	0621                	addi	a2,a2,8
    80003e4e:	060a                	slli	a2,a2,0x2
    80003e50:	0001c797          	auipc	a5,0x1c
    80003e54:	ce878793          	addi	a5,a5,-792 # 8001fb38 <log>
    80003e58:	97b2                	add	a5,a5,a2
    80003e5a:	44d8                	lw	a4,12(s1)
    80003e5c:	c7d8                	sw	a4,12(a5)
  if (i == log.lh.n) {  // Add new block to log?
    bpin(b);
    80003e5e:	8526                	mv	a0,s1
    80003e60:	ef1fe0ef          	jal	80002d50 <bpin>
    log.lh.n++;
    80003e64:	0001c717          	auipc	a4,0x1c
    80003e68:	cd470713          	addi	a4,a4,-812 # 8001fb38 <log>
    80003e6c:	571c                	lw	a5,40(a4)
    80003e6e:	2785                	addiw	a5,a5,1
    80003e70:	d71c                	sw	a5,40(a4)
    80003e72:	a80d                	j	80003ea4 <log_write+0xaa>
    panic("too big a transaction");
    80003e74:	00003517          	auipc	a0,0x3
    80003e78:	69c50513          	addi	a0,a0,1692 # 80007510 <etext+0x510>
    80003e7c:	965fc0ef          	jal	800007e0 <panic>
    panic("log_write outside of trans");
    80003e80:	00003517          	auipc	a0,0x3
    80003e84:	6a850513          	addi	a0,a0,1704 # 80007528 <etext+0x528>
    80003e88:	959fc0ef          	jal	800007e0 <panic>
  log.lh.block[i] = b->blockno;
    80003e8c:	00878693          	addi	a3,a5,8
    80003e90:	068a                	slli	a3,a3,0x2
    80003e92:	0001c717          	auipc	a4,0x1c
    80003e96:	ca670713          	addi	a4,a4,-858 # 8001fb38 <log>
    80003e9a:	9736                	add	a4,a4,a3
    80003e9c:	44d4                	lw	a3,12(s1)
    80003e9e:	c754                	sw	a3,12(a4)
  if (i == log.lh.n) {  // Add new block to log?
    80003ea0:	faf60fe3          	beq	a2,a5,80003e5e <log_write+0x64>
  }
  release(&log.lock);
    80003ea4:	0001c517          	auipc	a0,0x1c
    80003ea8:	c9450513          	addi	a0,a0,-876 # 8001fb38 <log>
    80003eac:	dbbfc0ef          	jal	80000c66 <release>
}
    80003eb0:	60e2                	ld	ra,24(sp)
    80003eb2:	6442                	ld	s0,16(sp)
    80003eb4:	64a2                	ld	s1,8(sp)
    80003eb6:	6902                	ld	s2,0(sp)
    80003eb8:	6105                	addi	sp,sp,32
    80003eba:	8082                	ret

0000000080003ebc <initsleeplock>:
#include "proc.h"
#include "sleeplock.h"

void
initsleeplock(struct sleeplock *lk, char *name)
{
    80003ebc:	1101                	addi	sp,sp,-32
    80003ebe:	ec06                	sd	ra,24(sp)
    80003ec0:	e822                	sd	s0,16(sp)
    80003ec2:	e426                	sd	s1,8(sp)
    80003ec4:	e04a                	sd	s2,0(sp)
    80003ec6:	1000                	addi	s0,sp,32
    80003ec8:	84aa                	mv	s1,a0
    80003eca:	892e                	mv	s2,a1
  initlock(&lk->lk, "sleep lock");
    80003ecc:	00003597          	auipc	a1,0x3
    80003ed0:	67c58593          	addi	a1,a1,1660 # 80007548 <etext+0x548>
    80003ed4:	0521                	addi	a0,a0,8
    80003ed6:	c79fc0ef          	jal	80000b4e <initlock>
  lk->name = name;
    80003eda:	0324b023          	sd	s2,32(s1)
  lk->locked = 0;
    80003ede:	0004a023          	sw	zero,0(s1)
  lk->pid = 0;
    80003ee2:	0204a423          	sw	zero,40(s1)
}
    80003ee6:	60e2                	ld	ra,24(sp)
    80003ee8:	6442                	ld	s0,16(sp)
    80003eea:	64a2                	ld	s1,8(sp)
    80003eec:	6902                	ld	s2,0(sp)
    80003eee:	6105                	addi	sp,sp,32
    80003ef0:	8082                	ret

0000000080003ef2 <acquiresleep>:

void
acquiresleep(struct sleeplock *lk)
{
    80003ef2:	1101                	addi	sp,sp,-32
    80003ef4:	ec06                	sd	ra,24(sp)
    80003ef6:	e822                	sd	s0,16(sp)
    80003ef8:	e426                	sd	s1,8(sp)
    80003efa:	e04a                	sd	s2,0(sp)
    80003efc:	1000                	addi	s0,sp,32
    80003efe:	84aa                	mv	s1,a0
  acquire(&lk->lk);
    80003f00:	00850913          	addi	s2,a0,8
    80003f04:	854a                	mv	a0,s2
    80003f06:	cc9fc0ef          	jal	80000bce <acquire>
  while (lk->locked) {
    80003f0a:	409c                	lw	a5,0(s1)
    80003f0c:	c799                	beqz	a5,80003f1a <acquiresleep+0x28>
    sleep(lk, &lk->lk);
    80003f0e:	85ca                	mv	a1,s2
    80003f10:	8526                	mv	a0,s1
    80003f12:	81afe0ef          	jal	80001f2c <sleep>
  while (lk->locked) {
    80003f16:	409c                	lw	a5,0(s1)
    80003f18:	fbfd                	bnez	a5,80003f0e <acquiresleep+0x1c>
  }
  lk->locked = 1;
    80003f1a:	4785                	li	a5,1
    80003f1c:	c09c                	sw	a5,0(s1)
  lk->pid = myproc()->pid;
    80003f1e:	a13fd0ef          	jal	80001930 <myproc>
    80003f22:	591c                	lw	a5,48(a0)
    80003f24:	d49c                	sw	a5,40(s1)
  release(&lk->lk);
    80003f26:	854a                	mv	a0,s2
    80003f28:	d3ffc0ef          	jal	80000c66 <release>
}
    80003f2c:	60e2                	ld	ra,24(sp)
    80003f2e:	6442                	ld	s0,16(sp)
    80003f30:	64a2                	ld	s1,8(sp)
    80003f32:	6902                	ld	s2,0(sp)
    80003f34:	6105                	addi	sp,sp,32
    80003f36:	8082                	ret

0000000080003f38 <releasesleep>:

void
releasesleep(struct sleeplock *lk)
{
    80003f38:	1101                	addi	sp,sp,-32
    80003f3a:	ec06                	sd	ra,24(sp)
    80003f3c:	e822                	sd	s0,16(sp)
    80003f3e:	e426                	sd	s1,8(sp)
    80003f40:	e04a                	sd	s2,0(sp)
    80003f42:	1000                	addi	s0,sp,32
    80003f44:	84aa                	mv	s1,a0
  acquire(&lk->lk);
    80003f46:	00850913          	addi	s2,a0,8
    80003f4a:	854a                	mv	a0,s2
    80003f4c:	c83fc0ef          	jal	80000bce <acquire>
  lk->locked = 0;
    80003f50:	0004a023          	sw	zero,0(s1)
  lk->pid = 0;
    80003f54:	0204a423          	sw	zero,40(s1)
  wakeup(lk);
    80003f58:	8526                	mv	a0,s1
    80003f5a:	81efe0ef          	jal	80001f78 <wakeup>
  release(&lk->lk);
    80003f5e:	854a                	mv	a0,s2
    80003f60:	d07fc0ef          	jal	80000c66 <release>
}
    80003f64:	60e2                	ld	ra,24(sp)
    80003f66:	6442                	ld	s0,16(sp)
    80003f68:	64a2                	ld	s1,8(sp)
    80003f6a:	6902                	ld	s2,0(sp)
    80003f6c:	6105                	addi	sp,sp,32
    80003f6e:	8082                	ret

0000000080003f70 <holdingsleep>:

int
holdingsleep(struct sleeplock *lk)
{
    80003f70:	7179                	addi	sp,sp,-48
    80003f72:	f406                	sd	ra,40(sp)
    80003f74:	f022                	sd	s0,32(sp)
    80003f76:	ec26                	sd	s1,24(sp)
    80003f78:	e84a                	sd	s2,16(sp)
    80003f7a:	1800                	addi	s0,sp,48
    80003f7c:	84aa                	mv	s1,a0
  int r;
  
  acquire(&lk->lk);
    80003f7e:	00850913          	addi	s2,a0,8
    80003f82:	854a                	mv	a0,s2
    80003f84:	c4bfc0ef          	jal	80000bce <acquire>
  r = lk->locked && (lk->pid == myproc()->pid);
    80003f88:	409c                	lw	a5,0(s1)
    80003f8a:	ef81                	bnez	a5,80003fa2 <holdingsleep+0x32>
    80003f8c:	4481                	li	s1,0
  release(&lk->lk);
    80003f8e:	854a                	mv	a0,s2
    80003f90:	cd7fc0ef          	jal	80000c66 <release>
  return r;
}
    80003f94:	8526                	mv	a0,s1
    80003f96:	70a2                	ld	ra,40(sp)
    80003f98:	7402                	ld	s0,32(sp)
    80003f9a:	64e2                	ld	s1,24(sp)
    80003f9c:	6942                	ld	s2,16(sp)
    80003f9e:	6145                	addi	sp,sp,48
    80003fa0:	8082                	ret
    80003fa2:	e44e                	sd	s3,8(sp)
  r = lk->locked && (lk->pid == myproc()->pid);
    80003fa4:	0284a983          	lw	s3,40(s1)
    80003fa8:	989fd0ef          	jal	80001930 <myproc>
    80003fac:	5904                	lw	s1,48(a0)
    80003fae:	413484b3          	sub	s1,s1,s3
    80003fb2:	0014b493          	seqz	s1,s1
    80003fb6:	69a2                	ld	s3,8(sp)
    80003fb8:	bfd9                	j	80003f8e <holdingsleep+0x1e>

0000000080003fba <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
    80003fba:	1141                	addi	sp,sp,-16
    80003fbc:	e406                	sd	ra,8(sp)
    80003fbe:	e022                	sd	s0,0(sp)
    80003fc0:	0800                	addi	s0,sp,16
  initlock(&ftable.lock, "ftable");
    80003fc2:	00003597          	auipc	a1,0x3
    80003fc6:	59658593          	addi	a1,a1,1430 # 80007558 <etext+0x558>
    80003fca:	0001c517          	auipc	a0,0x1c
    80003fce:	cb650513          	addi	a0,a0,-842 # 8001fc80 <ftable>
    80003fd2:	b7dfc0ef          	jal	80000b4e <initlock>
}
    80003fd6:	60a2                	ld	ra,8(sp)
    80003fd8:	6402                	ld	s0,0(sp)
    80003fda:	0141                	addi	sp,sp,16
    80003fdc:	8082                	ret

0000000080003fde <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
    80003fde:	1101                	addi	sp,sp,-32
    80003fe0:	ec06                	sd	ra,24(sp)
    80003fe2:	e822                	sd	s0,16(sp)
    80003fe4:	e426                	sd	s1,8(sp)
    80003fe6:	1000                	addi	s0,sp,32
  struct file *f;

  acquire(&ftable.lock);
    80003fe8:	0001c517          	auipc	a0,0x1c
    80003fec:	c9850513          	addi	a0,a0,-872 # 8001fc80 <ftable>
    80003ff0:	bdffc0ef          	jal	80000bce <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    80003ff4:	0001c497          	auipc	s1,0x1c
    80003ff8:	ca448493          	addi	s1,s1,-860 # 8001fc98 <ftable+0x18>
    80003ffc:	0001d717          	auipc	a4,0x1d
    80004000:	c3c70713          	addi	a4,a4,-964 # 80020c38 <disk>
    if(f->ref == 0){
    80004004:	40dc                	lw	a5,4(s1)
    80004006:	cf89                	beqz	a5,80004020 <filealloc+0x42>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    80004008:	02848493          	addi	s1,s1,40
    8000400c:	fee49ce3          	bne	s1,a4,80004004 <filealloc+0x26>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
    80004010:	0001c517          	auipc	a0,0x1c
    80004014:	c7050513          	addi	a0,a0,-912 # 8001fc80 <ftable>
    80004018:	c4ffc0ef          	jal	80000c66 <release>
  return 0;
    8000401c:	4481                	li	s1,0
    8000401e:	a809                	j	80004030 <filealloc+0x52>
      f->ref = 1;
    80004020:	4785                	li	a5,1
    80004022:	c0dc                	sw	a5,4(s1)
      release(&ftable.lock);
    80004024:	0001c517          	auipc	a0,0x1c
    80004028:	c5c50513          	addi	a0,a0,-932 # 8001fc80 <ftable>
    8000402c:	c3bfc0ef          	jal	80000c66 <release>
}
    80004030:	8526                	mv	a0,s1
    80004032:	60e2                	ld	ra,24(sp)
    80004034:	6442                	ld	s0,16(sp)
    80004036:	64a2                	ld	s1,8(sp)
    80004038:	6105                	addi	sp,sp,32
    8000403a:	8082                	ret

000000008000403c <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
    8000403c:	1101                	addi	sp,sp,-32
    8000403e:	ec06                	sd	ra,24(sp)
    80004040:	e822                	sd	s0,16(sp)
    80004042:	e426                	sd	s1,8(sp)
    80004044:	1000                	addi	s0,sp,32
    80004046:	84aa                	mv	s1,a0
  acquire(&ftable.lock);
    80004048:	0001c517          	auipc	a0,0x1c
    8000404c:	c3850513          	addi	a0,a0,-968 # 8001fc80 <ftable>
    80004050:	b7ffc0ef          	jal	80000bce <acquire>
  if(f->ref < 1)
    80004054:	40dc                	lw	a5,4(s1)
    80004056:	02f05063          	blez	a5,80004076 <filedup+0x3a>
    panic("filedup");
  f->ref++;
    8000405a:	2785                	addiw	a5,a5,1
    8000405c:	c0dc                	sw	a5,4(s1)
  release(&ftable.lock);
    8000405e:	0001c517          	auipc	a0,0x1c
    80004062:	c2250513          	addi	a0,a0,-990 # 8001fc80 <ftable>
    80004066:	c01fc0ef          	jal	80000c66 <release>
  return f;
}
    8000406a:	8526                	mv	a0,s1
    8000406c:	60e2                	ld	ra,24(sp)
    8000406e:	6442                	ld	s0,16(sp)
    80004070:	64a2                	ld	s1,8(sp)
    80004072:	6105                	addi	sp,sp,32
    80004074:	8082                	ret
    panic("filedup");
    80004076:	00003517          	auipc	a0,0x3
    8000407a:	4ea50513          	addi	a0,a0,1258 # 80007560 <etext+0x560>
    8000407e:	f62fc0ef          	jal	800007e0 <panic>

0000000080004082 <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
    80004082:	7139                	addi	sp,sp,-64
    80004084:	fc06                	sd	ra,56(sp)
    80004086:	f822                	sd	s0,48(sp)
    80004088:	f426                	sd	s1,40(sp)
    8000408a:	0080                	addi	s0,sp,64
    8000408c:	84aa                	mv	s1,a0
  struct file ff;

  acquire(&ftable.lock);
    8000408e:	0001c517          	auipc	a0,0x1c
    80004092:	bf250513          	addi	a0,a0,-1038 # 8001fc80 <ftable>
    80004096:	b39fc0ef          	jal	80000bce <acquire>
  if(f->ref < 1)
    8000409a:	40dc                	lw	a5,4(s1)
    8000409c:	04f05a63          	blez	a5,800040f0 <fileclose+0x6e>
    panic("fileclose");
  if(--f->ref > 0){
    800040a0:	37fd                	addiw	a5,a5,-1
    800040a2:	0007871b          	sext.w	a4,a5
    800040a6:	c0dc                	sw	a5,4(s1)
    800040a8:	04e04e63          	bgtz	a4,80004104 <fileclose+0x82>
    800040ac:	f04a                	sd	s2,32(sp)
    800040ae:	ec4e                	sd	s3,24(sp)
    800040b0:	e852                	sd	s4,16(sp)
    800040b2:	e456                	sd	s5,8(sp)
    release(&ftable.lock);
    return;
  }
  ff = *f;
    800040b4:	0004a903          	lw	s2,0(s1)
    800040b8:	0094ca83          	lbu	s5,9(s1)
    800040bc:	0104ba03          	ld	s4,16(s1)
    800040c0:	0184b983          	ld	s3,24(s1)
  f->ref = 0;
    800040c4:	0004a223          	sw	zero,4(s1)
  f->type = FD_NONE;
    800040c8:	0004a023          	sw	zero,0(s1)
  release(&ftable.lock);
    800040cc:	0001c517          	auipc	a0,0x1c
    800040d0:	bb450513          	addi	a0,a0,-1100 # 8001fc80 <ftable>
    800040d4:	b93fc0ef          	jal	80000c66 <release>

  if(ff.type == FD_PIPE){
    800040d8:	4785                	li	a5,1
    800040da:	04f90063          	beq	s2,a5,8000411a <fileclose+0x98>
    pipeclose(ff.pipe, ff.writable);
  } else if(ff.type == FD_INODE || ff.type == FD_DEVICE){
    800040de:	3979                	addiw	s2,s2,-2
    800040e0:	4785                	li	a5,1
    800040e2:	0527f563          	bgeu	a5,s2,8000412c <fileclose+0xaa>
    800040e6:	7902                	ld	s2,32(sp)
    800040e8:	69e2                	ld	s3,24(sp)
    800040ea:	6a42                	ld	s4,16(sp)
    800040ec:	6aa2                	ld	s5,8(sp)
    800040ee:	a00d                	j	80004110 <fileclose+0x8e>
    800040f0:	f04a                	sd	s2,32(sp)
    800040f2:	ec4e                	sd	s3,24(sp)
    800040f4:	e852                	sd	s4,16(sp)
    800040f6:	e456                	sd	s5,8(sp)
    panic("fileclose");
    800040f8:	00003517          	auipc	a0,0x3
    800040fc:	47050513          	addi	a0,a0,1136 # 80007568 <etext+0x568>
    80004100:	ee0fc0ef          	jal	800007e0 <panic>
    release(&ftable.lock);
    80004104:	0001c517          	auipc	a0,0x1c
    80004108:	b7c50513          	addi	a0,a0,-1156 # 8001fc80 <ftable>
    8000410c:	b5bfc0ef          	jal	80000c66 <release>
    begin_op();
    iput(ff.ip);
    end_op();
  }
}
    80004110:	70e2                	ld	ra,56(sp)
    80004112:	7442                	ld	s0,48(sp)
    80004114:	74a2                	ld	s1,40(sp)
    80004116:	6121                	addi	sp,sp,64
    80004118:	8082                	ret
    pipeclose(ff.pipe, ff.writable);
    8000411a:	85d6                	mv	a1,s5
    8000411c:	8552                	mv	a0,s4
    8000411e:	336000ef          	jal	80004454 <pipeclose>
    80004122:	7902                	ld	s2,32(sp)
    80004124:	69e2                	ld	s3,24(sp)
    80004126:	6a42                	ld	s4,16(sp)
    80004128:	6aa2                	ld	s5,8(sp)
    8000412a:	b7dd                	j	80004110 <fileclose+0x8e>
    begin_op();
    8000412c:	b4bff0ef          	jal	80003c76 <begin_op>
    iput(ff.ip);
    80004130:	854e                	mv	a0,s3
    80004132:	adcff0ef          	jal	8000340e <iput>
    end_op();
    80004136:	babff0ef          	jal	80003ce0 <end_op>
    8000413a:	7902                	ld	s2,32(sp)
    8000413c:	69e2                	ld	s3,24(sp)
    8000413e:	6a42                	ld	s4,16(sp)
    80004140:	6aa2                	ld	s5,8(sp)
    80004142:	b7f9                	j	80004110 <fileclose+0x8e>

0000000080004144 <filestat>:

// Get metadata about file f.
// addr is a user virtual address, pointing to a struct stat.
int
filestat(struct file *f, uint64 addr)
{
    80004144:	715d                	addi	sp,sp,-80
    80004146:	e486                	sd	ra,72(sp)
    80004148:	e0a2                	sd	s0,64(sp)
    8000414a:	fc26                	sd	s1,56(sp)
    8000414c:	f44e                	sd	s3,40(sp)
    8000414e:	0880                	addi	s0,sp,80
    80004150:	84aa                	mv	s1,a0
    80004152:	89ae                	mv	s3,a1
  struct proc *p = myproc();
    80004154:	fdcfd0ef          	jal	80001930 <myproc>
  struct stat st;
  
  if(f->type == FD_INODE || f->type == FD_DEVICE){
    80004158:	409c                	lw	a5,0(s1)
    8000415a:	37f9                	addiw	a5,a5,-2
    8000415c:	4705                	li	a4,1
    8000415e:	04f76063          	bltu	a4,a5,8000419e <filestat+0x5a>
    80004162:	f84a                	sd	s2,48(sp)
    80004164:	892a                	mv	s2,a0
    ilock(f->ip);
    80004166:	6c88                	ld	a0,24(s1)
    80004168:	924ff0ef          	jal	8000328c <ilock>
    stati(f->ip, &st);
    8000416c:	fb840593          	addi	a1,s0,-72
    80004170:	6c88                	ld	a0,24(s1)
    80004172:	c80ff0ef          	jal	800035f2 <stati>
    iunlock(f->ip);
    80004176:	6c88                	ld	a0,24(s1)
    80004178:	9c2ff0ef          	jal	8000333a <iunlock>
    if(copyout(p->pagetable, addr, (char *)&st, sizeof(st)) < 0)
    8000417c:	46e1                	li	a3,24
    8000417e:	fb840613          	addi	a2,s0,-72
    80004182:	85ce                	mv	a1,s3
    80004184:	05093503          	ld	a0,80(s2)
    80004188:	c5afd0ef          	jal	800015e2 <copyout>
    8000418c:	41f5551b          	sraiw	a0,a0,0x1f
    80004190:	7942                	ld	s2,48(sp)
      return -1;
    return 0;
  }
  return -1;
}
    80004192:	60a6                	ld	ra,72(sp)
    80004194:	6406                	ld	s0,64(sp)
    80004196:	74e2                	ld	s1,56(sp)
    80004198:	79a2                	ld	s3,40(sp)
    8000419a:	6161                	addi	sp,sp,80
    8000419c:	8082                	ret
  return -1;
    8000419e:	557d                	li	a0,-1
    800041a0:	bfcd                	j	80004192 <filestat+0x4e>

00000000800041a2 <fileread>:

// Read from file f.
// addr is a user virtual address.
int
fileread(struct file *f, uint64 addr, int n)
{
    800041a2:	7179                	addi	sp,sp,-48
    800041a4:	f406                	sd	ra,40(sp)
    800041a6:	f022                	sd	s0,32(sp)
    800041a8:	e84a                	sd	s2,16(sp)
    800041aa:	1800                	addi	s0,sp,48
  int r = 0;

  if(f->readable == 0)
    800041ac:	00854783          	lbu	a5,8(a0)
    800041b0:	cfd1                	beqz	a5,8000424c <fileread+0xaa>
    800041b2:	ec26                	sd	s1,24(sp)
    800041b4:	e44e                	sd	s3,8(sp)
    800041b6:	84aa                	mv	s1,a0
    800041b8:	89ae                	mv	s3,a1
    800041ba:	8932                	mv	s2,a2
    return -1;

  if(f->type == FD_PIPE){
    800041bc:	411c                	lw	a5,0(a0)
    800041be:	4705                	li	a4,1
    800041c0:	04e78363          	beq	a5,a4,80004206 <fileread+0x64>
    r = piperead(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    800041c4:	470d                	li	a4,3
    800041c6:	04e78763          	beq	a5,a4,80004214 <fileread+0x72>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
      return -1;
    r = devsw[f->major].read(1, addr, n);
  } else if(f->type == FD_INODE){
    800041ca:	4709                	li	a4,2
    800041cc:	06e79a63          	bne	a5,a4,80004240 <fileread+0x9e>
    ilock(f->ip);
    800041d0:	6d08                	ld	a0,24(a0)
    800041d2:	8baff0ef          	jal	8000328c <ilock>
    if((r = readi(f->ip, 1, addr, f->off, n)) > 0)
    800041d6:	874a                	mv	a4,s2
    800041d8:	5094                	lw	a3,32(s1)
    800041da:	864e                	mv	a2,s3
    800041dc:	4585                	li	a1,1
    800041de:	6c88                	ld	a0,24(s1)
    800041e0:	c3cff0ef          	jal	8000361c <readi>
    800041e4:	892a                	mv	s2,a0
    800041e6:	00a05563          	blez	a0,800041f0 <fileread+0x4e>
      f->off += r;
    800041ea:	509c                	lw	a5,32(s1)
    800041ec:	9fa9                	addw	a5,a5,a0
    800041ee:	d09c                	sw	a5,32(s1)
    iunlock(f->ip);
    800041f0:	6c88                	ld	a0,24(s1)
    800041f2:	948ff0ef          	jal	8000333a <iunlock>
    800041f6:	64e2                	ld	s1,24(sp)
    800041f8:	69a2                	ld	s3,8(sp)
  } else {
    panic("fileread");
  }

  return r;
}
    800041fa:	854a                	mv	a0,s2
    800041fc:	70a2                	ld	ra,40(sp)
    800041fe:	7402                	ld	s0,32(sp)
    80004200:	6942                	ld	s2,16(sp)
    80004202:	6145                	addi	sp,sp,48
    80004204:	8082                	ret
    r = piperead(f->pipe, addr, n);
    80004206:	6908                	ld	a0,16(a0)
    80004208:	388000ef          	jal	80004590 <piperead>
    8000420c:	892a                	mv	s2,a0
    8000420e:	64e2                	ld	s1,24(sp)
    80004210:	69a2                	ld	s3,8(sp)
    80004212:	b7e5                	j	800041fa <fileread+0x58>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
    80004214:	02451783          	lh	a5,36(a0)
    80004218:	03079693          	slli	a3,a5,0x30
    8000421c:	92c1                	srli	a3,a3,0x30
    8000421e:	4725                	li	a4,9
    80004220:	02d76863          	bltu	a4,a3,80004250 <fileread+0xae>
    80004224:	0792                	slli	a5,a5,0x4
    80004226:	0001c717          	auipc	a4,0x1c
    8000422a:	9ba70713          	addi	a4,a4,-1606 # 8001fbe0 <devsw>
    8000422e:	97ba                	add	a5,a5,a4
    80004230:	639c                	ld	a5,0(a5)
    80004232:	c39d                	beqz	a5,80004258 <fileread+0xb6>
    r = devsw[f->major].read(1, addr, n);
    80004234:	4505                	li	a0,1
    80004236:	9782                	jalr	a5
    80004238:	892a                	mv	s2,a0
    8000423a:	64e2                	ld	s1,24(sp)
    8000423c:	69a2                	ld	s3,8(sp)
    8000423e:	bf75                	j	800041fa <fileread+0x58>
    panic("fileread");
    80004240:	00003517          	auipc	a0,0x3
    80004244:	33850513          	addi	a0,a0,824 # 80007578 <etext+0x578>
    80004248:	d98fc0ef          	jal	800007e0 <panic>
    return -1;
    8000424c:	597d                	li	s2,-1
    8000424e:	b775                	j	800041fa <fileread+0x58>
      return -1;
    80004250:	597d                	li	s2,-1
    80004252:	64e2                	ld	s1,24(sp)
    80004254:	69a2                	ld	s3,8(sp)
    80004256:	b755                	j	800041fa <fileread+0x58>
    80004258:	597d                	li	s2,-1
    8000425a:	64e2                	ld	s1,24(sp)
    8000425c:	69a2                	ld	s3,8(sp)
    8000425e:	bf71                	j	800041fa <fileread+0x58>

0000000080004260 <filewrite>:
int
filewrite(struct file *f, uint64 addr, int n)
{
  int r, ret = 0;

  if(f->writable == 0)
    80004260:	00954783          	lbu	a5,9(a0)
    80004264:	10078b63          	beqz	a5,8000437a <filewrite+0x11a>
{
    80004268:	715d                	addi	sp,sp,-80
    8000426a:	e486                	sd	ra,72(sp)
    8000426c:	e0a2                	sd	s0,64(sp)
    8000426e:	f84a                	sd	s2,48(sp)
    80004270:	f052                	sd	s4,32(sp)
    80004272:	e85a                	sd	s6,16(sp)
    80004274:	0880                	addi	s0,sp,80
    80004276:	892a                	mv	s2,a0
    80004278:	8b2e                	mv	s6,a1
    8000427a:	8a32                	mv	s4,a2
    return -1;

  if(f->type == FD_PIPE){
    8000427c:	411c                	lw	a5,0(a0)
    8000427e:	4705                	li	a4,1
    80004280:	02e78763          	beq	a5,a4,800042ae <filewrite+0x4e>
    ret = pipewrite(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    80004284:	470d                	li	a4,3
    80004286:	02e78863          	beq	a5,a4,800042b6 <filewrite+0x56>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].write)
      return -1;
    ret = devsw[f->major].write(1, addr, n);
  } else if(f->type == FD_INODE){
    8000428a:	4709                	li	a4,2
    8000428c:	0ce79c63          	bne	a5,a4,80004364 <filewrite+0x104>
    80004290:	f44e                	sd	s3,40(sp)
    // the maximum log transaction size, including
    // i-node, indirect block, allocation blocks,
    // and 2 blocks of slop for non-aligned writes.
    int max = ((MAXOPBLOCKS-1-1-2) / 2) * BSIZE;
    int i = 0;
    while(i < n){
    80004292:	0ac05863          	blez	a2,80004342 <filewrite+0xe2>
    80004296:	fc26                	sd	s1,56(sp)
    80004298:	ec56                	sd	s5,24(sp)
    8000429a:	e45e                	sd	s7,8(sp)
    8000429c:	e062                	sd	s8,0(sp)
    int i = 0;
    8000429e:	4981                	li	s3,0
      int n1 = n - i;
      if(n1 > max)
    800042a0:	6b85                	lui	s7,0x1
    800042a2:	c00b8b93          	addi	s7,s7,-1024 # c00 <_entry-0x7ffff400>
    800042a6:	6c05                	lui	s8,0x1
    800042a8:	c00c0c1b          	addiw	s8,s8,-1024 # c00 <_entry-0x7ffff400>
    800042ac:	a8b5                	j	80004328 <filewrite+0xc8>
    ret = pipewrite(f->pipe, addr, n);
    800042ae:	6908                	ld	a0,16(a0)
    800042b0:	1fc000ef          	jal	800044ac <pipewrite>
    800042b4:	a04d                	j	80004356 <filewrite+0xf6>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].write)
    800042b6:	02451783          	lh	a5,36(a0)
    800042ba:	03079693          	slli	a3,a5,0x30
    800042be:	92c1                	srli	a3,a3,0x30
    800042c0:	4725                	li	a4,9
    800042c2:	0ad76e63          	bltu	a4,a3,8000437e <filewrite+0x11e>
    800042c6:	0792                	slli	a5,a5,0x4
    800042c8:	0001c717          	auipc	a4,0x1c
    800042cc:	91870713          	addi	a4,a4,-1768 # 8001fbe0 <devsw>
    800042d0:	97ba                	add	a5,a5,a4
    800042d2:	679c                	ld	a5,8(a5)
    800042d4:	c7dd                	beqz	a5,80004382 <filewrite+0x122>
    ret = devsw[f->major].write(1, addr, n);
    800042d6:	4505                	li	a0,1
    800042d8:	9782                	jalr	a5
    800042da:	a8b5                	j	80004356 <filewrite+0xf6>
      if(n1 > max)
    800042dc:	00048a9b          	sext.w	s5,s1
        n1 = max;

      begin_op();
    800042e0:	997ff0ef          	jal	80003c76 <begin_op>
      ilock(f->ip);
    800042e4:	01893503          	ld	a0,24(s2)
    800042e8:	fa5fe0ef          	jal	8000328c <ilock>
      if ((r = writei(f->ip, 1, addr + i, f->off, n1)) > 0)
    800042ec:	8756                	mv	a4,s5
    800042ee:	02092683          	lw	a3,32(s2)
    800042f2:	01698633          	add	a2,s3,s6
    800042f6:	4585                	li	a1,1
    800042f8:	01893503          	ld	a0,24(s2)
    800042fc:	c1cff0ef          	jal	80003718 <writei>
    80004300:	84aa                	mv	s1,a0
    80004302:	00a05763          	blez	a0,80004310 <filewrite+0xb0>
        f->off += r;
    80004306:	02092783          	lw	a5,32(s2)
    8000430a:	9fa9                	addw	a5,a5,a0
    8000430c:	02f92023          	sw	a5,32(s2)
      iunlock(f->ip);
    80004310:	01893503          	ld	a0,24(s2)
    80004314:	826ff0ef          	jal	8000333a <iunlock>
      end_op();
    80004318:	9c9ff0ef          	jal	80003ce0 <end_op>

      if(r != n1){
    8000431c:	029a9563          	bne	s5,s1,80004346 <filewrite+0xe6>
        // error from writei
        break;
      }
      i += r;
    80004320:	013489bb          	addw	s3,s1,s3
    while(i < n){
    80004324:	0149da63          	bge	s3,s4,80004338 <filewrite+0xd8>
      int n1 = n - i;
    80004328:	413a04bb          	subw	s1,s4,s3
      if(n1 > max)
    8000432c:	0004879b          	sext.w	a5,s1
    80004330:	fafbd6e3          	bge	s7,a5,800042dc <filewrite+0x7c>
    80004334:	84e2                	mv	s1,s8
    80004336:	b75d                	j	800042dc <filewrite+0x7c>
    80004338:	74e2                	ld	s1,56(sp)
    8000433a:	6ae2                	ld	s5,24(sp)
    8000433c:	6ba2                	ld	s7,8(sp)
    8000433e:	6c02                	ld	s8,0(sp)
    80004340:	a039                	j	8000434e <filewrite+0xee>
    int i = 0;
    80004342:	4981                	li	s3,0
    80004344:	a029                	j	8000434e <filewrite+0xee>
    80004346:	74e2                	ld	s1,56(sp)
    80004348:	6ae2                	ld	s5,24(sp)
    8000434a:	6ba2                	ld	s7,8(sp)
    8000434c:	6c02                	ld	s8,0(sp)
    }
    ret = (i == n ? n : -1);
    8000434e:	033a1c63          	bne	s4,s3,80004386 <filewrite+0x126>
    80004352:	8552                	mv	a0,s4
    80004354:	79a2                	ld	s3,40(sp)
  } else {
    panic("filewrite");
  }

  return ret;
}
    80004356:	60a6                	ld	ra,72(sp)
    80004358:	6406                	ld	s0,64(sp)
    8000435a:	7942                	ld	s2,48(sp)
    8000435c:	7a02                	ld	s4,32(sp)
    8000435e:	6b42                	ld	s6,16(sp)
    80004360:	6161                	addi	sp,sp,80
    80004362:	8082                	ret
    80004364:	fc26                	sd	s1,56(sp)
    80004366:	f44e                	sd	s3,40(sp)
    80004368:	ec56                	sd	s5,24(sp)
    8000436a:	e45e                	sd	s7,8(sp)
    8000436c:	e062                	sd	s8,0(sp)
    panic("filewrite");
    8000436e:	00003517          	auipc	a0,0x3
    80004372:	21a50513          	addi	a0,a0,538 # 80007588 <etext+0x588>
    80004376:	c6afc0ef          	jal	800007e0 <panic>
    return -1;
    8000437a:	557d                	li	a0,-1
}
    8000437c:	8082                	ret
      return -1;
    8000437e:	557d                	li	a0,-1
    80004380:	bfd9                	j	80004356 <filewrite+0xf6>
    80004382:	557d                	li	a0,-1
    80004384:	bfc9                	j	80004356 <filewrite+0xf6>
    ret = (i == n ? n : -1);
    80004386:	557d                	li	a0,-1
    80004388:	79a2                	ld	s3,40(sp)
    8000438a:	b7f1                	j	80004356 <filewrite+0xf6>

000000008000438c <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
    8000438c:	7179                	addi	sp,sp,-48
    8000438e:	f406                	sd	ra,40(sp)
    80004390:	f022                	sd	s0,32(sp)
    80004392:	ec26                	sd	s1,24(sp)
    80004394:	e052                	sd	s4,0(sp)
    80004396:	1800                	addi	s0,sp,48
    80004398:	84aa                	mv	s1,a0
    8000439a:	8a2e                	mv	s4,a1
  struct pipe *pi;

  pi = 0;
  *f0 = *f1 = 0;
    8000439c:	0005b023          	sd	zero,0(a1)
    800043a0:	00053023          	sd	zero,0(a0)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
    800043a4:	c3bff0ef          	jal	80003fde <filealloc>
    800043a8:	e088                	sd	a0,0(s1)
    800043aa:	c549                	beqz	a0,80004434 <pipealloc+0xa8>
    800043ac:	c33ff0ef          	jal	80003fde <filealloc>
    800043b0:	00aa3023          	sd	a0,0(s4)
    800043b4:	cd25                	beqz	a0,8000442c <pipealloc+0xa0>
    800043b6:	e84a                	sd	s2,16(sp)
    goto bad;
  if((pi = (struct pipe*)kalloc()) == 0)
    800043b8:	f46fc0ef          	jal	80000afe <kalloc>
    800043bc:	892a                	mv	s2,a0
    800043be:	c12d                	beqz	a0,80004420 <pipealloc+0x94>
    800043c0:	e44e                	sd	s3,8(sp)
    goto bad;
  pi->readopen = 1;
    800043c2:	4985                	li	s3,1
    800043c4:	23352023          	sw	s3,544(a0)
  pi->writeopen = 1;
    800043c8:	23352223          	sw	s3,548(a0)
  pi->nwrite = 0;
    800043cc:	20052e23          	sw	zero,540(a0)
  pi->nread = 0;
    800043d0:	20052c23          	sw	zero,536(a0)
  initlock(&pi->lock, "pipe");
    800043d4:	00003597          	auipc	a1,0x3
    800043d8:	1c458593          	addi	a1,a1,452 # 80007598 <etext+0x598>
    800043dc:	f72fc0ef          	jal	80000b4e <initlock>
  (*f0)->type = FD_PIPE;
    800043e0:	609c                	ld	a5,0(s1)
    800043e2:	0137a023          	sw	s3,0(a5)
  (*f0)->readable = 1;
    800043e6:	609c                	ld	a5,0(s1)
    800043e8:	01378423          	sb	s3,8(a5)
  (*f0)->writable = 0;
    800043ec:	609c                	ld	a5,0(s1)
    800043ee:	000784a3          	sb	zero,9(a5)
  (*f0)->pipe = pi;
    800043f2:	609c                	ld	a5,0(s1)
    800043f4:	0127b823          	sd	s2,16(a5)
  (*f1)->type = FD_PIPE;
    800043f8:	000a3783          	ld	a5,0(s4)
    800043fc:	0137a023          	sw	s3,0(a5)
  (*f1)->readable = 0;
    80004400:	000a3783          	ld	a5,0(s4)
    80004404:	00078423          	sb	zero,8(a5)
  (*f1)->writable = 1;
    80004408:	000a3783          	ld	a5,0(s4)
    8000440c:	013784a3          	sb	s3,9(a5)
  (*f1)->pipe = pi;
    80004410:	000a3783          	ld	a5,0(s4)
    80004414:	0127b823          	sd	s2,16(a5)
  return 0;
    80004418:	4501                	li	a0,0
    8000441a:	6942                	ld	s2,16(sp)
    8000441c:	69a2                	ld	s3,8(sp)
    8000441e:	a01d                	j	80004444 <pipealloc+0xb8>

 bad:
  if(pi)
    kfree((char*)pi);
  if(*f0)
    80004420:	6088                	ld	a0,0(s1)
    80004422:	c119                	beqz	a0,80004428 <pipealloc+0x9c>
    80004424:	6942                	ld	s2,16(sp)
    80004426:	a029                	j	80004430 <pipealloc+0xa4>
    80004428:	6942                	ld	s2,16(sp)
    8000442a:	a029                	j	80004434 <pipealloc+0xa8>
    8000442c:	6088                	ld	a0,0(s1)
    8000442e:	c10d                	beqz	a0,80004450 <pipealloc+0xc4>
    fileclose(*f0);
    80004430:	c53ff0ef          	jal	80004082 <fileclose>
  if(*f1)
    80004434:	000a3783          	ld	a5,0(s4)
    fileclose(*f1);
  return -1;
    80004438:	557d                	li	a0,-1
  if(*f1)
    8000443a:	c789                	beqz	a5,80004444 <pipealloc+0xb8>
    fileclose(*f1);
    8000443c:	853e                	mv	a0,a5
    8000443e:	c45ff0ef          	jal	80004082 <fileclose>
  return -1;
    80004442:	557d                	li	a0,-1
}
    80004444:	70a2                	ld	ra,40(sp)
    80004446:	7402                	ld	s0,32(sp)
    80004448:	64e2                	ld	s1,24(sp)
    8000444a:	6a02                	ld	s4,0(sp)
    8000444c:	6145                	addi	sp,sp,48
    8000444e:	8082                	ret
  return -1;
    80004450:	557d                	li	a0,-1
    80004452:	bfcd                	j	80004444 <pipealloc+0xb8>

0000000080004454 <pipeclose>:

void
pipeclose(struct pipe *pi, int writable)
{
    80004454:	1101                	addi	sp,sp,-32
    80004456:	ec06                	sd	ra,24(sp)
    80004458:	e822                	sd	s0,16(sp)
    8000445a:	e426                	sd	s1,8(sp)
    8000445c:	e04a                	sd	s2,0(sp)
    8000445e:	1000                	addi	s0,sp,32
    80004460:	84aa                	mv	s1,a0
    80004462:	892e                	mv	s2,a1
  acquire(&pi->lock);
    80004464:	f6afc0ef          	jal	80000bce <acquire>
  if(writable){
    80004468:	02090763          	beqz	s2,80004496 <pipeclose+0x42>
    pi->writeopen = 0;
    8000446c:	2204a223          	sw	zero,548(s1)
    wakeup(&pi->nread);
    80004470:	21848513          	addi	a0,s1,536
    80004474:	b05fd0ef          	jal	80001f78 <wakeup>
  } else {
    pi->readopen = 0;
    wakeup(&pi->nwrite);
  }
  if(pi->readopen == 0 && pi->writeopen == 0){
    80004478:	2204b783          	ld	a5,544(s1)
    8000447c:	e785                	bnez	a5,800044a4 <pipeclose+0x50>
    release(&pi->lock);
    8000447e:	8526                	mv	a0,s1
    80004480:	fe6fc0ef          	jal	80000c66 <release>
    kfree((char*)pi);
    80004484:	8526                	mv	a0,s1
    80004486:	d96fc0ef          	jal	80000a1c <kfree>
  } else
    release(&pi->lock);
}
    8000448a:	60e2                	ld	ra,24(sp)
    8000448c:	6442                	ld	s0,16(sp)
    8000448e:	64a2                	ld	s1,8(sp)
    80004490:	6902                	ld	s2,0(sp)
    80004492:	6105                	addi	sp,sp,32
    80004494:	8082                	ret
    pi->readopen = 0;
    80004496:	2204a023          	sw	zero,544(s1)
    wakeup(&pi->nwrite);
    8000449a:	21c48513          	addi	a0,s1,540
    8000449e:	adbfd0ef          	jal	80001f78 <wakeup>
    800044a2:	bfd9                	j	80004478 <pipeclose+0x24>
    release(&pi->lock);
    800044a4:	8526                	mv	a0,s1
    800044a6:	fc0fc0ef          	jal	80000c66 <release>
}
    800044aa:	b7c5                	j	8000448a <pipeclose+0x36>

00000000800044ac <pipewrite>:

int
pipewrite(struct pipe *pi, uint64 addr, int n)
{
    800044ac:	711d                	addi	sp,sp,-96
    800044ae:	ec86                	sd	ra,88(sp)
    800044b0:	e8a2                	sd	s0,80(sp)
    800044b2:	e4a6                	sd	s1,72(sp)
    800044b4:	e0ca                	sd	s2,64(sp)
    800044b6:	fc4e                	sd	s3,56(sp)
    800044b8:	f852                	sd	s4,48(sp)
    800044ba:	f456                	sd	s5,40(sp)
    800044bc:	1080                	addi	s0,sp,96
    800044be:	84aa                	mv	s1,a0
    800044c0:	8aae                	mv	s5,a1
    800044c2:	8a32                	mv	s4,a2
  int i = 0;
  struct proc *pr = myproc();
    800044c4:	c6cfd0ef          	jal	80001930 <myproc>
    800044c8:	89aa                	mv	s3,a0

  acquire(&pi->lock);
    800044ca:	8526                	mv	a0,s1
    800044cc:	f02fc0ef          	jal	80000bce <acquire>
  while(i < n){
    800044d0:	0b405a63          	blez	s4,80004584 <pipewrite+0xd8>
    800044d4:	f05a                	sd	s6,32(sp)
    800044d6:	ec5e                	sd	s7,24(sp)
    800044d8:	e862                	sd	s8,16(sp)
  int i = 0;
    800044da:	4901                	li	s2,0
    if(pi->nwrite == pi->nread + PIPESIZE){ //DOC: pipewrite-full
      wakeup(&pi->nread);
      sleep(&pi->nwrite, &pi->lock);
    } else {
      char ch;
      if(copyin(pr->pagetable, &ch, addr + i, 1) == -1)
    800044dc:	5b7d                	li	s6,-1
      wakeup(&pi->nread);
    800044de:	21848c13          	addi	s8,s1,536
      sleep(&pi->nwrite, &pi->lock);
    800044e2:	21c48b93          	addi	s7,s1,540
    800044e6:	a81d                	j	8000451c <pipewrite+0x70>
      release(&pi->lock);
    800044e8:	8526                	mv	a0,s1
    800044ea:	f7cfc0ef          	jal	80000c66 <release>
      return -1;
    800044ee:	597d                	li	s2,-1
    800044f0:	7b02                	ld	s6,32(sp)
    800044f2:	6be2                	ld	s7,24(sp)
    800044f4:	6c42                	ld	s8,16(sp)
  }
  wakeup(&pi->nread);
  release(&pi->lock);

  return i;
}
    800044f6:	854a                	mv	a0,s2
    800044f8:	60e6                	ld	ra,88(sp)
    800044fa:	6446                	ld	s0,80(sp)
    800044fc:	64a6                	ld	s1,72(sp)
    800044fe:	6906                	ld	s2,64(sp)
    80004500:	79e2                	ld	s3,56(sp)
    80004502:	7a42                	ld	s4,48(sp)
    80004504:	7aa2                	ld	s5,40(sp)
    80004506:	6125                	addi	sp,sp,96
    80004508:	8082                	ret
      wakeup(&pi->nread);
    8000450a:	8562                	mv	a0,s8
    8000450c:	a6dfd0ef          	jal	80001f78 <wakeup>
      sleep(&pi->nwrite, &pi->lock);
    80004510:	85a6                	mv	a1,s1
    80004512:	855e                	mv	a0,s7
    80004514:	a19fd0ef          	jal	80001f2c <sleep>
  while(i < n){
    80004518:	05495b63          	bge	s2,s4,8000456e <pipewrite+0xc2>
    if(pi->readopen == 0 || killed(pr)){
    8000451c:	2204a783          	lw	a5,544(s1)
    80004520:	d7e1                	beqz	a5,800044e8 <pipewrite+0x3c>
    80004522:	854e                	mv	a0,s3
    80004524:	c41fd0ef          	jal	80002164 <killed>
    80004528:	f161                	bnez	a0,800044e8 <pipewrite+0x3c>
    if(pi->nwrite == pi->nread + PIPESIZE){ //DOC: pipewrite-full
    8000452a:	2184a783          	lw	a5,536(s1)
    8000452e:	21c4a703          	lw	a4,540(s1)
    80004532:	2007879b          	addiw	a5,a5,512
    80004536:	fcf70ae3          	beq	a4,a5,8000450a <pipewrite+0x5e>
      if(copyin(pr->pagetable, &ch, addr + i, 1) == -1)
    8000453a:	4685                	li	a3,1
    8000453c:	01590633          	add	a2,s2,s5
    80004540:	faf40593          	addi	a1,s0,-81
    80004544:	0509b503          	ld	a0,80(s3)
    80004548:	97efd0ef          	jal	800016c6 <copyin>
    8000454c:	03650e63          	beq	a0,s6,80004588 <pipewrite+0xdc>
      pi->data[pi->nwrite++ % PIPESIZE] = ch;
    80004550:	21c4a783          	lw	a5,540(s1)
    80004554:	0017871b          	addiw	a4,a5,1
    80004558:	20e4ae23          	sw	a4,540(s1)
    8000455c:	1ff7f793          	andi	a5,a5,511
    80004560:	97a6                	add	a5,a5,s1
    80004562:	faf44703          	lbu	a4,-81(s0)
    80004566:	00e78c23          	sb	a4,24(a5)
      i++;
    8000456a:	2905                	addiw	s2,s2,1
    8000456c:	b775                	j	80004518 <pipewrite+0x6c>
    8000456e:	7b02                	ld	s6,32(sp)
    80004570:	6be2                	ld	s7,24(sp)
    80004572:	6c42                	ld	s8,16(sp)
  wakeup(&pi->nread);
    80004574:	21848513          	addi	a0,s1,536
    80004578:	a01fd0ef          	jal	80001f78 <wakeup>
  release(&pi->lock);
    8000457c:	8526                	mv	a0,s1
    8000457e:	ee8fc0ef          	jal	80000c66 <release>
  return i;
    80004582:	bf95                	j	800044f6 <pipewrite+0x4a>
  int i = 0;
    80004584:	4901                	li	s2,0
    80004586:	b7fd                	j	80004574 <pipewrite+0xc8>
    80004588:	7b02                	ld	s6,32(sp)
    8000458a:	6be2                	ld	s7,24(sp)
    8000458c:	6c42                	ld	s8,16(sp)
    8000458e:	b7dd                	j	80004574 <pipewrite+0xc8>

0000000080004590 <piperead>:

int
piperead(struct pipe *pi, uint64 addr, int n)
{
    80004590:	715d                	addi	sp,sp,-80
    80004592:	e486                	sd	ra,72(sp)
    80004594:	e0a2                	sd	s0,64(sp)
    80004596:	fc26                	sd	s1,56(sp)
    80004598:	f84a                	sd	s2,48(sp)
    8000459a:	f44e                	sd	s3,40(sp)
    8000459c:	f052                	sd	s4,32(sp)
    8000459e:	ec56                	sd	s5,24(sp)
    800045a0:	0880                	addi	s0,sp,80
    800045a2:	84aa                	mv	s1,a0
    800045a4:	892e                	mv	s2,a1
    800045a6:	8ab2                	mv	s5,a2
  int i;
  struct proc *pr = myproc();
    800045a8:	b88fd0ef          	jal	80001930 <myproc>
    800045ac:	8a2a                	mv	s4,a0
  char ch;

  acquire(&pi->lock);
    800045ae:	8526                	mv	a0,s1
    800045b0:	e1efc0ef          	jal	80000bce <acquire>
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    800045b4:	2184a703          	lw	a4,536(s1)
    800045b8:	21c4a783          	lw	a5,540(s1)
    if(killed(pr)){
      release(&pi->lock);
      return -1;
    }
    sleep(&pi->nread, &pi->lock); //DOC: piperead-sleep
    800045bc:	21848993          	addi	s3,s1,536
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    800045c0:	02f71563          	bne	a4,a5,800045ea <piperead+0x5a>
    800045c4:	2244a783          	lw	a5,548(s1)
    800045c8:	cb85                	beqz	a5,800045f8 <piperead+0x68>
    if(killed(pr)){
    800045ca:	8552                	mv	a0,s4
    800045cc:	b99fd0ef          	jal	80002164 <killed>
    800045d0:	ed19                	bnez	a0,800045ee <piperead+0x5e>
    sleep(&pi->nread, &pi->lock); //DOC: piperead-sleep
    800045d2:	85a6                	mv	a1,s1
    800045d4:	854e                	mv	a0,s3
    800045d6:	957fd0ef          	jal	80001f2c <sleep>
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    800045da:	2184a703          	lw	a4,536(s1)
    800045de:	21c4a783          	lw	a5,540(s1)
    800045e2:	fef701e3          	beq	a4,a5,800045c4 <piperead+0x34>
    800045e6:	e85a                	sd	s6,16(sp)
    800045e8:	a809                	j	800045fa <piperead+0x6a>
    800045ea:	e85a                	sd	s6,16(sp)
    800045ec:	a039                	j	800045fa <piperead+0x6a>
      release(&pi->lock);
    800045ee:	8526                	mv	a0,s1
    800045f0:	e76fc0ef          	jal	80000c66 <release>
      return -1;
    800045f4:	59fd                	li	s3,-1
    800045f6:	a8b1                	j	80004652 <piperead+0xc2>
    800045f8:	e85a                	sd	s6,16(sp)
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    800045fa:	4981                	li	s3,0
    if(pi->nread == pi->nwrite)
      break;
    ch = pi->data[pi->nread++ % PIPESIZE];
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1)
    800045fc:	5b7d                	li	s6,-1
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    800045fe:	05505263          	blez	s5,80004642 <piperead+0xb2>
    if(pi->nread == pi->nwrite)
    80004602:	2184a783          	lw	a5,536(s1)
    80004606:	21c4a703          	lw	a4,540(s1)
    8000460a:	02f70c63          	beq	a4,a5,80004642 <piperead+0xb2>
    ch = pi->data[pi->nread++ % PIPESIZE];
    8000460e:	0017871b          	addiw	a4,a5,1
    80004612:	20e4ac23          	sw	a4,536(s1)
    80004616:	1ff7f793          	andi	a5,a5,511
    8000461a:	97a6                	add	a5,a5,s1
    8000461c:	0187c783          	lbu	a5,24(a5)
    80004620:	faf40fa3          	sb	a5,-65(s0)
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1)
    80004624:	4685                	li	a3,1
    80004626:	fbf40613          	addi	a2,s0,-65
    8000462a:	85ca                	mv	a1,s2
    8000462c:	050a3503          	ld	a0,80(s4)
    80004630:	fb3fc0ef          	jal	800015e2 <copyout>
    80004634:	01650763          	beq	a0,s6,80004642 <piperead+0xb2>
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    80004638:	2985                	addiw	s3,s3,1
    8000463a:	0905                	addi	s2,s2,1
    8000463c:	fd3a93e3          	bne	s5,s3,80004602 <piperead+0x72>
    80004640:	89d6                	mv	s3,s5
      break;
  }
  wakeup(&pi->nwrite);  //DOC: piperead-wakeup
    80004642:	21c48513          	addi	a0,s1,540
    80004646:	933fd0ef          	jal	80001f78 <wakeup>
  release(&pi->lock);
    8000464a:	8526                	mv	a0,s1
    8000464c:	e1afc0ef          	jal	80000c66 <release>
    80004650:	6b42                	ld	s6,16(sp)
  return i;
}
    80004652:	854e                	mv	a0,s3
    80004654:	60a6                	ld	ra,72(sp)
    80004656:	6406                	ld	s0,64(sp)
    80004658:	74e2                	ld	s1,56(sp)
    8000465a:	7942                	ld	s2,48(sp)
    8000465c:	79a2                	ld	s3,40(sp)
    8000465e:	7a02                	ld	s4,32(sp)
    80004660:	6ae2                	ld	s5,24(sp)
    80004662:	6161                	addi	sp,sp,80
    80004664:	8082                	ret

0000000080004666 <flags2perm>:

static int loadseg(pde_t *, uint64, struct inode *, uint, uint);

// map ELF permissions to PTE permission bits.
int flags2perm(int flags)
{
    80004666:	1141                	addi	sp,sp,-16
    80004668:	e422                	sd	s0,8(sp)
    8000466a:	0800                	addi	s0,sp,16
    8000466c:	87aa                	mv	a5,a0
    int perm = 0;
    if(flags & 0x1)
    8000466e:	8905                	andi	a0,a0,1
    80004670:	050e                	slli	a0,a0,0x3
      perm = PTE_X;
    if(flags & 0x2)
    80004672:	8b89                	andi	a5,a5,2
    80004674:	c399                	beqz	a5,8000467a <flags2perm+0x14>
      perm |= PTE_W;
    80004676:	00456513          	ori	a0,a0,4
    return perm;
}
    8000467a:	6422                	ld	s0,8(sp)
    8000467c:	0141                	addi	sp,sp,16
    8000467e:	8082                	ret

0000000080004680 <kexec>:
//
// the implementation of the exec() system call
//
int
kexec(char *path, char **argv)
{
    80004680:	df010113          	addi	sp,sp,-528
    80004684:	20113423          	sd	ra,520(sp)
    80004688:	20813023          	sd	s0,512(sp)
    8000468c:	ffa6                	sd	s1,504(sp)
    8000468e:	fbca                	sd	s2,496(sp)
    80004690:	0c00                	addi	s0,sp,528
    80004692:	892a                	mv	s2,a0
    80004694:	dea43c23          	sd	a0,-520(s0)
    80004698:	e0b43023          	sd	a1,-512(s0)
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pagetable_t pagetable = 0, oldpagetable;
  struct proc *p = myproc();
    8000469c:	a94fd0ef          	jal	80001930 <myproc>
    800046a0:	84aa                	mv	s1,a0

  begin_op();
    800046a2:	dd4ff0ef          	jal	80003c76 <begin_op>

  // Open the executable file.
  if((ip = namei(path)) == 0){
    800046a6:	854a                	mv	a0,s2
    800046a8:	bfaff0ef          	jal	80003aa2 <namei>
    800046ac:	c931                	beqz	a0,80004700 <kexec+0x80>
    800046ae:	f3d2                	sd	s4,480(sp)
    800046b0:	8a2a                	mv	s4,a0
    end_op();
    return -1;
  }
  ilock(ip);
    800046b2:	bdbfe0ef          	jal	8000328c <ilock>

  // Read the ELF header.
  if(readi(ip, 0, (uint64)&elf, 0, sizeof(elf)) != sizeof(elf))
    800046b6:	04000713          	li	a4,64
    800046ba:	4681                	li	a3,0
    800046bc:	e5040613          	addi	a2,s0,-432
    800046c0:	4581                	li	a1,0
    800046c2:	8552                	mv	a0,s4
    800046c4:	f59fe0ef          	jal	8000361c <readi>
    800046c8:	04000793          	li	a5,64
    800046cc:	00f51a63          	bne	a0,a5,800046e0 <kexec+0x60>
    goto bad;

  // Is this really an ELF file?
  if(elf.magic != ELF_MAGIC)
    800046d0:	e5042703          	lw	a4,-432(s0)
    800046d4:	464c47b7          	lui	a5,0x464c4
    800046d8:	57f78793          	addi	a5,a5,1407 # 464c457f <_entry-0x39b3ba81>
    800046dc:	02f70663          	beq	a4,a5,80004708 <kexec+0x88>

 bad:
  if(pagetable)
    proc_freepagetable(pagetable, sz);
  if(ip){
    iunlockput(ip);
    800046e0:	8552                	mv	a0,s4
    800046e2:	db5fe0ef          	jal	80003496 <iunlockput>
    end_op();
    800046e6:	dfaff0ef          	jal	80003ce0 <end_op>
  }
  return -1;
    800046ea:	557d                	li	a0,-1
    800046ec:	7a1e                	ld	s4,480(sp)
}
    800046ee:	20813083          	ld	ra,520(sp)
    800046f2:	20013403          	ld	s0,512(sp)
    800046f6:	74fe                	ld	s1,504(sp)
    800046f8:	795e                	ld	s2,496(sp)
    800046fa:	21010113          	addi	sp,sp,528
    800046fe:	8082                	ret
    end_op();
    80004700:	de0ff0ef          	jal	80003ce0 <end_op>
    return -1;
    80004704:	557d                	li	a0,-1
    80004706:	b7e5                	j	800046ee <kexec+0x6e>
    80004708:	ebda                	sd	s6,464(sp)
  if((pagetable = proc_pagetable(p)) == 0)
    8000470a:	8526                	mv	a0,s1
    8000470c:	b2afd0ef          	jal	80001a36 <proc_pagetable>
    80004710:	8b2a                	mv	s6,a0
    80004712:	2c050d63          	beqz	a0,800049ec <kexec+0x36c>
    80004716:	f7ce                	sd	s3,488(sp)
    80004718:	efd6                	sd	s5,472(sp)
    8000471a:	e7de                	sd	s7,456(sp)
    8000471c:	e3e2                	sd	s8,448(sp)
    8000471e:	ff66                	sd	s9,440(sp)
    80004720:	fb6a                	sd	s10,432(sp)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    80004722:	e7042d03          	lw	s10,-400(s0)
    80004726:	e8845783          	lhu	a5,-376(s0)
    8000472a:	12078963          	beqz	a5,8000485c <kexec+0x1dc>
    8000472e:	f76e                	sd	s11,424(sp)
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
    80004730:	4901                	li	s2,0
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    80004732:	4d81                	li	s11,0
    if(ph.vaddr % PGSIZE != 0)
    80004734:	6c85                	lui	s9,0x1
    80004736:	fffc8793          	addi	a5,s9,-1 # fff <_entry-0x7ffff001>
    8000473a:	def43823          	sd	a5,-528(s0)

  for(i = 0; i < sz; i += PGSIZE){
    pa = walkaddr(pagetable, va + i);
    if(pa == 0)
      panic("loadseg: address should exist");
    if(sz - i < PGSIZE)
    8000473e:	6a85                	lui	s5,0x1
    80004740:	a085                	j	800047a0 <kexec+0x120>
      panic("loadseg: address should exist");
    80004742:	00003517          	auipc	a0,0x3
    80004746:	e5e50513          	addi	a0,a0,-418 # 800075a0 <etext+0x5a0>
    8000474a:	896fc0ef          	jal	800007e0 <panic>
    if(sz - i < PGSIZE)
    8000474e:	2481                	sext.w	s1,s1
      n = sz - i;
    else
      n = PGSIZE;
    if(readi(ip, 0, (uint64)pa, offset+i, n) != n)
    80004750:	8726                	mv	a4,s1
    80004752:	012c06bb          	addw	a3,s8,s2
    80004756:	4581                	li	a1,0
    80004758:	8552                	mv	a0,s4
    8000475a:	ec3fe0ef          	jal	8000361c <readi>
    8000475e:	2501                	sext.w	a0,a0
    80004760:	24a49c63          	bne	s1,a0,800049b8 <kexec+0x338>
  for(i = 0; i < sz; i += PGSIZE){
    80004764:	012a893b          	addw	s2,s5,s2
    80004768:	03397363          	bgeu	s2,s3,8000478e <kexec+0x10e>
    pa = walkaddr(pagetable, va + i);
    8000476c:	02091593          	slli	a1,s2,0x20
    80004770:	9181                	srli	a1,a1,0x20
    80004772:	95de                	add	a1,a1,s7
    80004774:	855a                	mv	a0,s6
    80004776:	83bfc0ef          	jal	80000fb0 <walkaddr>
    8000477a:	862a                	mv	a2,a0
    if(pa == 0)
    8000477c:	d179                	beqz	a0,80004742 <kexec+0xc2>
    if(sz - i < PGSIZE)
    8000477e:	412984bb          	subw	s1,s3,s2
    80004782:	0004879b          	sext.w	a5,s1
    80004786:	fcfcf4e3          	bgeu	s9,a5,8000474e <kexec+0xce>
    8000478a:	84d6                	mv	s1,s5
    8000478c:	b7c9                	j	8000474e <kexec+0xce>
    sz = sz1;
    8000478e:	e0843903          	ld	s2,-504(s0)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    80004792:	2d85                	addiw	s11,s11,1
    80004794:	038d0d1b          	addiw	s10,s10,56 # 1038 <_entry-0x7fffefc8>
    80004798:	e8845783          	lhu	a5,-376(s0)
    8000479c:	08fdd063          	bge	s11,a5,8000481c <kexec+0x19c>
    if(readi(ip, 0, (uint64)&ph, off, sizeof(ph)) != sizeof(ph))
    800047a0:	2d01                	sext.w	s10,s10
    800047a2:	03800713          	li	a4,56
    800047a6:	86ea                	mv	a3,s10
    800047a8:	e1840613          	addi	a2,s0,-488
    800047ac:	4581                	li	a1,0
    800047ae:	8552                	mv	a0,s4
    800047b0:	e6dfe0ef          	jal	8000361c <readi>
    800047b4:	03800793          	li	a5,56
    800047b8:	1cf51863          	bne	a0,a5,80004988 <kexec+0x308>
    if(ph.type != ELF_PROG_LOAD)
    800047bc:	e1842783          	lw	a5,-488(s0)
    800047c0:	4705                	li	a4,1
    800047c2:	fce798e3          	bne	a5,a4,80004792 <kexec+0x112>
    if(ph.memsz < ph.filesz)
    800047c6:	e4043483          	ld	s1,-448(s0)
    800047ca:	e3843783          	ld	a5,-456(s0)
    800047ce:	1cf4e163          	bltu	s1,a5,80004990 <kexec+0x310>
    if(ph.vaddr + ph.memsz < ph.vaddr)
    800047d2:	e2843783          	ld	a5,-472(s0)
    800047d6:	94be                	add	s1,s1,a5
    800047d8:	1cf4e063          	bltu	s1,a5,80004998 <kexec+0x318>
    if(ph.vaddr % PGSIZE != 0)
    800047dc:	df043703          	ld	a4,-528(s0)
    800047e0:	8ff9                	and	a5,a5,a4
    800047e2:	1a079f63          	bnez	a5,800049a0 <kexec+0x320>
    if((sz1 = uvmalloc(pagetable, sz, ph.vaddr + ph.memsz, flags2perm(ph.flags))) == 0)
    800047e6:	e1c42503          	lw	a0,-484(s0)
    800047ea:	e7dff0ef          	jal	80004666 <flags2perm>
    800047ee:	86aa                	mv	a3,a0
    800047f0:	8626                	mv	a2,s1
    800047f2:	85ca                	mv	a1,s2
    800047f4:	855a                	mv	a0,s6
    800047f6:	a93fc0ef          	jal	80001288 <uvmalloc>
    800047fa:	e0a43423          	sd	a0,-504(s0)
    800047fe:	1a050563          	beqz	a0,800049a8 <kexec+0x328>
    if(loadseg(pagetable, ph.vaddr, ip, ph.off, ph.filesz) < 0)
    80004802:	e2843b83          	ld	s7,-472(s0)
    80004806:	e2042c03          	lw	s8,-480(s0)
    8000480a:	e3842983          	lw	s3,-456(s0)
  for(i = 0; i < sz; i += PGSIZE){
    8000480e:	00098463          	beqz	s3,80004816 <kexec+0x196>
    80004812:	4901                	li	s2,0
    80004814:	bfa1                	j	8000476c <kexec+0xec>
    sz = sz1;
    80004816:	e0843903          	ld	s2,-504(s0)
    8000481a:	bfa5                	j	80004792 <kexec+0x112>
    8000481c:	7dba                	ld	s11,424(sp)
  iunlockput(ip);
    8000481e:	8552                	mv	a0,s4
    80004820:	c77fe0ef          	jal	80003496 <iunlockput>
  end_op();
    80004824:	cbcff0ef          	jal	80003ce0 <end_op>
  p = myproc();
    80004828:	908fd0ef          	jal	80001930 <myproc>
    8000482c:	8aaa                	mv	s5,a0
  uint64 oldsz = p->sz;
    8000482e:	04853c83          	ld	s9,72(a0)
  sz = PGROUNDUP(sz);
    80004832:	6985                	lui	s3,0x1
    80004834:	19fd                	addi	s3,s3,-1 # fff <_entry-0x7ffff001>
    80004836:	99ca                	add	s3,s3,s2
    80004838:	77fd                	lui	a5,0xfffff
    8000483a:	00f9f9b3          	and	s3,s3,a5
  if((sz1 = uvmalloc(pagetable, sz, sz + (USERSTACK+1)*PGSIZE, PTE_W)) == 0)
    8000483e:	4691                	li	a3,4
    80004840:	6609                	lui	a2,0x2
    80004842:	964e                	add	a2,a2,s3
    80004844:	85ce                	mv	a1,s3
    80004846:	855a                	mv	a0,s6
    80004848:	a41fc0ef          	jal	80001288 <uvmalloc>
    8000484c:	892a                	mv	s2,a0
    8000484e:	e0a43423          	sd	a0,-504(s0)
    80004852:	e519                	bnez	a0,80004860 <kexec+0x1e0>
  if(pagetable)
    80004854:	e1343423          	sd	s3,-504(s0)
    80004858:	4a01                	li	s4,0
    8000485a:	a285                	j	800049ba <kexec+0x33a>
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
    8000485c:	4901                	li	s2,0
    8000485e:	b7c1                	j	8000481e <kexec+0x19e>
  uvmclear(pagetable, sz-(USERSTACK+1)*PGSIZE);
    80004860:	75f9                	lui	a1,0xffffe
    80004862:	95aa                	add	a1,a1,a0
    80004864:	855a                	mv	a0,s6
    80004866:	bf9fc0ef          	jal	8000145e <uvmclear>
  stackbase = sp - USERSTACK*PGSIZE;
    8000486a:	7bfd                	lui	s7,0xfffff
    8000486c:	9bca                	add	s7,s7,s2
  for(argc = 0; argv[argc]; argc++) {
    8000486e:	e0043783          	ld	a5,-512(s0)
    80004872:	6388                	ld	a0,0(a5)
    80004874:	cd39                	beqz	a0,800048d2 <kexec+0x252>
    80004876:	e9040993          	addi	s3,s0,-368
    8000487a:	f9040c13          	addi	s8,s0,-112
    8000487e:	4481                	li	s1,0
    sp -= strlen(argv[argc]) + 1;
    80004880:	d92fc0ef          	jal	80000e12 <strlen>
    80004884:	0015079b          	addiw	a5,a0,1
    80004888:	40f907b3          	sub	a5,s2,a5
    sp -= sp % 16; // riscv sp must be 16-byte aligned
    8000488c:	ff07f913          	andi	s2,a5,-16
    if(sp < stackbase)
    80004890:	13796063          	bltu	s2,s7,800049b0 <kexec+0x330>
    if(copyout(pagetable, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
    80004894:	e0043d03          	ld	s10,-512(s0)
    80004898:	000d3a03          	ld	s4,0(s10)
    8000489c:	8552                	mv	a0,s4
    8000489e:	d74fc0ef          	jal	80000e12 <strlen>
    800048a2:	0015069b          	addiw	a3,a0,1
    800048a6:	8652                	mv	a2,s4
    800048a8:	85ca                	mv	a1,s2
    800048aa:	855a                	mv	a0,s6
    800048ac:	d37fc0ef          	jal	800015e2 <copyout>
    800048b0:	10054263          	bltz	a0,800049b4 <kexec+0x334>
    ustack[argc] = sp;
    800048b4:	0129b023          	sd	s2,0(s3)
  for(argc = 0; argv[argc]; argc++) {
    800048b8:	0485                	addi	s1,s1,1
    800048ba:	008d0793          	addi	a5,s10,8
    800048be:	e0f43023          	sd	a5,-512(s0)
    800048c2:	008d3503          	ld	a0,8(s10)
    800048c6:	c909                	beqz	a0,800048d8 <kexec+0x258>
    if(argc >= MAXARG)
    800048c8:	09a1                	addi	s3,s3,8
    800048ca:	fb899be3          	bne	s3,s8,80004880 <kexec+0x200>
  ip = 0;
    800048ce:	4a01                	li	s4,0
    800048d0:	a0ed                	j	800049ba <kexec+0x33a>
  sp = sz;
    800048d2:	e0843903          	ld	s2,-504(s0)
  for(argc = 0; argv[argc]; argc++) {
    800048d6:	4481                	li	s1,0
  ustack[argc] = 0;
    800048d8:	00349793          	slli	a5,s1,0x3
    800048dc:	f9078793          	addi	a5,a5,-112 # ffffffffffffef90 <end+0xffffffff7ffde218>
    800048e0:	97a2                	add	a5,a5,s0
    800048e2:	f007b023          	sd	zero,-256(a5)
  sp -= (argc+1) * sizeof(uint64);
    800048e6:	00148693          	addi	a3,s1,1
    800048ea:	068e                	slli	a3,a3,0x3
    800048ec:	40d90933          	sub	s2,s2,a3
  sp -= sp % 16;
    800048f0:	ff097913          	andi	s2,s2,-16
  sz = sz1;
    800048f4:	e0843983          	ld	s3,-504(s0)
  if(sp < stackbase)
    800048f8:	f5796ee3          	bltu	s2,s7,80004854 <kexec+0x1d4>
  if(copyout(pagetable, sp, (char *)ustack, (argc+1)*sizeof(uint64)) < 0)
    800048fc:	e9040613          	addi	a2,s0,-368
    80004900:	85ca                	mv	a1,s2
    80004902:	855a                	mv	a0,s6
    80004904:	cdffc0ef          	jal	800015e2 <copyout>
    80004908:	0e054463          	bltz	a0,800049f0 <kexec+0x370>
  p->trapframe->a1 = sp;
    8000490c:	058ab783          	ld	a5,88(s5) # 1058 <_entry-0x7fffefa8>
    80004910:	0727bc23          	sd	s2,120(a5)
  for(last=s=path; *s; s++)
    80004914:	df843783          	ld	a5,-520(s0)
    80004918:	0007c703          	lbu	a4,0(a5)
    8000491c:	cf11                	beqz	a4,80004938 <kexec+0x2b8>
    8000491e:	0785                	addi	a5,a5,1
    if(*s == '/')
    80004920:	02f00693          	li	a3,47
    80004924:	a039                	j	80004932 <kexec+0x2b2>
      last = s+1;
    80004926:	def43c23          	sd	a5,-520(s0)
  for(last=s=path; *s; s++)
    8000492a:	0785                	addi	a5,a5,1
    8000492c:	fff7c703          	lbu	a4,-1(a5)
    80004930:	c701                	beqz	a4,80004938 <kexec+0x2b8>
    if(*s == '/')
    80004932:	fed71ce3          	bne	a4,a3,8000492a <kexec+0x2aa>
    80004936:	bfc5                	j	80004926 <kexec+0x2a6>
  safestrcpy(p->name, last, sizeof(p->name));
    80004938:	4641                	li	a2,16
    8000493a:	df843583          	ld	a1,-520(s0)
    8000493e:	158a8513          	addi	a0,s5,344
    80004942:	c9efc0ef          	jal	80000de0 <safestrcpy>
  oldpagetable = p->pagetable;
    80004946:	050ab503          	ld	a0,80(s5)
  p->pagetable = pagetable;
    8000494a:	056ab823          	sd	s6,80(s5)
  p->sz = sz;
    8000494e:	e0843783          	ld	a5,-504(s0)
    80004952:	04fab423          	sd	a5,72(s5)
  p->mem_reqs = 0;
    80004956:	160ab423          	sd	zero,360(s5)
  p->trapframe->epc = elf.entry;  // initial program counter = ulib.c:start()
    8000495a:	058ab783          	ld	a5,88(s5)
    8000495e:	e6843703          	ld	a4,-408(s0)
    80004962:	ef98                	sd	a4,24(a5)
  p->trapframe->sp = sp; // initial stack pointer
    80004964:	058ab783          	ld	a5,88(s5)
    80004968:	0327b823          	sd	s2,48(a5)
  proc_freepagetable(oldpagetable, oldsz);
    8000496c:	85e6                	mv	a1,s9
    8000496e:	94cfd0ef          	jal	80001aba <proc_freepagetable>
  return argc; // this ends up in a0, the first argument to main(argc, argv)
    80004972:	0004851b          	sext.w	a0,s1
    80004976:	79be                	ld	s3,488(sp)
    80004978:	7a1e                	ld	s4,480(sp)
    8000497a:	6afe                	ld	s5,472(sp)
    8000497c:	6b5e                	ld	s6,464(sp)
    8000497e:	6bbe                	ld	s7,456(sp)
    80004980:	6c1e                	ld	s8,448(sp)
    80004982:	7cfa                	ld	s9,440(sp)
    80004984:	7d5a                	ld	s10,432(sp)
    80004986:	b3a5                	j	800046ee <kexec+0x6e>
    80004988:	e1243423          	sd	s2,-504(s0)
    8000498c:	7dba                	ld	s11,424(sp)
    8000498e:	a035                	j	800049ba <kexec+0x33a>
    80004990:	e1243423          	sd	s2,-504(s0)
    80004994:	7dba                	ld	s11,424(sp)
    80004996:	a015                	j	800049ba <kexec+0x33a>
    80004998:	e1243423          	sd	s2,-504(s0)
    8000499c:	7dba                	ld	s11,424(sp)
    8000499e:	a831                	j	800049ba <kexec+0x33a>
    800049a0:	e1243423          	sd	s2,-504(s0)
    800049a4:	7dba                	ld	s11,424(sp)
    800049a6:	a811                	j	800049ba <kexec+0x33a>
    800049a8:	e1243423          	sd	s2,-504(s0)
    800049ac:	7dba                	ld	s11,424(sp)
    800049ae:	a031                	j	800049ba <kexec+0x33a>
  ip = 0;
    800049b0:	4a01                	li	s4,0
    800049b2:	a021                	j	800049ba <kexec+0x33a>
    800049b4:	4a01                	li	s4,0
  if(pagetable)
    800049b6:	a011                	j	800049ba <kexec+0x33a>
    800049b8:	7dba                	ld	s11,424(sp)
    proc_freepagetable(pagetable, sz);
    800049ba:	e0843583          	ld	a1,-504(s0)
    800049be:	855a                	mv	a0,s6
    800049c0:	8fafd0ef          	jal	80001aba <proc_freepagetable>
  return -1;
    800049c4:	557d                	li	a0,-1
  if(ip){
    800049c6:	000a1b63          	bnez	s4,800049dc <kexec+0x35c>
    800049ca:	79be                	ld	s3,488(sp)
    800049cc:	7a1e                	ld	s4,480(sp)
    800049ce:	6afe                	ld	s5,472(sp)
    800049d0:	6b5e                	ld	s6,464(sp)
    800049d2:	6bbe                	ld	s7,456(sp)
    800049d4:	6c1e                	ld	s8,448(sp)
    800049d6:	7cfa                	ld	s9,440(sp)
    800049d8:	7d5a                	ld	s10,432(sp)
    800049da:	bb11                	j	800046ee <kexec+0x6e>
    800049dc:	79be                	ld	s3,488(sp)
    800049de:	6afe                	ld	s5,472(sp)
    800049e0:	6b5e                	ld	s6,464(sp)
    800049e2:	6bbe                	ld	s7,456(sp)
    800049e4:	6c1e                	ld	s8,448(sp)
    800049e6:	7cfa                	ld	s9,440(sp)
    800049e8:	7d5a                	ld	s10,432(sp)
    800049ea:	b9dd                	j	800046e0 <kexec+0x60>
    800049ec:	6b5e                	ld	s6,464(sp)
    800049ee:	b9cd                	j	800046e0 <kexec+0x60>
  sz = sz1;
    800049f0:	e0843983          	ld	s3,-504(s0)
    800049f4:	b585                	j	80004854 <kexec+0x1d4>

00000000800049f6 <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
    800049f6:	7179                	addi	sp,sp,-48
    800049f8:	f406                	sd	ra,40(sp)
    800049fa:	f022                	sd	s0,32(sp)
    800049fc:	ec26                	sd	s1,24(sp)
    800049fe:	e84a                	sd	s2,16(sp)
    80004a00:	1800                	addi	s0,sp,48
    80004a02:	892e                	mv	s2,a1
    80004a04:	84b2                	mv	s1,a2
  int fd;
  struct file *f;

  argint(n, &fd);
    80004a06:	fdc40593          	addi	a1,s0,-36
    80004a0a:	e27fd0ef          	jal	80002830 <argint>
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
    80004a0e:	fdc42703          	lw	a4,-36(s0)
    80004a12:	47bd                	li	a5,15
    80004a14:	02e7e963          	bltu	a5,a4,80004a46 <argfd+0x50>
    80004a18:	f19fc0ef          	jal	80001930 <myproc>
    80004a1c:	fdc42703          	lw	a4,-36(s0)
    80004a20:	01a70793          	addi	a5,a4,26
    80004a24:	078e                	slli	a5,a5,0x3
    80004a26:	953e                	add	a0,a0,a5
    80004a28:	611c                	ld	a5,0(a0)
    80004a2a:	c385                	beqz	a5,80004a4a <argfd+0x54>
    return -1;
  if(pfd)
    80004a2c:	00090463          	beqz	s2,80004a34 <argfd+0x3e>
    *pfd = fd;
    80004a30:	00e92023          	sw	a4,0(s2)
  if(pf)
    *pf = f;
  return 0;
    80004a34:	4501                	li	a0,0
  if(pf)
    80004a36:	c091                	beqz	s1,80004a3a <argfd+0x44>
    *pf = f;
    80004a38:	e09c                	sd	a5,0(s1)
}
    80004a3a:	70a2                	ld	ra,40(sp)
    80004a3c:	7402                	ld	s0,32(sp)
    80004a3e:	64e2                	ld	s1,24(sp)
    80004a40:	6942                	ld	s2,16(sp)
    80004a42:	6145                	addi	sp,sp,48
    80004a44:	8082                	ret
    return -1;
    80004a46:	557d                	li	a0,-1
    80004a48:	bfcd                	j	80004a3a <argfd+0x44>
    80004a4a:	557d                	li	a0,-1
    80004a4c:	b7fd                	j	80004a3a <argfd+0x44>

0000000080004a4e <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
    80004a4e:	1101                	addi	sp,sp,-32
    80004a50:	ec06                	sd	ra,24(sp)
    80004a52:	e822                	sd	s0,16(sp)
    80004a54:	e426                	sd	s1,8(sp)
    80004a56:	1000                	addi	s0,sp,32
    80004a58:	84aa                	mv	s1,a0
  int fd;
  struct proc *p = myproc();
    80004a5a:	ed7fc0ef          	jal	80001930 <myproc>
    80004a5e:	862a                	mv	a2,a0

  for(fd = 0; fd < NOFILE; fd++){
    80004a60:	0d050793          	addi	a5,a0,208
    80004a64:	4501                	li	a0,0
    80004a66:	46c1                	li	a3,16
    if(p->ofile[fd] == 0){
    80004a68:	6398                	ld	a4,0(a5)
    80004a6a:	cb19                	beqz	a4,80004a80 <fdalloc+0x32>
  for(fd = 0; fd < NOFILE; fd++){
    80004a6c:	2505                	addiw	a0,a0,1
    80004a6e:	07a1                	addi	a5,a5,8
    80004a70:	fed51ce3          	bne	a0,a3,80004a68 <fdalloc+0x1a>
      p->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
    80004a74:	557d                	li	a0,-1
}
    80004a76:	60e2                	ld	ra,24(sp)
    80004a78:	6442                	ld	s0,16(sp)
    80004a7a:	64a2                	ld	s1,8(sp)
    80004a7c:	6105                	addi	sp,sp,32
    80004a7e:	8082                	ret
      p->ofile[fd] = f;
    80004a80:	01a50793          	addi	a5,a0,26
    80004a84:	078e                	slli	a5,a5,0x3
    80004a86:	963e                	add	a2,a2,a5
    80004a88:	e204                	sd	s1,0(a2)
      return fd;
    80004a8a:	b7f5                	j	80004a76 <fdalloc+0x28>

0000000080004a8c <create>:
  return -1;
}

static struct inode*
create(char *path, short type, short major, short minor)
{
    80004a8c:	715d                	addi	sp,sp,-80
    80004a8e:	e486                	sd	ra,72(sp)
    80004a90:	e0a2                	sd	s0,64(sp)
    80004a92:	fc26                	sd	s1,56(sp)
    80004a94:	f84a                	sd	s2,48(sp)
    80004a96:	f44e                	sd	s3,40(sp)
    80004a98:	ec56                	sd	s5,24(sp)
    80004a9a:	e85a                	sd	s6,16(sp)
    80004a9c:	0880                	addi	s0,sp,80
    80004a9e:	8b2e                	mv	s6,a1
    80004aa0:	89b2                	mv	s3,a2
    80004aa2:	8936                	mv	s2,a3
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
    80004aa4:	fb040593          	addi	a1,s0,-80
    80004aa8:	814ff0ef          	jal	80003abc <nameiparent>
    80004aac:	84aa                	mv	s1,a0
    80004aae:	10050a63          	beqz	a0,80004bc2 <create+0x136>
    return 0;

  ilock(dp);
    80004ab2:	fdafe0ef          	jal	8000328c <ilock>

  if((ip = dirlookup(dp, name, 0)) != 0){
    80004ab6:	4601                	li	a2,0
    80004ab8:	fb040593          	addi	a1,s0,-80
    80004abc:	8526                	mv	a0,s1
    80004abe:	d7ffe0ef          	jal	8000383c <dirlookup>
    80004ac2:	8aaa                	mv	s5,a0
    80004ac4:	c129                	beqz	a0,80004b06 <create+0x7a>
    iunlockput(dp);
    80004ac6:	8526                	mv	a0,s1
    80004ac8:	9cffe0ef          	jal	80003496 <iunlockput>
    ilock(ip);
    80004acc:	8556                	mv	a0,s5
    80004ace:	fbefe0ef          	jal	8000328c <ilock>
    if(type == T_FILE && (ip->type == T_FILE || ip->type == T_DEVICE))
    80004ad2:	4789                	li	a5,2
    80004ad4:	02fb1463          	bne	s6,a5,80004afc <create+0x70>
    80004ad8:	044ad783          	lhu	a5,68(s5)
    80004adc:	37f9                	addiw	a5,a5,-2
    80004ade:	17c2                	slli	a5,a5,0x30
    80004ae0:	93c1                	srli	a5,a5,0x30
    80004ae2:	4705                	li	a4,1
    80004ae4:	00f76c63          	bltu	a4,a5,80004afc <create+0x70>
  ip->nlink = 0;
  iupdate(ip);
  iunlockput(ip);
  iunlockput(dp);
  return 0;
}
    80004ae8:	8556                	mv	a0,s5
    80004aea:	60a6                	ld	ra,72(sp)
    80004aec:	6406                	ld	s0,64(sp)
    80004aee:	74e2                	ld	s1,56(sp)
    80004af0:	7942                	ld	s2,48(sp)
    80004af2:	79a2                	ld	s3,40(sp)
    80004af4:	6ae2                	ld	s5,24(sp)
    80004af6:	6b42                	ld	s6,16(sp)
    80004af8:	6161                	addi	sp,sp,80
    80004afa:	8082                	ret
    iunlockput(ip);
    80004afc:	8556                	mv	a0,s5
    80004afe:	999fe0ef          	jal	80003496 <iunlockput>
    return 0;
    80004b02:	4a81                	li	s5,0
    80004b04:	b7d5                	j	80004ae8 <create+0x5c>
    80004b06:	f052                	sd	s4,32(sp)
  if((ip = ialloc(dp->dev, type)) == 0){
    80004b08:	85da                	mv	a1,s6
    80004b0a:	4088                	lw	a0,0(s1)
    80004b0c:	e10fe0ef          	jal	8000311c <ialloc>
    80004b10:	8a2a                	mv	s4,a0
    80004b12:	cd15                	beqz	a0,80004b4e <create+0xc2>
  ilock(ip);
    80004b14:	f78fe0ef          	jal	8000328c <ilock>
  ip->major = major;
    80004b18:	053a1323          	sh	s3,70(s4)
  ip->minor = minor;
    80004b1c:	052a1423          	sh	s2,72(s4)
  ip->nlink = 1;
    80004b20:	4905                	li	s2,1
    80004b22:	052a1523          	sh	s2,74(s4)
  iupdate(ip);
    80004b26:	8552                	mv	a0,s4
    80004b28:	eb0fe0ef          	jal	800031d8 <iupdate>
  if(type == T_DIR){  // Create . and .. entries.
    80004b2c:	032b0763          	beq	s6,s2,80004b5a <create+0xce>
  if(dirlink(dp, name, ip->inum) < 0)
    80004b30:	004a2603          	lw	a2,4(s4)
    80004b34:	fb040593          	addi	a1,s0,-80
    80004b38:	8526                	mv	a0,s1
    80004b3a:	ecffe0ef          	jal	80003a08 <dirlink>
    80004b3e:	06054563          	bltz	a0,80004ba8 <create+0x11c>
  iunlockput(dp);
    80004b42:	8526                	mv	a0,s1
    80004b44:	953fe0ef          	jal	80003496 <iunlockput>
  return ip;
    80004b48:	8ad2                	mv	s5,s4
    80004b4a:	7a02                	ld	s4,32(sp)
    80004b4c:	bf71                	j	80004ae8 <create+0x5c>
    iunlockput(dp);
    80004b4e:	8526                	mv	a0,s1
    80004b50:	947fe0ef          	jal	80003496 <iunlockput>
    return 0;
    80004b54:	8ad2                	mv	s5,s4
    80004b56:	7a02                	ld	s4,32(sp)
    80004b58:	bf41                	j	80004ae8 <create+0x5c>
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
    80004b5a:	004a2603          	lw	a2,4(s4)
    80004b5e:	00003597          	auipc	a1,0x3
    80004b62:	a6258593          	addi	a1,a1,-1438 # 800075c0 <etext+0x5c0>
    80004b66:	8552                	mv	a0,s4
    80004b68:	ea1fe0ef          	jal	80003a08 <dirlink>
    80004b6c:	02054e63          	bltz	a0,80004ba8 <create+0x11c>
    80004b70:	40d0                	lw	a2,4(s1)
    80004b72:	00003597          	auipc	a1,0x3
    80004b76:	a5658593          	addi	a1,a1,-1450 # 800075c8 <etext+0x5c8>
    80004b7a:	8552                	mv	a0,s4
    80004b7c:	e8dfe0ef          	jal	80003a08 <dirlink>
    80004b80:	02054463          	bltz	a0,80004ba8 <create+0x11c>
  if(dirlink(dp, name, ip->inum) < 0)
    80004b84:	004a2603          	lw	a2,4(s4)
    80004b88:	fb040593          	addi	a1,s0,-80
    80004b8c:	8526                	mv	a0,s1
    80004b8e:	e7bfe0ef          	jal	80003a08 <dirlink>
    80004b92:	00054b63          	bltz	a0,80004ba8 <create+0x11c>
    dp->nlink++;  // for ".."
    80004b96:	04a4d783          	lhu	a5,74(s1)
    80004b9a:	2785                	addiw	a5,a5,1
    80004b9c:	04f49523          	sh	a5,74(s1)
    iupdate(dp);
    80004ba0:	8526                	mv	a0,s1
    80004ba2:	e36fe0ef          	jal	800031d8 <iupdate>
    80004ba6:	bf71                	j	80004b42 <create+0xb6>
  ip->nlink = 0;
    80004ba8:	040a1523          	sh	zero,74(s4)
  iupdate(ip);
    80004bac:	8552                	mv	a0,s4
    80004bae:	e2afe0ef          	jal	800031d8 <iupdate>
  iunlockput(ip);
    80004bb2:	8552                	mv	a0,s4
    80004bb4:	8e3fe0ef          	jal	80003496 <iunlockput>
  iunlockput(dp);
    80004bb8:	8526                	mv	a0,s1
    80004bba:	8ddfe0ef          	jal	80003496 <iunlockput>
  return 0;
    80004bbe:	7a02                	ld	s4,32(sp)
    80004bc0:	b725                	j	80004ae8 <create+0x5c>
    return 0;
    80004bc2:	8aaa                	mv	s5,a0
    80004bc4:	b715                	j	80004ae8 <create+0x5c>

0000000080004bc6 <sys_dup>:
{
    80004bc6:	7179                	addi	sp,sp,-48
    80004bc8:	f406                	sd	ra,40(sp)
    80004bca:	f022                	sd	s0,32(sp)
    80004bcc:	1800                	addi	s0,sp,48
  if(argfd(0, 0, &f) < 0)
    80004bce:	fd840613          	addi	a2,s0,-40
    80004bd2:	4581                	li	a1,0
    80004bd4:	4501                	li	a0,0
    80004bd6:	e21ff0ef          	jal	800049f6 <argfd>
    return -1;
    80004bda:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0)
    80004bdc:	02054363          	bltz	a0,80004c02 <sys_dup+0x3c>
    80004be0:	ec26                	sd	s1,24(sp)
    80004be2:	e84a                	sd	s2,16(sp)
  if((fd=fdalloc(f)) < 0)
    80004be4:	fd843903          	ld	s2,-40(s0)
    80004be8:	854a                	mv	a0,s2
    80004bea:	e65ff0ef          	jal	80004a4e <fdalloc>
    80004bee:	84aa                	mv	s1,a0
    return -1;
    80004bf0:	57fd                	li	a5,-1
  if((fd=fdalloc(f)) < 0)
    80004bf2:	00054d63          	bltz	a0,80004c0c <sys_dup+0x46>
  filedup(f);
    80004bf6:	854a                	mv	a0,s2
    80004bf8:	c44ff0ef          	jal	8000403c <filedup>
  return fd;
    80004bfc:	87a6                	mv	a5,s1
    80004bfe:	64e2                	ld	s1,24(sp)
    80004c00:	6942                	ld	s2,16(sp)
}
    80004c02:	853e                	mv	a0,a5
    80004c04:	70a2                	ld	ra,40(sp)
    80004c06:	7402                	ld	s0,32(sp)
    80004c08:	6145                	addi	sp,sp,48
    80004c0a:	8082                	ret
    80004c0c:	64e2                	ld	s1,24(sp)
    80004c0e:	6942                	ld	s2,16(sp)
    80004c10:	bfcd                	j	80004c02 <sys_dup+0x3c>

0000000080004c12 <sys_read>:
{
    80004c12:	7179                	addi	sp,sp,-48
    80004c14:	f406                	sd	ra,40(sp)
    80004c16:	f022                	sd	s0,32(sp)
    80004c18:	1800                	addi	s0,sp,48
  argaddr(1, &p);
    80004c1a:	fd840593          	addi	a1,s0,-40
    80004c1e:	4505                	li	a0,1
    80004c20:	c2dfd0ef          	jal	8000284c <argaddr>
  argint(2, &n);
    80004c24:	fe440593          	addi	a1,s0,-28
    80004c28:	4509                	li	a0,2
    80004c2a:	c07fd0ef          	jal	80002830 <argint>
  if(argfd(0, 0, &f) < 0)
    80004c2e:	fe840613          	addi	a2,s0,-24
    80004c32:	4581                	li	a1,0
    80004c34:	4501                	li	a0,0
    80004c36:	dc1ff0ef          	jal	800049f6 <argfd>
    80004c3a:	87aa                	mv	a5,a0
    return -1;
    80004c3c:	557d                	li	a0,-1
  if(argfd(0, 0, &f) < 0)
    80004c3e:	0007ca63          	bltz	a5,80004c52 <sys_read+0x40>
  return fileread(f, p, n);
    80004c42:	fe442603          	lw	a2,-28(s0)
    80004c46:	fd843583          	ld	a1,-40(s0)
    80004c4a:	fe843503          	ld	a0,-24(s0)
    80004c4e:	d54ff0ef          	jal	800041a2 <fileread>
}
    80004c52:	70a2                	ld	ra,40(sp)
    80004c54:	7402                	ld	s0,32(sp)
    80004c56:	6145                	addi	sp,sp,48
    80004c58:	8082                	ret

0000000080004c5a <sys_write>:
{
    80004c5a:	7179                	addi	sp,sp,-48
    80004c5c:	f406                	sd	ra,40(sp)
    80004c5e:	f022                	sd	s0,32(sp)
    80004c60:	1800                	addi	s0,sp,48
  argaddr(1, &p);
    80004c62:	fd840593          	addi	a1,s0,-40
    80004c66:	4505                	li	a0,1
    80004c68:	be5fd0ef          	jal	8000284c <argaddr>
  argint(2, &n);
    80004c6c:	fe440593          	addi	a1,s0,-28
    80004c70:	4509                	li	a0,2
    80004c72:	bbffd0ef          	jal	80002830 <argint>
  if(argfd(0, 0, &f) < 0)
    80004c76:	fe840613          	addi	a2,s0,-24
    80004c7a:	4581                	li	a1,0
    80004c7c:	4501                	li	a0,0
    80004c7e:	d79ff0ef          	jal	800049f6 <argfd>
    80004c82:	87aa                	mv	a5,a0
    return -1;
    80004c84:	557d                	li	a0,-1
  if(argfd(0, 0, &f) < 0)
    80004c86:	0007ca63          	bltz	a5,80004c9a <sys_write+0x40>
  return filewrite(f, p, n);
    80004c8a:	fe442603          	lw	a2,-28(s0)
    80004c8e:	fd843583          	ld	a1,-40(s0)
    80004c92:	fe843503          	ld	a0,-24(s0)
    80004c96:	dcaff0ef          	jal	80004260 <filewrite>
}
    80004c9a:	70a2                	ld	ra,40(sp)
    80004c9c:	7402                	ld	s0,32(sp)
    80004c9e:	6145                	addi	sp,sp,48
    80004ca0:	8082                	ret

0000000080004ca2 <sys_close>:
{
    80004ca2:	1101                	addi	sp,sp,-32
    80004ca4:	ec06                	sd	ra,24(sp)
    80004ca6:	e822                	sd	s0,16(sp)
    80004ca8:	1000                	addi	s0,sp,32
  if(argfd(0, &fd, &f) < 0)
    80004caa:	fe040613          	addi	a2,s0,-32
    80004cae:	fec40593          	addi	a1,s0,-20
    80004cb2:	4501                	li	a0,0
    80004cb4:	d43ff0ef          	jal	800049f6 <argfd>
    return -1;
    80004cb8:	57fd                	li	a5,-1
  if(argfd(0, &fd, &f) < 0)
    80004cba:	02054063          	bltz	a0,80004cda <sys_close+0x38>
  myproc()->ofile[fd] = 0;
    80004cbe:	c73fc0ef          	jal	80001930 <myproc>
    80004cc2:	fec42783          	lw	a5,-20(s0)
    80004cc6:	07e9                	addi	a5,a5,26
    80004cc8:	078e                	slli	a5,a5,0x3
    80004cca:	953e                	add	a0,a0,a5
    80004ccc:	00053023          	sd	zero,0(a0)
  fileclose(f);
    80004cd0:	fe043503          	ld	a0,-32(s0)
    80004cd4:	baeff0ef          	jal	80004082 <fileclose>
  return 0;
    80004cd8:	4781                	li	a5,0
}
    80004cda:	853e                	mv	a0,a5
    80004cdc:	60e2                	ld	ra,24(sp)
    80004cde:	6442                	ld	s0,16(sp)
    80004ce0:	6105                	addi	sp,sp,32
    80004ce2:	8082                	ret

0000000080004ce4 <sys_fstat>:
{
    80004ce4:	1101                	addi	sp,sp,-32
    80004ce6:	ec06                	sd	ra,24(sp)
    80004ce8:	e822                	sd	s0,16(sp)
    80004cea:	1000                	addi	s0,sp,32
  argaddr(1, &st);
    80004cec:	fe040593          	addi	a1,s0,-32
    80004cf0:	4505                	li	a0,1
    80004cf2:	b5bfd0ef          	jal	8000284c <argaddr>
  if(argfd(0, 0, &f) < 0)
    80004cf6:	fe840613          	addi	a2,s0,-24
    80004cfa:	4581                	li	a1,0
    80004cfc:	4501                	li	a0,0
    80004cfe:	cf9ff0ef          	jal	800049f6 <argfd>
    80004d02:	87aa                	mv	a5,a0
    return -1;
    80004d04:	557d                	li	a0,-1
  if(argfd(0, 0, &f) < 0)
    80004d06:	0007c863          	bltz	a5,80004d16 <sys_fstat+0x32>
  return filestat(f, st);
    80004d0a:	fe043583          	ld	a1,-32(s0)
    80004d0e:	fe843503          	ld	a0,-24(s0)
    80004d12:	c32ff0ef          	jal	80004144 <filestat>
}
    80004d16:	60e2                	ld	ra,24(sp)
    80004d18:	6442                	ld	s0,16(sp)
    80004d1a:	6105                	addi	sp,sp,32
    80004d1c:	8082                	ret

0000000080004d1e <sys_link>:
{
    80004d1e:	7169                	addi	sp,sp,-304
    80004d20:	f606                	sd	ra,296(sp)
    80004d22:	f222                	sd	s0,288(sp)
    80004d24:	1a00                	addi	s0,sp,304
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80004d26:	08000613          	li	a2,128
    80004d2a:	ed040593          	addi	a1,s0,-304
    80004d2e:	4501                	li	a0,0
    80004d30:	b39fd0ef          	jal	80002868 <argstr>
    return -1;
    80004d34:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80004d36:	0c054e63          	bltz	a0,80004e12 <sys_link+0xf4>
    80004d3a:	08000613          	li	a2,128
    80004d3e:	f5040593          	addi	a1,s0,-176
    80004d42:	4505                	li	a0,1
    80004d44:	b25fd0ef          	jal	80002868 <argstr>
    return -1;
    80004d48:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80004d4a:	0c054463          	bltz	a0,80004e12 <sys_link+0xf4>
    80004d4e:	ee26                	sd	s1,280(sp)
  begin_op();
    80004d50:	f27fe0ef          	jal	80003c76 <begin_op>
  if((ip = namei(old)) == 0){
    80004d54:	ed040513          	addi	a0,s0,-304
    80004d58:	d4bfe0ef          	jal	80003aa2 <namei>
    80004d5c:	84aa                	mv	s1,a0
    80004d5e:	c53d                	beqz	a0,80004dcc <sys_link+0xae>
  ilock(ip);
    80004d60:	d2cfe0ef          	jal	8000328c <ilock>
  if(ip->type == T_DIR){
    80004d64:	04449703          	lh	a4,68(s1)
    80004d68:	4785                	li	a5,1
    80004d6a:	06f70663          	beq	a4,a5,80004dd6 <sys_link+0xb8>
    80004d6e:	ea4a                	sd	s2,272(sp)
  ip->nlink++;
    80004d70:	04a4d783          	lhu	a5,74(s1)
    80004d74:	2785                	addiw	a5,a5,1
    80004d76:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    80004d7a:	8526                	mv	a0,s1
    80004d7c:	c5cfe0ef          	jal	800031d8 <iupdate>
  iunlock(ip);
    80004d80:	8526                	mv	a0,s1
    80004d82:	db8fe0ef          	jal	8000333a <iunlock>
  if((dp = nameiparent(new, name)) == 0)
    80004d86:	fd040593          	addi	a1,s0,-48
    80004d8a:	f5040513          	addi	a0,s0,-176
    80004d8e:	d2ffe0ef          	jal	80003abc <nameiparent>
    80004d92:	892a                	mv	s2,a0
    80004d94:	cd21                	beqz	a0,80004dec <sys_link+0xce>
  ilock(dp);
    80004d96:	cf6fe0ef          	jal	8000328c <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
    80004d9a:	00092703          	lw	a4,0(s2)
    80004d9e:	409c                	lw	a5,0(s1)
    80004da0:	04f71363          	bne	a4,a5,80004de6 <sys_link+0xc8>
    80004da4:	40d0                	lw	a2,4(s1)
    80004da6:	fd040593          	addi	a1,s0,-48
    80004daa:	854a                	mv	a0,s2
    80004dac:	c5dfe0ef          	jal	80003a08 <dirlink>
    80004db0:	02054b63          	bltz	a0,80004de6 <sys_link+0xc8>
  iunlockput(dp);
    80004db4:	854a                	mv	a0,s2
    80004db6:	ee0fe0ef          	jal	80003496 <iunlockput>
  iput(ip);
    80004dba:	8526                	mv	a0,s1
    80004dbc:	e52fe0ef          	jal	8000340e <iput>
  end_op();
    80004dc0:	f21fe0ef          	jal	80003ce0 <end_op>
  return 0;
    80004dc4:	4781                	li	a5,0
    80004dc6:	64f2                	ld	s1,280(sp)
    80004dc8:	6952                	ld	s2,272(sp)
    80004dca:	a0a1                	j	80004e12 <sys_link+0xf4>
    end_op();
    80004dcc:	f15fe0ef          	jal	80003ce0 <end_op>
    return -1;
    80004dd0:	57fd                	li	a5,-1
    80004dd2:	64f2                	ld	s1,280(sp)
    80004dd4:	a83d                	j	80004e12 <sys_link+0xf4>
    iunlockput(ip);
    80004dd6:	8526                	mv	a0,s1
    80004dd8:	ebefe0ef          	jal	80003496 <iunlockput>
    end_op();
    80004ddc:	f05fe0ef          	jal	80003ce0 <end_op>
    return -1;
    80004de0:	57fd                	li	a5,-1
    80004de2:	64f2                	ld	s1,280(sp)
    80004de4:	a03d                	j	80004e12 <sys_link+0xf4>
    iunlockput(dp);
    80004de6:	854a                	mv	a0,s2
    80004de8:	eaefe0ef          	jal	80003496 <iunlockput>
  ilock(ip);
    80004dec:	8526                	mv	a0,s1
    80004dee:	c9efe0ef          	jal	8000328c <ilock>
  ip->nlink--;
    80004df2:	04a4d783          	lhu	a5,74(s1)
    80004df6:	37fd                	addiw	a5,a5,-1
    80004df8:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    80004dfc:	8526                	mv	a0,s1
    80004dfe:	bdafe0ef          	jal	800031d8 <iupdate>
  iunlockput(ip);
    80004e02:	8526                	mv	a0,s1
    80004e04:	e92fe0ef          	jal	80003496 <iunlockput>
  end_op();
    80004e08:	ed9fe0ef          	jal	80003ce0 <end_op>
  return -1;
    80004e0c:	57fd                	li	a5,-1
    80004e0e:	64f2                	ld	s1,280(sp)
    80004e10:	6952                	ld	s2,272(sp)
}
    80004e12:	853e                	mv	a0,a5
    80004e14:	70b2                	ld	ra,296(sp)
    80004e16:	7412                	ld	s0,288(sp)
    80004e18:	6155                	addi	sp,sp,304
    80004e1a:	8082                	ret

0000000080004e1c <sys_unlink>:
{
    80004e1c:	7151                	addi	sp,sp,-240
    80004e1e:	f586                	sd	ra,232(sp)
    80004e20:	f1a2                	sd	s0,224(sp)
    80004e22:	1980                	addi	s0,sp,240
  if(argstr(0, path, MAXPATH) < 0)
    80004e24:	08000613          	li	a2,128
    80004e28:	f3040593          	addi	a1,s0,-208
    80004e2c:	4501                	li	a0,0
    80004e2e:	a3bfd0ef          	jal	80002868 <argstr>
    80004e32:	16054063          	bltz	a0,80004f92 <sys_unlink+0x176>
    80004e36:	eda6                	sd	s1,216(sp)
  begin_op();
    80004e38:	e3ffe0ef          	jal	80003c76 <begin_op>
  if((dp = nameiparent(path, name)) == 0){
    80004e3c:	fb040593          	addi	a1,s0,-80
    80004e40:	f3040513          	addi	a0,s0,-208
    80004e44:	c79fe0ef          	jal	80003abc <nameiparent>
    80004e48:	84aa                	mv	s1,a0
    80004e4a:	c945                	beqz	a0,80004efa <sys_unlink+0xde>
  ilock(dp);
    80004e4c:	c40fe0ef          	jal	8000328c <ilock>
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    80004e50:	00002597          	auipc	a1,0x2
    80004e54:	77058593          	addi	a1,a1,1904 # 800075c0 <etext+0x5c0>
    80004e58:	fb040513          	addi	a0,s0,-80
    80004e5c:	9cbfe0ef          	jal	80003826 <namecmp>
    80004e60:	10050e63          	beqz	a0,80004f7c <sys_unlink+0x160>
    80004e64:	00002597          	auipc	a1,0x2
    80004e68:	76458593          	addi	a1,a1,1892 # 800075c8 <etext+0x5c8>
    80004e6c:	fb040513          	addi	a0,s0,-80
    80004e70:	9b7fe0ef          	jal	80003826 <namecmp>
    80004e74:	10050463          	beqz	a0,80004f7c <sys_unlink+0x160>
    80004e78:	e9ca                	sd	s2,208(sp)
  if((ip = dirlookup(dp, name, &off)) == 0)
    80004e7a:	f2c40613          	addi	a2,s0,-212
    80004e7e:	fb040593          	addi	a1,s0,-80
    80004e82:	8526                	mv	a0,s1
    80004e84:	9b9fe0ef          	jal	8000383c <dirlookup>
    80004e88:	892a                	mv	s2,a0
    80004e8a:	0e050863          	beqz	a0,80004f7a <sys_unlink+0x15e>
  ilock(ip);
    80004e8e:	bfefe0ef          	jal	8000328c <ilock>
  if(ip->nlink < 1)
    80004e92:	04a91783          	lh	a5,74(s2)
    80004e96:	06f05763          	blez	a5,80004f04 <sys_unlink+0xe8>
  if(ip->type == T_DIR && !isdirempty(ip)){
    80004e9a:	04491703          	lh	a4,68(s2)
    80004e9e:	4785                	li	a5,1
    80004ea0:	06f70963          	beq	a4,a5,80004f12 <sys_unlink+0xf6>
  memset(&de, 0, sizeof(de));
    80004ea4:	4641                	li	a2,16
    80004ea6:	4581                	li	a1,0
    80004ea8:	fc040513          	addi	a0,s0,-64
    80004eac:	df7fb0ef          	jal	80000ca2 <memset>
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80004eb0:	4741                	li	a4,16
    80004eb2:	f2c42683          	lw	a3,-212(s0)
    80004eb6:	fc040613          	addi	a2,s0,-64
    80004eba:	4581                	li	a1,0
    80004ebc:	8526                	mv	a0,s1
    80004ebe:	85bfe0ef          	jal	80003718 <writei>
    80004ec2:	47c1                	li	a5,16
    80004ec4:	08f51b63          	bne	a0,a5,80004f5a <sys_unlink+0x13e>
  if(ip->type == T_DIR){
    80004ec8:	04491703          	lh	a4,68(s2)
    80004ecc:	4785                	li	a5,1
    80004ece:	08f70d63          	beq	a4,a5,80004f68 <sys_unlink+0x14c>
  iunlockput(dp);
    80004ed2:	8526                	mv	a0,s1
    80004ed4:	dc2fe0ef          	jal	80003496 <iunlockput>
  ip->nlink--;
    80004ed8:	04a95783          	lhu	a5,74(s2)
    80004edc:	37fd                	addiw	a5,a5,-1
    80004ede:	04f91523          	sh	a5,74(s2)
  iupdate(ip);
    80004ee2:	854a                	mv	a0,s2
    80004ee4:	af4fe0ef          	jal	800031d8 <iupdate>
  iunlockput(ip);
    80004ee8:	854a                	mv	a0,s2
    80004eea:	dacfe0ef          	jal	80003496 <iunlockput>
  end_op();
    80004eee:	df3fe0ef          	jal	80003ce0 <end_op>
  return 0;
    80004ef2:	4501                	li	a0,0
    80004ef4:	64ee                	ld	s1,216(sp)
    80004ef6:	694e                	ld	s2,208(sp)
    80004ef8:	a849                	j	80004f8a <sys_unlink+0x16e>
    end_op();
    80004efa:	de7fe0ef          	jal	80003ce0 <end_op>
    return -1;
    80004efe:	557d                	li	a0,-1
    80004f00:	64ee                	ld	s1,216(sp)
    80004f02:	a061                	j	80004f8a <sys_unlink+0x16e>
    80004f04:	e5ce                	sd	s3,200(sp)
    panic("unlink: nlink < 1");
    80004f06:	00002517          	auipc	a0,0x2
    80004f0a:	6ca50513          	addi	a0,a0,1738 # 800075d0 <etext+0x5d0>
    80004f0e:	8d3fb0ef          	jal	800007e0 <panic>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    80004f12:	04c92703          	lw	a4,76(s2)
    80004f16:	02000793          	li	a5,32
    80004f1a:	f8e7f5e3          	bgeu	a5,a4,80004ea4 <sys_unlink+0x88>
    80004f1e:	e5ce                	sd	s3,200(sp)
    80004f20:	02000993          	li	s3,32
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80004f24:	4741                	li	a4,16
    80004f26:	86ce                	mv	a3,s3
    80004f28:	f1840613          	addi	a2,s0,-232
    80004f2c:	4581                	li	a1,0
    80004f2e:	854a                	mv	a0,s2
    80004f30:	eecfe0ef          	jal	8000361c <readi>
    80004f34:	47c1                	li	a5,16
    80004f36:	00f51c63          	bne	a0,a5,80004f4e <sys_unlink+0x132>
    if(de.inum != 0)
    80004f3a:	f1845783          	lhu	a5,-232(s0)
    80004f3e:	efa1                	bnez	a5,80004f96 <sys_unlink+0x17a>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    80004f40:	29c1                	addiw	s3,s3,16
    80004f42:	04c92783          	lw	a5,76(s2)
    80004f46:	fcf9efe3          	bltu	s3,a5,80004f24 <sys_unlink+0x108>
    80004f4a:	69ae                	ld	s3,200(sp)
    80004f4c:	bfa1                	j	80004ea4 <sys_unlink+0x88>
      panic("isdirempty: readi");
    80004f4e:	00002517          	auipc	a0,0x2
    80004f52:	69a50513          	addi	a0,a0,1690 # 800075e8 <etext+0x5e8>
    80004f56:	88bfb0ef          	jal	800007e0 <panic>
    80004f5a:	e5ce                	sd	s3,200(sp)
    panic("unlink: writei");
    80004f5c:	00002517          	auipc	a0,0x2
    80004f60:	6a450513          	addi	a0,a0,1700 # 80007600 <etext+0x600>
    80004f64:	87dfb0ef          	jal	800007e0 <panic>
    dp->nlink--;
    80004f68:	04a4d783          	lhu	a5,74(s1)
    80004f6c:	37fd                	addiw	a5,a5,-1
    80004f6e:	04f49523          	sh	a5,74(s1)
    iupdate(dp);
    80004f72:	8526                	mv	a0,s1
    80004f74:	a64fe0ef          	jal	800031d8 <iupdate>
    80004f78:	bfa9                	j	80004ed2 <sys_unlink+0xb6>
    80004f7a:	694e                	ld	s2,208(sp)
  iunlockput(dp);
    80004f7c:	8526                	mv	a0,s1
    80004f7e:	d18fe0ef          	jal	80003496 <iunlockput>
  end_op();
    80004f82:	d5ffe0ef          	jal	80003ce0 <end_op>
  return -1;
    80004f86:	557d                	li	a0,-1
    80004f88:	64ee                	ld	s1,216(sp)
}
    80004f8a:	70ae                	ld	ra,232(sp)
    80004f8c:	740e                	ld	s0,224(sp)
    80004f8e:	616d                	addi	sp,sp,240
    80004f90:	8082                	ret
    return -1;
    80004f92:	557d                	li	a0,-1
    80004f94:	bfdd                	j	80004f8a <sys_unlink+0x16e>
    iunlockput(ip);
    80004f96:	854a                	mv	a0,s2
    80004f98:	cfefe0ef          	jal	80003496 <iunlockput>
    goto bad;
    80004f9c:	694e                	ld	s2,208(sp)
    80004f9e:	69ae                	ld	s3,200(sp)
    80004fa0:	bff1                	j	80004f7c <sys_unlink+0x160>

0000000080004fa2 <sys_open>:

uint64
sys_open(void)
{
    80004fa2:	7131                	addi	sp,sp,-192
    80004fa4:	fd06                	sd	ra,184(sp)
    80004fa6:	f922                	sd	s0,176(sp)
    80004fa8:	0180                	addi	s0,sp,192
  int fd, omode;
  struct file *f;
  struct inode *ip;
  int n;

  argint(1, &omode);
    80004faa:	f4c40593          	addi	a1,s0,-180
    80004fae:	4505                	li	a0,1
    80004fb0:	881fd0ef          	jal	80002830 <argint>
  if((n = argstr(0, path, MAXPATH)) < 0)
    80004fb4:	08000613          	li	a2,128
    80004fb8:	f5040593          	addi	a1,s0,-176
    80004fbc:	4501                	li	a0,0
    80004fbe:	8abfd0ef          	jal	80002868 <argstr>
    80004fc2:	87aa                	mv	a5,a0
    return -1;
    80004fc4:	557d                	li	a0,-1
  if((n = argstr(0, path, MAXPATH)) < 0)
    80004fc6:	0a07c263          	bltz	a5,8000506a <sys_open+0xc8>
    80004fca:	f526                	sd	s1,168(sp)

  begin_op();
    80004fcc:	cabfe0ef          	jal	80003c76 <begin_op>

  if(omode & O_CREATE){
    80004fd0:	f4c42783          	lw	a5,-180(s0)
    80004fd4:	2007f793          	andi	a5,a5,512
    80004fd8:	c3d5                	beqz	a5,8000507c <sys_open+0xda>
    ip = create(path, T_FILE, 0, 0);
    80004fda:	4681                	li	a3,0
    80004fdc:	4601                	li	a2,0
    80004fde:	4589                	li	a1,2
    80004fe0:	f5040513          	addi	a0,s0,-176
    80004fe4:	aa9ff0ef          	jal	80004a8c <create>
    80004fe8:	84aa                	mv	s1,a0
    if(ip == 0){
    80004fea:	c541                	beqz	a0,80005072 <sys_open+0xd0>
      end_op();
      return -1;
    }
  }

  if(ip->type == T_DEVICE && (ip->major < 0 || ip->major >= NDEV)){
    80004fec:	04449703          	lh	a4,68(s1)
    80004ff0:	478d                	li	a5,3
    80004ff2:	00f71763          	bne	a4,a5,80005000 <sys_open+0x5e>
    80004ff6:	0464d703          	lhu	a4,70(s1)
    80004ffa:	47a5                	li	a5,9
    80004ffc:	0ae7ed63          	bltu	a5,a4,800050b6 <sys_open+0x114>
    80005000:	f14a                	sd	s2,160(sp)
    iunlockput(ip);
    end_op();
    return -1;
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
    80005002:	fddfe0ef          	jal	80003fde <filealloc>
    80005006:	892a                	mv	s2,a0
    80005008:	c179                	beqz	a0,800050ce <sys_open+0x12c>
    8000500a:	ed4e                	sd	s3,152(sp)
    8000500c:	a43ff0ef          	jal	80004a4e <fdalloc>
    80005010:	89aa                	mv	s3,a0
    80005012:	0a054a63          	bltz	a0,800050c6 <sys_open+0x124>
    iunlockput(ip);
    end_op();
    return -1;
  }

  if(ip->type == T_DEVICE){
    80005016:	04449703          	lh	a4,68(s1)
    8000501a:	478d                	li	a5,3
    8000501c:	0cf70263          	beq	a4,a5,800050e0 <sys_open+0x13e>
    f->type = FD_DEVICE;
    f->major = ip->major;
  } else {
    f->type = FD_INODE;
    80005020:	4789                	li	a5,2
    80005022:	00f92023          	sw	a5,0(s2)
    f->off = 0;
    80005026:	02092023          	sw	zero,32(s2)
  }
  f->ip = ip;
    8000502a:	00993c23          	sd	s1,24(s2)
  f->readable = !(omode & O_WRONLY);
    8000502e:	f4c42783          	lw	a5,-180(s0)
    80005032:	0017c713          	xori	a4,a5,1
    80005036:	8b05                	andi	a4,a4,1
    80005038:	00e90423          	sb	a4,8(s2)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
    8000503c:	0037f713          	andi	a4,a5,3
    80005040:	00e03733          	snez	a4,a4
    80005044:	00e904a3          	sb	a4,9(s2)

  if((omode & O_TRUNC) && ip->type == T_FILE){
    80005048:	4007f793          	andi	a5,a5,1024
    8000504c:	c791                	beqz	a5,80005058 <sys_open+0xb6>
    8000504e:	04449703          	lh	a4,68(s1)
    80005052:	4789                	li	a5,2
    80005054:	08f70d63          	beq	a4,a5,800050ee <sys_open+0x14c>
    itrunc(ip);
  }

  iunlock(ip);
    80005058:	8526                	mv	a0,s1
    8000505a:	ae0fe0ef          	jal	8000333a <iunlock>
  end_op();
    8000505e:	c83fe0ef          	jal	80003ce0 <end_op>

  return fd;
    80005062:	854e                	mv	a0,s3
    80005064:	74aa                	ld	s1,168(sp)
    80005066:	790a                	ld	s2,160(sp)
    80005068:	69ea                	ld	s3,152(sp)
}
    8000506a:	70ea                	ld	ra,184(sp)
    8000506c:	744a                	ld	s0,176(sp)
    8000506e:	6129                	addi	sp,sp,192
    80005070:	8082                	ret
      end_op();
    80005072:	c6ffe0ef          	jal	80003ce0 <end_op>
      return -1;
    80005076:	557d                	li	a0,-1
    80005078:	74aa                	ld	s1,168(sp)
    8000507a:	bfc5                	j	8000506a <sys_open+0xc8>
    if((ip = namei(path)) == 0){
    8000507c:	f5040513          	addi	a0,s0,-176
    80005080:	a23fe0ef          	jal	80003aa2 <namei>
    80005084:	84aa                	mv	s1,a0
    80005086:	c11d                	beqz	a0,800050ac <sys_open+0x10a>
    ilock(ip);
    80005088:	a04fe0ef          	jal	8000328c <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
    8000508c:	04449703          	lh	a4,68(s1)
    80005090:	4785                	li	a5,1
    80005092:	f4f71de3          	bne	a4,a5,80004fec <sys_open+0x4a>
    80005096:	f4c42783          	lw	a5,-180(s0)
    8000509a:	d3bd                	beqz	a5,80005000 <sys_open+0x5e>
      iunlockput(ip);
    8000509c:	8526                	mv	a0,s1
    8000509e:	bf8fe0ef          	jal	80003496 <iunlockput>
      end_op();
    800050a2:	c3ffe0ef          	jal	80003ce0 <end_op>
      return -1;
    800050a6:	557d                	li	a0,-1
    800050a8:	74aa                	ld	s1,168(sp)
    800050aa:	b7c1                	j	8000506a <sys_open+0xc8>
      end_op();
    800050ac:	c35fe0ef          	jal	80003ce0 <end_op>
      return -1;
    800050b0:	557d                	li	a0,-1
    800050b2:	74aa                	ld	s1,168(sp)
    800050b4:	bf5d                	j	8000506a <sys_open+0xc8>
    iunlockput(ip);
    800050b6:	8526                	mv	a0,s1
    800050b8:	bdefe0ef          	jal	80003496 <iunlockput>
    end_op();
    800050bc:	c25fe0ef          	jal	80003ce0 <end_op>
    return -1;
    800050c0:	557d                	li	a0,-1
    800050c2:	74aa                	ld	s1,168(sp)
    800050c4:	b75d                	j	8000506a <sys_open+0xc8>
      fileclose(f);
    800050c6:	854a                	mv	a0,s2
    800050c8:	fbbfe0ef          	jal	80004082 <fileclose>
    800050cc:	69ea                	ld	s3,152(sp)
    iunlockput(ip);
    800050ce:	8526                	mv	a0,s1
    800050d0:	bc6fe0ef          	jal	80003496 <iunlockput>
    end_op();
    800050d4:	c0dfe0ef          	jal	80003ce0 <end_op>
    return -1;
    800050d8:	557d                	li	a0,-1
    800050da:	74aa                	ld	s1,168(sp)
    800050dc:	790a                	ld	s2,160(sp)
    800050de:	b771                	j	8000506a <sys_open+0xc8>
    f->type = FD_DEVICE;
    800050e0:	00f92023          	sw	a5,0(s2)
    f->major = ip->major;
    800050e4:	04649783          	lh	a5,70(s1)
    800050e8:	02f91223          	sh	a5,36(s2)
    800050ec:	bf3d                	j	8000502a <sys_open+0x88>
    itrunc(ip);
    800050ee:	8526                	mv	a0,s1
    800050f0:	a8afe0ef          	jal	8000337a <itrunc>
    800050f4:	b795                	j	80005058 <sys_open+0xb6>

00000000800050f6 <sys_mkdir>:

uint64
sys_mkdir(void)
{
    800050f6:	7175                	addi	sp,sp,-144
    800050f8:	e506                	sd	ra,136(sp)
    800050fa:	e122                	sd	s0,128(sp)
    800050fc:	0900                	addi	s0,sp,144
  char path[MAXPATH];
  struct inode *ip;

  begin_op();
    800050fe:	b79fe0ef          	jal	80003c76 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
    80005102:	08000613          	li	a2,128
    80005106:	f7040593          	addi	a1,s0,-144
    8000510a:	4501                	li	a0,0
    8000510c:	f5cfd0ef          	jal	80002868 <argstr>
    80005110:	02054363          	bltz	a0,80005136 <sys_mkdir+0x40>
    80005114:	4681                	li	a3,0
    80005116:	4601                	li	a2,0
    80005118:	4585                	li	a1,1
    8000511a:	f7040513          	addi	a0,s0,-144
    8000511e:	96fff0ef          	jal	80004a8c <create>
    80005122:	c911                	beqz	a0,80005136 <sys_mkdir+0x40>
    end_op();
    return -1;
  }
  iunlockput(ip);
    80005124:	b72fe0ef          	jal	80003496 <iunlockput>
  end_op();
    80005128:	bb9fe0ef          	jal	80003ce0 <end_op>
  return 0;
    8000512c:	4501                	li	a0,0
}
    8000512e:	60aa                	ld	ra,136(sp)
    80005130:	640a                	ld	s0,128(sp)
    80005132:	6149                	addi	sp,sp,144
    80005134:	8082                	ret
    end_op();
    80005136:	babfe0ef          	jal	80003ce0 <end_op>
    return -1;
    8000513a:	557d                	li	a0,-1
    8000513c:	bfcd                	j	8000512e <sys_mkdir+0x38>

000000008000513e <sys_mknod>:

uint64
sys_mknod(void)
{
    8000513e:	7135                	addi	sp,sp,-160
    80005140:	ed06                	sd	ra,152(sp)
    80005142:	e922                	sd	s0,144(sp)
    80005144:	1100                	addi	s0,sp,160
  struct inode *ip;
  char path[MAXPATH];
  int major, minor;

  begin_op();
    80005146:	b31fe0ef          	jal	80003c76 <begin_op>
  argint(1, &major);
    8000514a:	f6c40593          	addi	a1,s0,-148
    8000514e:	4505                	li	a0,1
    80005150:	ee0fd0ef          	jal	80002830 <argint>
  argint(2, &minor);
    80005154:	f6840593          	addi	a1,s0,-152
    80005158:	4509                	li	a0,2
    8000515a:	ed6fd0ef          	jal	80002830 <argint>
  if((argstr(0, path, MAXPATH)) < 0 ||
    8000515e:	08000613          	li	a2,128
    80005162:	f7040593          	addi	a1,s0,-144
    80005166:	4501                	li	a0,0
    80005168:	f00fd0ef          	jal	80002868 <argstr>
    8000516c:	02054563          	bltz	a0,80005196 <sys_mknod+0x58>
     (ip = create(path, T_DEVICE, major, minor)) == 0){
    80005170:	f6841683          	lh	a3,-152(s0)
    80005174:	f6c41603          	lh	a2,-148(s0)
    80005178:	458d                	li	a1,3
    8000517a:	f7040513          	addi	a0,s0,-144
    8000517e:	90fff0ef          	jal	80004a8c <create>
  if((argstr(0, path, MAXPATH)) < 0 ||
    80005182:	c911                	beqz	a0,80005196 <sys_mknod+0x58>
    end_op();
    return -1;
  }
  iunlockput(ip);
    80005184:	b12fe0ef          	jal	80003496 <iunlockput>
  end_op();
    80005188:	b59fe0ef          	jal	80003ce0 <end_op>
  return 0;
    8000518c:	4501                	li	a0,0
}
    8000518e:	60ea                	ld	ra,152(sp)
    80005190:	644a                	ld	s0,144(sp)
    80005192:	610d                	addi	sp,sp,160
    80005194:	8082                	ret
    end_op();
    80005196:	b4bfe0ef          	jal	80003ce0 <end_op>
    return -1;
    8000519a:	557d                	li	a0,-1
    8000519c:	bfcd                	j	8000518e <sys_mknod+0x50>

000000008000519e <sys_chdir>:

uint64
sys_chdir(void)
{
    8000519e:	7135                	addi	sp,sp,-160
    800051a0:	ed06                	sd	ra,152(sp)
    800051a2:	e922                	sd	s0,144(sp)
    800051a4:	e14a                	sd	s2,128(sp)
    800051a6:	1100                	addi	s0,sp,160
  char path[MAXPATH];
  struct inode *ip;
  struct proc *p = myproc();
    800051a8:	f88fc0ef          	jal	80001930 <myproc>
    800051ac:	892a                	mv	s2,a0
  
  begin_op();
    800051ae:	ac9fe0ef          	jal	80003c76 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = namei(path)) == 0){
    800051b2:	08000613          	li	a2,128
    800051b6:	f6040593          	addi	a1,s0,-160
    800051ba:	4501                	li	a0,0
    800051bc:	eacfd0ef          	jal	80002868 <argstr>
    800051c0:	04054363          	bltz	a0,80005206 <sys_chdir+0x68>
    800051c4:	e526                	sd	s1,136(sp)
    800051c6:	f6040513          	addi	a0,s0,-160
    800051ca:	8d9fe0ef          	jal	80003aa2 <namei>
    800051ce:	84aa                	mv	s1,a0
    800051d0:	c915                	beqz	a0,80005204 <sys_chdir+0x66>
    end_op();
    return -1;
  }
  ilock(ip);
    800051d2:	8bafe0ef          	jal	8000328c <ilock>
  if(ip->type != T_DIR){
    800051d6:	04449703          	lh	a4,68(s1)
    800051da:	4785                	li	a5,1
    800051dc:	02f71963          	bne	a4,a5,8000520e <sys_chdir+0x70>
    iunlockput(ip);
    end_op();
    return -1;
  }
  iunlock(ip);
    800051e0:	8526                	mv	a0,s1
    800051e2:	958fe0ef          	jal	8000333a <iunlock>
  iput(p->cwd);
    800051e6:	15093503          	ld	a0,336(s2)
    800051ea:	a24fe0ef          	jal	8000340e <iput>
  end_op();
    800051ee:	af3fe0ef          	jal	80003ce0 <end_op>
  p->cwd = ip;
    800051f2:	14993823          	sd	s1,336(s2)
  return 0;
    800051f6:	4501                	li	a0,0
    800051f8:	64aa                	ld	s1,136(sp)
}
    800051fa:	60ea                	ld	ra,152(sp)
    800051fc:	644a                	ld	s0,144(sp)
    800051fe:	690a                	ld	s2,128(sp)
    80005200:	610d                	addi	sp,sp,160
    80005202:	8082                	ret
    80005204:	64aa                	ld	s1,136(sp)
    end_op();
    80005206:	adbfe0ef          	jal	80003ce0 <end_op>
    return -1;
    8000520a:	557d                	li	a0,-1
    8000520c:	b7fd                	j	800051fa <sys_chdir+0x5c>
    iunlockput(ip);
    8000520e:	8526                	mv	a0,s1
    80005210:	a86fe0ef          	jal	80003496 <iunlockput>
    end_op();
    80005214:	acdfe0ef          	jal	80003ce0 <end_op>
    return -1;
    80005218:	557d                	li	a0,-1
    8000521a:	64aa                	ld	s1,136(sp)
    8000521c:	bff9                	j	800051fa <sys_chdir+0x5c>

000000008000521e <sys_exec>:

uint64
sys_exec(void)
{
    8000521e:	7121                	addi	sp,sp,-448
    80005220:	ff06                	sd	ra,440(sp)
    80005222:	fb22                	sd	s0,432(sp)
    80005224:	0380                	addi	s0,sp,448
  char path[MAXPATH], *argv[MAXARG];
  int i;
  uint64 uargv, uarg;

  argaddr(1, &uargv);
    80005226:	e4840593          	addi	a1,s0,-440
    8000522a:	4505                	li	a0,1
    8000522c:	e20fd0ef          	jal	8000284c <argaddr>
  if(argstr(0, path, MAXPATH) < 0) {
    80005230:	08000613          	li	a2,128
    80005234:	f5040593          	addi	a1,s0,-176
    80005238:	4501                	li	a0,0
    8000523a:	e2efd0ef          	jal	80002868 <argstr>
    8000523e:	87aa                	mv	a5,a0
    return -1;
    80005240:	557d                	li	a0,-1
  if(argstr(0, path, MAXPATH) < 0) {
    80005242:	0c07c463          	bltz	a5,8000530a <sys_exec+0xec>
    80005246:	f726                	sd	s1,424(sp)
    80005248:	f34a                	sd	s2,416(sp)
    8000524a:	ef4e                	sd	s3,408(sp)
    8000524c:	eb52                	sd	s4,400(sp)
  }
  memset(argv, 0, sizeof(argv));
    8000524e:	10000613          	li	a2,256
    80005252:	4581                	li	a1,0
    80005254:	e5040513          	addi	a0,s0,-432
    80005258:	a4bfb0ef          	jal	80000ca2 <memset>
  for(i=0;; i++){
    if(i >= NELEM(argv)){
    8000525c:	e5040493          	addi	s1,s0,-432
  memset(argv, 0, sizeof(argv));
    80005260:	89a6                	mv	s3,s1
    80005262:	4901                	li	s2,0
    if(i >= NELEM(argv)){
    80005264:	02000a13          	li	s4,32
      goto bad;
    }
    if(fetchaddr(uargv+sizeof(uint64)*i, (uint64*)&uarg) < 0){
    80005268:	00391513          	slli	a0,s2,0x3
    8000526c:	e4040593          	addi	a1,s0,-448
    80005270:	e4843783          	ld	a5,-440(s0)
    80005274:	953e                	add	a0,a0,a5
    80005276:	d30fd0ef          	jal	800027a6 <fetchaddr>
    8000527a:	02054663          	bltz	a0,800052a6 <sys_exec+0x88>
      goto bad;
    }
    if(uarg == 0){
    8000527e:	e4043783          	ld	a5,-448(s0)
    80005282:	c3a9                	beqz	a5,800052c4 <sys_exec+0xa6>
      argv[i] = 0;
      break;
    }
    argv[i] = kalloc();
    80005284:	87bfb0ef          	jal	80000afe <kalloc>
    80005288:	85aa                	mv	a1,a0
    8000528a:	00a9b023          	sd	a0,0(s3)
    if(argv[i] == 0)
    8000528e:	cd01                	beqz	a0,800052a6 <sys_exec+0x88>
      goto bad;
    if(fetchstr(uarg, argv[i], PGSIZE) < 0)
    80005290:	6605                	lui	a2,0x1
    80005292:	e4043503          	ld	a0,-448(s0)
    80005296:	d5afd0ef          	jal	800027f0 <fetchstr>
    8000529a:	00054663          	bltz	a0,800052a6 <sys_exec+0x88>
    if(i >= NELEM(argv)){
    8000529e:	0905                	addi	s2,s2,1
    800052a0:	09a1                	addi	s3,s3,8
    800052a2:	fd4913e3          	bne	s2,s4,80005268 <sys_exec+0x4a>
    kfree(argv[i]);

  return ret;

 bad:
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    800052a6:	f5040913          	addi	s2,s0,-176
    800052aa:	6088                	ld	a0,0(s1)
    800052ac:	c931                	beqz	a0,80005300 <sys_exec+0xe2>
    kfree(argv[i]);
    800052ae:	f6efb0ef          	jal	80000a1c <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    800052b2:	04a1                	addi	s1,s1,8
    800052b4:	ff249be3          	bne	s1,s2,800052aa <sys_exec+0x8c>
  return -1;
    800052b8:	557d                	li	a0,-1
    800052ba:	74ba                	ld	s1,424(sp)
    800052bc:	791a                	ld	s2,416(sp)
    800052be:	69fa                	ld	s3,408(sp)
    800052c0:	6a5a                	ld	s4,400(sp)
    800052c2:	a0a1                	j	8000530a <sys_exec+0xec>
      argv[i] = 0;
    800052c4:	0009079b          	sext.w	a5,s2
    800052c8:	078e                	slli	a5,a5,0x3
    800052ca:	fd078793          	addi	a5,a5,-48
    800052ce:	97a2                	add	a5,a5,s0
    800052d0:	e807b023          	sd	zero,-384(a5)
  int ret = kexec(path, argv);
    800052d4:	e5040593          	addi	a1,s0,-432
    800052d8:	f5040513          	addi	a0,s0,-176
    800052dc:	ba4ff0ef          	jal	80004680 <kexec>
    800052e0:	892a                	mv	s2,a0
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    800052e2:	f5040993          	addi	s3,s0,-176
    800052e6:	6088                	ld	a0,0(s1)
    800052e8:	c511                	beqz	a0,800052f4 <sys_exec+0xd6>
    kfree(argv[i]);
    800052ea:	f32fb0ef          	jal	80000a1c <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    800052ee:	04a1                	addi	s1,s1,8
    800052f0:	ff349be3          	bne	s1,s3,800052e6 <sys_exec+0xc8>
  return ret;
    800052f4:	854a                	mv	a0,s2
    800052f6:	74ba                	ld	s1,424(sp)
    800052f8:	791a                	ld	s2,416(sp)
    800052fa:	69fa                	ld	s3,408(sp)
    800052fc:	6a5a                	ld	s4,400(sp)
    800052fe:	a031                	j	8000530a <sys_exec+0xec>
  return -1;
    80005300:	557d                	li	a0,-1
    80005302:	74ba                	ld	s1,424(sp)
    80005304:	791a                	ld	s2,416(sp)
    80005306:	69fa                	ld	s3,408(sp)
    80005308:	6a5a                	ld	s4,400(sp)
}
    8000530a:	70fa                	ld	ra,440(sp)
    8000530c:	745a                	ld	s0,432(sp)
    8000530e:	6139                	addi	sp,sp,448
    80005310:	8082                	ret

0000000080005312 <sys_pipe>:

uint64
sys_pipe(void)
{
    80005312:	7139                	addi	sp,sp,-64
    80005314:	fc06                	sd	ra,56(sp)
    80005316:	f822                	sd	s0,48(sp)
    80005318:	f426                	sd	s1,40(sp)
    8000531a:	0080                	addi	s0,sp,64
  uint64 fdarray; // user pointer to array of two integers
  struct file *rf, *wf;
  int fd0, fd1;
  struct proc *p = myproc();
    8000531c:	e14fc0ef          	jal	80001930 <myproc>
    80005320:	84aa                	mv	s1,a0

  argaddr(0, &fdarray);
    80005322:	fd840593          	addi	a1,s0,-40
    80005326:	4501                	li	a0,0
    80005328:	d24fd0ef          	jal	8000284c <argaddr>
  if(pipealloc(&rf, &wf) < 0)
    8000532c:	fc840593          	addi	a1,s0,-56
    80005330:	fd040513          	addi	a0,s0,-48
    80005334:	858ff0ef          	jal	8000438c <pipealloc>
    return -1;
    80005338:	57fd                	li	a5,-1
  if(pipealloc(&rf, &wf) < 0)
    8000533a:	0a054463          	bltz	a0,800053e2 <sys_pipe+0xd0>
  fd0 = -1;
    8000533e:	fcf42223          	sw	a5,-60(s0)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
    80005342:	fd043503          	ld	a0,-48(s0)
    80005346:	f08ff0ef          	jal	80004a4e <fdalloc>
    8000534a:	fca42223          	sw	a0,-60(s0)
    8000534e:	08054163          	bltz	a0,800053d0 <sys_pipe+0xbe>
    80005352:	fc843503          	ld	a0,-56(s0)
    80005356:	ef8ff0ef          	jal	80004a4e <fdalloc>
    8000535a:	fca42023          	sw	a0,-64(s0)
    8000535e:	06054063          	bltz	a0,800053be <sys_pipe+0xac>
      p->ofile[fd0] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  }
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    80005362:	4691                	li	a3,4
    80005364:	fc440613          	addi	a2,s0,-60
    80005368:	fd843583          	ld	a1,-40(s0)
    8000536c:	68a8                	ld	a0,80(s1)
    8000536e:	a74fc0ef          	jal	800015e2 <copyout>
    80005372:	00054e63          	bltz	a0,8000538e <sys_pipe+0x7c>
     copyout(p->pagetable, fdarray+sizeof(fd0), (char *)&fd1, sizeof(fd1)) < 0){
    80005376:	4691                	li	a3,4
    80005378:	fc040613          	addi	a2,s0,-64
    8000537c:	fd843583          	ld	a1,-40(s0)
    80005380:	0591                	addi	a1,a1,4
    80005382:	68a8                	ld	a0,80(s1)
    80005384:	a5efc0ef          	jal	800015e2 <copyout>
    p->ofile[fd1] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  }
  return 0;
    80005388:	4781                	li	a5,0
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    8000538a:	04055c63          	bgez	a0,800053e2 <sys_pipe+0xd0>
    p->ofile[fd0] = 0;
    8000538e:	fc442783          	lw	a5,-60(s0)
    80005392:	07e9                	addi	a5,a5,26
    80005394:	078e                	slli	a5,a5,0x3
    80005396:	97a6                	add	a5,a5,s1
    80005398:	0007b023          	sd	zero,0(a5)
    p->ofile[fd1] = 0;
    8000539c:	fc042783          	lw	a5,-64(s0)
    800053a0:	07e9                	addi	a5,a5,26
    800053a2:	078e                	slli	a5,a5,0x3
    800053a4:	94be                	add	s1,s1,a5
    800053a6:	0004b023          	sd	zero,0(s1)
    fileclose(rf);
    800053aa:	fd043503          	ld	a0,-48(s0)
    800053ae:	cd5fe0ef          	jal	80004082 <fileclose>
    fileclose(wf);
    800053b2:	fc843503          	ld	a0,-56(s0)
    800053b6:	ccdfe0ef          	jal	80004082 <fileclose>
    return -1;
    800053ba:	57fd                	li	a5,-1
    800053bc:	a01d                	j	800053e2 <sys_pipe+0xd0>
    if(fd0 >= 0)
    800053be:	fc442783          	lw	a5,-60(s0)
    800053c2:	0007c763          	bltz	a5,800053d0 <sys_pipe+0xbe>
      p->ofile[fd0] = 0;
    800053c6:	07e9                	addi	a5,a5,26
    800053c8:	078e                	slli	a5,a5,0x3
    800053ca:	97a6                	add	a5,a5,s1
    800053cc:	0007b023          	sd	zero,0(a5)
    fileclose(rf);
    800053d0:	fd043503          	ld	a0,-48(s0)
    800053d4:	caffe0ef          	jal	80004082 <fileclose>
    fileclose(wf);
    800053d8:	fc843503          	ld	a0,-56(s0)
    800053dc:	ca7fe0ef          	jal	80004082 <fileclose>
    return -1;
    800053e0:	57fd                	li	a5,-1
}
    800053e2:	853e                	mv	a0,a5
    800053e4:	70e2                	ld	ra,56(sp)
    800053e6:	7442                	ld	s0,48(sp)
    800053e8:	74a2                	ld	s1,40(sp)
    800053ea:	6121                	addi	sp,sp,64
    800053ec:	8082                	ret
	...

00000000800053f0 <kernelvec>:
.globl kerneltrap
.globl kernelvec
.align 4
kernelvec:
        # make room to save registers.
        addi sp, sp, -256
    800053f0:	7111                	addi	sp,sp,-256

        # save caller-saved registers.
        sd ra, 0(sp)
    800053f2:	e006                	sd	ra,0(sp)
        # sd sp, 8(sp)
        sd gp, 16(sp)
    800053f4:	e80e                	sd	gp,16(sp)
        sd tp, 24(sp)
    800053f6:	ec12                	sd	tp,24(sp)
        sd t0, 32(sp)
    800053f8:	f016                	sd	t0,32(sp)
        sd t1, 40(sp)
    800053fa:	f41a                	sd	t1,40(sp)
        sd t2, 48(sp)
    800053fc:	f81e                	sd	t2,48(sp)
        sd a0, 72(sp)
    800053fe:	e4aa                	sd	a0,72(sp)
        sd a1, 80(sp)
    80005400:	e8ae                	sd	a1,80(sp)
        sd a2, 88(sp)
    80005402:	ecb2                	sd	a2,88(sp)
        sd a3, 96(sp)
    80005404:	f0b6                	sd	a3,96(sp)
        sd a4, 104(sp)
    80005406:	f4ba                	sd	a4,104(sp)
        sd a5, 112(sp)
    80005408:	f8be                	sd	a5,112(sp)
        sd a6, 120(sp)
    8000540a:	fcc2                	sd	a6,120(sp)
        sd a7, 128(sp)
    8000540c:	e146                	sd	a7,128(sp)
        sd t3, 216(sp)
    8000540e:	edf2                	sd	t3,216(sp)
        sd t4, 224(sp)
    80005410:	f1f6                	sd	t4,224(sp)
        sd t5, 232(sp)
    80005412:	f5fa                	sd	t5,232(sp)
        sd t6, 240(sp)
    80005414:	f9fe                	sd	t6,240(sp)

        # call the C trap handler in trap.c
        call kerneltrap
    80005416:	aa0fd0ef          	jal	800026b6 <kerneltrap>

        # restore registers.
        ld ra, 0(sp)
    8000541a:	6082                	ld	ra,0(sp)
        # ld sp, 8(sp)
        ld gp, 16(sp)
    8000541c:	61c2                	ld	gp,16(sp)
        # not tp (contains hartid), in case we moved CPUs
        ld t0, 32(sp)
    8000541e:	7282                	ld	t0,32(sp)
        ld t1, 40(sp)
    80005420:	7322                	ld	t1,40(sp)
        ld t2, 48(sp)
    80005422:	73c2                	ld	t2,48(sp)
        ld a0, 72(sp)
    80005424:	6526                	ld	a0,72(sp)
        ld a1, 80(sp)
    80005426:	65c6                	ld	a1,80(sp)
        ld a2, 88(sp)
    80005428:	6666                	ld	a2,88(sp)
        ld a3, 96(sp)
    8000542a:	7686                	ld	a3,96(sp)
        ld a4, 104(sp)
    8000542c:	7726                	ld	a4,104(sp)
        ld a5, 112(sp)
    8000542e:	77c6                	ld	a5,112(sp)
        ld a6, 120(sp)
    80005430:	7866                	ld	a6,120(sp)
        ld a7, 128(sp)
    80005432:	688a                	ld	a7,128(sp)
        ld t3, 216(sp)
    80005434:	6e6e                	ld	t3,216(sp)
        ld t4, 224(sp)
    80005436:	7e8e                	ld	t4,224(sp)
        ld t5, 232(sp)
    80005438:	7f2e                	ld	t5,232(sp)
        ld t6, 240(sp)
    8000543a:	7fce                	ld	t6,240(sp)

        addi sp, sp, 256
    8000543c:	6111                	addi	sp,sp,256

        # return to whatever we were doing in the kernel.
        sret
    8000543e:	10200073          	sret
	...

000000008000544e <plicinit>:
// the riscv Platform Level Interrupt Controller (PLIC).
//

void
plicinit(void)
{
    8000544e:	1141                	addi	sp,sp,-16
    80005450:	e422                	sd	s0,8(sp)
    80005452:	0800                	addi	s0,sp,16
  // set desired IRQ priorities non-zero (otherwise disabled).
  *(uint32*)(PLIC + UART0_IRQ*4) = 1;
    80005454:	0c0007b7          	lui	a5,0xc000
    80005458:	4705                	li	a4,1
    8000545a:	d798                	sw	a4,40(a5)
  *(uint32*)(PLIC + VIRTIO0_IRQ*4) = 1;
    8000545c:	0c0007b7          	lui	a5,0xc000
    80005460:	c3d8                	sw	a4,4(a5)
}
    80005462:	6422                	ld	s0,8(sp)
    80005464:	0141                	addi	sp,sp,16
    80005466:	8082                	ret

0000000080005468 <plicinithart>:

void
plicinithart(void)
{
    80005468:	1141                	addi	sp,sp,-16
    8000546a:	e406                	sd	ra,8(sp)
    8000546c:	e022                	sd	s0,0(sp)
    8000546e:	0800                	addi	s0,sp,16
  int hart = cpuid();
    80005470:	c94fc0ef          	jal	80001904 <cpuid>
  
  // set enable bits for this hart's S-mode
  // for the uart and virtio disk.
  *(uint32*)PLIC_SENABLE(hart) = (1 << UART0_IRQ) | (1 << VIRTIO0_IRQ);
    80005474:	0085171b          	slliw	a4,a0,0x8
    80005478:	0c0027b7          	lui	a5,0xc002
    8000547c:	97ba                	add	a5,a5,a4
    8000547e:	40200713          	li	a4,1026
    80005482:	08e7a023          	sw	a4,128(a5) # c002080 <_entry-0x73ffdf80>

  // set this hart's S-mode priority threshold to 0.
  *(uint32*)PLIC_SPRIORITY(hart) = 0;
    80005486:	00d5151b          	slliw	a0,a0,0xd
    8000548a:	0c2017b7          	lui	a5,0xc201
    8000548e:	97aa                	add	a5,a5,a0
    80005490:	0007a023          	sw	zero,0(a5) # c201000 <_entry-0x73dff000>
}
    80005494:	60a2                	ld	ra,8(sp)
    80005496:	6402                	ld	s0,0(sp)
    80005498:	0141                	addi	sp,sp,16
    8000549a:	8082                	ret

000000008000549c <plic_claim>:

// ask the PLIC what interrupt we should serve.
int
plic_claim(void)
{
    8000549c:	1141                	addi	sp,sp,-16
    8000549e:	e406                	sd	ra,8(sp)
    800054a0:	e022                	sd	s0,0(sp)
    800054a2:	0800                	addi	s0,sp,16
  int hart = cpuid();
    800054a4:	c60fc0ef          	jal	80001904 <cpuid>
  int irq = *(uint32*)PLIC_SCLAIM(hart);
    800054a8:	00d5151b          	slliw	a0,a0,0xd
    800054ac:	0c2017b7          	lui	a5,0xc201
    800054b0:	97aa                	add	a5,a5,a0
  return irq;
}
    800054b2:	43c8                	lw	a0,4(a5)
    800054b4:	60a2                	ld	ra,8(sp)
    800054b6:	6402                	ld	s0,0(sp)
    800054b8:	0141                	addi	sp,sp,16
    800054ba:	8082                	ret

00000000800054bc <plic_complete>:

// tell the PLIC we've served this IRQ.
void
plic_complete(int irq)
{
    800054bc:	1101                	addi	sp,sp,-32
    800054be:	ec06                	sd	ra,24(sp)
    800054c0:	e822                	sd	s0,16(sp)
    800054c2:	e426                	sd	s1,8(sp)
    800054c4:	1000                	addi	s0,sp,32
    800054c6:	84aa                	mv	s1,a0
  int hart = cpuid();
    800054c8:	c3cfc0ef          	jal	80001904 <cpuid>
  *(uint32*)PLIC_SCLAIM(hart) = irq;
    800054cc:	00d5151b          	slliw	a0,a0,0xd
    800054d0:	0c2017b7          	lui	a5,0xc201
    800054d4:	97aa                	add	a5,a5,a0
    800054d6:	c3c4                	sw	s1,4(a5)
}
    800054d8:	60e2                	ld	ra,24(sp)
    800054da:	6442                	ld	s0,16(sp)
    800054dc:	64a2                	ld	s1,8(sp)
    800054de:	6105                	addi	sp,sp,32
    800054e0:	8082                	ret

00000000800054e2 <free_desc>:
}

// mark a descriptor as free.
static void
free_desc(int i)
{
    800054e2:	1141                	addi	sp,sp,-16
    800054e4:	e406                	sd	ra,8(sp)
    800054e6:	e022                	sd	s0,0(sp)
    800054e8:	0800                	addi	s0,sp,16
  if(i >= NUM)
    800054ea:	479d                	li	a5,7
    800054ec:	04a7ca63          	blt	a5,a0,80005540 <free_desc+0x5e>
    panic("free_desc 1");
  if(disk.free[i])
    800054f0:	0001b797          	auipc	a5,0x1b
    800054f4:	74878793          	addi	a5,a5,1864 # 80020c38 <disk>
    800054f8:	97aa                	add	a5,a5,a0
    800054fa:	0187c783          	lbu	a5,24(a5)
    800054fe:	e7b9                	bnez	a5,8000554c <free_desc+0x6a>
    panic("free_desc 2");
  disk.desc[i].addr = 0;
    80005500:	00451693          	slli	a3,a0,0x4
    80005504:	0001b797          	auipc	a5,0x1b
    80005508:	73478793          	addi	a5,a5,1844 # 80020c38 <disk>
    8000550c:	6398                	ld	a4,0(a5)
    8000550e:	9736                	add	a4,a4,a3
    80005510:	00073023          	sd	zero,0(a4)
  disk.desc[i].len = 0;
    80005514:	6398                	ld	a4,0(a5)
    80005516:	9736                	add	a4,a4,a3
    80005518:	00072423          	sw	zero,8(a4)
  disk.desc[i].flags = 0;
    8000551c:	00071623          	sh	zero,12(a4)
  disk.desc[i].next = 0;
    80005520:	00071723          	sh	zero,14(a4)
  disk.free[i] = 1;
    80005524:	97aa                	add	a5,a5,a0
    80005526:	4705                	li	a4,1
    80005528:	00e78c23          	sb	a4,24(a5)
  wakeup(&disk.free[0]);
    8000552c:	0001b517          	auipc	a0,0x1b
    80005530:	72450513          	addi	a0,a0,1828 # 80020c50 <disk+0x18>
    80005534:	a45fc0ef          	jal	80001f78 <wakeup>
}
    80005538:	60a2                	ld	ra,8(sp)
    8000553a:	6402                	ld	s0,0(sp)
    8000553c:	0141                	addi	sp,sp,16
    8000553e:	8082                	ret
    panic("free_desc 1");
    80005540:	00002517          	auipc	a0,0x2
    80005544:	0d050513          	addi	a0,a0,208 # 80007610 <etext+0x610>
    80005548:	a98fb0ef          	jal	800007e0 <panic>
    panic("free_desc 2");
    8000554c:	00002517          	auipc	a0,0x2
    80005550:	0d450513          	addi	a0,a0,212 # 80007620 <etext+0x620>
    80005554:	a8cfb0ef          	jal	800007e0 <panic>

0000000080005558 <virtio_disk_init>:
{
    80005558:	1101                	addi	sp,sp,-32
    8000555a:	ec06                	sd	ra,24(sp)
    8000555c:	e822                	sd	s0,16(sp)
    8000555e:	e426                	sd	s1,8(sp)
    80005560:	e04a                	sd	s2,0(sp)
    80005562:	1000                	addi	s0,sp,32
  initlock(&disk.vdisk_lock, "virtio_disk");
    80005564:	00002597          	auipc	a1,0x2
    80005568:	0cc58593          	addi	a1,a1,204 # 80007630 <etext+0x630>
    8000556c:	0001b517          	auipc	a0,0x1b
    80005570:	7f450513          	addi	a0,a0,2036 # 80020d60 <disk+0x128>
    80005574:	ddafb0ef          	jal	80000b4e <initlock>
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    80005578:	100017b7          	lui	a5,0x10001
    8000557c:	4398                	lw	a4,0(a5)
    8000557e:	2701                	sext.w	a4,a4
    80005580:	747277b7          	lui	a5,0x74727
    80005584:	97678793          	addi	a5,a5,-1674 # 74726976 <_entry-0xb8d968a>
    80005588:	18f71063          	bne	a4,a5,80005708 <virtio_disk_init+0x1b0>
     *R(VIRTIO_MMIO_VERSION) != 2 ||
    8000558c:	100017b7          	lui	a5,0x10001
    80005590:	0791                	addi	a5,a5,4 # 10001004 <_entry-0x6fffeffc>
    80005592:	439c                	lw	a5,0(a5)
    80005594:	2781                	sext.w	a5,a5
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    80005596:	4709                	li	a4,2
    80005598:	16e79863          	bne	a5,a4,80005708 <virtio_disk_init+0x1b0>
     *R(VIRTIO_MMIO_DEVICE_ID) != 2 ||
    8000559c:	100017b7          	lui	a5,0x10001
    800055a0:	07a1                	addi	a5,a5,8 # 10001008 <_entry-0x6fffeff8>
    800055a2:	439c                	lw	a5,0(a5)
    800055a4:	2781                	sext.w	a5,a5
     *R(VIRTIO_MMIO_VERSION) != 2 ||
    800055a6:	16e79163          	bne	a5,a4,80005708 <virtio_disk_init+0x1b0>
     *R(VIRTIO_MMIO_VENDOR_ID) != 0x554d4551){
    800055aa:	100017b7          	lui	a5,0x10001
    800055ae:	47d8                	lw	a4,12(a5)
    800055b0:	2701                	sext.w	a4,a4
     *R(VIRTIO_MMIO_DEVICE_ID) != 2 ||
    800055b2:	554d47b7          	lui	a5,0x554d4
    800055b6:	55178793          	addi	a5,a5,1361 # 554d4551 <_entry-0x2ab2baaf>
    800055ba:	14f71763          	bne	a4,a5,80005708 <virtio_disk_init+0x1b0>
  *R(VIRTIO_MMIO_STATUS) = status;
    800055be:	100017b7          	lui	a5,0x10001
    800055c2:	0607a823          	sw	zero,112(a5) # 10001070 <_entry-0x6fffef90>
  *R(VIRTIO_MMIO_STATUS) = status;
    800055c6:	4705                	li	a4,1
    800055c8:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    800055ca:	470d                	li	a4,3
    800055cc:	dbb8                	sw	a4,112(a5)
  uint64 features = *R(VIRTIO_MMIO_DEVICE_FEATURES);
    800055ce:	10001737          	lui	a4,0x10001
    800055d2:	4b14                	lw	a3,16(a4)
  features &= ~(1 << VIRTIO_RING_F_INDIRECT_DESC);
    800055d4:	c7ffe737          	lui	a4,0xc7ffe
    800055d8:	75f70713          	addi	a4,a4,1887 # ffffffffc7ffe75f <end+0xffffffff47fdd9e7>
  *R(VIRTIO_MMIO_DRIVER_FEATURES) = features;
    800055dc:	8ef9                	and	a3,a3,a4
    800055de:	10001737          	lui	a4,0x10001
    800055e2:	d314                	sw	a3,32(a4)
  *R(VIRTIO_MMIO_STATUS) = status;
    800055e4:	472d                	li	a4,11
    800055e6:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    800055e8:	07078793          	addi	a5,a5,112
  status = *R(VIRTIO_MMIO_STATUS);
    800055ec:	439c                	lw	a5,0(a5)
    800055ee:	0007891b          	sext.w	s2,a5
  if(!(status & VIRTIO_CONFIG_S_FEATURES_OK))
    800055f2:	8ba1                	andi	a5,a5,8
    800055f4:	12078063          	beqz	a5,80005714 <virtio_disk_init+0x1bc>
  *R(VIRTIO_MMIO_QUEUE_SEL) = 0;
    800055f8:	100017b7          	lui	a5,0x10001
    800055fc:	0207a823          	sw	zero,48(a5) # 10001030 <_entry-0x6fffefd0>
  if(*R(VIRTIO_MMIO_QUEUE_READY))
    80005600:	100017b7          	lui	a5,0x10001
    80005604:	04478793          	addi	a5,a5,68 # 10001044 <_entry-0x6fffefbc>
    80005608:	439c                	lw	a5,0(a5)
    8000560a:	2781                	sext.w	a5,a5
    8000560c:	10079a63          	bnez	a5,80005720 <virtio_disk_init+0x1c8>
  uint32 max = *R(VIRTIO_MMIO_QUEUE_NUM_MAX);
    80005610:	100017b7          	lui	a5,0x10001
    80005614:	03478793          	addi	a5,a5,52 # 10001034 <_entry-0x6fffefcc>
    80005618:	439c                	lw	a5,0(a5)
    8000561a:	2781                	sext.w	a5,a5
  if(max == 0)
    8000561c:	10078863          	beqz	a5,8000572c <virtio_disk_init+0x1d4>
  if(max < NUM)
    80005620:	471d                	li	a4,7
    80005622:	10f77b63          	bgeu	a4,a5,80005738 <virtio_disk_init+0x1e0>
  disk.desc = kalloc();
    80005626:	cd8fb0ef          	jal	80000afe <kalloc>
    8000562a:	0001b497          	auipc	s1,0x1b
    8000562e:	60e48493          	addi	s1,s1,1550 # 80020c38 <disk>
    80005632:	e088                	sd	a0,0(s1)
  disk.avail = kalloc();
    80005634:	ccafb0ef          	jal	80000afe <kalloc>
    80005638:	e488                	sd	a0,8(s1)
  disk.used = kalloc();
    8000563a:	cc4fb0ef          	jal	80000afe <kalloc>
    8000563e:	87aa                	mv	a5,a0
    80005640:	e888                	sd	a0,16(s1)
  if(!disk.desc || !disk.avail || !disk.used)
    80005642:	6088                	ld	a0,0(s1)
    80005644:	10050063          	beqz	a0,80005744 <virtio_disk_init+0x1ec>
    80005648:	0001b717          	auipc	a4,0x1b
    8000564c:	5f873703          	ld	a4,1528(a4) # 80020c40 <disk+0x8>
    80005650:	0e070a63          	beqz	a4,80005744 <virtio_disk_init+0x1ec>
    80005654:	0e078863          	beqz	a5,80005744 <virtio_disk_init+0x1ec>
  memset(disk.desc, 0, PGSIZE);
    80005658:	6605                	lui	a2,0x1
    8000565a:	4581                	li	a1,0
    8000565c:	e46fb0ef          	jal	80000ca2 <memset>
  memset(disk.avail, 0, PGSIZE);
    80005660:	0001b497          	auipc	s1,0x1b
    80005664:	5d848493          	addi	s1,s1,1496 # 80020c38 <disk>
    80005668:	6605                	lui	a2,0x1
    8000566a:	4581                	li	a1,0
    8000566c:	6488                	ld	a0,8(s1)
    8000566e:	e34fb0ef          	jal	80000ca2 <memset>
  memset(disk.used, 0, PGSIZE);
    80005672:	6605                	lui	a2,0x1
    80005674:	4581                	li	a1,0
    80005676:	6888                	ld	a0,16(s1)
    80005678:	e2afb0ef          	jal	80000ca2 <memset>
  *R(VIRTIO_MMIO_QUEUE_NUM) = NUM;
    8000567c:	100017b7          	lui	a5,0x10001
    80005680:	4721                	li	a4,8
    80005682:	df98                	sw	a4,56(a5)
  *R(VIRTIO_MMIO_QUEUE_DESC_LOW) = (uint64)disk.desc;
    80005684:	4098                	lw	a4,0(s1)
    80005686:	100017b7          	lui	a5,0x10001
    8000568a:	08e7a023          	sw	a4,128(a5) # 10001080 <_entry-0x6fffef80>
  *R(VIRTIO_MMIO_QUEUE_DESC_HIGH) = (uint64)disk.desc >> 32;
    8000568e:	40d8                	lw	a4,4(s1)
    80005690:	100017b7          	lui	a5,0x10001
    80005694:	08e7a223          	sw	a4,132(a5) # 10001084 <_entry-0x6fffef7c>
  *R(VIRTIO_MMIO_DRIVER_DESC_LOW) = (uint64)disk.avail;
    80005698:	649c                	ld	a5,8(s1)
    8000569a:	0007869b          	sext.w	a3,a5
    8000569e:	10001737          	lui	a4,0x10001
    800056a2:	08d72823          	sw	a3,144(a4) # 10001090 <_entry-0x6fffef70>
  *R(VIRTIO_MMIO_DRIVER_DESC_HIGH) = (uint64)disk.avail >> 32;
    800056a6:	9781                	srai	a5,a5,0x20
    800056a8:	10001737          	lui	a4,0x10001
    800056ac:	08f72a23          	sw	a5,148(a4) # 10001094 <_entry-0x6fffef6c>
  *R(VIRTIO_MMIO_DEVICE_DESC_LOW) = (uint64)disk.used;
    800056b0:	689c                	ld	a5,16(s1)
    800056b2:	0007869b          	sext.w	a3,a5
    800056b6:	10001737          	lui	a4,0x10001
    800056ba:	0ad72023          	sw	a3,160(a4) # 100010a0 <_entry-0x6fffef60>
  *R(VIRTIO_MMIO_DEVICE_DESC_HIGH) = (uint64)disk.used >> 32;
    800056be:	9781                	srai	a5,a5,0x20
    800056c0:	10001737          	lui	a4,0x10001
    800056c4:	0af72223          	sw	a5,164(a4) # 100010a4 <_entry-0x6fffef5c>
  *R(VIRTIO_MMIO_QUEUE_READY) = 0x1;
    800056c8:	10001737          	lui	a4,0x10001
    800056cc:	4785                	li	a5,1
    800056ce:	c37c                	sw	a5,68(a4)
    disk.free[i] = 1;
    800056d0:	00f48c23          	sb	a5,24(s1)
    800056d4:	00f48ca3          	sb	a5,25(s1)
    800056d8:	00f48d23          	sb	a5,26(s1)
    800056dc:	00f48da3          	sb	a5,27(s1)
    800056e0:	00f48e23          	sb	a5,28(s1)
    800056e4:	00f48ea3          	sb	a5,29(s1)
    800056e8:	00f48f23          	sb	a5,30(s1)
    800056ec:	00f48fa3          	sb	a5,31(s1)
  status |= VIRTIO_CONFIG_S_DRIVER_OK;
    800056f0:	00496913          	ori	s2,s2,4
  *R(VIRTIO_MMIO_STATUS) = status;
    800056f4:	100017b7          	lui	a5,0x10001
    800056f8:	0727a823          	sw	s2,112(a5) # 10001070 <_entry-0x6fffef90>
}
    800056fc:	60e2                	ld	ra,24(sp)
    800056fe:	6442                	ld	s0,16(sp)
    80005700:	64a2                	ld	s1,8(sp)
    80005702:	6902                	ld	s2,0(sp)
    80005704:	6105                	addi	sp,sp,32
    80005706:	8082                	ret
    panic("could not find virtio disk");
    80005708:	00002517          	auipc	a0,0x2
    8000570c:	f3850513          	addi	a0,a0,-200 # 80007640 <etext+0x640>
    80005710:	8d0fb0ef          	jal	800007e0 <panic>
    panic("virtio disk FEATURES_OK unset");
    80005714:	00002517          	auipc	a0,0x2
    80005718:	f4c50513          	addi	a0,a0,-180 # 80007660 <etext+0x660>
    8000571c:	8c4fb0ef          	jal	800007e0 <panic>
    panic("virtio disk should not be ready");
    80005720:	00002517          	auipc	a0,0x2
    80005724:	f6050513          	addi	a0,a0,-160 # 80007680 <etext+0x680>
    80005728:	8b8fb0ef          	jal	800007e0 <panic>
    panic("virtio disk has no queue 0");
    8000572c:	00002517          	auipc	a0,0x2
    80005730:	f7450513          	addi	a0,a0,-140 # 800076a0 <etext+0x6a0>
    80005734:	8acfb0ef          	jal	800007e0 <panic>
    panic("virtio disk max queue too short");
    80005738:	00002517          	auipc	a0,0x2
    8000573c:	f8850513          	addi	a0,a0,-120 # 800076c0 <etext+0x6c0>
    80005740:	8a0fb0ef          	jal	800007e0 <panic>
    panic("virtio disk kalloc");
    80005744:	00002517          	auipc	a0,0x2
    80005748:	f9c50513          	addi	a0,a0,-100 # 800076e0 <etext+0x6e0>
    8000574c:	894fb0ef          	jal	800007e0 <panic>

0000000080005750 <virtio_disk_rw>:
  return 0;
}

void
virtio_disk_rw(struct buf *b, int write)
{
    80005750:	7159                	addi	sp,sp,-112
    80005752:	f486                	sd	ra,104(sp)
    80005754:	f0a2                	sd	s0,96(sp)
    80005756:	eca6                	sd	s1,88(sp)
    80005758:	e8ca                	sd	s2,80(sp)
    8000575a:	e4ce                	sd	s3,72(sp)
    8000575c:	e0d2                	sd	s4,64(sp)
    8000575e:	fc56                	sd	s5,56(sp)
    80005760:	f85a                	sd	s6,48(sp)
    80005762:	f45e                	sd	s7,40(sp)
    80005764:	f062                	sd	s8,32(sp)
    80005766:	ec66                	sd	s9,24(sp)
    80005768:	1880                	addi	s0,sp,112
    8000576a:	8a2a                	mv	s4,a0
    8000576c:	8bae                	mv	s7,a1
  uint64 sector = b->blockno * (BSIZE / 512);
    8000576e:	00c52c83          	lw	s9,12(a0)
    80005772:	001c9c9b          	slliw	s9,s9,0x1
    80005776:	1c82                	slli	s9,s9,0x20
    80005778:	020cdc93          	srli	s9,s9,0x20

  acquire(&disk.vdisk_lock);
    8000577c:	0001b517          	auipc	a0,0x1b
    80005780:	5e450513          	addi	a0,a0,1508 # 80020d60 <disk+0x128>
    80005784:	c4afb0ef          	jal	80000bce <acquire>
  for(int i = 0; i < 3; i++){
    80005788:	4981                	li	s3,0
  for(int i = 0; i < NUM; i++){
    8000578a:	44a1                	li	s1,8
      disk.free[i] = 0;
    8000578c:	0001bb17          	auipc	s6,0x1b
    80005790:	4acb0b13          	addi	s6,s6,1196 # 80020c38 <disk>
  for(int i = 0; i < 3; i++){
    80005794:	4a8d                	li	s5,3
  int idx[3];
  while(1){
    if(alloc3_desc(idx) == 0) {
      break;
    }
    sleep(&disk.free[0], &disk.vdisk_lock);
    80005796:	0001bc17          	auipc	s8,0x1b
    8000579a:	5cac0c13          	addi	s8,s8,1482 # 80020d60 <disk+0x128>
    8000579e:	a8b9                	j	800057fc <virtio_disk_rw+0xac>
      disk.free[i] = 0;
    800057a0:	00fb0733          	add	a4,s6,a5
    800057a4:	00070c23          	sb	zero,24(a4) # 10001018 <_entry-0x6fffefe8>
    idx[i] = alloc_desc();
    800057a8:	c19c                	sw	a5,0(a1)
    if(idx[i] < 0){
    800057aa:	0207c563          	bltz	a5,800057d4 <virtio_disk_rw+0x84>
  for(int i = 0; i < 3; i++){
    800057ae:	2905                	addiw	s2,s2,1
    800057b0:	0611                	addi	a2,a2,4 # 1004 <_entry-0x7fffeffc>
    800057b2:	05590963          	beq	s2,s5,80005804 <virtio_disk_rw+0xb4>
    idx[i] = alloc_desc();
    800057b6:	85b2                	mv	a1,a2
  for(int i = 0; i < NUM; i++){
    800057b8:	0001b717          	auipc	a4,0x1b
    800057bc:	48070713          	addi	a4,a4,1152 # 80020c38 <disk>
    800057c0:	87ce                	mv	a5,s3
    if(disk.free[i]){
    800057c2:	01874683          	lbu	a3,24(a4)
    800057c6:	fee9                	bnez	a3,800057a0 <virtio_disk_rw+0x50>
  for(int i = 0; i < NUM; i++){
    800057c8:	2785                	addiw	a5,a5,1
    800057ca:	0705                	addi	a4,a4,1
    800057cc:	fe979be3          	bne	a5,s1,800057c2 <virtio_disk_rw+0x72>
    idx[i] = alloc_desc();
    800057d0:	57fd                	li	a5,-1
    800057d2:	c19c                	sw	a5,0(a1)
      for(int j = 0; j < i; j++)
    800057d4:	01205d63          	blez	s2,800057ee <virtio_disk_rw+0x9e>
        free_desc(idx[j]);
    800057d8:	f9042503          	lw	a0,-112(s0)
    800057dc:	d07ff0ef          	jal	800054e2 <free_desc>
      for(int j = 0; j < i; j++)
    800057e0:	4785                	li	a5,1
    800057e2:	0127d663          	bge	a5,s2,800057ee <virtio_disk_rw+0x9e>
        free_desc(idx[j]);
    800057e6:	f9442503          	lw	a0,-108(s0)
    800057ea:	cf9ff0ef          	jal	800054e2 <free_desc>
    sleep(&disk.free[0], &disk.vdisk_lock);
    800057ee:	85e2                	mv	a1,s8
    800057f0:	0001b517          	auipc	a0,0x1b
    800057f4:	46050513          	addi	a0,a0,1120 # 80020c50 <disk+0x18>
    800057f8:	f34fc0ef          	jal	80001f2c <sleep>
  for(int i = 0; i < 3; i++){
    800057fc:	f9040613          	addi	a2,s0,-112
    80005800:	894e                	mv	s2,s3
    80005802:	bf55                	j	800057b6 <virtio_disk_rw+0x66>
  }

  // format the three descriptors.
  // qemu's virtio-blk.c reads them.

  struct virtio_blk_req *buf0 = &disk.ops[idx[0]];
    80005804:	f9042503          	lw	a0,-112(s0)
    80005808:	00451693          	slli	a3,a0,0x4

  if(write)
    8000580c:	0001b797          	auipc	a5,0x1b
    80005810:	42c78793          	addi	a5,a5,1068 # 80020c38 <disk>
    80005814:	00a50713          	addi	a4,a0,10
    80005818:	0712                	slli	a4,a4,0x4
    8000581a:	973e                	add	a4,a4,a5
    8000581c:	01703633          	snez	a2,s7
    80005820:	c710                	sw	a2,8(a4)
    buf0->type = VIRTIO_BLK_T_OUT; // write the disk
  else
    buf0->type = VIRTIO_BLK_T_IN; // read the disk
  buf0->reserved = 0;
    80005822:	00072623          	sw	zero,12(a4)
  buf0->sector = sector;
    80005826:	01973823          	sd	s9,16(a4)

  disk.desc[idx[0]].addr = (uint64) buf0;
    8000582a:	6398                	ld	a4,0(a5)
    8000582c:	9736                	add	a4,a4,a3
  struct virtio_blk_req *buf0 = &disk.ops[idx[0]];
    8000582e:	0a868613          	addi	a2,a3,168
    80005832:	963e                	add	a2,a2,a5
  disk.desc[idx[0]].addr = (uint64) buf0;
    80005834:	e310                	sd	a2,0(a4)
  disk.desc[idx[0]].len = sizeof(struct virtio_blk_req);
    80005836:	6390                	ld	a2,0(a5)
    80005838:	00d605b3          	add	a1,a2,a3
    8000583c:	4741                	li	a4,16
    8000583e:	c598                	sw	a4,8(a1)
  disk.desc[idx[0]].flags = VRING_DESC_F_NEXT;
    80005840:	4805                	li	a6,1
    80005842:	01059623          	sh	a6,12(a1)
  disk.desc[idx[0]].next = idx[1];
    80005846:	f9442703          	lw	a4,-108(s0)
    8000584a:	00e59723          	sh	a4,14(a1)

  disk.desc[idx[1]].addr = (uint64) b->data;
    8000584e:	0712                	slli	a4,a4,0x4
    80005850:	963a                	add	a2,a2,a4
    80005852:	058a0593          	addi	a1,s4,88
    80005856:	e20c                	sd	a1,0(a2)
  disk.desc[idx[1]].len = BSIZE;
    80005858:	0007b883          	ld	a7,0(a5)
    8000585c:	9746                	add	a4,a4,a7
    8000585e:	40000613          	li	a2,1024
    80005862:	c710                	sw	a2,8(a4)
  if(write)
    80005864:	001bb613          	seqz	a2,s7
    80005868:	0016161b          	slliw	a2,a2,0x1
    disk.desc[idx[1]].flags = 0; // device reads b->data
  else
    disk.desc[idx[1]].flags = VRING_DESC_F_WRITE; // device writes b->data
  disk.desc[idx[1]].flags |= VRING_DESC_F_NEXT;
    8000586c:	00166613          	ori	a2,a2,1
    80005870:	00c71623          	sh	a2,12(a4)
  disk.desc[idx[1]].next = idx[2];
    80005874:	f9842583          	lw	a1,-104(s0)
    80005878:	00b71723          	sh	a1,14(a4)

  disk.info[idx[0]].status = 0xff; // device writes 0 on success
    8000587c:	00250613          	addi	a2,a0,2
    80005880:	0612                	slli	a2,a2,0x4
    80005882:	963e                	add	a2,a2,a5
    80005884:	577d                	li	a4,-1
    80005886:	00e60823          	sb	a4,16(a2)
  disk.desc[idx[2]].addr = (uint64) &disk.info[idx[0]].status;
    8000588a:	0592                	slli	a1,a1,0x4
    8000588c:	98ae                	add	a7,a7,a1
    8000588e:	03068713          	addi	a4,a3,48
    80005892:	973e                	add	a4,a4,a5
    80005894:	00e8b023          	sd	a4,0(a7)
  disk.desc[idx[2]].len = 1;
    80005898:	6398                	ld	a4,0(a5)
    8000589a:	972e                	add	a4,a4,a1
    8000589c:	01072423          	sw	a6,8(a4)
  disk.desc[idx[2]].flags = VRING_DESC_F_WRITE; // device writes the status
    800058a0:	4689                	li	a3,2
    800058a2:	00d71623          	sh	a3,12(a4)
  disk.desc[idx[2]].next = 0;
    800058a6:	00071723          	sh	zero,14(a4)

  // record struct buf for virtio_disk_intr().
  b->disk = 1;
    800058aa:	010a2223          	sw	a6,4(s4)
  disk.info[idx[0]].b = b;
    800058ae:	01463423          	sd	s4,8(a2)

  // tell the device the first index in our chain of descriptors.
  disk.avail->ring[disk.avail->idx % NUM] = idx[0];
    800058b2:	6794                	ld	a3,8(a5)
    800058b4:	0026d703          	lhu	a4,2(a3)
    800058b8:	8b1d                	andi	a4,a4,7
    800058ba:	0706                	slli	a4,a4,0x1
    800058bc:	96ba                	add	a3,a3,a4
    800058be:	00a69223          	sh	a0,4(a3)

  __sync_synchronize();
    800058c2:	0ff0000f          	fence

  // tell the device another avail ring entry is available.
  disk.avail->idx += 1; // not % NUM ...
    800058c6:	6798                	ld	a4,8(a5)
    800058c8:	00275783          	lhu	a5,2(a4)
    800058cc:	2785                	addiw	a5,a5,1
    800058ce:	00f71123          	sh	a5,2(a4)

  __sync_synchronize();
    800058d2:	0ff0000f          	fence

  *R(VIRTIO_MMIO_QUEUE_NOTIFY) = 0; // value is queue number
    800058d6:	100017b7          	lui	a5,0x10001
    800058da:	0407a823          	sw	zero,80(a5) # 10001050 <_entry-0x6fffefb0>

  // Wait for virtio_disk_intr() to say request has finished.
  while(b->disk == 1) {
    800058de:	004a2783          	lw	a5,4(s4)
    sleep(b, &disk.vdisk_lock);
    800058e2:	0001b917          	auipc	s2,0x1b
    800058e6:	47e90913          	addi	s2,s2,1150 # 80020d60 <disk+0x128>
  while(b->disk == 1) {
    800058ea:	4485                	li	s1,1
    800058ec:	01079a63          	bne	a5,a6,80005900 <virtio_disk_rw+0x1b0>
    sleep(b, &disk.vdisk_lock);
    800058f0:	85ca                	mv	a1,s2
    800058f2:	8552                	mv	a0,s4
    800058f4:	e38fc0ef          	jal	80001f2c <sleep>
  while(b->disk == 1) {
    800058f8:	004a2783          	lw	a5,4(s4)
    800058fc:	fe978ae3          	beq	a5,s1,800058f0 <virtio_disk_rw+0x1a0>
  }

  disk.info[idx[0]].b = 0;
    80005900:	f9042903          	lw	s2,-112(s0)
    80005904:	00290713          	addi	a4,s2,2
    80005908:	0712                	slli	a4,a4,0x4
    8000590a:	0001b797          	auipc	a5,0x1b
    8000590e:	32e78793          	addi	a5,a5,814 # 80020c38 <disk>
    80005912:	97ba                	add	a5,a5,a4
    80005914:	0007b423          	sd	zero,8(a5)
    int flag = disk.desc[i].flags;
    80005918:	0001b997          	auipc	s3,0x1b
    8000591c:	32098993          	addi	s3,s3,800 # 80020c38 <disk>
    80005920:	00491713          	slli	a4,s2,0x4
    80005924:	0009b783          	ld	a5,0(s3)
    80005928:	97ba                	add	a5,a5,a4
    8000592a:	00c7d483          	lhu	s1,12(a5)
    int nxt = disk.desc[i].next;
    8000592e:	854a                	mv	a0,s2
    80005930:	00e7d903          	lhu	s2,14(a5)
    free_desc(i);
    80005934:	bafff0ef          	jal	800054e2 <free_desc>
    if(flag & VRING_DESC_F_NEXT)
    80005938:	8885                	andi	s1,s1,1
    8000593a:	f0fd                	bnez	s1,80005920 <virtio_disk_rw+0x1d0>
  free_chain(idx[0]);

  release(&disk.vdisk_lock);
    8000593c:	0001b517          	auipc	a0,0x1b
    80005940:	42450513          	addi	a0,a0,1060 # 80020d60 <disk+0x128>
    80005944:	b22fb0ef          	jal	80000c66 <release>
}
    80005948:	70a6                	ld	ra,104(sp)
    8000594a:	7406                	ld	s0,96(sp)
    8000594c:	64e6                	ld	s1,88(sp)
    8000594e:	6946                	ld	s2,80(sp)
    80005950:	69a6                	ld	s3,72(sp)
    80005952:	6a06                	ld	s4,64(sp)
    80005954:	7ae2                	ld	s5,56(sp)
    80005956:	7b42                	ld	s6,48(sp)
    80005958:	7ba2                	ld	s7,40(sp)
    8000595a:	7c02                	ld	s8,32(sp)
    8000595c:	6ce2                	ld	s9,24(sp)
    8000595e:	6165                	addi	sp,sp,112
    80005960:	8082                	ret

0000000080005962 <virtio_disk_intr>:

void
virtio_disk_intr()
{
    80005962:	1101                	addi	sp,sp,-32
    80005964:	ec06                	sd	ra,24(sp)
    80005966:	e822                	sd	s0,16(sp)
    80005968:	e426                	sd	s1,8(sp)
    8000596a:	1000                	addi	s0,sp,32
  acquire(&disk.vdisk_lock);
    8000596c:	0001b497          	auipc	s1,0x1b
    80005970:	2cc48493          	addi	s1,s1,716 # 80020c38 <disk>
    80005974:	0001b517          	auipc	a0,0x1b
    80005978:	3ec50513          	addi	a0,a0,1004 # 80020d60 <disk+0x128>
    8000597c:	a52fb0ef          	jal	80000bce <acquire>
  // we've seen this interrupt, which the following line does.
  // this may race with the device writing new entries to
  // the "used" ring, in which case we may process the new
  // completion entries in this interrupt, and have nothing to do
  // in the next interrupt, which is harmless.
  *R(VIRTIO_MMIO_INTERRUPT_ACK) = *R(VIRTIO_MMIO_INTERRUPT_STATUS) & 0x3;
    80005980:	100017b7          	lui	a5,0x10001
    80005984:	53b8                	lw	a4,96(a5)
    80005986:	8b0d                	andi	a4,a4,3
    80005988:	100017b7          	lui	a5,0x10001
    8000598c:	d3f8                	sw	a4,100(a5)

  __sync_synchronize();
    8000598e:	0ff0000f          	fence

  // the device increments disk.used->idx when it
  // adds an entry to the used ring.

  while(disk.used_idx != disk.used->idx){
    80005992:	689c                	ld	a5,16(s1)
    80005994:	0204d703          	lhu	a4,32(s1)
    80005998:	0027d783          	lhu	a5,2(a5) # 10001002 <_entry-0x6fffeffe>
    8000599c:	04f70663          	beq	a4,a5,800059e8 <virtio_disk_intr+0x86>
    __sync_synchronize();
    800059a0:	0ff0000f          	fence
    int id = disk.used->ring[disk.used_idx % NUM].id;
    800059a4:	6898                	ld	a4,16(s1)
    800059a6:	0204d783          	lhu	a5,32(s1)
    800059aa:	8b9d                	andi	a5,a5,7
    800059ac:	078e                	slli	a5,a5,0x3
    800059ae:	97ba                	add	a5,a5,a4
    800059b0:	43dc                	lw	a5,4(a5)

    if(disk.info[id].status != 0)
    800059b2:	00278713          	addi	a4,a5,2
    800059b6:	0712                	slli	a4,a4,0x4
    800059b8:	9726                	add	a4,a4,s1
    800059ba:	01074703          	lbu	a4,16(a4)
    800059be:	e321                	bnez	a4,800059fe <virtio_disk_intr+0x9c>
      panic("virtio_disk_intr status");

    struct buf *b = disk.info[id].b;
    800059c0:	0789                	addi	a5,a5,2
    800059c2:	0792                	slli	a5,a5,0x4
    800059c4:	97a6                	add	a5,a5,s1
    800059c6:	6788                	ld	a0,8(a5)
    b->disk = 0;   // disk is done with buf
    800059c8:	00052223          	sw	zero,4(a0)
    wakeup(b);
    800059cc:	dacfc0ef          	jal	80001f78 <wakeup>

    disk.used_idx += 1;
    800059d0:	0204d783          	lhu	a5,32(s1)
    800059d4:	2785                	addiw	a5,a5,1
    800059d6:	17c2                	slli	a5,a5,0x30
    800059d8:	93c1                	srli	a5,a5,0x30
    800059da:	02f49023          	sh	a5,32(s1)
  while(disk.used_idx != disk.used->idx){
    800059de:	6898                	ld	a4,16(s1)
    800059e0:	00275703          	lhu	a4,2(a4)
    800059e4:	faf71ee3          	bne	a4,a5,800059a0 <virtio_disk_intr+0x3e>
  }

  release(&disk.vdisk_lock);
    800059e8:	0001b517          	auipc	a0,0x1b
    800059ec:	37850513          	addi	a0,a0,888 # 80020d60 <disk+0x128>
    800059f0:	a76fb0ef          	jal	80000c66 <release>
}
    800059f4:	60e2                	ld	ra,24(sp)
    800059f6:	6442                	ld	s0,16(sp)
    800059f8:	64a2                	ld	s1,8(sp)
    800059fa:	6105                	addi	sp,sp,32
    800059fc:	8082                	ret
      panic("virtio_disk_intr status");
    800059fe:	00002517          	auipc	a0,0x2
    80005a02:	cfa50513          	addi	a0,a0,-774 # 800076f8 <etext+0x6f8>
    80005a06:	ddbfa0ef          	jal	800007e0 <panic>
	...

0000000080006000 <_trampoline>:
    80006000:	14051073          	csrw	sscratch,a0
    80006004:	02000537          	lui	a0,0x2000
    80006008:	357d                	addiw	a0,a0,-1 # 1ffffff <_entry-0x7e000001>
    8000600a:	0536                	slli	a0,a0,0xd
    8000600c:	02153423          	sd	ra,40(a0)
    80006010:	02253823          	sd	sp,48(a0)
    80006014:	02353c23          	sd	gp,56(a0)
    80006018:	04453023          	sd	tp,64(a0)
    8000601c:	04553423          	sd	t0,72(a0)
    80006020:	04653823          	sd	t1,80(a0)
    80006024:	04753c23          	sd	t2,88(a0)
    80006028:	f120                	sd	s0,96(a0)
    8000602a:	f524                	sd	s1,104(a0)
    8000602c:	fd2c                	sd	a1,120(a0)
    8000602e:	e150                	sd	a2,128(a0)
    80006030:	e554                	sd	a3,136(a0)
    80006032:	e958                	sd	a4,144(a0)
    80006034:	ed5c                	sd	a5,152(a0)
    80006036:	0b053023          	sd	a6,160(a0)
    8000603a:	0b153423          	sd	a7,168(a0)
    8000603e:	0b253823          	sd	s2,176(a0)
    80006042:	0b353c23          	sd	s3,184(a0)
    80006046:	0d453023          	sd	s4,192(a0)
    8000604a:	0d553423          	sd	s5,200(a0)
    8000604e:	0d653823          	sd	s6,208(a0)
    80006052:	0d753c23          	sd	s7,216(a0)
    80006056:	0f853023          	sd	s8,224(a0)
    8000605a:	0f953423          	sd	s9,232(a0)
    8000605e:	0fa53823          	sd	s10,240(a0)
    80006062:	0fb53c23          	sd	s11,248(a0)
    80006066:	11c53023          	sd	t3,256(a0)
    8000606a:	11d53423          	sd	t4,264(a0)
    8000606e:	11e53823          	sd	t5,272(a0)
    80006072:	11f53c23          	sd	t6,280(a0)
    80006076:	140022f3          	csrr	t0,sscratch
    8000607a:	06553823          	sd	t0,112(a0)
    8000607e:	00853103          	ld	sp,8(a0)
    80006082:	02053203          	ld	tp,32(a0)
    80006086:	01053283          	ld	t0,16(a0)
    8000608a:	00053303          	ld	t1,0(a0)
    8000608e:	12000073          	sfence.vma
    80006092:	18031073          	csrw	satp,t1
    80006096:	12000073          	sfence.vma
    8000609a:	9282                	jalr	t0

000000008000609c <userret>:
    8000609c:	12000073          	sfence.vma
    800060a0:	18051073          	csrw	satp,a0
    800060a4:	12000073          	sfence.vma
    800060a8:	02000537          	lui	a0,0x2000
    800060ac:	357d                	addiw	a0,a0,-1 # 1ffffff <_entry-0x7e000001>
    800060ae:	0536                	slli	a0,a0,0xd
    800060b0:	02853083          	ld	ra,40(a0)
    800060b4:	03053103          	ld	sp,48(a0)
    800060b8:	03853183          	ld	gp,56(a0)
    800060bc:	04053203          	ld	tp,64(a0)
    800060c0:	04853283          	ld	t0,72(a0)
    800060c4:	05053303          	ld	t1,80(a0)
    800060c8:	05853383          	ld	t2,88(a0)
    800060cc:	7120                	ld	s0,96(a0)
    800060ce:	7524                	ld	s1,104(a0)
    800060d0:	7d2c                	ld	a1,120(a0)
    800060d2:	6150                	ld	a2,128(a0)
    800060d4:	6554                	ld	a3,136(a0)
    800060d6:	6958                	ld	a4,144(a0)
    800060d8:	6d5c                	ld	a5,152(a0)
    800060da:	0a053803          	ld	a6,160(a0)
    800060de:	0a853883          	ld	a7,168(a0)
    800060e2:	0b053903          	ld	s2,176(a0)
    800060e6:	0b853983          	ld	s3,184(a0)
    800060ea:	0c053a03          	ld	s4,192(a0)
    800060ee:	0c853a83          	ld	s5,200(a0)
    800060f2:	0d053b03          	ld	s6,208(a0)
    800060f6:	0d853b83          	ld	s7,216(a0)
    800060fa:	0e053c03          	ld	s8,224(a0)
    800060fe:	0e853c83          	ld	s9,232(a0)
    80006102:	0f053d03          	ld	s10,240(a0)
    80006106:	0f853d83          	ld	s11,248(a0)
    8000610a:	10053e03          	ld	t3,256(a0)
    8000610e:	10853e83          	ld	t4,264(a0)
    80006112:	11053f03          	ld	t5,272(a0)
    80006116:	11853f83          	ld	t6,280(a0)
    8000611a:	7928                	ld	a0,112(a0)
    8000611c:	10200073          	sret
	...
